...
description: Provision TNF full-auto command+skill artifacts across agent runtimes and execute autonomous self-improvement loops without per-run human prompting.
---

# TNF Full-Auto Network Autopilot

Use this skill when Codex or any agent must self-bootstrap full-auto execution across the TNF agent network.

## Objective

1. Install shared full-auto slash-command artifacts into available agent runtimes.
...
- `TNF_SUPER_ADMIN_INPUT_TOKEN` available to the invoking process for protected commands.
...
- If provisioning fails for a specific target root, continue processing remaining targets and report skipped roots.
- If `full-auto once` fails, stop and inspect `tnf full-auto status` before moving to `start`.
- If `full-auto start --strict` fails, treat as blocking reliability event and remediate before resuming.
