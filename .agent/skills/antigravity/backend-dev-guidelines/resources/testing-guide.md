...
describe('UserService Integration', () => {
  ...
  beforeAll(async () => {
    // Create test data
    testUser = await DatabaseService.main.user.create({
      data: {
        email: 'test@test.com',
  ...
  afterAll(async () => {
    // Cleanup
    await DatabaseService.main.user.delete({ where: { id: testUser.id } });
  });
...
describe('PermissionService', () => {
  ...
  beforeAll(async () => {
    // Create test post
    const post = await DatabaseService.main.post.create({
  ...
  afterAll(async () => {
    // Cleanup
    await DatabaseService.main.post.delete({
      where: { id: instanceId },
    });
  });
