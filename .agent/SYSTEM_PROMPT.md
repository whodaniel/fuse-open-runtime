...
### 2.5. Load Canonical Runtime Snapshot

```bash
# Get current TNF registry state (generated by tnf:onboard)
cat .agent/runtime-state.json 2>/dev/null
```

Treat `.agent/runtime-state.json` as the authoritative bootstrap state for
current agents, models, harnesses, MCPs, and active sessions.
