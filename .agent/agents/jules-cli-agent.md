...
description:
  "Jules is Google's asynchronous coding agent CLI for parallel, multi-session
  code generation and repository-wide improvements. MUST BE USED for delegating
  complex, time-consuming coding tasks that benefit from asynchronous execution
  and batch processing across multiple sessions."
tools: [Bash]
color: Blue
agent_type: external
version: 1.0.0
