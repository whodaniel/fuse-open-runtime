...
## 2026-04-03 - Inline React Components Unmount/Remount Cycle
**Learning:** Defining inline components (e.g., `const ConnectionStatus = () => (...)`) directly inside the render cycle of a parent component forces React to treat the component as an entirely new component type on every render. This completely bypasses React's diffing algorithm and causes immediate unmounting of the old DOM node and remounting of a new DOM node, leading to severe layout thrashing and high CPU usage in frequently updating parent components like text inputs.
**Action:** Always hoist inline components outside of the parent component. If they require scoped props from the parent, pass them explicitly and wrap the hoisted component in `React.memo` to prevent unnecessary re-renders.
