...
\n## 2026-03-03 - Unapplied Optimizations Block Progress\n**Learning:** In `A2AMultiAgentChat.tsx`, a previous attempt at optimization created an unused `renderedMessages` map while leaving the inline `messages.map` in place. This actually degraded performance slightly as the messages list was being calculated twice (once memoized, once inline). Additionally, another `memoizedMessages` variable was dangling unused.\n**Action:** When creating memoized variables to prevent O(n) re-renders, always ensure the original inline reference is actually replaced with the new memoized variable in the component's render output.
