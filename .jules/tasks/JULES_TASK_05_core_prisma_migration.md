...
package has TypeScript stack overflow issues due to Drizzle 7's recursive types.

There are 6 files in packages/core/src that use DrizzleClient:
...
## Task: Migrate Core Package Drizzle Service to Drizzle

Migrate the drizzle.service.ts in packages/core to use the Drizzle infrastructure
from @the-new-fuse/database.

### Steps:

1. First, search for and understand the current drizzle.service.ts in
   packages/core:
   - `find packages/core -name "drizzle.service.ts" -o -name "*drizzle*"`
...
4. Update imports in files that use the old Drizzle service
5. Run `pnpm type-check` in packages/core to verify no TypeScript errors

### Success Criteria:

- packages/core compiles without Drizzle-related stack overflow errors
