...
class TheiaBuilder {
  ...
  checkNodeVersion() {
    ...
    if (majorVersion < 20) {
      logError(`Theia ${CONFIG.theiaVersion} requires Node.js >= ${CONFIG.minNodeVersion}`);
  ...
  installDependencies() {
    ...
    if (fs.existsSync(nodeModulesPath)) {
      ...
      if (nodeModulesContent.length > 0) {
        logSuccess('Dependencies already installed, skipping installation');
        return true;
      }
    }

    // Use the detected package manager (should be yarn for Theia)
  ...
  buildTheia() {
    ...
    try {
      ...
      let theiaCmd, buildArgs;
      if (fs.existsSync('./node_modules/.bin/theia')) {
        theiaCmd = './node_modules/.bin/theia';
        buildArgs = this.buildMode === 'production' ?
          ['build', '--mode', 'production'] :
          ['build', '--mode', 'development'];
      } else {
        // Use the package manager to run theia CLI
        if (this.packageManager === 'yarn') {
          theiaCmd = 'yarn';
          buildArgs = ['theia', 'build', '--mode', this.buildMode === 'production' ? 'production' : 'development'];
        } else {
          theiaCmd = 'npx';
          buildArgs = ['@theia/cli', 'build', '--mode', this.buildMode === 'production' ? 'production' : 'development'];
        }
      }
