import {
  ...
  CellValue,
  Column,
  SelectOption as ColumnSelectOption,
  DataType,
  Row,
  Table,
  evaluateFormula,
...
import { ArrowUpIcon, PencilIcon, PlusIcon, TrashIcon } from './Icons';
...
const TableCell: React.FC<TableCellProps> = ({
  ...
  useEffect(() => {
    if (
      ...
      (column.type === DataType.TEXT ||
        ...
        column.type === DataType.LONG_TEXT ||
        column.type === DataType.DATE ||
        column.type === DataType.URL ||
        column.type === DataType.EMAIL)
    ) {
  ...
  const handleBlur = (e: React.FocusEvent) => {
    if (
      column.type === DataType.SINGLE_SELECT &&
  ...
  const handleDoubleClick = () => {
    if (
      ...
      column.type !== DataType.LINKED_RECORD &&
      column.type !== DataType.ATTACHMENT &&
      column.type !== DataType.VOTES
    ) {
      setIsEditing(true);
    }
  };
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      const existingAttachments = Array.isArray(currentValue)
        ? (currentValue as AttachmentFile[])
        : [];
  ...
  const removeAttachment = (fileId: string) => {
    const updatedAttachments = (
      Array.isArray(currentValue) ? (currentValue as AttachmentFile[]) : []
  ...
  const renderEditingControl = (): React.ReactNode => {
    switch (column.type) {
      ...
      case DataType.DATE:
        return (
          <input
            ...
            onChange={(e) =>
              setCurrentValue(e.target.value ? new Date(e.target.value).toISOString() : null)
            }
            onBlur={handleBlur}
            onKeyDown={handleKeyDown}
            className="w-full h-full p-2 border-2 border-sky-500 outline-none box-border text-sm"
      ...
      case DataType.SINGLE_SELECT:
        const selectOptions: SelectOptionItem[] = (column.options || []).map(
          (opt: ColumnSelectOption) => ({
            value: opt.id,
            label: (
              <span className={`px-2 py-0.5 text-xs rounded-full ${opt.colorClass}`}>
                {opt.name}
              </span>
            ),
        ...
        return (
          <div ref={selectRef} className="w-full h-full p-0.5 bg-sky-100">
            <SelectInput
              options={[
                { value: '', label: <span className="text-slate-400">Clear selection</span> },
                ...selectOptions,
              ]}
  ...
  const renderViewControl = (): React.ReactNode => {
    ...
    switch (column.type) {
      case DataType.BOOLEAN:
        return (
          <div
            ...
            onClick={() => !isReadOnly && onUpdateCell(!value)}
          >
            <input
              type="checkbox"
              readOnly
              checked={!!value}
              className={`form-checkbox h-5 w-5 text-sky-600 rounded border-slate-400 focus:ring-sky-500 ${isReadOnly ? 'cursor-not-allowed opacity-70' : 'cursor-pointer'}`}
            />
          </div>
        );
      case DataType.SINGLE_SELECT:
        const selectedOption = (column.options || []).find(
          (opt: ColumnSelectOption) => opt.id === value
        );
        return selectedOption ? (
          <span className={`px-2 py-0.5 text-xs rounded-full ${selectedOption.colorClass}`}>
            {selectedOption.name}
          </span>
        ) : (
          emptyDisplay
      ...
      case DataType.URL:
        return (
          <a
            href={String(value).startsWith('http') ? String(value) : `http://${String(value)}`}
            target="_blank"
      ...
      case DataType.LINKED_RECORD: {
          ...
          return (
            <button
              onClick={() =>
                ...
                onOpenLinkRecordModal(row.id, column.id, column.linkedTableId, [])
              }
        ...
        const primaryColumnOfLinkedTable = linkedTable?.columns.find(
        ...
        return (
          <button
            onClick={() =>
              column.linkedTableId &&
              onOpenLinkRecordModal(row.id, column.id, column.linkedTableId, currentLinkedIds)
            }
          ...
          >
            {currentLinkedIds.map((linkedId) => {
              ...
              return (
                <span
                  key={linkedId}
                  className="text-xs bg-sky-100 text-sky-700 px-1.5 py-0.5 rounded-full my-0.5 truncate max-w-full"
                >
                  {displayValue}
                </span>
              );
      ...
      case DataType.FORMULA:
        ...
        if (formulaResult.value === null || formulaResult.value === undefined) {
          ...
          formulaDisplayValue = formulaResult.value
            .map((i: any) => (typeof i === 'object' ? JSON.stringify(i) : String(i)))
            .join(', ');
        ...
        return (
          <span className="text-slate-700 italic truncate">
            {formulaResult.error ? `⚠️ ${formulaResult.error}` : formulaDisplayValue}
          </span>
        );
        ...
        return (
          <div className="w-full h-full p-1 space-y-0.5">
            {attachments.map((file) => (
              <div
                key={file.id}
                className="flex items-center justify-between text-xs bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded-full group"
              >
                <span className="truncate">{file.name}</span>
                <button
                  onClick={() => removeAttachment(file.id)}
                  className="ml-1 text-slate-400 hover:text-red-500 opacity-0 group-hover:opacity-100"
  ...
  if (
    ...
    return (
      <td className="p-0 border-b border-r border-slate-300 relative">{renderEditingControl()}</td>
    );
  }
  ...
  return (
    ...
    >
      <div className="h-full w-full p-2 overflow-hidden">{renderViewControl()}</div>
      {!isReadOnly &&
        column.type !== DataType.BOOLEAN &&
        column.type !== DataType.LINKED_RECORD &&
        column.type !== DataType.ATTACHMENT &&
        column.type !== DataType.VOTES && (
          <button
            className="absolute top-0.5 right-0.5 p-0.5 bg-slate-100 rounded-sm opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer text-slate-500 hover:text-slate-700"
