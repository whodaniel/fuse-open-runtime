...
import React, { useEffect, useRef, useState } from 'react'; // Column is not used
...
import {
  ...
  TimelineIcon,
  TrashIcon,
} from './Icons'; // NEW_VIEW_DEFAULT_NAME is not used
...
const FILTER_OPERATOR_OPTIONS: Record<string, { label: string; applicableTypes: DataType[] }> = {
  EQUALS: {
    label: 'Equals',
    applicableTypes: [DataType.TEXT, DataType.NUMBER, DataType.DATE, DataType.BOOLEAN],
  },
  NOT_EQUALS: {
...
const ViewTypeIconDisplay: React.FC<{ type: ViewType; className?: string }> = ({
  type,
  ...
  switch (type) {
    ...
    case ViewType.KANBAN:
      return <KanbanIcon className={className} />;
...
const Toolbar: React.FC<ToolbarProps> = ({
  ...
  const columnOptions: SelectOptionItem[] = table.columns.map((col: Column) => ({
    value: col.id,
    label: col.name,
  }));
  ...
  const allColumnOptionsForLabel: SelectOptionItem[] = table.columns.map((col: Column) => ({
    value: col.id,
    label: col.name,
  }));
  ...
  const handleConfirmAddView = () => {
    ...
    if (newViewType === ViewType.KANBAN) {
      const potentialGroupByCols = table.columns.filter(
        (c: Column) =>
          c.type === DataType.SINGLE_SELECT ||
          c.type === DataType.LINKED_RECORD ||
    ...
    <div className="p-2 border-b border-slate-300 bg-slate-50 flex items-center space-x-3 text-sm text-slate-700 sticky top-0 z-10">
      ...
      <div className="relative" ref={viewDropdownRef}>
        ...
        >
          ...
          />
          <ChevronDownIcon
            className={`w-4 h-4 ml-1 transition-transform ${isViewDropdownOpen ? 'rotate-180' : ''}`}
          />
        </button>
        {isViewDropdownOpen && (
      ...
      <div className="relative" ref={filterDropdownRef}>
        ...
        {isFilterDropdownOpen && (
          <div className="absolute left-0 mt-1 w-[450px] bg-white rounded-md shadow-lg p-3 z-30 border border-slate-200 space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar">
              ...
              <div key={filter.id} className="space-y-1 p-2 bg-slate-50 rounded">
                <div className="grid grid-cols-[1fr_1fr_auto] gap-2 items-center">
                  <SelectInput
                    ...
                    onChange={(val) => onUpdateFilter(filter.id, { columnId: val, value: '' })}
                  />{' '}
                  {/* Reset value on column change */}
                  <SelectInput
                    options={getApplicableFilterOperators(filter.columnId)}
                    value={filter.operator}
                    onChange={(val) =>
                      onUpdateFilter(filter.id, { operator: val as FilterOperator })
                    }
                  />
                  <button
                    onClick={() => onDeleteFilter(filter.id)}
                    className="text-red-500 hover:text-red-700 justify-self-end p-1"
                    title="Delete filter"
                ...
                {isValueNeededForOperator(filter.operator) &&
                  (table.columns.find((c: Column) => c.id === filter.columnId)?.type ===
                  DataType.BOOLEAN ? (
                    <SelectInput
                      options={[
                      ...
                      onChange={(val) =>
                        onUpdateFilter(filter.id, { value: val === 'true' ? true : false })
                      }
                      className="w-full"
                    />
                  ) : (
                    <input
                      ...
                      onChange={(e) =>
                        onUpdateFilter(filter.id, {
                          value:
                            getFilterInputType(filter.columnId) === 'checkbox'
                              ? e.target.checked
                              : e.target.value,
                        })
                      }
            ...
            <button
              onClick={onAddFilter}
              className="w-full mt-2 px-3 py-1.5 text-sm text-sky-600 hover:bg-sky-100 rounded-md border border-sky-200"
            >
      ...
      <div className="relative" ref={sortDropdownRef}>
        ...
        >
          <SortAscendingIcon className="w-4 h-4 mr-1" /> Sort{' '}
            ...
            {view.sorts.map((sortRule: any) => (
              ...
              >
                <SelectInput
                  ...
                  onChange={(val) => onUpdateSort(sortRule.id, { columnId: val })}
                  className="flex-grow"
                />
                <SelectInput
                  options={[
                  ...
                  onChange={(val) =>
                    onUpdateSort(sortRule.id, { direction: val as 'ASC' | 'DESC' })
                  }
                  className="w-32"
                />
                <button
                  onClick={() => onDeleteSort(sortRule.id)}
                  className="text-red-500 hover:text-red-700 p-1"
                  title="Delete sort rule"
      ...
      >
        ...
        <p className="text-xs text-slate-500 mt-2">
          Note: Calendar and Gallery views are placeholders.
        </p>
      </Modal>
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end space-x-2">
            <button
              onClick={() => setIsConfigureKanbanModalOpen(false)}
              className="px-4 py-2 text-sm text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-md"
              aria-label="Cancel"
            >
              Cancel
      ...
      >
        <div className="space-y-3">
          <label htmlFor="kanbanGroupBy" className="block text-sm font-medium text-slate-700">
            Group by column:
          </label>
          <SelectInput
          ...
          />
          {!kanbanGroupByColumnId && (
            <p className="text-xs text-red-500">Please select a column to group by.</p>
          )}
          <p className="text-xs text-slate-500 mt-1">
            Choose a column whose unique values will become the Kanban lanes.
          </p>
        </div>
      </Modal>
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end space-x-2">
            <button
              onClick={() => setIsConfigureTimelineModalOpen(false)}
              className="px-4 py-2 text-sm text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-md"
              aria-label="Cancel"
            >
              Cancel
      ...
      >
        <div className="space-y-4">
          <div>
            ...
            >
              Start Date Column <span className="text-red-500">*</span>
            </label>
            <SelectInput
            ...
            {!timelineStartDateCol && (
              <p className="text-xs text-red-500 mt-1">Start date column is required.</p>
            )}
          </div>
          <div>
            <label
            ...
            />
            <p className="text-xs text-slate-500 mt-1">
              If not set, items will have a default duration.
            </p>
          </div>
          <div>
            <label
            ...
            <SelectInput
              options={[
                { value: '', label: 'Default (Primary Column)' },
                ...allColumnOptionsForLabel,
              ]}
            ...
            />
            <p className="text-xs text-slate-500 mt-1">
              Column to display as the item label on the timeline.
            </p>
          </div>
        </div>
      </Modal>
