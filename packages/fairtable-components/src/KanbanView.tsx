...
const KanbanView: React.FC<KanbanViewProps> = ({
  ...
  const [draggedItem, setDraggedItem] = useState<{ rowId: string; originalLaneId: string } | null>(null);
  ...
  if (!groupByColumn) {
    return <div className="p-4 text-center text-red-500">Kanban view configuration error: Group By column not found.</div>;
  }
  ...
  return (
    <div className="flex-grow overflow-x-auto overflow-y-hidden p-4 flex space-x-4 bg-slate-100 custom-scrollbar">
      {lanes.map(lane => (
        ...
        >
          <div className={`p-3 border-b border-slate-300 ${lane.colorClass ? `${lane.colorClass} text-opacity-75 rounded-t-lg` : 'text-slate-700'}`}>
            <h3 className={`font-semibold text-sm truncate ${lane.colorClass ? '' : ''}`}>{lane.title}</h3>
            <span className="text-xs opacity-80">{lane.rows.length} card{lane.rows.length === 1 ? '' : 's'}</span>
          </div>
            ...
            {lane.rows.map(row => (
              ...
              >
                ...
                {columnsToDisplay.slice(0, 3).map(col => {
                    if(col.id === primaryDisplayColumn?.id || col.id === groupByColumn.id) return null;
                    ...
                    let displayValue: React.ReactNode = String(cellValue ?? '');
                     if (cellValue === null || cellValue === undefined || String(cellValue).trim() === '') {
                        ...
                        displayValue = <button 
                                          onClick={() => col.linkedTableId && onOpenLinkRecordModal(row.id, col.id, col.linkedTableId, cellValue as string[])}
                                          className="text-xs text-sky-600 hover:underline"
                                       >
                                          {cellValue.length} link(s)
                                       </button>
                    } else if (col.type === DataType.BOOLEAN) {
                        displayValue = <input type="checkbox" checked={!!cellValue} readOnly className="form-checkbox h-3.5 w-3.5"/>
                    }
