...
const DataTypeDisplay: React.FC<{ type: DataType }> = ({ type }) => {
  ...
  switch (type) {
    case DataType.LINKED_RECORD:
      iconComponent = <LinkIcon className="w-3.5 h-3.5" />;
      break;
    case DataType.FORMULA:
      iconComponent = <FormulaIcon className="w-3.5 h-3.5" />;
      break;
    case DataType.DATE:
      iconComponent = <DateIcon className="w-3.5 h-3.5" />;
      break;
    case DataType.ATTACHMENT:
      iconComponent = <AttachmentIcon className="w-3.5 h-3.5" />;
      break;
    case DataType.URL:
      iconComponent = <UrlIcon className="w-3.5 h-3.5" />;
      break;
    case DataType.EMAIL:
      iconComponent = <EmailIcon className="w-3.5 h-3.5" />;
      break;
    case DataType.CREATED_TIME:
    case DataType.LAST_MODIFIED_TIME:
      iconComponent = <ClockIcon className="w-3.5 h-3.5" />;
      break;
    case DataType.VOTES:
      iconComponent = <ArrowUpIcon className="w-3.5 h-3.5" />;
      break;
    default:
      iconComponent = <span className="text-xs">{typeInfo?.icon || type.charAt(0)}</span>;
  }
  return (
...
const ColumnHeader: React.FC<ColumnHeaderProps> = ({
  ...
  const [editingLinkedTableId, setEditingLinkedTableId] = useState<string | undefined>(
    column.linkedTableId
  );
  const [editingFormulaString, setEditingFormulaString] = useState<string | undefined>(
  ...
  const handleAddOption = () => {
    ...
    setEditingOptions([
      ...editingOptions,
      { id: generateId(), name: newOptionName.trim(), colorClass: selectedColor },
    ]);
    setNewOptionName('');
  };
  ...
  return (
    <th
      ...
      style={{
        minWidth: `${column.width || DEFAULT_COLUMN_WIDTH}px`,
    ...
    >
      <div className="flex items-center justify-between h-full px-2 py-2">
        {!readOnlyColumnType && (
          <div
            className="flex items-center cursor-grab mr-1 text-slate-400 group-hover:text-slate-500"
        ...
        <div className="flex items-center overflow-hidden flex-grow">
          ...
          <EditableText
            ...
            disabled={
              readOnlyColumnType && column.type !== DataType.VOTES /* Votes name can be edited */
            }
          />
        </div>
        {!(
          column.type === DataType.CREATED_TIME || column.type === DataType.LAST_MODIFIED_TIME
        ) && ( // No menu for pure read-only types
          <div ref={menuRef} className="relative">
            ...
            {isMenuOpen && (
              <div className="absolute right-0 mt-1 w-56 bg-white rounded-md shadow-lg py-1 z-20 border border-slate-200">
                {column.type !== DataType.VOTES && (
                  /* Votes type cannot be changed from menu */ <button
                    onClick={() => {
      ...
      >
        <div className="space-y-4">
          <div>
            <label htmlFor="columnName" className="block text-sm font-medium text-slate-700 mb-1">
              Column Name
            </label>
            <input
              id="columnName"
          ...
          <div>
            <label htmlFor="columnType" className="block text-sm font-medium text-slate-700 mb-1">
              Column Type
            </label>
            <SelectInput
              options={dataTypeOptionsForSelect.filter(
                (dt) =>
                  dt.value !== DataType.VOTES &&
                  dt.value !== DataType.CREATED_TIME &&
                  dt.value !== DataType.LAST_MODIFIED_TIME
              )} // Cannot change TO Votes type, or other read-only
              value={editingType}
          ...
          {editingType === DataType.SINGLE_SELECT && (
            <div className="border-t border-slate-200 pt-4 mt-4">
              ...
              <div className="mt-3 flex items-center space-x-2">
                ...
                />
                <SelectInput
                  options={SINGLE_SELECT_COLOR_PALETTE.map((c: any) => ({
                    value: c.value,
                    label: (
                      <span className={`px-2 py-0.5 text-xs rounded-full ${c.value}`}>
                        {c.name}
                      </span>
                    ),
                  }))}
          ...
          {editingType === DataType.LINKED_RECORD && (
            <div className="border-t border-slate-200 pt-4 mt-4">
              ...
              />
              {tableOptionsForSelect.length === 0 && (
                <p className="text-xs text-slate-500 mt-1">No other tables available to link.</p>
              )}
            </div>
          )}
          {editingType === DataType.FORMULA && (
            <div className="border-t border-slate-200 pt-4 mt-4">
              ...
              <p className="text-xs text-slate-500 mt-1">
                Note: Formula evaluation is not yet fully implemented.
              </p>
            </div>
          )}
        </div>
