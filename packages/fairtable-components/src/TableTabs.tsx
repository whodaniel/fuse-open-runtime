...
import { PlusIcon, TableCellsIcon, TrashIcon } from './Icons';
  ...
  return (
    <div className="flex items-center border-b border-slate-300 bg-slate-100 px-2 pt-2 custom-scrollbar overflow-x-auto">
      {tables.map((table) => (
        ...
        >
          <TableCellsIcon
            className={`w-4 h-4 mr-2 ${activeTableId === table.id ? 'text-sky-500' : 'text-slate-500 group-hover:text-slate-700'}`}
          />
          <EditableText
      ...
      >
        <p className="text-sm text-slate-600">
          ...
          action cannot be undone. Any columns in other tables linking to this table will have their
          links cleared.
        </p>
      </Modal>
    </div>
