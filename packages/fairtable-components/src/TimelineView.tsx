import {
  ...
  Row,
  Table,
  TIMELINE_DEFAULT_ITEM_DURATION_DAYS,
  TimelineViewOptions,
  View,
...
const TimelineView: React.FC<TimelineViewProps> = ({
  ...
  const startDateColumn = useMemo(
    () => table.columns.find((c: Column) => c.id === startDateColumnId && c.type === DataType.DATE),
    [table.columns, startDateColumnId]
  );
  const endDateColumn = useMemo(
    () => table.columns.find((c: Column) => c.id === endDateColumnId && c.type === DataType.DATE),
    [table.columns, endDateColumnId]
  );
  const labelColumn = useMemo(
    () =>
      table.columns.find((c: Column) => c.id === labelColumnId) ||
      table.columns.find((c: Column) => c.id === table.columnOrder[0]),
    [table.columns, labelColumnId, table.columnOrder]
  );
    ...
    return rowsToDisplay
      .map((row) => {
        ...
        const label = labelColumn
          ? String(row.data[labelColumn.id] ?? `Item ${row.id.slice(-4)}`)
          : `Item ${row.id.slice(-4)}`;
  ...
  if (!startDateColumnId || !startDateColumn) {
    return (
      <div className="p-4 text-center text-red-500">
        Timeline view requires a valid Start Date column (Date type) to be configured.
      </div>
    );
  }
  if (endDateColumnId && !endDateColumn) {
    return (
      <div className="p-4 text-center text-red-500">
        Timeline view has an End Date column configured, but it's not a valid Date type column.
      </div>
    );
  }
  ...
  return (
    <div className="flex-grow overflow-auto custom-scrollbar bg-white">
      <div style={{ width: LABEL_WIDTH + totalDays * DAY_WIDTH, position: 'relative' }}>
        {/* Header: Dates */}
        <div
          className="sticky top-0 z-20 flex bg-slate-50 border-b border-slate-300"
        ...
        <div className="relative">
          {timelineItems.map((item, rowIndex) => {
            ...
            return (
              <div
                ...
                style={{
                  height: ROW_HEIGHT_TIMELINE,
                  top: HEADER_HEIGHT + rowIndex * ROW_HEIGHT_TIMELINE,
                }}
              >
                <div className="sticky left-0 z-10 w-[150px] px-2 py-1 border-r border-slate-300 bg-white group-hover:bg-slate-50 flex items-center">
                  <span className="text-xs text-slate-700 truncate" title={item.label}>
                    {item.label}
                  </span>
                </div>
                <div
          ...
          {dateHeaders.map(({ dayIndex }) => (
            <div
              ...
              style={{
                left: LABEL_WIDTH + dayIndex * DAY_WIDTH,
                zIndex: -1,
                height: HEADER_HEIGHT + timelineItems.length * ROW_HEIGHT_TIMELINE,
              }}
            ></div>
          ))}
        </div>
        {timelineItems.length === 0 && (
          <div
            ...
            style={{ top: HEADER_HEIGHT, position: 'absolute', width: '100%' }}
          >
