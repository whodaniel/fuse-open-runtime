...
import { WorkflowStep, WorkflowMetrics, MonitoringEvent } from '../types.js';
...
export class WorkflowMonitor extends EventEmitter {
  ...
  updateMetrics(workflowId: string, metrics: Partial<WorkflowMetrics>): void {
    const currentMetrics = this.activeWorkflows.get(workflowId) || {} as WorkflowMetrics;
    const updatedMetrics = { ...currentMetrics, ...metrics } as WorkflowMetrics;
