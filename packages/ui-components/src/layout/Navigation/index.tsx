...
const Navigation: React.FC = () => {
    ...
    const renderRoutes = (routeObj: RouteConfig, level = 0) => {
        return Object.entries(routeObj).map(([key, value]) => {
            if (typeof value === 'string') {
                return (
                    <div key={value} className={`pl-${level * 4} py-2 hover:bg-gray-100 dark:hover:bg-gray-800`}>
                        <Link to={value} className="text-blue-600 dark:text-blue-400 hover:underline flex items-center">
                ...
                return (
                    <div key={key} className={`pl-${level * 4} py-2`}>
                        <h3 className="font-semibold text-gray-800 dark:text-gray-200 mb-2">
                            {key}
                        </h3>
                        {renderRoutes(value, level + 1)}
                    </div>
                );
            }
        });
            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="mt-8 p-4 bg-blue-50 dark:bg-blue-900 rounded-lg">
                    ...
                    <div className="flex flex-wrap gap-4">
                        <Link to="/admin/overview" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                            Admin Dashboard
                        </Link>
                        <Link to="/api/docs" className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">
                            API Documentation
                        </Link>
                        <Link to="/status" className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">
                            System Status
                        </Link>
                    </div>
                </div>
