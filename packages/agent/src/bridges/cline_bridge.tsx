...
export class ClineBridge {
  ...
  constructor() {
    ...
    this.communication = new DirectCommunication('cline_ai', 'cascade_ai', AgentRole.ARCHITECT);
  }
  ...
  async initialize(): Promise<boolean> {
    try {
      ...
      this.logger.error(
        `Failed to initialize Cline bridge: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
  ...
  async sendTask(task: unknown): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to send task: ${error instanceof Error ? error.message : String(error)}`
      );
      throw error;
    }
  }
  ...
  async onResult(callback: (result: unknown) => Promise<void>): Promise<void> {
    try {
      ...
      this.client.on('message', async (channel: string, message: string) => {
        if (channel === 'AI_RESULT_CHANNEL') {
          try {
            ...
            this.logger.error(
              `Error processing result: ${error instanceof Error ? error.message : String(error)}`
            );
          }
        }
      });
      ...
      this.logger.error(
        `Failed to set up result handler: ${error instanceof Error ? error.message : String(error)}`
      );
      throw error;
    }
  }
