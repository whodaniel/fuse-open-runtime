 ...
 */
class MCPErrorHandler extends events_1.EventEmitter {
    config;
    logger;
    ...
    constructor(config = {}, logger) {
        ...
        this.logger = logger || new Logger_1.Logger('MCPErrorHandler');
        this.statistics = {
            totalErrors: 0,
            errorsByCategory: {},
            errorsBySeverity: {},
  ...
  registerErrorHandler(errorCode: number, handler: ErrorHandler): void {
    this.errorHandlers.set(errorCode, handler);`, this.logger.debug(`Registered error handler for code: ${errorCode}`));
    }
    /**
  ...
  private logError(error: MCPErrorClass, context: ErrorContext): void {
    ...
    switch (error.severity) {`, error_1.ErrorSeverity.CRITICAL, `
        this.logger.error(`, CRITICAL, ERROR, $, { error, : .message }, logData);
                break;
                error_1.ErrorSeverity.HIGH;
                `
  ...
  private initializeDefaultErrorHandlers(): void {
    ...
    this.registerErrorHandler(-1, {
      ...
      canHandle: () => true,
      handle: async (error, context) => {`, this.logger.debug(Generic, handler, processing, error, $, { error, : .code } `` `);
