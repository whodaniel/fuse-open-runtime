...
describe('CircuitBreaker', () => {
    ...
    beforeEach(() => {
        ...
        circuitBreaker = new CircuitBreaker_1.CircuitBreaker('test-circuit', {
            failureThreshold: 3,
            successThreshold: 2,
            timeout: 1000,
    ...
    describe('State Transitions', () => {
        ...
        it('should transition to half-open after timeout', async () => {
            const shortTimeoutCircuit = new CircuitBreaker_1.CircuitBreaker('short-timeout', {
                failureThreshold: 2,
            ...
            expect(shortTimeoutCircuit.getState()).toBe(CircuitBreaker_1.CircuitState.OPEN);
            // Wait for timeout
        ...
        it('should close circuit after successful operations in half-open state', async () => {
            const shortTimeoutCircuit = new CircuitBreaker_1.CircuitBreaker('half-open-test', {
                failureThreshold: 2,
                successThreshold: 2,
                timeout: 50
            }, mockLogger);
            ...
            for (let i = 0; i < 2; i++) {
                ...
                if (i === 0) {
                    expect(shortTimeoutCircuit.getState()).toBe(CircuitBreaker_1.CircuitState.HALF_OPEN);
                }
            }
            expect(shortTimeoutCircuit.getState()).toBe(CircuitBreaker_1.CircuitState.CLOSED);
        });
        it('should reopen circuit on failure in half-open state', async () => {
            const shortTimeoutCircuit = new CircuitBreaker_1.CircuitBreaker('reopen-test', {
                failureThreshold: 2,
                timeout: 50
            }, mockLogger);
            // Open the circuit
    ...
    describe('Events', () => {
        it('should emit state change events', (done) => {
            circuitBreaker.once('stateChange', (name, oldState, newState) => {
                ...
                expect(oldState).toBe(CircuitBreaker_1.CircuitState.CLOSED);
                expect(newState).toBe(CircuitBreaker_1.CircuitState.OPEN);
                done();
            });
    ...
    describe('Rolling Window', () => {
        it('should use rolling window for failure detection', async () => {
            const rollingWindowCircuit = new CircuitBreaker_1.CircuitBreaker('rolling-window', {
                failureThreshold: 3,
                rollingWindowSize: 5,
            ...
            expect(rollingWindowCircuit.getState()).toBe(CircuitBreaker_1.CircuitState.OPEN);
        });
    });
});
describe('CircuitBreakerManager', () => {
    ...
    beforeEach(() => {
        ...
        manager = new CircuitBreaker_1.CircuitBreakerManager(mockLogger);
    });
    describe('Circuit Breaker Management', () => {
        ...
        it('should reset all circuit breakers', async () => {
            ...
            expect(cb1.getState()).toBe(CircuitBreaker_1.CircuitState.OPEN);
            expect(cb2.getState()).toBe(CircuitBreaker_1.CircuitState.OPEN);
            manager.resetAll();
            expect(cb1.getState()).toBe(CircuitBreaker_1.CircuitState.CLOSED);
            expect(cb2.getState()).toBe(CircuitBreaker_1.CircuitState.CLOSED);
        });
    });
