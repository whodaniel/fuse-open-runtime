...
describe('System Resilience Integration', () => {
    ...
    beforeEach(() => {
        ...
        circuitBreakerManager = new CircuitBreaker_1.CircuitBreakerManager(mockLogger);
        const degradationConfig = {
            serviceName: 'test-service',
            levels: {
                [GracefulDegradation_1.ServiceLevel.FULL]: {
                    name: GracefulDegradation_1.ServiceLevel.FULL,
                    description: 'Full service',
                    availableFeatures: ['read', 'write', 'delete'],
                    disabledFeatures: [],
                    fallbackHandlers: new Map()
                },
                [GracefulDegradation_1.ServiceLevel.DEGRADED]: {
                    name: GracefulDegradation_1.ServiceLevel.DEGRADED,
                    description: 'Degraded service',
                    availableFeatures: ['read', 'write'],
                    disabledFeatures: ['delete'],
                    fallbackHandlers: new Map()
                },
                [GracefulDegradation_1.ServiceLevel.MINIMAL]: {
                    name: GracefulDegradation_1.ServiceLevel.MINIMAL,
                    description: 'Minimal service',
                    availableFeatures: ['read'],
                    disabledFeatures: ['write', 'delete'],
                    fallbackHandlers: new Map()
                },
                [GracefulDegradation_1.ServiceLevel.OFFLINE]: {
                    name: GracefulDegradation_1.ServiceLevel.OFFLINE,
        ...
        degradationManager = new GracefulDegradation_1.GracefulDegradationManager(degradationConfig, mockLogger);
        ...
        }, degradationManager, mockLogger);
        errorHandler = new MCPErrorHandler_1.MCPErrorHandler({
            enableAutoRecovery: true,
            maxRecoveryAttempts: 2,
            statisticsInterval: 0,
            enableLogging: true
        }, mockLogger);
        errorMonitor = new ErrorMonitor_1.ErrorMonitor({
            metricsInterval: 100,
    ...
    describe('Circuit Breaker Integration', () => {
        ...
        it('should coordinate circuit breaker with graceful degradation', async () => {
            ...
            degradationManager.registerFallbackHandler(GracefulDegradation_1.ServiceLevel.DEGRADED, 'read', {
                name: 'cached-read',
    ...
    describe('Failover Integration', () => {
        it('should integrate failover with circuit breakers', async () => {
            ...
            let tertiaryCalls = 0;
            const serviceCall = async (endpoint) => {
                switch (endpoint.id) {
                    case 'primary':
                        primaryCalls++;
                        if (primaryCalls <= 3) {
        ...
        it('should trigger degradation when all endpoints fail', async () => {
            ...
            expect(degradationManager.getServiceStatus().currentLevel).not.toBe(GracefulDegradation_1.ServiceLevel.FULL);
        });
    });
    describe('Error Monitoring Integration', () => {
        it('should monitor errors across all resilience components', async () => {
            ...
            const errors = [
                new error_1.MCPErrorClass(error_1.MCPErrorCode.CONNECTION_TIMEOUT, 'Connection timeout'),
                new error_1.MCPErrorClass(error_1.MCPErrorCode.SERVICE_UNAVAILABLE, 'Service unavailable'),
                new error_1.MCPErrorClass(error_1.MCPErrorCode.RESOURCE_NOT_FOUND, 'Resource not found'),
                new error_1.MCPErrorClass(error_1.MCPErrorCode.SYSTEM_OVERLOADED, 'System overloaded', { severity: error_1.ErrorSeverity.CRITICAL })
            ];
...
it('should trigger alerts based on system resilience state', async () => {
    ...
    for (let i = 0; i < 10; i++) {
        errorMonitor.recordError(`
          new MCPErrorClass(MCPErrorCode.SYSTEM_OVERLOADED, Error ${i}`, { severity: error_1.ErrorSeverity.HIGH });
    }
});
...
describe('End-to-End Resilience Scenarios', () => {
    it('should handle cascading failures gracefully', async () => {
        ...
        degradationManager.registerFallbackHandler(GracefulDegradation_1.ServiceLevel.MINIMAL, 'read', {
        ...
        const cascadingFailureService = async (endpoint) => {
...
//# sourceMappingURL=SystemResilience.integration.test.js.map