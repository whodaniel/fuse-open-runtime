...
describe('ConnectionManager', () => {
    ...
    describe('Connection Creation', () => {
        ...
        it('should enforce connection pool limits', async () => {
            ...
            await expect(connectionManager.createConnection('ws://localhost:8085', defaultOptions)).rejects.toThrow('Connection pool limit exceeded');
        });
        it('should handle connection timeout', async () => {
            // Mock WebSocket that never connects
            class TimeoutWebSocket extends events_1.EventEmitter {
                readyState = WebSocket.CONNECTING;
                constructor(url) {
                    super();
            ...
            global.WebSocket = TimeoutWebSocket;
            const shortTimeoutOptions = { ...defaultOptions, timeout: 50 };
            await expect(connectionManager.createConnection('ws://localhost:8080', shortTimeoutOptions)).rejects.toThrow('Connection timeout');
            // Restore mock
            global.WebSocket = MockWebSocket;
        });
    });
    describe('Connection Management', () => {
        ...
        it('should get connection status correctly', async () => {
            const endpoint = 'ws://localhost:8080';
            expect(connectionManager.getConnectionStatus(endpoint)).toBe(IMCPConnection_1.ConnectionStatus.DISCONNECTED);
            await connectionManager.createConnection(endpoint, defaultOptions);
            expect(connectionManager.getConnectionStatus(endpoint)).toBe(IMCPConnection_1.ConnectionStatus.CONNECTED);
        });
    ...
    describe('Connection Retry Logic', () => {
        it('should retry connection with exponential backoff', async () => {
            ...
            class RetryWebSocket extends events_1.EventEmitter {
                readyState = WebSocket.CONNECTING;
                constructor(url) {
                    super();
                    attemptCount++;
        ...
        it('should fail after max retry attempts', async () => {
            ...
            class FailingWebSocket extends events_1.EventEmitter {
                readyState = WebSocket.CONNECTING;
                constructor(url) {
                    super();
                    setTimeout(() => {
            ...
            await expect(connectionManager.createConnection('ws://localhost:8080', defaultOptions)).rejects.toThrow('Connection failed');
            // Restore mock
            global.WebSocket = MockWebSocket;
        });
    });
    ...
    describe('Error Handling', () => {
        it('should handle WebSocket errors gracefully', async () => {
            ...
            class ErrorWebSocket extends events_1.EventEmitter {
                readyState = WebSocket.CONNECTING;
                constructor(url) {
                    super();
                    setTimeout(() => {
