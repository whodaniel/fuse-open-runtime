...
class TheNewFuseMCPServer {
    ...
    setupToolHandlers() {
        ...
        throw new McpError(
          ErrorCode.InternalError,` `Error executing tool ${name}`, $, { error, instanceof: Error ? error.message : String(error) });
                }
            }
            finally { }
        });
...
`;
    ...
    ;
    transport = new sse_js_1.SSEServerTransport('/message', httpServer);
}
