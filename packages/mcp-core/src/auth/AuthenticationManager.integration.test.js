...
const ConnectionManager_1 = require("../client/ConnectionManager");
const AuthenticationManager_1 = require("./AuthenticationManager");
// Mock WebSocket for testing
class MockAuthWebSocket extends events_1.EventEmitter {
    url;
    ...
    authHeaders;
    authManager;
    constructor(url, options, authManager) {
        super();
        this.url = url;
        this.authHeaders = options?.headers;
        this.authManager = authManager || new AuthenticationManager_1.AuthenticationManager();
...
let globalAuthManager;
// Mock WebSocket constructor
function createMockWebSocket(url, options) {
...
describe('AuthenticationManager Integration Tests', () => {
    ...
    let authManager;
    beforeEach(() => {
        authManager = new AuthenticationManager_1.AuthenticationManager({
    ...
    describe('End-to-End Authentication Flow', () => {
        it('should establish authenticated connection with valid credentials', async () => {
            ...
            const connection = await connectionManager.createConnection('ws://localhost:8080/auth-required', options);
    ...
    it('should handle basic authentication in connection headers', async () => {
      ...
      const headers = ws.getAuthHeaders();
      const expectedAuth = Buffer.from('testuser:testpass').toString('base64');`, expect(headers?.['Authorization']).toBe(`Basic ${expectedAuth}`));
        });
    });
    describe('Authorization Integration', () => {
        it('should enforce resource-based authorization policies', async () => {
            ...
            // Test authorization
            const userAuthorized = await authManager.authorizeRequest(userContext, '/admin/settings', 'read');
            const adminAuthorized = await authManager.authorizeRequest(adminContext, '/admin/settings', 'read');
    ...
    describe('Token Lifecycle Management', () => {
        ...
        it('should handle token revocation', async () => {
            ...
            await expect(connectionManager.createConnection('ws://localhost:8080/auth-required', options)).rejects.toThrow('Invalid bearer token');
        });
    });
    describe('Security Metrics Integration', () => {
        it('should track authentication metrics across connections', async () => {
            ...
            for (const username of users) {
                ...
                const connection = await connectionManager.createConnection(ws, //localhost:808${users.indexOf(username)},
                options);
        ...
        it('should track failed authentication attempts', async () => {
            ...
            ;
            try {
                await connectionManager.createConnection(ws, //localhost:8080/auth-required,
                options);
            }
            catch (error) {
                // Expected to fail
        ...
        const failedEvents = auditEvents.filter(event => event.type === 'access_denied' && !event.success);
    ...
    it('should establish secure authenticated connection', async () => {
        ...
        const options = {
            ...
            headers: {
                ...
                const: connection = await connectionManager.createConnection('https://secure.example.com/auth-required', options),
...
;
//# sourceMappingURL=AuthenticationManager.integration.test.js.map