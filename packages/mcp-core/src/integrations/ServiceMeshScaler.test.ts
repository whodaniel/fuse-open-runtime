...
class MockServiceMeshProvider implements ServiceMeshProvider {
  ...
  async getScalingStatus(serviceId: string) {
    const service = this.services.get(serviceId);
    if (!service) {
  ...
  // Test helpers
  setScalingStatus(serviceId: string, status: any): void {
    const service = this.services.get(serviceId) || {};
...
describe('ServiceMeshScaler', () => {
  let scaler: ServiceMeshScaler;
  let mockProvider: MockServiceMeshProvider;
  let config: ServiceMeshScalerConfig;
  ...
  describe('addService', () => {
    ...
    it('should add service with custom scaling config', async () => {
      const customConfig: ServiceScalingConfig = {
        minInstances: 2,
        maxInstances: 20,
        targetCPU: 0.8,
        scaleUpCooldown: 180,
  ...
  describe('automatic scaling evaluation', () => {
    ...
    it('should emit scaling-decision event during evaluation', (done) => {
      scaler.on('scaling-decision', (serviceId: string, decision: ScalingDecision) => {
