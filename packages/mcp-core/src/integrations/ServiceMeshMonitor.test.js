...
const ServiceMeshMonitor_1 = require("./ServiceMeshMonitor");
...
class MockServiceMeshProvider {
    ...
    // Test helpers
    setServiceHealth(serviceId, health) {
        const service = this.services.get(serviceId) || {};
        service.health = {
            serviceId,
            status: common_1.ServiceStatus.ONLINE,
...
describe('ServiceMeshMonitor', () => {
    ...
    beforeEach(() => {
        ...
        monitor = new ServiceMeshMonitor_1.ServiceMeshMonitor(mockProvider, config);
    });
    afterEach(async () => {
    ...
    describe('getServicesByHealthStatus', () => {
        beforeEach(async () => {
            ...
            mockProvider.setServiceHealth('healthy-service', { status: common_1.ServiceStatus.ONLINE });
            mockProvider.setServiceHealth('unhealthy-service', { status: common_1.ServiceStatus.OFFLINE });
    ...
    describe('alerting', () => {
        ...
        it('should generate health score alert when below threshold', (done) => {
            ...
            monitor.on('alert', (serviceId, alert) => {
                expect(serviceId).toBe('test-service');
...
//# sourceMappingURL=ServiceMeshMonitor.test.js.map