...
export const RelayIntegration = {
     ...
     */
    async registerMCPService(serviceInfo, config) {
        if (!relayCore || !relayCore.registerService) {
            return false;
        }
        try {
            const relayServiceInfo = {
                id: serviceInfo.id,
     ...
     */
    async unregisterMCPService(serviceId) {
    ...
    subscribeToEvents(eventTypes, callback) {
        ...
        try {
            const subscription = relayCore.subscribe(eventTypes, (event) => {
    ...
    async discoverServices(query) {
        ...
        try {
            ...
            return services.filter((service) => service.metadata?.protocol === 'mcp');
        }
        catch (error) {
     ...
     */
    setupHeartbeat(serviceId, config) {
        ...
        return {
            ...
            createEventBridge() {
                ...
                return {
                    ...
                    publish: async (eventType, data, metadata) => {
