...
const KubernetesServiceMeshProvider_1 = require("./providers/KubernetesServiceMeshProvider");
const common_1 = require("../types/common");
describe('MCPServiceMesh Integration Tests', () => {
    let serviceMesh;
    let kubernetesProvider;
    let mockMCPService;
    let mockMeshRegistration;
    beforeAll(() => {
        // Initialize Kubernetes provider with test configuration
        kubernetesProvider = new KubernetesServiceMeshProvider_1.KubernetesServiceMeshProvider({
        ...
        serviceMesh = new MCPServiceMesh_1.MCPServiceMesh({
    ...
    describe('End-to-End Service Registration and Management', () => {
        it('should register, monitor, and manage a complete MCP service lifecycle', async () => {
            ...
            const registrationResult = await serviceMesh.registerService(mockMCPService, mockMeshRegistration);
            ...
            expect(health.status).toBe(common_1.ServiceStatus.ONLINE);
            ...
            const scalingResult = await serviceMesh.configureScaling('integration-test-service', scalingConfig);
            ...
            const healthUpdateResult = await serviceMesh.updateServiceHealth('integration-test-service', updatedHealth);
            expect(healthUpdateResult.success).toBe(true);
            expect(healthUpdateResult.metadata?.healthStatus).toBe(common_1.ServiceStatus.DEGRADED);
            ...
            expect(updatedHealthCheck.status).toBe(common_1.ServiceStatus.DEGRADED);
...
it('should register and manage multiple services simultaneously', async () => {
    ...
    const registrationPromises = services.map(({ mcp, mesh }) => serviceMesh.registerService(mcp, mesh));
    ...
    it('should handle concurrent operations correctly', async () => {
      const concurrentServices = Array.from({ length: 5 }, (_, i) => ({
        mcp: {
          ...
          ...mockMeshRegistration,
          serviceId: concurrent-service-${i}`, serviceName, concurrent - service - $, { i } `
        }
      }));
...
//# sourceMappingURL=MCPServiceMesh.integration.test.js.map