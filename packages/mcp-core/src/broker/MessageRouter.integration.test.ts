...
describe('MessageRouter Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    messageRouter['sendNotificationToService'] = jest
      .fn()
      .mockImplementation(async (notification, serviceId) => {
  ...
  describe('Broadcast Notifications', () => {
    ...
    it('should handle partial failures in broadcast gracefully', async () => {
      ...
      await expect(
        messageRouter.broadcastNotification(broadcastNotification)
  ...
  describe('Performance and Metrics', () => {
    ...
    it('should handle high-throughput routing scenarios', async () => {
      ...
      const responses = await Promise.all(requests.map((req) => messageRouter.routeRequest(req)));
      ...
      console.log(
        `Processed ${requestCount} requests in ${totalTime}ms (${((requestCount / totalTime) * 1000).toFixed(2)} req/sec)`
      );
    });
  });
