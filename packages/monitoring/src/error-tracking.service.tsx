...
export class ErrorTrackingService {
  ...
  async getErrorStats(): Promise<void> {fingerprint: string): Promise<ErrorStats | null> {
    try {
      ...
      await this.drizzle.(errorEvent as any).create({
        data await this.drizzle.(errorEvent as any).findMany({
  ...
  async listRecentErrors(page  = (error as any).timestamp.toISOString().split(): Promise<void> {'T')[0];
    ...
    try {
      ...
      const [errors, total] = await (Promise as any).all([
        this.drizzle.(errorEvent as any).findMany({
          where,
          orderBy: { timestamp: desc' },
          skip: (page - 1) * limit,
          take: limit,
        }),
        this.drizzle.(errorEvent as any).count({ where }),
      ]);
