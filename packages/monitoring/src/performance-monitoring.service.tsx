...
export class PerformanceMonitoringService {
  ...
  async recordMetric(metric this.normalizeMetric(): Promise<void> {metric);
      ...
      // Store in database
      await this.db.(performanceMetric as any).create({
        data true,
    tags = {},
  }: {
    operation: string;
  ...
  async getMetrics(): Promise<void> {{
    ...
    try {
      ...
      const [metrics, total]   = await this.db.(performanceMetric as any).findMany( {
          where,
          orderBy(this as any) (aggregatedMetrics as any).slice((page - 1) * limit, page * limit);
        
        return { metrics await (Promise as any).all([
        this.db.(performanceMetric as any).findMany({
          where,
          orderBy: { timestamp: desc' },
          skip: (page - 1) * limit,
          take: limit,
        }),
        this.db.(performanceMetric as any).count({ where }): (metric as any).tags ? (JSON as any).parse((metric as any).tags: unknown): null,
  ...
  private normalizeMetric(metric: PerformanceMetric): PerformanceMetric {
    ...
    const matchingThresholds): void {
      const isExceeded: unknown){
        ...
        try {
          await this.db.(performanceAlert as any).create({
            data: {
