...
var __esDecorate =
  ...
  function (ctor, descriptorIn, decorators, contextIn, initializers, extraInitializers): any {
    function accept(f): any {
...
var __runInitializers =
  ...
  function (thisArg, initializers, value): any {
...
let AG2Bridge = (exports.AG2Bridge = (() => {
  ...
  var AG2Bridge = (_classThis = class {
    constructor(config?: AG2BridgeConfig) {
      ...
      this.config = {
        pythonServiceUrl:
          config?.pythonServiceUrl ?? process.env.AG2_PYTHON_URL ?? 'http://localhost:8000',
    ...
    async getMessages(conversationId: string): Promise<any[]> {
      try {
        const response = await this.withRetry(() =>
          this.axiosInstance.get(`/messages/${conversationId}`)
        );
        if (!response.data?.data || !Array.isArray(response.data.data)) {
    ...
    async executeTool(toolId: string, args: any): Promise<any> {
      try {
        const response = await this.withRetry(() =>
          this.axiosInstance.post(`/tools/${toolId}/execute`, { args })
        );
        return response.data;
      } catch (error) {
        const errorMessage =
