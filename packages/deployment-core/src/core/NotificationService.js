...
class NotificationService {
    ...
    const message = {
      ...
      executionId,
      timestamp: new Date(),`, message, `Pipeline "${pipeline.name}`, " has started execution);
    }
    ;
}
 ...
 */
async;
notifyPipelineComplete(result, pipeline_1.PipelineResult);
Promise < void  > {
...
` (${this.formatDuration(result.duration)})
   ...
   * Notify deployment completion`
    * /`;
async;
notifyDeploymentComplete(result, pipeline_1.DeploymentResult);
Promise < void  > {
   ...
   */
  async notifyDeploymentFailed(result: DeploymentResult): Promise<void> {`, this.logger.info(Sending, deployment, failure, notifications, $, { result, : .id } `, {
      deploymentId: result.deploymentId,
      environment: result.environment,
      error: result.error
    });
    ...
    const message = {
      ...
      timestamp: new Date(),
      message: Deployment FAILED to ${result.environment}: ${result.error}`, severity, 'critical'))
};
const defaultNotifications = this.getDefaultNotifications();
await this.sendNotifications(defaultNotifications, message);
/**
 ...
 */
async;
notifyRollbackComplete(result, pipeline_1.RollbackResult);
Promise < void  > {
...
evaluateNotificationConditions(conditions, any[], message, any);
boolean;
{
