...
export class PipelineValidator {
  ...
  private validateTaskParameters(task: PipelineTask, errors: string[], taskPrefix: string): void {
    switch (task.type) {
      case 'docker':
  ...
  private validateTriggers(triggers: any[], errors: string[]): void {
  ...
  private validateEnvironment(environment: any, errors: string[]): void {
    if (!environment.name) {
  ...
  private validateQualityGates(qualityGates: any[], errors: string[]): void {
  ...
  private validateNotifications(notifications: any[], errors: string[]): void {
  ...
  private validateResources(pipeline: PipelineDefinition, errors: string[]): void {
  ...
  private validateStageConditions(conditions: any[], errors: string[], stagePrefix: string): void {
  ...
  private validateTaskConditions(conditions: any[], errors: string[], taskPrefix: string): void {
    conditions?.forEach((condition, index) => {
      if (!condition.type) {
  ...
  private validateTaskArtifacts(artifacts: any[], errors: string[], taskPrefix: string): void {
  ...
  private validateServices(services: any[], errors: string[]): void {
  ...
  private validateCanaryConfig(canaryConfig: any, errors: string[]): void {
    if (!canaryConfig.steps || canaryConfig.steps.length === 0) {
      errors.push('Canary configuration must have at least one step');
    }

    canaryConfig.steps?.forEach((step: any, index: number) => {
  ...
  private validateBlueGreenConfig(blueGreenConfig: any, errors: string[]): void {
  ...
  private validateHealthChecks(healthChecks: any[], errors: string[]): void {
  ...
  private validateApprovals(approvals: any[], errors: string[]): void {
