...
export class GCPProvider implements ResourceProvider {
  ...
  constructor(projectId: string, region: string = 'us-central1', zone: string = 'us-central1-a') {
  ...
  async import(importConfig: ResourceImportConfig): Promise<ResourceProvisionResult> {
  ...
  async refresh(resource: ResourceDefinition): Promise<ResourceDefinition> {
