...
const InfrastructureManager_1 = require("./InfrastructureManager");
...
describe('InfrastructureManager', () => {
    ...
    beforeEach(() => {
        ...
        const templateParser = new TemplateParser_1.TemplateParser();
        const stateManager = new StateManager_1.StateManager(new StateManager_1.InMemoryStateStorage());
        const resourceProvisioner = new ResourceProvisioner_1.ResourceProvisioner();
        const templateValidator = new TemplateValidator_1.TemplateValidator();
        const changeAnalyzer = new ChangeAnalyzer_1.ChangeAnalyzer();
        const metricsCollector = new MetricsCollector_1.MetricsCollector(mockLogger);
        infrastructureManager = new InfrastructureManager_1.InfrastructureManager(templateParser, stateManager, resourceProvisioner, templateValidator, changeAnalyzer, metricsCollector);
    });
    describe('provisionInfrastructure', () => {
        it('should provision infrastructure from a valid template', async () => {
            const template = {
                ...
                version: '1.0.0',
                provider: infrastructure_1.CloudProvider.GCP,
                resources: [
                    {
                        type: infrastructure_1.ResourceType.COMPUTE_ENGINE,
                        name: 'web-server',
                        properties: {
                            cpu: '500m',
                            memory: '1Gi',
        ...
        expect(result.resources[0].resourceType).toBe(infrastructure_1.ResourceType.COMPUTE_ENGINE);
    ...
    it('should fail provisioning with invalid template', async () => {
        const invalidTemplate = {
            ...
            name: '',
            version: '',
            provider: infrastructure_1.CloudProvider.KUBERNETES,
            resources: [],
            variables: [],
            outputs: [],
            dependencies: [],
    ...
    it('should update existing infrastructure', async () => {
        ...
        const template = {
            ...
            metadata: {
                ...
                expect(provisionResult) { }, : .success
            }
        };
    }).toBe(true);
    ...
    it('should list all infrastructure instances', async () => {
        ...
        const template1 = {
            ...
            metadata: {
                ...
                updatedAt: new Date(),
                version: '1.0.0',
                const: template2, InfrastructureTemplate: infrastructure_1.InfrastructureTemplate = {
                    ...
                    metadata: {
                        ...
                        expect(allInfrastructure) { }, : .length
                    }
                }
            }
        };
