        ...
        try {
            ...
            for (const rule of this.rules) {
                try {
                    ...
                    issues.push({
                        rule: rule.name,
                        severity: infrastructure_1.ErrorSeverity.ERROR,
      ...
      for (const [name, rule] of this.customRules) {
        try {
          ...
          issues.push({
            ...
            // Categorize issues
            const errors = issues
                .filter(i => i.severity === infrastructure_1.ErrorSeverity.ERROR)
                .map(i => ({
                code: i.rule,
                message: i.message,
                path: i.path,
            ...
            const warnings = issues
                .filter(i => i.severity === infrastructure_1.ErrorSeverity.WARNING)
                .map(i => ({
                code: i.rule,
                message: i.message,
                path: i.path,
            ...
            const suggestions = issues
                .filter(i => i.severity === infrastructure_1.ErrorSeverity.INFO && i.suggestion)
                .map(i => ({
            ...
            const mappedWarnings = warnings.map(warn => ({
                ...
                severity: infrastructure_1.ErrorSeverity.WARNING
            }));
            const mappedSuggestions = suggestions.map(sugg => ({
                ...sugg,
                rule: sugg.type || 'unknown',
                severity: infrastructure_1.ErrorSeverity.INFO,
            ...
            }, { error, instanceof: Error, error, message: , 'Unknown error':  }, path, severity;
        }
        ;
        return {
            isValid: false,
...
addCustomRule(name, string, rule, ValidationRule);
void {
...
getSuggestionType(ruleName, string);
infrastructure_1.SuggestionType;
{
    if (ruleName.includes('security'))
        return infrastructure_1.SuggestionType.SECURITY;
    if (ruleName.includes('cost'))
        return infrastructure_1.SuggestionType.COST;
    if (ruleName.includes('performance'))
        return infrastructure_1.SuggestionType.PERFORMANCE;
    return infrastructure_1.SuggestionType.OPTIMIZATION;
}
...
class ResourceNamingRule {
    ...
    validate(template) {
        ...
        template.resources.forEach((resource, index) => {
            ...
            severity: this.severity,`, message, 'Resource name should use lowercase letters, numbers, and hyphens only', path, template.resources[$], { index } `].name,
        ...
        if (reservedNames.includes(resource.name.toLowerCase())) {
          issues.push({
            rule: this.name,
            severity: ErrorSeverity.ERROR,
            message: Resource name "${resource.name}" is reserved,`, path, template.resources[$], { index }, name, suggestion, 'Choose a different name that is not reserved');
                }
            }
        });
    }
 ...
 > 63;
{
    issues.push({
        ...
        path: template.resources[$]
    }, { index }, name, suggestion, 'Shorten the resource name');
}
;
;
return issues;
class DependencyValidationRule {
            ...
            if (resource.dependencies) {
                resource.dependencies.forEach((dep, depIndex) => {
                    ...
                    if (!resourceNames.has(dep)) {
                        issues.push({} `
              rule: this.name,`, severity, this.severity, `
              message: Resource depends on non-existent resource: ${dep},`, path, template.resources[$], { index }, dependencies[$], { depIndex } `],
    ...
    circularDeps.forEach(cycle => {
      issues.push({`, rule, this.name, `
        severity: this.severity,
        message: Circular dependency detected: ${cycle.join(' -> ')}`, path, 'template.resources', suggestion, 'Break the circular dependency by removing or restructuring dependencies');
                    }
                });
            }
        });
...
class VariableValidationRule {
    ...
    if(, variable, type) {
        issues.push({
          ...
          severity: this.severity,`, message, 'Variable type is required', path, template.variables[$], { index } `].type
        });
      }
      ...
      if (variable.defaultValue !== undefined && variable.type) {
        ...
        if (!isValidType) {
          issues.push({
            rule: this.name,`, severity, this.severity, `
            message: Default value type does not match variable type ${variable.type},`, path, `template.variables[${index}].defaultValue,
      ...
      if (variable.required && variable.defaultValue === undefined) {
        issues.push({
          rule: this.name,
          severity: ErrorSeverity.WARNING,`, message, 'Required variable has no default value', `
      ...
      secretPatterns.forEach(pattern => {
        if (pattern.test(resourceStr)) {
          issues.push({
            rule: this.name,
            severity: ErrorSeverity.ERROR,
            message: 'Hardcoded secrets detected in resource properties',`, path, `template.resources[${index}].properties,
        ...
        sourceRanges.forEach((range: string, rangeIndex: number) => {
          if (range === '0.0.0.0/0' && resource.properties.allowed) {
            ...
            if (!allowedPorts.includes('80') && !allowedPorts.includes('443')) {
              issues.push({
                rule: this.name,
                severity: this.severity,`, message, 'Firewall rule allows access from anywhere on non-standard port', `
                path: template.resources[${index}].properties.sourceRanges[${rangeIndex}`, suggestion, 'Restrict access to specific IP ranges or use standard ports');
    }
    ;
}
;
// Check for unencrypted storage
if (resource.type === infrastructure_1.ResourceType.CLOUD_STORAGE) {
    if (!resource.properties.encryption || !resource.properties.encryption.enabled) {
        issues.push({
            ...
            path: template.resources[$]
        }, { index }, properties, suggestion, 'Enable encryption for Cloud Storage buckets');
    }
    ;
}
;
...
class PerformanceValidationRule {
    ...
    validate(template) {
        ...
        template.resources.forEach((resource, index) => {
            ...
            if (resource.type === infrastructure_1.ResourceType.COMPUTE_ENGINE) {
            ...
            message: 'Compute Engine instance may be undersized',`, path, template.resources[$], { index }, properties.machineType `,
    ...
    template.resources.forEach((resource, index) => {
      ...
      if (resource.type === ResourceType.COMPUTE_ENGINE) {
        ...
        if (machineType && (machineType.includes('n1-highmem') || machineType.includes('n1-highcpu'))) {
          issues.push({
            rule: this.name,
            severity: this.severity,`, message, 'High-performance machine type detected - consider if full capacity is needed', `
            path: template.resources[${index}`, properties.machineType, suggestion, 'Review if a smaller machine type would be sufficient');
                }
            }
        });
    }
...
if (resource.type === infrastructure_1.ResourceType.COMPUTE_ENGINE && !resource.properties.autoScaling) {
    issues.push({
        rule: this.name,
        severity: this.severity,
        message: 'Compute Engine instance without auto-scaling may lead to over-provisioning',
        path: template.resources[$]
    }, { index }, properties, suggestion, 'Consider using Managed Instance Groups with auto-scaling to optimize costs');
}
;
;
return issues;
class BestPracticesRule {
    ...
    validate(template) {
        ...
        template.resources.forEach((resource, index) => {
          ...
          message: 'Resource missing tags',`, path, template.resources[$], { index }, tags, suggestion, 'Add tags for better resource management and cost tracking');
            }
        });
    }
 ...
 && !resource.tags.environment;
{
    issues.push({
        ...
        message: 'Resource missing environment tag',
          ...
          path: template.resources[${index}].tags`, suggestion, 'Add environment tag to identify resource environment');
}
;
;
