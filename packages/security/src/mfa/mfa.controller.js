...
const current_user_decorator_1 = require("../jwt/current-user.decorator");
let MfaController = class MfaController {
    ...
    async setupMfa(user) {
        const { secret, qrCode } = await this.mfaService.generateMfaSecret(user.id, user.email);
        return {
            secret,
            qrCode,
    ...
    async enableMfa(user, token) {
        const success = await this.mfaService.enableMfa(user.id, token);
        if (!success) {
