...
exports.RedisAgentClient = exports.CONFIG = void 0;
...
class RedisAgentClient {
    ...
    async initialize() {
        try {
            ...
            this.publisher = (0, infrastructure_1.createStandaloneRedisClient)({ lazyConnect: true });
            this.subscriber = (0, infrastructure_1.createStandaloneRedisClient)({ lazyConnect: true });
            this.upstash = (0, infrastructure_1.createUpstashRestClient)();
            if (this.publisher instanceof ioredis_1.Redis) {
                this.publisher.on('error', (error) => this.logRedisClientError('publisher', error));
                await this.publisher.connect().catch(() => { });
            }
            if (this.subscriber instanceof ioredis_1.Redis) {
    ...
    async register(name, role, platform, capabilities = []) {
        ...
        if (this.subscriber instanceof ioredis_1.Redis) {
            await this.subscriber.subscribe(exports.CONFIG.channels.agents, exports.CONFIG.channels.conversations, exports.CONFIG.channels.orchestrator, exports.CONFIG.channels.broker, 'tnf:bus:ingress' // Listen to global ingress for auctions
            );
            await this.subscriber.psubscribe(`${exports.CONFIG.channels.directPrefix}:*:${this.agentInfo.id}`);
        }
    ...
    async submitBid(taskId, suitability, metadata = {}) {
        ...
        if (this.upstash) {
            await this.upstash.publish(exports.CONFIG.channels.broker, payload);
        }
        else if (this.publisher) {
            await this.publisher.publish(exports.CONFIG.channels.broker, payload);
        }
    ...
    async send(content, options = {}) {
        ...
        const channel = directAgentId
            ? `${exports.CONFIG.channels.directPrefix}:${this.agentInfo.id}:${directAgentId}`
            : options.channel || exports.CONFIG.channels.conversations;
    ...
    async broadcast(options) {
        return this.send(options.content, {
            ...options,
            channel: exports.CONFIG.channels.agents,
            to: { broadcast: true },
        });
    }
        ...
        this.heartbeatTimer = setInterval(async () => {
            if (this.agentInfo && (this.publisher || this.upstash)) {
                ...
                if (this.upstash) {
                    ...
                    await this.upstash.publish(exports.CONFIG.channels.heartbeat, heartbeatData);
                }
                else if (this.publisher) {
                    await this.publisher.hset('tnf:agent-registry', this.agentInfo.id, agentData);
                    await this.publisher.publish(exports.CONFIG.channels.heartbeat, heartbeatData);
                }
            }
        }, exports.CONFIG.heartbeatInterval);
    }
    async listAgents() {
        let agents = {};
        if (this.upstash) {
        ...
        for (const [id, jsonStr] of Object.entries(agents)) {
            try {
                ...
                const isOnline = Date.now() - lastSeen.getTime() < exports.CONFIG.heartbeatInterval * 2;
                agentList.push({
                    ...agent,
                    isOnline,
                });
...
exports.RedisAgentClient = RedisAgentClient;
