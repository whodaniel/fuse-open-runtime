...
/**
 * The New Fuse - AI Studio Batch Video Processor
 *
 ...
 *   pnpm tnf-agent ai-studio:batch --start=1 --end=100
 *   pnpm tnf-agent ai-studio:batch --resume --batch-size=25
 */
...
const TRANSCRIPTS_DIR = config.transcriptsDir;
...
function loadVideoLibrary(): Video[] {
  if (!fs.existsSync(LIBRARY_FILE)) {
    console.error(`[TNF] Library file not found: ${LIBRARY_FILE}`);
    console.error('Create a video-library.html file with your video list.');
    process.exit(1);
  }
...
async function callGeminiAPI(prompt: string, maxRetries = 3): Promise<string> {
...
async function fetchTranscript(videoId: string): Promise<string> {
  try {
    ...
    execSync(cmd, { cwd: TRANSCRIPTS_DIR, stdio: 'pipe', timeout: 60000 });

    const possibleFiles = [
      path.join(TRANSCRIPTS_DIR, `${tempFilePrefix}.en.json3`),
      path.join(TRANSCRIPTS_DIR, `${tempFilePrefix}.en-US.json3`),
      path.join(TRANSCRIPTS_DIR, `${tempFilePrefix}.en-GB.json3`),
    ];
...
async function processVideo(
  ...
  console.log(`\n${'═'.repeat(70)}`);
  console.log(
    `[TNF] #${video.index}: ${video.title.substring(0, 60)}${video.title.length > 60 ? '...' : ''}`
  );
...
*Generated by The New Fuse - AI Video Intelligence Suite*
`;
...
export async function batchProcess(args: string[]): Promise<void> {
  if (!API_KEY) {
    console.error('[TNF] Error: GEMINI_API_KEY environment variable not set');
  ...
  console.log('[TNF] AI Studio Batch Video Processor\n' + '═'.repeat(70) + '\n');

  const progress = loadProgress();
  const allVideos = loadVideoLibrary();
  console.log(`[TNF] ${allVideos.length} videos in library\n`);
  ...
  if (resumeMode) {
    ...
    console.log(`[TNF] Resume mode: ${videosToProcess.length} remaining\n`);
  } else if (startIndex && endIndex) {
    videosToProcess = allVideos.filter((v) => v.index >= startIndex && v.index <= endIndex);
    console.log(`[TNF] Range #${startIndex}-#${endIndex}\n`);
  }
  ...
  console.log(`[TNF] Processing ${videosToProcess.length} videos (batches of ${batchSize})\n`);
  ...
  for (let i = 0; i < videosToProcess.length; i += batchSize) {
    ...
    console.log(`\n${'━'.repeat(70)}`);
    console.log(
      `[TNF] Batch ${batchNum}: ${i + 1}-${Math.min(i + batchSize, videosToProcess.length)}`
    );
    console.log(`${'━'.repeat(70)}`);
    ...
    saveProgress(progress);
    console.log(`\n[TNF] Progress: ${progress.completed.length}/${allVideos.length} total`);

    if (i + batchSize < videosToProcess.length) {
      console.log('[TNF] Pausing 3s...\n');
      await new Promise((r) => setTimeout(r, 3000));
    }
  }

  console.log(`\n${'═'.repeat(70)}`);
  console.log('[TNF] BATCH PROCESSING COMPLETE!');
  console.log(`${'═'.repeat(70)}\n`);
