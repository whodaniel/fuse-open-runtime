export class PromptTemplateServiceImpl {
    templates = new Map();
    versions = new Map();
    snippets = new Map();
    executionResults = [];
...
{{format_template}}`,
    ...
    async compileTemplate(templateId, versionId, variables) {
        const template = this.templates.get(templateId);
        if (!template)
            throw new Error(`Template not found: ${templateId}`);
    ...
    async executeTemplate(templateId, versionId, variables) {
    ...
    async recordExecution(result) {
        ...
        if (result.versionId) {
            ...
            if (version) {
                ...
                version.metrics.avgResponseTime = Math.round((version.metrics.avgResponseTime * (version.metrics.totalRuns - 1) + result.responseTime) /
