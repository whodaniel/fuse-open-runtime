...
const ModularPromptTemplatingSystem: React.FC<ModularPromptTemplatingSystemProps> = ({
  ...
  const activeVersion = currentVersion || (template.versions && template.versions.length > 0 ? template.versions[0] : undefined);
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{Object.keys(activeVersion?.variables || {}).length}</p>
            </div>
            ...
            {template.analytics.popularVariables.map((variable, index) => (
              <div key={variable} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-500">{100 - (index * 20)}%</span> {/* Example percentage logic */}
                </div>
              </div>
            ))}
          </div>
