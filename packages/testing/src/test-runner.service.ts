...
export class TestRunnerService {
  ...
  async runAgentWorkflowTests(config?: Partial<TestConfiguration>): Promise<TestRunResult> {
    ...
    try {
      ...
      this.logger.log(
        `Test run completed: ${runId} - ${results.passed}/${results.results.length} passed`,
      );
    } catch (error: any) {
  ...
  async getTestSchedules(): Promise<TestSchedule[]> {
    return Array.from(this.testSchedules.values()).sort((a, b) => a.name.localeCompare(b.name));
  }
  ...
  async getTestAnalytics(days: number = 30): Promise<{
    ...
    const recentRuns = Array.from(this.testRuns.values()).filter(run => run.startTime > cutoffDate);
  ...
  private generateRecommendations(testRun: TestRunResult): string[] {
    ...
    if (testRun.duration && testRun.duration > 300000) {
      // 5 minutes
    ...
    if (longRunningTests.length > 0) {
      recommendations.push(
        'Optimize long-running test cases: ' + longRunningTests.map(t => t.name).join(', '),
      );
    }
  ...
  private generateDailyTrends(
    runs: TestRunResult[],
    days: number,
  ): Array<{ date: string; runs: number; passed: number; failed: number }> {
    const trends = [];
  ...
  private generateTestCaseAnalytics(
    runs: TestRunResult[],
  ): Array<{ name: string; successRate: number; averageDuration: number }> {
    const testCaseStats = new Map<
      string,
      { total: number; passed: number; totalDuration: number }
    >();
  ...
  private getTopFailures(
    runs: TestRunResult[],
  ): Array<{ testCase: string; failures: number; lastFailure: number }> {
  ...
  async getHealthStatus(): Promise<{
    ...
    const recentRuns = Array.from(this.testRuns.values()).filter(
      run => run.startTime > Date.now() - 24 * 60 * 60 * 1000,
    ); // Last 24 hours
