import { AttachmentFile, CellValue, Column, DataType, Row, Table } from './types.js';
...
const getColumnValue = (
  ...
  switch (targetColumn.type) {
    ...
    case DataType.SINGLE_SELECT:
      if (typeof value === 'string' && targetColumn.options) {
        // value is optionId
...
export const evaluateFormula = (
  ...
  for (const token of tokens) {
    if (typeof token === 'string' && token.startsWith('{') && token.endsWith('}')) {
      ...
      const colValResult = getColumnValue(
        fieldName,
        currentRow.data,
        columns,
        currentTableRows,
      ...
      typeof token === 'number' ||
      (typeof token === 'string' && !precedence.hasOwnProperty(token))
    ) {
      // Number or string literal
      RPNQueue.push(token);
    } else if (typeof token === 'string' && precedence.hasOwnProperty(token)) {
      // Operator
    ...
    if (typeof item === 'string' && ['+', '-', '*', '/', '&'].includes(item)) {
      ...
      if (item === '&') {
        ...
        if (isNaN(valA as number) || isNaN(valB as number)) {
          ...
          if (
            (isNaN(valA as number) && a_raw !== null && a_raw !== undefined) ||
            (isNaN(valB as number) && b_raw !== null && b_raw !== undefined)
          ) {
            return {
              value: null,
              error: `Cannot perform arithmetic operation on non-numeric value.`,
            };
          }
