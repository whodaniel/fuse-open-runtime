...
const Badge = React.forwardRef(({ className, variant, size, dismissible, onDismiss, children, ...props }, ref) => {
    return (<div ref={ref} className={cn(badgeVariants({ variant, size }), className)} {...props}>
        {children}
        {dismissible && (<button type="button" className="ml-1 -mr-1 h-3.5 w-3.5 rounded-full text-primary-foreground/70 hover:text-primary-foreground" onClick={onDismiss}>
