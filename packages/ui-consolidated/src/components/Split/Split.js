 ...
 * </Split>
 */
const Split = React.forwardRef(({ className, direction = 'horizontal', initialSizes = [50, 50], minSize = 10, resizable = true, gutterSize = 4, onChange, children, ...props }, ref) => {
    ...
    // Handle mouse move
    React.useEffect(() => {
        if (!isDragging || dragIndex === -1 || !containerRef.current)
            return;
        const handleMouseMove = (e) => {
            ...
            const sizeDiff = newSizePercentage -
                newSizes.slice(0, panelIndex + 1).reduce((acc, size) => acc + size, 0) +
                newSizes[panelIndex];
            // Ensure minimum size
            if (newSizes[panelIndex] + sizeDiff < minSize ||
    ...
    return (_jsx("div", { 
        ...
        }, className: cn('flex', direction === 'horizontal' ? 'flex-row' : 'flex-col', isDragging && 'select-none', className), ...props, children: childrenArray.map((child, index) => (_jsxs(React.Fragment, { children: [_jsx("div", { ref: (el) => {
                        panelRefs.current[index] = el;
                    }, className: cn('panel', 'flex-shrink-0 overflow-auto'), children: child }), index < childrenArray.length - 1 && (_jsx("div", { ref: (el) => {
                        gutterRefs.current[index] = el;
                    }, className: cn('gutter', direction === 'horizontal'
                        ? 'gutterHorizontal cursor-col-resize'
                        : 'gutterVertical cursor-row-resize', resizable ? 'bg-border hover:bg-primary/20' : 'bg-transparent', isDragging && dragIndex === index && 'bg-primary'), onMouseDown: (e) => handleGutterMouseDown(index, e) }))] }, index))) }));
});
