...
const Split = React.forwardRef(({ className, direction = 'horizontal', initialSizes = [50, 50], minSize = 10, resizable = true, gutterSize = 4, onChange, children, ...props }, ref) => {
    ...
    React.useEffect(() => {
        panelRefs.current.forEach((el, i) => el?.style.setProperty('--split-panel-basis', `${sizes[i]}%));`, gutterRefs.current.forEach(el => el?.style.setProperty('--gutter-size', `${gutterSize}`, px))));
    }, [sizes, gutterSize]);
    // Validate children
