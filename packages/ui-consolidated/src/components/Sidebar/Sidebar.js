...
const Sidebar = React.forwardRef(({ className, variant, width, position, mobile, open, onClose, items = [], onNavigate, collapsible = false, collapsed = false, onCollapse, header, footer, children, ...props }, ref) => {
    ...
    // Default header
    const defaultHeader = (<div className="h-16 flex items-center justify-between px-4 border-b">
        {!collapsed && <div className="text-xl font-semibold">The New Fuse</div>}
        {mobile && (<button type="button" className="p-2 rounded-md text-muted-foreground hover:bg-accent hover:text-accent-foreground" onClick={onClose}>
            <span className="sr-only">Close sidebar</span>
            <svg className="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/>
            </svg>
          </button>)}
        {collapsible && !mobile && (<button type="button" className="p-2 rounded-md text-muted-foreground hover:bg-accent hover:text-accent-foreground" onClick={handleCollapseToggle}>
    ...
    const mobileTransformClass = mobile
        ...
        : '';
    return (<>
        <aside ref={ref} className={cn(sidebarVariants({ variant, width: sidebarWidth, position, mobile }), mobileTransformClass, className)} role="navigation" aria-label="Sidebar navigation" tabIndex={-1} {...props}>
          {/* Header */}
          {header || defaultHeader}
          
          {/* Navigation */}
          {items.length > 0 && (<nav className="p-4 space-y-1" aria-label="Sidebar main navigation">
              {items.map((item) => (<a key={item.key || item.href} href={item.href} className={cn('flex items-center px-4 py-2 text-sm font-medium rounded-md focus:outline-none focus-visible:ring-2 focus-visible:ring-ring', item.active
                  ...
                  {item.icon && (<span className={cn('flex-shrink-0', !collapsed && 'mr-3')} aria-hidden="true">
                      {item.icon}
                    </span>)}
        ...
        {/* Overlay for mobile */}
        {mobile && open && (<div className="fixed inset-0 z-30 bg-black bg-opacity-50 lg:hidden" onClick={onClose}/>)}
      </>);
});
