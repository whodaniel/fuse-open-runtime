...
export function GraphChart({ type, data, title, xAxisKey = "timestamp", yAxisKey = "value", series, stacked = false, height = 300, width = "100%", colors = defaultColors, className = "", }) {
    ...
    const renderChart = () => {
        switch (type) {
            ...
            <Legend />
            {series
                        ? series.map((s, index) => (<Line key={s.key} type="monotone" dataKey={s.key} name={s.label} stroke={s.color || colors[index % colors.length]}/>))
            ...
            <Legend />
            {series
                        ? series.map((s, index) => (<Bar key={s.key} dataKey={s.key} name={s.label} fill={s.color || colors[index % colors.length]} stackId={stacked ? "stack" : undefined}/>))
                        : <Bar dataKey={yAxisKey} fill={colors[0]}/>}
          </BarChart>);
            case "pie":
                return (<PieChart>
            <Pie data={chartData} dataKey="value" nameKey="label" cx="50%" cy="50%" outerRadius={80} label>
              {chartData.map((entry, index) => (<Cell key={entry.id} fill={String(entry.color || colors[index % colors.length])}/>))}
            </Pie>
            <Tooltip />
            <Legend />
          </PieChart>);
            ...
            <Legend />
            {series
                        ? series.map((s, index) => (<Area key={s.key} type="monotone" dataKey={s.key} name={s.label} fill={s.color || colors[index % colors.length]} stroke={s.color || colors[index % colors.length]} stackId={stacked ? "stack" : undefined} fillOpacity={0.6}/>))
