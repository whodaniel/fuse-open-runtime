...
const Pagination = React.forwardRef(({ className, variant, count, page = 1, onPageChange, siblingCount = 1, boundaryCount = 1, showControls = true, showFirstLast = false, size, ...props }, ref) => {
    ...
    return (<PaginationContext.Provider value={count,
            page,
            onPageChange} handlePageChange variant size/>);
}, 
    >
        <nav ref={ref} role="navigation" aria-label="pagination" className={cn(paginationVariants({ variant }), className)} {...props}>
          {showFirstLast && (<PaginationItem variant={variant} size={size} onClick={() => handlePageChange(1)} disabled={page === 1} aria-label="Go to first page">
          ...
          {showControls && (<PaginationItem variant={variant} size={size} onClick={() => handlePageChange(page - 1)} disabled={page === 1} aria-label="Go to previous page">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4">
                <polyline points="15 18 9 12 15 6"></polyline>
              </svg>
            </PaginationItem>)}
          
          {range.map((item, index) => (<React.Fragment key={index}>
              {item === 'ellipsis' ? (<span className="flex h-9 w-9 items-center justify-center text-sm">...</span>) : (<PaginationItem variant={variant} size={size} isActive={item === page} onClick={() => handlePageChange(item)} aria-label={`}Go to page ${item}`} aria-current={item === page ? 'page' : undefined}>
                  {item}
                </PaginationItem>)}
            </React.Fragment>))}
          
          {showControls && (<PaginationItem variant={variant} size={size} onClick={() => handlePageChange(page + 1)} disabled={page === count} aria-label="Go to next page">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4">
                <polyline points="9 18 15 12 9 6"></polyline>
              </svg>
            </PaginationItem>)}
          
          {showFirstLast && (<PaginationItem variant={variant} size={size} onClick={() => handlePageChange(count)} disabled={page === count} aria-label="Go to last page">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4">
...
const PaginationItem = React.forwardRef(({ className, variant, size, isActive, children, ...props }, ref) => {
    ...
    return (<button ref={ref} type="button" className={cn(paginationItemVariants({
