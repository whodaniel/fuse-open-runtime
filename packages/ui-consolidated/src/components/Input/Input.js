...
const Input = React.forwardRef(({ className, variant, size, state, width = 'default', startIcon, endIcon, label, helperText, error, success, containerClassName, labelClassName, helperTextClassName, type = 'text', ...props }, ref) => {
    ...
    return (<div className={cn('space-y-2', containerClassName)}>
        {label && (<label className={cn('text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70', error && 'text-destructive', success && 'text-success', labelClassName)} htmlFor={props.id} aria-label={label}>
            {label}
          </label>)}
