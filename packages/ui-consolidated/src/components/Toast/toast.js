...
const Toast = React.forwardRef(({ title, description, variant = 'default', duration = 3000, action, ...props }, ref) => {
    ...
    return (<div ref={ref} className={cn("rounded-lg border shadow-lg p-4", variantStyles[toastVariant] // Type assertion to help TypeScript
        )} {...props}>
      {title && <div className="font-medium">{title}</div>}
      {description && <div className="mt-1 text-sm">{description}</div>}
      {action && (<div className="mt-2 flex justify-end">
          {action}
        </div>)}
    </div>);
});
