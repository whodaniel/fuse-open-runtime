...
export const Tooltip = forwardRef(({ children, content, side = 'top', className = '' }, ref) => {
    ...
    return (<div className="relative inline-flex" ref={ref}>
        <div onMouseEnter={() => setIsVisible(true)} onMouseLeave={() => setIsVisible(false)} onFocus={() => setIsVisible(true)} onBlur={() => setIsVisible(false)}>
          {children}
        </div>
        
        {isVisible && (<div className={`absolute z-50 px-3 py-2 text-sm text-white bg-black rounded-md ${positionClasses[side]}} ${className}`} role="tooltip">
            {content}
          </div>)}
      </div>);
});
