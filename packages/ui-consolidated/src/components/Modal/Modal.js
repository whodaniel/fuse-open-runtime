...
;
    >
        <div ref={ref} className="fixed inset-0 z-50" role="dialog" aria-modal="true" {...props}>
          <div className={cn(modalOverlayVariants({ position }))} onClick={closeOnClickOutside ? () => onOpenChange?.(false) : undefined}>
            {children}
          </div>
        </div>;
...
const ModalContent = React.forwardRef((props, ref) => {
    ...
    return (<ModalPortal>
      <ModalOverlay />
      <DialogPrimitive.Content ref={ref} className={`fixed left-[50%] top-[50%] z-50 grid w-full translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-lg ${sizeClasses[size]}} ${className || ''}`} {...restProps}>
        {children}
 ...
 */
const ModalHeader = ({ className, ...props }) => (<div className={cn('flex flex-col space-y-1.5 text-center sm:text-left', className)} {...props}/>);
ModalHeader.displayName = 'ModalHeader';
/**
 * Modal title component for displaying the modal title
 */
const ModalTitle = React.forwardRef(({ className, ...props }, ref) => (<DialogPrimitive.Title ref={ref} className={cn('text-lg font-semibold leading-none tracking-tight', className)} {...props}/>));
ModalTitle.displayName = DialogPrimitive.Title.displayName;
/**
 * Modal description component for displaying the modal description
 */
const ModalDescription = React.forwardRef(({ className, ...props }, ref) => (<DialogPrimitive.Description ref={ref} className={cn('text-sm text-muted-foreground', className)} {...props}/>));
ModalDescription.displayName = DialogPrimitive.Description.displayName;
/**
 * Modal footer component for displaying the modal footer
 */
const ModalFooter = ({ className, ...props }) => (<div className={cn('flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2', className)} {...props}/>);
...
const ModalCloseButton = React.forwardRef(({ className, onClick, ...props }, ref) => {
    ...
    if (!showCloseButton)
        return null;
    return (<button ref={ref} className={cn('absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none', className)} onClick={(e) => {
            onClick?.(e);
            setOpen(false);
        }} type="button" aria-label="Close" {...props}>
 ...
 */
const ModalOverlay = ({ position = 'default' }) => (<div className={cn(modalOverlayVariants({ position }))}/>);
