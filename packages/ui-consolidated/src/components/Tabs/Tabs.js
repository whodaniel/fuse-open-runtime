...
const Tabs = React.forwardRef(({ defaultValue, value, onValueChange, variant = 'default', fullWidth = false, className, children, ...props }, ref) => {
    ...
    return (<TabsContext.Provider value={value} tabValue onValueChange:handleValueChange variant fullWidth/>);
}, 
    >
...
const TabsList = React.forwardRef(({ className, ...props }, ref) => {
    ...
    return (<div ref={ref} className={cn(tabsListVariants({ variant, fullWidth }), className)} role="tablist" aria-orientation="horizontal" {...props}/>);
});
...
const TabsTrigger = React.forwardRef(({ className, value, icon, badge, children, ...props }, ref) => {
    ...
    return (<button ref={ref} type="button" role="tab" aria-selected={isActive} className={cn(tabsTriggerVariants({ variant, fullWidth }), className)} onClick={() => onValueChange(value)} data-state={isActive ? 'active' : 'inactive'} {...props}>
...
const TabsContent = React.forwardRef(({ className, value, children, ...props }, ref) => {
    ...
    if (!isActive)
        return null;
    return (<div ref={ref} role="tabpanel" className={cn(tabsContentVariants({ variant }), className)} data-state={isActive ? 'active' : 'inactive'} {...props}>
        {children}
      </div>);
});
