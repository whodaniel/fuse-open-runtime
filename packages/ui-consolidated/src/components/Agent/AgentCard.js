...
import { PlayCircle, PauseCircle, Edit, Trash, Info, ExternalLink, Settings } from 'lucide-react';
const statusColors = {
...
export const AgentCard = ({ agent, onSelect, onStart, onStop, onEdit, onDelete, onViewDetails, onConfigureEndpoints, className = '', compact = false, showProtocols = false, showEndpoints = false, showSecurity = false, }) => {
    ...
    return (<Card className={className} variant="default" size={compact ? "sm" : "default"} hoverable={true}>
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        {!compact && (<div>
            ...
            <div className="mt-1 flex flex-wrap gap-1">
              {agent.capabilities.map((capability) => (<Badge key={capability} className="px-2 py-1 rounded-full bg-blue-100 text-blue-800 text-xs">
                  {capability}
                </Badge>))}
            </div>
          </div>)}

        {showProtocols && agent.protocols && agent.protocols.length > 0 && (<div>
            <h4 className="text-sm font-medium text-gray-700">Protocols</h4>
            <div className="mt-1 flex flex-wrap gap-1">
              {agent.protocols.map((protocol) => (<Badge key={protocol} className="px-2 py-1 rounded-full bg-purple-100 text-purple-800 text-xs">
                  {protocol}
                </Badge>))}
            </div>
          </div>)}

        {showEndpoints && agent.endpoints && (<div>
            <h4 className="text-sm font-medium text-gray-700">Endpoints</h4>
            <div className="mt-1 space-y-1 text-xs text-gray-500">
              {agent.endpoints.discovery && (<div className="flex items-center">
              ...
              {agent.endpoints.messaging && (<div className="flex items-center">
        ...
        {showSecurity && agent.security && (<div>
        ...
        {!compact && agent.metadata && (<div className="flex items-center justify-between text-sm text-gray-500">
            {agent.metadata.version && <span>Version: {agent.metadata.version}</span>}
            {agent.metadata.lastActive && (<span>
      ...
      <CardFooter className="flex justify-end space-x-2">
        {onSelect && (<Button variant="ghost" size="sm" onClick={() => onSelect(agent)}>
            Select
          </Button>)}

        {onViewDetails && (<Button variant="ghost" size="sm" onClick={() => onViewDetails(agent)} icon={<Info className="h-4 w-4"/>}>
            Details
          </Button>)}

        {onConfigureEndpoints && (<Button variant="ghost" size="sm" onClick={() => onConfigureEndpoints(agent)} icon={<Settings className="h-4 w-4"/>}>
            Configure
          </Button>)}

        {isActive && onStop && (<Button variant="ghost" size="sm" onClick={() => onStop(agent)} icon={<PauseCircle className="h-4 w-4"/>}>
            Stop
          </Button>)}

        {!isActive && onStart && (<Button variant="ghost" size="sm" onClick={() => onStart(agent)} icon={<PlayCircle className="h-4 w-4"/>}>
            Start
          </Button>)}

        {onEdit && (<Button variant="ghost" size="sm" onClick={() => onEdit(agent)} icon={<Edit className="h-4 w-4"/>}>
            Edit
          </Button>)}

        {onDelete && (<Button variant="ghost" size="sm" onClick={() => onDelete(agent)} icon={<Trash className="h-4 w-4"/>}>
            Delete
          </Button>)}
      </CardFooter>
    </Card>);
