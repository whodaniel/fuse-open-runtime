...
const Accordion = React.forwardRef(({ className, variant, type = 'single', defaultValue, value, onValueChange, collapsible = true, children, ...props }, ref) => {
    ...
    return (<AccordionContext.Provider value={value} currentValue onValueChange:handleValueChange type collapsible variant:variant/> || 'default',
    );
}, 
    >
...
const AccordionItem = React.forwardRef(({ className, variant, value, disabled = false, children, ...props }, ref) => {
    ...
    return (<AccordionItemContext.Provider value={value,
            isOpen,
            isDisabled} disabled toggle/>);
}, 
    >
        <div ref={ref} className={cn(accordionItemVariants({ variant: variant || accordionVariant }), disabled && 'opacity-50 cursor-not-allowed', className)} data-state={isOpen ? 'open' : 'closed'} data-disabled={disabled || undefined} {...props}>
          {children}
        </div>);
...
const AccordionTrigger = React.forwardRef(({ className, variant, showChevron = true, chevron, children, ...props }, ref) => {
    ...
    return (<button ref={ref} type="button" className={cn(accordionTriggerVariants({ variant: variant || accordionVariant }), className)} onClick={toggle} onKeyDown={(e) => {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                toggle();
            }
        }} disabled={isDisabled} aria-expanded={isOpen} aria-controls={`}accordion-content-${String(props.id || '')}`} data-state={isOpen ? 'open' : 'closed'} {...props}>
        {children}
        {showChevron && (<div className="shrink-0 transition-transform duration-200">
            {chevron || (<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4 transition-transform duration-200">
...
const AccordionContent = React.forwardRef(({ className, variant, children, ...props }, ref) => {
    ...
    if (!isOpen)
        return null;
    return (<div ref={ref} className={cn(accordionContentVariants({ variant: variant || accordionVariant }), className)} data-state={isOpen ? 'open' : 'closed'} {...props}>
