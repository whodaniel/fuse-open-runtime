...
// Stub implementations until modules are available
...
async;
getSuggestedConnections(node, any);
{
    return [];
}
const styles = {
...
`;
const TaskNode = ({ data }) => {
    const nodeClass = $, { styles, taskNode }, $, { data, status };
...
return (<div className={nodeClass}>
      <div className="font-medium">{data.label}</div>
      {data.metadata && (<div className={styles.metadataList}>
...
const AgentNode = ({ data }) => {
    return (<div className={styles.agentNode}>
      <div className="font-medium">{data.label}</div>
      {data.status && (`
  ...
  return (
    <div className={containerClass}>
      <ReactFlow
        ...
        snapGrid={config.snapGrid as [number, number]}
