...
import { A2AError } from '@the-new-fuse/a2a-core';
export function useA2A(config) {
    ...
    const connect = useCallback(async () => {
      ...
      socket.on('heartbeat:received', (_heartbeat: AgentHeartbeat) => {
  ...
  const sendRequest = useCallback(async (
    ...
    return new Promise((resolve, reject) => {
      const timeoutHandle = setTimeout(() => {
        pendingRequestsRef.current.delete(requestId);`, reject(new A2AError(`Request timeout after ${timeout}`, ms `, 'TIMEOUT'));
      }, timeout);
