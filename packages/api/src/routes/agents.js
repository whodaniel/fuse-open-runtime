...
const agentService = new AgentService(null, null);
import { sendSuccess, sendNoContent } from '../utils/response.util';
import { ApiError } from '../middleware/error.middleware';
import { validateParams } from '../middleware/validation.middleware';
import { agentIdSchema } from '../schemas/agent.schema';
...
router.get('/:id', authenticate, validateParams(agentIdSchema), async (req, res, next) => {
    try {
        if (!req.user) {
            return next(new ApiError('Authentication required', 401));
        }
...
router.delete('/:id', authenticate, validateParams(agentIdSchema), async (req, res, next) => {
    try {
        if (!req.user) {
            return next(new ApiError('Authentication required', 401));
        }
