...
// Use WorkflowModel as WorkflowDefinition and WorkflowStepDefinition as WorkflowStep
    ...
    this.steps.forEach((step: WorkflowStep) => {
      ...
      if (step.conditions) {
        step.conditions = step.conditions.filter(
          (cond: WorkflowCondition) => cond.nextStepId !== stepId
        );
      }
    });
  }
      ...
      if (step.conditions) {
        step.conditions.forEach((cond: WorkflowCondition) => {
          if (!stepIds.has(cond.nextStepId)) {
            errors.push(
              `Step ${step.id} has condition pointing to non-existent step: ${cond.nextStepId}`
            );
          }
        });
      }
