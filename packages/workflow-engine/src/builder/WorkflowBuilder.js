...
class WorkflowBuilder extends events_1.EventEmitter {
    logger;
    config;
    ...
    createWorkflow(name, description) {
        ...
        this.currentWorkflow = {
            ...
            status: WorkflowTypes_js_1.WorkflowStatus.DRAFT,
            version: '1.0.0',
            tags: [],
            isTemplate: false,
            createdAt: new Date(),
    ...
    removeNode(nodeId) {
        ...
        this.currentWorkflow.definition.connections = this.currentWorkflow.definition.connections.filter(c => c.sourceNodeId !== nodeId && c.targetNodeId !== nodeId);
        // Remove the node
     ...
     */
    addConnection(sourceNodeId, sourceOutputId, targetNodeId, targetInputId, condition) {
        if (!this.currentWorkflow) {
            throw new Error('No workflow loaded');
        }
    ...
    initializeNodeTemplates() {
        ...
        this.nodeTemplates.set(WorkflowTypes_js_1.WorkflowNodeType.START, {
            inputs: [],
            outputs: [{ id: 'output', name: 'Start', type: WorkflowTypes_js_1.VariableType.OBJECT, description: 'Workflow started' }],
            config: {}
        });
        this.nodeTemplates.set(WorkflowTypes_js_1.WorkflowNodeType.END, {
            inputs: [{ id: 'input', name: 'End', type: WorkflowTypes_js_1.VariableType.OBJECT, required: false, description: 'Workflow result' }],
            outputs: [],
            config: {}
        });
        this.nodeTemplates.set(WorkflowTypes_js_1.WorkflowNodeType.AGENT_TASK, {
            inputs: [
                { id: 'task', name: 'Task', type: WorkflowTypes_js_1.VariableType.STRING, required: true, description: 'Task description' },
                { id: 'context', name: 'Context', type: WorkflowTypes_js_1.VariableType.OBJECT, required: false, description: 'Task context' }
            ],
            outputs: [
                { id: 'result', name: 'Result', type: WorkflowTypes_js_1.VariableType.OBJECT, description: 'Task result' },
                { id: 'agentId', name: 'Agent ID', type: WorkflowTypes_js_1.VariableType.STRING, description: 'Executing agent ID' }
            ],
            config: {
                priority: 'medium',
                expectedDuration: 30
    ...
    wouldCreateCircle(sourceNodeId, targetNodeId) {
        if (!this.currentWorkflow)
            return false;
        ...
        while (stack.length > 0) {
            ...
            const outgoingConnections = this.currentWorkflow.definition.connections.filter(c => c.sourceNodeId === currentId);
    ...
    addStartNode() {
        this.addNode(WorkflowTypes_js_1.WorkflowNodeType.START, 'Start', { x: 100, y: 100 });
    }
    addEndNode() {
        this.addNode(WorkflowTypes_js_1.WorkflowNodeType.END, 'End', { x: 500, y: 100 });
    }
    getDefaultSettings() {
        return {
            parallel: false,
        ...
        for (const node of this.currentWorkflow.definition.nodes) {
            switch (node.type) {
                case WorkflowTypes_js_1.WorkflowNodeType.AGENT_TASK:
                    totalTime += node.config?.expectedDuration || 30;
                    break;
