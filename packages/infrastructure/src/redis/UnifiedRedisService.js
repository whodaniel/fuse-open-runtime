...
let UnifiedRedisService = (UnifiedRedisService_1 = class UnifiedRedisService {
  redisConfig;
  logger = new common_1.Logger(UnifiedRedisService_1.name);
  mainClient;
  pubSubClient;
  upstashClient;
  subscribers = new Map();
  ...
  async initializeConnections() {
    ...
    try {
      if (this.redisConfig.isClusterMode()) {
        ...
        this.mainClient = new ioredis_1.default.Cluster(nodes, {
          redisOptions: config,
          ...this.redisConfig.getConfiguration().cluster,
        });
        this.pubSubClient = new ioredis_1.default.Cluster(nodes, {
          redisOptions: config,
          ...this.redisConfig.getConfiguration().cluster,
        });
      } else {
        this.mainClient = new ioredis_1.default(config);
        this.pubSubClient = new ioredis_1.default(config);
      }
  ...
  async subscribe(channel, callback) {
    return this.executeOperation('subscribe', channel, async () => {
      ...
      const subscriber =
        this.pubSubClient instanceof ioredis_1.default.Cluster
          ? new ioredis_1.default.Cluster(this.redisConfig.getClusterNodes(), {
              redisOptions: config,
            })
          : new ioredis_1.default(config);
...
exports.UnifiedRedisService =
  UnifiedRedisService =
  UnifiedRedisService_1 =
    __decorate(
      [(0, common_1.Injectable)(), __metadata('design:paramtypes', [RedisConfig_1.RedisConfig])],
      UnifiedRedisService
    );
