...
type EnhancedDragEvent = d3.D3DragEvent<
  SVGGElement,
  d3.HierarchyPointNode<NodeData>,
  { x: number; y: number }
>; // Subject is {x, y} position
...
export const EnhancedTimelineView: React.FC<EnhancedTimelineViewProps> = ({
  ...
  const handleDrag = useCallback(
    ...
    [zoom]
  ); // Add zoom as dependency for guideline stroke width
  ...
  const initializeDrag = useCallback(() => {
    return d3
      .drag<any, d3.HierarchyPointNode<NodeData>>() // Use 'any' for element type to avoid strict typing issues
      .subject(function (d) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (rootCandidates.length === 1 && hasParentIds) {
        ...
        try {
          ...
          console.warn(
            'Stratify failed, likely due to data structure issues (e.g., cycles, multiple roots). Falling back to virtual root.',
            stratifyError
          );
        }
      }
      ...
      if (useStratify) {
        ...
        if (!useStratify && rootCandidates.length !== 1) {
          ...
          console.warn(
            "Data doesn't form a single tree structure or lacks parent IDs. Creating virtual root."
          );
        }
        ...
        const rootChildren = nodeDataEvents.filter(
          (e) => !e.parentId || !nodeDataEvents.find((p) => p.id === e.parentId)
        );
      ...
      console.error(
        'Unexpected error creating hierarchy, falling back to basic virtual root:',
        error
      );
      const virtualRootData: NodeData = {
        id: 'virtual-root',
      ...
      const rootChildren = nodeDataEvents.filter(
        (e) => !e.parentId || !nodeDataEvents.find((p) => e.parentId === p.id)
      );
      rootNode = d3.hierarchy<NodeData>(virtualRootData, (d) =>
        d.id === 'virtual-root'
          ? rootChildren
    ...
    g.append('g')
      ...
      .attr('stroke', (d: d3.HierarchyPointLink<NodeData>) => {
        // Use D3 type
        ...
        if (sourceEvent.mergedFrom?.includes(targetEvent.id)) return branchColor.merged;
        // Check parent relationship using hierarchy structure
        if (d.target.parent && d.target.parent.id === d.source.id) return branchColor.active;
        return branchColor.inactive; // Default link color
      })
      .attr(
        'stroke-width',
        (
          d: d3.HierarchyPointLink<NodeData> // Use D3 type
    ...
    nodeGroup.each(function (this: SVGGElement, d: d3.HierarchyPointNode<NodeData>) {
      // Use D3 type
      const node = d3.select(this);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-96">
            <h3 className="text-lg font-bold mb-4">
              Create New Branch from "{selectedEvent.data.title}"
            </h3>
            <input
              type="text"
              value={newBranchName}
