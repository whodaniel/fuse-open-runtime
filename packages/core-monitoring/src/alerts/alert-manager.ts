...
export interface AlertRule {
  ...
  threshold: number;
  duration: number; // How long the condition must be true (ms)
...
export interface AlertManagerConfig {
  evaluationInterval: number; // How often to evaluate rules (ms)
  notificationCooldown: number; // Min time between notifications (ms)
  autoResolveAfter?: number; // Auto-resolve alerts after this time (ms)
  enabled: boolean;
}
