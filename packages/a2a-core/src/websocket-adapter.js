...
let A2AWebSocketAdapter = A2AWebSocketAdapter_1 = class A2AWebSocketAdapter extends events_1.EventEmitter {
    constructor(config, redisAdapter) {
        ...
        this.logger = new common_1.Logger(A2AWebSocketAdapter_1.name);
    ...
    async handleDisconnect(client) {
        ...
        if (agentId) {
            ...
            try {
                await this.redisAdapter.updateAgentStatus(agentId, types_js_1.AgentStatus.OFFLINE);
            }
            catch (error) {
    ...
    async handleAuthenticate(client, data) {
        try {
            // Basic validation
            if (!data.agentId) {
                throw new types_js_1.A2AValidationError('Agent ID is required', 'agentId');
            }
            ...
            await this.redisAdapter.updateAgentStatus(data.agentId, types_js_1.AgentStatus.ONLINE);
    ...
    async handleSendMessage(client, data) {
        try {
            if (!client.isAuthenticated || !client.agentId) {
                throw new types_js_1.A2AError('Not authenticated', 'NOT_AUTHENTICATED');
            }
            // Validate the message
            const validatedMessage = types_js_1.A2AMessageSchema.parse({
                ...data,
            ...
            client.emit('message:error', {
                code: error instanceof types_js_1.A2AError ? error.code : 'SEND_ERROR',
    ...
    async handleSendRequest(client, data) {
        try {
            ...
            client.emit('request:error', {
                code: error instanceof types_js_1.A2AError ? error.code : 'REQUEST_ERROR',
    ...
    async handleSendBroadcast(client, data) {
        try {
            ...
            client.emit('broadcast:error', {
                code: error instanceof types_js_1.A2AError ? error.code : 'BROADCAST_ERROR',
    ...
    async handleJoinConversation(client, data) {
        try {
            ...
            client.emit('conversation:error', {
                code: error instanceof types_js_1.A2AError ? error.code : 'CONVERSATION_ERROR',
    ...
    async handleDiscoverAgents(client, data) {
        try {
            ...
            client.emit('discovery:error', {
                code: error instanceof types_js_1.A2AError ? error.code : 'DISCOVERY_ERROR',
    ...
    async handleSendHeartbeat(client, data) {
        try {
            ...
            client.emit('heartbeat:error', {
                code: error instanceof types_js_1.A2AError ? error.code : 'HEARTBEAT_ERROR',
...
exports.A2AWebSocketAdapter = A2AWebSocketAdapter = A2AWebSocketAdapter_1 = __decorate([
    ...
    __metadata("design:paramtypes", [Object, redis_adapter_js_1.A2ARedisAdapter])
], A2AWebSocketAdapter);
