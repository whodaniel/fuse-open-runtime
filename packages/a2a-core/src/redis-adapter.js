...
import { A2AMessageSchema, AgentRegistrationSchema, AgentHeartbeatSchema, ConversationSchema, AgentStatus, A2AMessageType, A2APriority, A2AError, A2AValidationError, A2ATimeoutError, A2AConnectionError } from './types';
let A2ARedisAdapter = A2ARedisAdapter_1 = class A2ARedisAdapter extends EventEmitter {
    ...
    async sendMessage(message) {
        try {
            ...
            if (validatedMessage.toAgent) {
                ...
                // Broadcast
                await this.redisService.publish(`${this.keyPrefix}messages:global`, { message: validatedMessage });
            }
    ...
    async sendHeartbeat(heartbeat) {
        try {
            ...
            // Publish heartbeat
            await this.redisService.publish(`${this.keyPrefix}heartbeats:global`, { message: JSON.stringify(validatedHeartbeat) });
    ...
    async publishSystemEvent(type, data) {
        await this.redisService.publish(`${this.keyPrefix}events:global`, { message: { type, data, timestamp: Date.now() } });
    }
};
