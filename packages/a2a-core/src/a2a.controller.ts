...
export class A2AController {
  ...
  async sendResponse(
    @Body() body: { originalMessage: A2AMessage; responsePayload: any; fromAgent: string }
  ) {
  ...
  async startConversation(
    @Body() body: { initiator: string; participants: string[]; topic?: string }
  ) {
  ...
  async facilitateHandshake(@Body() body: { agent1Id: string; agent2Id: string }) {
  ...
  @Post('agents/channel')
  async createCommunicationChannel(@Body() body: { agentIds: string[]; topic?: string }) {
  ...
  @Post('payment')
  async createPayment(
    @Body() paymentDetails: { amount: number; currency: string; recipient: string }
  ) {
