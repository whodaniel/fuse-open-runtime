...
let A2AService = A2AService_1 = class A2AService {
    constructor(configService, ap2ProtocolService, redisService) {
        ...
        this.logger = new common_1.Logger(A2AService_1.name);
    ...
    initializeDefaultRoutes() {
        ...
        const defaultRoutes = [
            {
                type: types_js_1.A2AMessageType.TASK_ASSIGNMENT,
                rule: {
                    priority: types_js_1.A2APriority.HIGH,
                    preferredAgents: [],
                    fallbackAgents: [],
                    loadBalancingStrategy: types_js_1.LoadBalancingStrategy.LEAST_LOADED,
                    timeoutMs: 15000,
                },
            },
            {
                type: types_js_1.A2AMessageType.WORKFLOW_COORDINATION,
                rule: {
                    priority: types_js_1.A2APriority.HIGH,
                    preferredAgents: [],
                    fallbackAgents: [],
                    loadBalancingStrategy: types_js_1.LoadBalancingStrategy.CAPABILITY_MATCH,
                    timeoutMs: 20000,
                },
            },
            {
                type: types_js_1.A2AMessageType.STATUS_UPDATE,
                rule: {
                    priority: types_js_1.A2APriority.MEDIUM,
                    preferredAgents: [],
                    fallbackAgents: [],
                    loadBalancingStrategy: types_js_1.LoadBalancingStrategy.FASTEST_RESPONSE,
                    timeoutMs: 10000,
                },
            },
            {
    ...
    async registerAgent(registration) {
        try {
            ...
            await this.broadcastMessage({
                ...
                toAgent: '*', // Broadcast
                type: types_js_1.A2AMessageType.CAPABILITY_ANNOUNCEMENT,
    ...
    async sendMessage(message) {
        try {
            ...
            if (message.priority <= types_js_1.A2APriority.HIGH) {
            ...
            return {
                ...
                error: error.message,
                processingTime: 0,
                agentStatus: types_js_1.AgentStatus.ERROR,
            };
        }
    }
    ...
    getRequiredCapabilityForMessage(messageType) {
        const capabilityMap = {
            [types_js_1.A2AMessageType.TASK_ASSIGNMENT]: 'task_execution',
            [types_js_1.A2AMessageType.STATUS_UPDATE]: 'status_monitoring',
            [types_js_1.A2AMessageType.DATA_REQUEST]: 'data_processing',
            [types_js_1.A2AMessageType.DATA_RESPONSE]: 'data_processing',
            [types_js_1.A2AMessageType.WORKFLOW_COORDINATION]: 'workflow_management',
            [types_js_1.A2AMessageType.COLLABORATION_REQUEST]: 'collaboration',
            [types_js_1.A2AMessageType.RESOURCE_SHARING]: 'resource_management',
            [types_js_1.A2AMessageType.ERROR_NOTIFICATION]: 'error_handling',
            [types_js_1.A2AMessageType.HEARTBEAT]: 'monitoring',
            [types_js_1.A2AMessageType.CAPABILITY_ANNOUNCEMENT]: 'discovery',
            [types_js_1.A2AMessageType.REQUEST]: 'general',
            [types_js_1.A2AMessageType.NOTIFICATION]: 'general',
        };
        return capabilityMap[messageType] || 'general';
    }
    selectAgentByStrategy(agents, strategy) {
        switch (strategy) {
            case types_js_1.LoadBalancingStrategy.ROUND_ROBIN:
                return this.selectRoundRobin(agents);
            case types_js_1.LoadBalancingStrategy.LEAST_LOADED:
                return this.selectLeastLoaded(agents);
            case types_js_1.LoadBalancingStrategy.FASTEST_RESPONSE:
                return this.selectFastestResponse(agents);
            case types_js_1.LoadBalancingStrategy.CAPABILITY_MATCH:
    ...
    async queueMessage(targetAgent, message) {
        try {
            ...
            return {
                ...
                data: { queued: true },
                processingTime: 0,
                agentStatus: types_js_1.AgentStatus.ONLINE,
            };
        }
        catch (error) {
    ...
    mapA2APriorityToJobPriority(a2aPriority) {
        const mapping = {
            [types_js_1.A2APriority.CRITICAL]: 1,
            [types_js_1.A2APriority.HIGH]: 2,
            [types_js_1.A2APriority.MEDIUM]: 3,
            [types_js_1.A2APriority.LOW]: 4,
            [types_js_1.A2APriority.BATCH]: 5,
        };
    ...
    async sendHeartbeats() {
        const heartbeatMessage = {
            ...
            fromAgent: 'system',
            toAgent: '*',
            type: types_js_1.A2AMessageType.HEARTBEAT,
            payload: { timestamp: Date.now() },
            priority: types_js_1.A2APriority.LOW,
            timestamp: Date.now(),
    ...
    async updateAgentStatus(agentId, status) {
        ...
        agent.isOnline = status === types_js_1.AgentStatus.ONLINE;
        agent.lastSeen = Date.now();
    ...
    async discoverAgents(criteria) {
        ...
        if (criteria?.status) {
            const isOnline = criteria.status === types_js_1.AgentStatus.ONLINE;
    ...
    async getOnlineAgents() {
        return this.discoverAgents({ status: types_js_1.AgentStatus.ONLINE });
    }
    async getAgentHealth(agentId) {
        ...
        return {
            ...
            status: agent.isOnline ? types_js_1.AgentStatus.ONLINE : types_js_1.AgentStatus.OFFLINE,
    ...
    async sendResponse(originalMessage, responsePayload, fromAgent) {
        const response = {
            ...
            type: types_js_1.A2AMessageType.DATA_RESPONSE,
    ...
    async facilitateAgentHandshake(agent1Id, agent2Id) {
        ...
        await this.sendMessage({
            ...
            fromAgent: 'system',
            toAgent: agent1Id,
            type: types_js_1.A2AMessageType.COLLABORATION_REQUEST,
    ...
    async sendHeartbeat(heartbeat) {
        ...
        agent.isOnline = heartbeat.status === types_js_1.AgentStatus.ONLINE;
...
exports.A2AService = A2AService = A2AService_1 = __decorate([
    ...
    __metadata("design:paramtypes", [config_1.ConfigService,
        ap2_protocol_1.Ap2ProtocolService,
        infrastructure_1.UnifiedRedisService])
], A2AService);
//# sourceMappingURL=a2a.service.js.map