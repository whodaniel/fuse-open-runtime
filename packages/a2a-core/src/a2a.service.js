...
import { AgentStatus, A2AMessageType, A2APriority, LoadBalancingStrategy } from './types';
let A2AService = A2AService_1 = class A2AService {
    ...
    metricsInterval;
    redis;
    constructor(configService, ap2ProtocolService) {
    ...
    async registerAgent(registration) {
        try {
            ...
            await this.redis.setex(`agent:${registration.agentId}`, 3600, // 1 hour TTL
    ...
    async updateAgentStatus(agentId, status) {
        ...
        // Update in Redis
        await this.redis.setex(`agent:status:${agentId}`, 3600, JSON.stringify({ status, timestamp: Date.now() }));
    ...
    async sendHeartbeat(heartbeat) {
        ...
        await this.redis.setex(`heartbeat:${heartbeat.agentId}`, this.config.heartbeatInterval * 2, JSON.stringify(heartbeat));
