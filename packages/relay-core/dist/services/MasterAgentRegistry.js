...
class MasterAgentRegistry extends events_1.EventEmitter {
    constructor(drizzle, logger, blockchainConfig, vcPrivateKey) {
        ...
        this.fairtableAdapters = null;
        this.spreadsheetRowMap = new Map();
    ...
    async syncAllAgentsToSpreadsheet() {
        ...
        try {
            ...
            this.logger.info(`📊 All agents synced to spreadsheet (table=${this.spreadsheetIntegration.tableId || 'n/a'}, rows=${this.spreadsheetRowMap.size})`);
        }
        catch (error) {
            this.spreadsheetIntegration.syncStatus = 'failed';
            this.spreadsheetIntegration.errorMessage =
    ...
    loadFairtableAdapters() {
        if (this.fairtableAdapters)
        ...
        try {
            ...
            const module = require('@the-new-fuse/fairtable-adapters');
            if (!module?.convertLegacyKanbanToAirtable || !module?.validateDataCompatibility) {
                this.logger.warn('📊 Fairtable adapters loaded but required conversion utilities are missing');
                return null;
            }
            this.fairtableAdapters = module;
            this.logger.info(`📊 Fairtable adapters loaded${module.ADAPTER_VERSION ? ` (v${module.ADAPTER_VERSION})` : ''}`);
            return this.fairtableAdapters;
        }
        catch (error) {
            this.logger.warn(`📊 Unable to load fairtable adapters: ${error instanceof Error ? error.message : String(error)}`);
            return null;
        }
    }
    buildLegacySpreadsheetData() {
    ...
    async rebuildSpreadsheetSnapshot() {
        const adapters = this.loadFairtableAdapters();
        ...
        const conversion = adapters.convertLegacyKanbanToAirtable(legacyData, 'tnf_master_agent_registry', 'TNF Master Agent Registry');
