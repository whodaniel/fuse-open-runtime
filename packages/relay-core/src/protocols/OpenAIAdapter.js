...
class OpenAIAdapter {
  ...
  private createOpenAIFunctionCall(payload: any): any {
    const functionCall = {
      ...
      content: null,
      tool_calls: [
        {`, id, payload.toolCallId || call_$, { Date, : .now() } ``, type, 'function',  || {});
    }
}
...
return functionCall;
createOpenAIToolOutput(payload, any);
any;
{
    return {
...
createOpenAIAssistantMessage(payload, any);
any;
{
    return {
...
createOpenAIMessage(payload, any);
any;
{
    return {
        role: payload.role || 'user',
