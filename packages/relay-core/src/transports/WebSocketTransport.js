...
class WebSocketTransport extends events_1.EventEmitter {
    ...
    async start() {
      ...
      return true;
    } catch (error) {`, this.logger.error(`Failed to start WebSocket server: ${error instanceof Error ? error.message : String(error)}`));
            return false;
        }
        finally {
        }
    ...
    handleConnection(ws) {
        ...
        const heartbeatWs = ws;
        heartbeatWs.isAlive = true;
        this.clients.set(clientId, heartbeatWs);
        `
...
`, heartbeatWs.on('error', (error) => {
      ...
      id: `, welcome_$, { clientId }, type, source, target, payload, timestamp;
        ().toISOString(),
        ;
    }
    ;
