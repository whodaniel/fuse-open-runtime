...
class FileTransport extends events_1.EventEmitter {
    ...
    async start() {
        ...
        try {
            ...
            this.watcher = chokidar_1.default.watch(path_1.default.join(this.queueDir, '*', 'outbox', '*.json'), {
                ignored: /\.|~$/,
                persistent: true,
                awaitWriteFinish: {
      ...
      return true;
    } catch (error) {`, this.logger.error(`Failed to start file transport: ${error instanceof Error ? error.message : String(error)}`));
            return false;
        }
        finally {
        }
    ...
    async send(message) {
        ...
        try {
            ...
            `
      this.logger.error(Failed to send file message to ${targetId}`;
            $;
            {
