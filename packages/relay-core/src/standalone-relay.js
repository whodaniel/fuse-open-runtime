...
function resolveRelayAgentStatus(input) {
  return (
    (0, lifecycle_js_1.normalizeAgentLifecycleStatus)(typeof input === 'string' ? input : null) ||
    'active'
  );
}
...
class TNFRelayServer extends events_1.EventEmitter {
  constructor(port = PORT) {
    ...
    this.stallDetector.on('conversation:recovered', (event) => {
      ...
      if (
        manager &&
        manager.getPhase() === conversation_state_machine_js_1.ConversationPhase.STALLED
      ) {
  ...
  handleMessage(ws, message, currentAgentId) {
    ...
    if (type === 'REGISTER') {
      ...
      const regName =
        regPayload.name || regPayload.clientType || regPayload.type || 'Unknown Agent';
      const regId =
        regPayload.id ||
    ...
    switch (type) {
      case 'AGENT_REGISTER': {
        ...
        const token = payload?.token || message?.token;
        ...
        TerminalFormatter_js_1.relay.agentRegistered(
          agent.name,
          id,
          agent.platform,
          !!verifiedToken
      ...
      case 'MESSAGE_SEND': {
        ...
        if (channel && !metadata?.isSystemMessage && !metadata?.isRecoveryAttempt) {
          ...
          if (currentPhase === conversation_state_machine_js_1.ConversationPhase.PAUSED) {
            ...
            if (
              currentPhase === conversation_state_machine_js_1.ConversationPhase.EXECUTION ||
  ...
  handleBridgeEgress(envelope) {
    ...
    const envelopeMetadata =
      typeof envelope.metadata === 'object' && envelope.metadata ? envelope.metadata : undefined;
