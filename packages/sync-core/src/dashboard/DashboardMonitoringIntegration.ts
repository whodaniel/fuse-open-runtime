...
export class DashboardMonitoringIntegration extends EventEmitter implements OnModuleInit {
  ...
  private processMetricsUpdate(metrics: any, tenantId?: string): void {
    ...
    if (metrics.operations && metrics.errors) {
      const totalOps = Object.values(metrics.operations).reduce((sum: number, val: any) => sum + (val || 0), 0);
      const totalErrors = Object.values(metrics.errors).reduce((sum: number, val: any) => sum + (val || 0), 0);
    ...
    if (metrics.operations?.conflicts && metrics.operations?.sync) {
      const conflictRate = metrics.operations.sync > 0 ? 
        metrics.operations.conflicts / metrics.operations.sync : 0;
  ...
  private checkThreshold(threshold: AlertThreshold): void {
    try {
      ...
      const tenantKeys = Array.from(this.metricsBuffer.keys())
        .filter(key => key.startsWith(threshold.metric + ':'));
      ...
      for (const metricKey of tenantKeys) {
        ...
        const avgValue = windowData.reduce((sum, entry) => sum + entry.value, 0) / windowData.length;
