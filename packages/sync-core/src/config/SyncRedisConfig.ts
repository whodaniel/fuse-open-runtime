...
export class SyncRedisConfig {
    ...
    return {
      ...
      fileSync: {
        changes: (tenantId?: string) =>
          tenantId
            ? `${prefix}:sync:files:tenant:${tenantId}:changes`
            : `${prefix}:sync:files:global:changes`,
        checksums: (tenantId?: string) =>
          tenantId
            ? `${prefix}:sync:files:tenant:${tenantId}:checksums`
            : `${prefix}:sync:files:global:checksums`,
        watchers: `${prefix}:sync:files:watchers`,
        conflicts: (tenantId?: string) =>
          tenantId
            ? `${prefix}:sync:files:tenant:${tenantId}:conflicts`
            : `${prefix}:sync:files:global:conflicts`,
      },
      ...
      channels: {
        ...
        tenantWorkflows: (tenantId: string) =>
          `${prefix}:sync:channel:tenant:${tenantId}:workflows`,
      ...
      locks: {
        ...
        fileWatcher: (path: string) =>
          `${prefix}:lock:filewatcher:${Buffer.from(path).toString('base64')}`,
      },
  ...
  sanitizeResourceId(resourceId: string): string {
    ...
    return resourceId.replace(/[^a-zA-Z0-9_-]/g, '_').substring(0, 128); // Limit length
  }
}
