...
async function loadCredentialsAndRunServer() {
  ...
  if (hasEnvCredentials()) {
    ...
    auth = new google.auth.OAuth2(client_id, client_secret, 'http://localhost');
    auth.setCredentials({ refresh_token });
  } else {
    // Fall back to file-based credentials
    if (!fs.existsSync(credentialsPath)) {
      console.error(
        "Credentials not found. Please run with 'auth' argument first, or set GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, and GOOGLE_REFRESH_TOKEN environment variables."
      );
      process.exit(1);
    }
