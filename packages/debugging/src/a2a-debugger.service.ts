...
export class A2ADebuggerService extends EventEmitter implements OnModuleInit, OnModuleDestroy {
  ...
  async updateAgentActivity(agentId: string, activity: {
    messagesSent?: number;
  ...
  async exportDebugSession(sessionId: string): Promise<{
    ...
    const conversations = session.conversations.map(id => this.conversations.get(id)).filter(Boolean);
    const analytics = messages.map(msg => this.messageAnalytics.get(msg.messageId)).filter(Boolean);
  ...
  async getDebugDashboard(): Promise<{
    ...
    const avgLatency = allMessages.length > 0 
      ? allMessages.reduce((sum, msg) => sum + (msg.performanceMetrics.totalLatency || 0), 0) / allMessages.length
      : 0;
  ...
  private findOrCreateConversation(message: A2ADebugMessage): string {
    ...
    for (const [id, conversation] of this.conversations) {
      if (conversation.participants.includes(message.fromAgent) &&
  ...
  private updateAgentMetrics(agentInfo: AgentDebugInfo): void {
    ...
    agentInfo.performanceMetrics.errorRate = total > 0 
      ? (agentInfo.messageStats.failed / total) * 100 
      : 0;
    
    agentInfo.performanceMetrics.reliability = Math.max(0, 100 - agentInfo.performanceMetrics.errorRate) / 100;
  }

  private calculateConversationSummary(conversation: ConversationTrace): ConversationTrace['summary'] {
    ...
    const avgLatency = messages.length > 0
      ? messages.reduce((sum, msg) => sum + (msg.performanceMetrics.totalLatency || 0), 0) / messages.length
      : 0;
  ...
  private identifyBottlenecks(conversation: ConversationTrace): string[] {
    ...
    const highLatencyMessages = messages.filter(msg => 
      (msg.performanceMetrics.totalLatency || 0) > 2000
    );
  ...
  private identifyPatterns(conversation: ConversationTrace): string[] {
    ...
    const typeFrequency = messageTypes.reduce((acc, type) => {
  ...
  private performCleanup(): void {
    ...
    for (const messageId of messageIds) {
      ...
      if (message && (now - message.timestamp) > 3600000) { // 1 hour
