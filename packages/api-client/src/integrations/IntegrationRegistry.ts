...
import { Integration, IntegrationRegistry, IntegrationType } from './types';
...
export class IntegrationRegistryImpl implements IntegrationRegistry {
  ...
  async connectIntegration(id: string): Promise<boolean> {
    ...
    try {
      ...
      if (error instanceof Error) {
        this.logger.error(
          `Failed to connect to integration: ${integration.name} (${integration.id})`,
          {
            error: error.message,
            stack: error.stack,
          }
  ...
  async disconnectIntegration(id: string): Promise<boolean> {
    ...
    try {
      ...
      if (error instanceof Error) {
        this.logger.error(
          `Failed to disconnect from integration: ${integration.name} (${integration.id})`,
          {
            error: error.message,
            stack: error.stack,
          }
  ...
  async executeAction(
    ...
    try {
      this.logger.debug(`Executing action ${action} on ${integration.name} (${integration.id})`, {
        params,
      });
      ...
      if (error instanceof Error) {
        this.logger.error(
          `Failed to execute action ${action} on ${integration.name} (${integration.id})`,
          {
            error: error.message,
            stack: error.stack,
            params,
