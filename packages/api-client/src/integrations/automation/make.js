...
async;
createScenario(teamId, string, name, string, blueprint ?  : any);
Promise < any > {
    try: {
        const: payload, any = {
            name,
...
Promise < any > {
  ...
  private async getScenarioExecutionHistory(scenarioId: string, options: any = {}): Promise<any> {
    try {`, let, url = /api/v2 / scenarios / $, { scenarioId } `/executions;
...
async;
listDataStoreRecords(dataStoreId, string, options, any = {});
Promise < any > {
    try: {
        let, url = /api/v2 / data - stores / $
    }
};
{
    dataStoreId;
...
if (options) {
   ...
   */`;
        async;
        createDataStoreRecord(dataStoreId, string, data, any);
        Promise < any > {} `
    try {
...
async;
createWebhook(scenarioId, string, hookUrl, string);
Promise < any > {
    try: {
  ...
  private async listWebhooks(scenarioId: string): Promise<any> {`),
        try: {} `
      return await this.apiClient.get(` / api / v2 / hooks ? scenarioId = $ : 
    }
};
{
    scenarioId;
