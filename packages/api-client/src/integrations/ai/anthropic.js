...
new Error(Unsupported, Anthropic, action, $, { action });
async;
createChatCompletion(model, string, messages, (Array), // Content can be complex for vision
system ?  : string, options ?  : Record);
Promise < any > {
    if(, model) {
...
for (let i = 1; i < messages.length; i++) {
    if (messages[i].role === messages[i - 1].role) {
        `
            throw new Error(Messages must alternate between "user" and "assistant" roles. Found consecutive ${messages[i].role}`;
        roles.;
        ;
    }
}
...
try {
    ...
    console.error(Anthropic, chat, completion, error);
    for (model; $; { model })
        : $;
    {
...
listKnownModels();
Array < { id: string, name: string, description: string, context_length: number } > {
