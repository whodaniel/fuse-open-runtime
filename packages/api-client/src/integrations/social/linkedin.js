...
export class LinkedInIntegration {
  ...
  async connect(): Promise<boolean> {
    ...
    async createPost(text, visibility = 'PUBLIC', media) {
        try {
            const payload = {} `
        author: urn:li:person:${this.getProfileId()}`, lifecycleState, specificContent;
             && media.length > 0 ? 'IMAGE' : 'NONE',
                visibility;
            {
...
async;
shareUpdate(content, any, visibility = 'PUBLIC');
Promise < any > {
    try: {
        const: payload = {
            content,
...
async;
createCompanyPost(companyId, string, content, any);
Promise < any > {} `
    try {`;
const payload = {
...
;
async;
getAnalytics(companyId, string, timeRange, string);
Promise < any > {
    try: {
...
async;
searchPeople(keywords, string, filters ?  : any);
Promise < any > {
    try: {
        let, url = /v2/people - search ? keywords = $ : 
    }
};
{
...
async;
searchCompanies(keywords, string, filters ?  : any);
Promise < any > {
    try: {} `
