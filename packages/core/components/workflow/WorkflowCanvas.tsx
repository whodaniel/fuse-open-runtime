...
var WorkflowCanvas = function (_a) {
  var nodeTypes = _a.nodeTypes,
    ...
    onEdgeDoubleClick = _a.onEdgeDoubleClick,
    _b = _a.className,
    className = _b === void 0 ? '' : _b;
  var _c = (0, WorkflowContext_1.useWorkflow)(),
    workflow = _c.workflow,
    setWorkflow = _c.setWorkflow,
    selectNode = _c.selectNode,
    selectEdge = _c.selectEdge;
  var onNodesChange = (0, react_1.useCallback)(
    function (changes) {
      setWorkflow(function (prev) {
        ...
        changes.forEach(function (change) {
          switch (change.type) {
            case 'position':
              var node = nextNodes.find(function (n) {
                return n.id === change.id;
              });
              if (node) {
              ...
              break;
            case 'remove':
              var index = nextNodes.findIndex(function (n) {
                return n.id === change.id;
              });
              if (index !== -1) {
  ...
  var onEdgesChange = (0, react_1.useCallback)(
    function (changes) {
      setWorkflow(function (prev) {
        ...
        changes.forEach(function (change) {
          switch (change.type) {
            case 'remove':
              var index = nextEdges.findIndex(function (e) {
  ...
  return (
    <div className={'h-full w-full '.concat(className)}>
      <reactflow_1.default
        ...
        onEdgeClick={function (_, edge) {
          return selectEdge(edge);
        }}
