...
var WorkflowCanvas = function (_a) {
    ...
    var onConnect = (0, react_1.useCallback)(function (connection) {
        setWorkflow(function (prev) {
            return (__assign(__assign({}, prev), { edges: __spreadArray(__spreadArray([], prev.edges, true), [
                    {
    ...
    return className = { "h-full w-full ": .concat(className) } >
        nodes;
    {
        workflow.nodes;
    }
    ...
    onConnect = { onConnect };
    onNodeClick = { function(_, node) { return selectNode(node); } };
    onEdgeClick = { function(_, edge) { return selectEdge(edge); } };
