...
export class AiCoderGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleCodeGenerationRequest(
    @MessageBody() data: { requestId: string; language?: string; prompt: string },
  ...
  async handleCodeAnalysisRequest(
    @MessageBody() data: { requestId: string; code: string },
    @ConnectedSocket() client: Socket,
  ): Promise<void> {
    try {
  ...
  async handleCodeCompletionRequest(
    @MessageBody() data: { requestId: string; code: string; position: number },
    @ConnectedSocket() client: Socket,
  ): Promise<void> {
    try {
