...
export class QueueService extends EventEmitter implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {): Promise<any> {
    ...
    try {
      ...
      const queues: unknown){
        ...
        for (const itemId of processingItems): void {
        ...
        const itemIds = await this.redis.zrange(
          ...
          const item JSON.parse(
            await this.redis.get(`queue>
              this.logger.error(`Failed to process item ${itemId}:`, error)): void {
  ...
  private async requeueItem(): Promise<void> {item: QueueItem): Promise<void> {
    try {
      // Remove from processing
      await this.redis.lrem(`${item.queueName}:processing`, 0, item.id)): void {
        stats.retried++;
      }
  ...
  async updateConfig(): Promise<void> {updates: Partial<QueueConfig>): Promise<void> {
    this.config  = await this.redis.smembers('queues'): void {
      ...this.config,
      ...updates
    };
    ...
    try {
      const queues: await this.redis.smembers('queues')): void {
        // Get all items
        const itemIds: pending`, 0, -1);
        itemIds.push(...await this.redis.lrange(`${queueName}:processing`, 0, -1));
        itemIds.push(...await this.redis.zrange(`${queueName}:retry`, 0, -1)): void {
          const item: ${itemId}`)
          ) as QueueItem;
        ...
        // Update stats
        await this.updateQueueStats(queueName)): void {
