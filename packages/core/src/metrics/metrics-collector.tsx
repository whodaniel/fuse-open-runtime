  ...
  constructor(
    ...
    if (!this.histograms.has(name)) {
      ...
      this.histograms.set(name, histogram): string, labels: Record<string, string>  = new Histogram( {
        ...
        buckets,
        registers {}): void {
    const counter: string, value: number, labels: Record<string, string>  = this.counters.get(name)): void {
      counter.inc(labels);
    }
  }

  public setGauge(name {}): void {
    const gauge: string, value: number, labels: Record<string, string>  = this.gauges.get(name)): void {
      gauge.set(labels, value);
    }
  }

  public observeHistogram(name {}): void {
    const histogram = this.histograms.get(name)): void {
