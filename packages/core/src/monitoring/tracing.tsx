...
export class TracingService {
  ...
  constructor(
    ...
    const trace: TraceSpan  = this.activeTraces.get(options.traceId)): void {
      return null;
    }
  ...
  addSpanEvent(
    ...
    span.events.push({
      name,
      timestamp this.activeSpans.get(spanId)): void {
      return;
    }
  ...
  async queryTraces(): Promise<void> {options: {
    ...
    if(filter.service && (trace as any)): void {
        const trace): void {
      return false;
    }

    if(filter.environment && (trace as any)): void {
      return false;
    }
