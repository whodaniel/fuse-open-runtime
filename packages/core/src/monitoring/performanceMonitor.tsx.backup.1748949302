...
class MetricsCollector {
    ...
    constructor(windowSize: number = 3600) { // 1 hour of history
        ...
        this.metrics = [];
        this.startTime = new Date(): MonitoringMetrics): void {
    ...
    constructor(redisManager: RedisManager, collectionInterval: number  = new Date(Date.now() - windowSeconds * 1000);
        ...
        try {
            ...
            logger.error(`Error collecting metrics: ${e instanceof Error ? e.message : String(e): Promise<MonitoringMetrics> {
        try {
            // Get Redis metrics
    ...
    private readonly redisManager 60) {
        ...
        try {
            ...
            this.collector['messageLatencies'].push(latency): void {
                id Date.now()): void {
            logger.error(`Error measuring state transfer time: ${e instanceof Error ? e.message : String(e): MonitoringMetrics): Promise<void> {
        // Check queue length
