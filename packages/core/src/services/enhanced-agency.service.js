...
enabledCategories: config?.enabledCategories || [',
    this.eventEmitter.emit('service.request.', this.eventEmitter.emit('agency.', orderBy, { createdAt: '',
        this: .eventEmitter.emit('agency.', experienceLevel ?  : JUNIOR, ' | INTERMEDIATE' | SENIOR, ' | ', this.eventEmitter.emit('providers.', orderBy, { createdAt: 'desc' }, { isAvailable: 'desc' }, { averageRating: 'desc',
            by: ['categoryId',
                orderBy, { _count: { id: '',
                        timeframe: string = 30, ';: ,
                        const: timeframeDays = parseInt(timeframe.replace('')),
                        const: completedRequests = requests.filter(r => r.status === ''),
                        SIMPLE, ': MODERATE, ': COMPL, 'EX': '', EXPERT, ': ,
                        ...
                        date: date.toISOString().split('T', date, date.toISOString().split('')),
