...
let RateLimiter = (() => {
    ...
    var RateLimiter = class {
        ...
        configService;
        logger = new common_1.Logger(RateLimiter.name);
        config;
        ...
        constructor(configService) {
            ...
            this.config = {
                maxRequests: this.configService.get('CODE_EXECUTION_RATE_LIMIT_MAX_REQUESTS', 10),
                windowMs: this.configService.get('CODE_EXECUTION_RATE_LIMIT_WINDOW_MS', 60000), // 1 minute
                skipList: this.configService.get('CODE_EXECUTION_RATE_LIMIT_SKIP_LIST', '').split(',').filter(Boolean),
            };
