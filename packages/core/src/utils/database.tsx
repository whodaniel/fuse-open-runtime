...
// Database configuration options interface
...
// Query result type
type QueryResult = OkPacket | RowDataPacket[] | ResultSetHeader | RowDataPacket[][];

// Shard configuration interface
interface ShardConfig extends DatabaseConfigOptions {
    uri: string;
}
...
export class DatabaseConfig extends EventEmitter {
    ...
    async initShards(shardConfigs: Record<string, ShardConfig>): Promise<void> {
        for (const [shardName, config] of Object.entries(shardConfigs)) {
            await this.addShard(shardName, config);
        }
    }

    async addShard(shardName: string, config: ShardConfig): Promise<void> {
        try {
            ...
            // pool.on('error', (err) => { ... }); // Consider adding explicit pool error handling if needed
    ...
    async getConnection(shard: string): Promise<PoolConnection> {
        ...
        try {
            ...
            const connectionAcquisitionDuration = Date.now() - startTime;
            this.metrics.latency[shard].push(connectionAcquisitionDuration);
            ...
            const originalQuery = wrappedConnection.query.bind(wrappedConnection);
            const self = this;
            ...
            wrappedConnection.query = async function <T extends QueryResult>(sql: string | QueryOptions, values?: unknown): Promise<[T, FieldPacket[]]> {
                ...
                try {
                    ...
                    (self as any).metrics.latency[shard].push(queryDuration);
    ...
    private async updateRedisMetrics(shard: string, metric: string, value: number): Promise<void> {
        if (this.redis) {
            try {
                await this.redis.hincrby(`db:metrics:${shard}`, metric, value);
            } catch (error: unknown) {
                logger.error(`Failed to update Redis metrics: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }

    // Use the defined interface for return type
        ...
        await Promise.all(
            Object.entries(this.shardMap).map(async ([shard, pool]) => {
                try {
                    ...
                    logger.error(`Error closing pool for shard ${shard}: ${error instanceof Error ? error.message : String(error)}`);
                }
            })
        );
...
export async function initializeDatabase(): Promise<DatabaseConfig> {
    ...
    await dbConfig.initShards(defaultShards);
    return dbConfig;
}
