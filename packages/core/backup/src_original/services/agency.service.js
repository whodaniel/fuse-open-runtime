...
import { Agency, AgencyTier, SubscriptionStatus, UserRole, AgencySubscription } from '@prisma/client';
import * as bcrypt from 'bcrypt';
import { v4 as uuidv4 } from 'uuid';
let AgencyService = AgencyService_1 = class AgencyService {
    constructor(prisma, config, templateService, billingService, cacheService, eventEmitter) {
        this.prisma = prisma;
        this.config = config;
    ...
    async createAgency(data) {
        ...
        // Emit events
        this.eventEmitter.emit('agency.', created, ', {, agencyId, agency.id, subdomain, agency.subdomain, tier, agency.subscriptionTier, adminEmail, data.adminEmail);
    }
    ;
...
AgencyService = AgencyService_1 = __decorate([
    ...
    __metadata('design:paramtypes', [PrismaService,
        ConfigService, typeof (_a = typeof AgencyTemplateService !== 'undefined' && AgencyTemplateService) === 'function' ? _a : Object, typeof (_b = typeof BillingService !== 'undefined' && BillingService) === 'function' ? _b : Object, typeof (_c = typeof CacheService !== 'undefined' && CacheService) === 'function' ? _c : Object, EventEmitter2])
], AgencyService);
export { AgencyService };
 && data.tier !== AgencyTier.TRIAL;
{
    try {
...
// Check cache first
let agency = await this.cacheService.get(cacheKey);
if (!agency) {
    // Database lookup
 ...
 */
async;
getAgencyWithStats(agencyId, string);
Promise < AgencyWithStats > {
 ...
 */
async;
updateAgency(agencyId, string, updates, Partial < Pick < Agency, name, ' | settings' | branding, '>>,, actorId, string);
Promise < Agency > {
    ...
    this: .eventEmitter.emit('agency.', updated, ", {, agencyId, changes, updates, actorId)
};
;
 ...
 */
async;
setAgencyStatus(agencyId, string, isActive, boolean, reason, string, actorId, string);
Promise < Agency > {
    const: agency = await this.prisma.$transaction(async (tx) => {
        ...
        await tx.agencyAuditLog.create({
            data: {
                agencyId,
                action: isActive ? ACTIVATE_AGENCY : , ": ': SUSPEND_AGENCY, ',: actorId,
    ...
    // Clear cache
    await, this: .clearAgencyCache(agency.subdomain),
    this: .eventEmitter.emit('agency.', status_changed, ', {, agencyId, isActive, reason, actorId)
};
;
return agency;
/**
 ...
 */
getAgencyFeatures(tier, AgencyTier);
AgencyFeatures;
{
    const featureMap = {
        [AgencyTier.TRIAL]: {
...
const agenciesWithStats = await Promise.all(agencies.map(async (agency) => {
    ...
    return {};
}, ...agency, stats, subscription));
;
;
return { agencies: agenciesWithStats, total };
async;
validateAgencyCreation(data, CreateAgencyDto);
Promise < void  > {
    : .isValidSubdomain(data.subdomain)
};
{
...
getDefaultSettings(tier, AgencyTier);
any;
{
    return {
...
async;
provisionDefaultResources(tx, any, agencyId, string, template ?  : string);
Promise < void  > {
    const: templateData = await this.templateService.getTemplate(template || basic, ');)
    // Create default agents
    ,
    // Create default agents
    for(, agentTemplate, of, templateData) { }, : .agents
};
{
...
async;
getActiveSubscription(agencyId, string);
...
;
async;
clearAgencyCache(subdomain, string);
Promise < void  > {
