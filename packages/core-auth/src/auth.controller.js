...
let AuthController = class AuthController {
  ...
  async githubCallback(@Req() req: Request, @Res() res: Response) {
    ...
    const tokens = await this.authService.generateTokens(user);` `
    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';`, res.redirect($, { frontendUrl } / auth / callback ? token = $ : , { tokens, : .accessToken } & refresh, $, { tokens, : .refreshToken } `);
  }
