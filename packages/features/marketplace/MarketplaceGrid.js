...
var MarketplaceGrid = function () {
    ...
    if (loading) {
        return className = "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 p-6" >
            { __spreadArray([], Array) { } }(8), true;
        map(function (_, i) { return key = { i }; className = "animate-pulse bg-gray-200 rounded-lg h-96" /  > ; });
    }
}
    < /div>;
if (error) {
    return className = "flex items-center justify-center h-64" >
        className;
    "text-center" >
        className;
...
return className = "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 p-6" >
    { items, : .map(function (item) { return key = { item, : .id }; item = { item }; onSelect = { selectItem }; onDownload = { function(item) { return downloadItem(item.id); } }; onPurchase = { function(item) { return purchaseItem(item.id); } } /  > ; }) };
/div>;
;
;
