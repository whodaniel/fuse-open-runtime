    ...
    var renderRating = function () {
        return className = "flex items-center" >
            { [1, 2, 3, 4, 5]: .map(function (star) {
                    return key = { star };
                    className = { "w-4 h-4 ": .concat(star <= item.rating ? 'text-yellow-400' : 'text-gray-300') };
                    fill = "currentColor";
                    viewBox = "0 0 20 20" >
                        fillRule;
                    "evenodd";
                    d = "M10 15.585l-6.327 3.89 1.42-7.897L.18 6.974l7.924-1.092L10 0l1.896 5.882 7.924 1.092-4.913 4.604 1.42 7.897z";
                    clipRule = "evenodd" /  >
                        /svg>;
                }) };
    }
...
return className = "h-full transition-shadow hover:shadow-md cursor-pointer";
onClick = { function() { return onSelect === null || onSelect === void 0 ? void 0 : onSelect(item); } } >
    { /* Thumbnail */};
{
...
"flex items-start justify-between" >
    className;
"text-lg font-medium text-gray-900" > { item, : .title } < /h3>
    < p;
...
className = "mt-4 flex items-center justify-between" >
    className;
"flex items-center" >
    { item, : .author.avatar ? src = { item, : .author.avatar } : , alt = { item, : .author.name }, className = "w-6 h-6 rounded-full" /  >  }(className, "w-6 h-6 rounded-full bg-gray-200" /  > );
className;
...
className = "flex space-x-2" >
    { item, : .price > 0 ? variant = "primary" : , onClick = { function(e) {
                e.stopPropagation();
                onPurchase === null || onPurchase === void 0 ? void 0 : onPurchase(item);
            } } >
            Purchase
            < /Button_1.Button> }(variant, "outline", onClick = { function(e) {
            e.stopPropagation();
