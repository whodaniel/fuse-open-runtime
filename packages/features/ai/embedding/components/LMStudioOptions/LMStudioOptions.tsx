...
function LMStudioModelSelection({ settings, basePath = null }) {
    ...
    useEffect(() => {
        async function fetchModels(): Promise<void> {) {
            ...
            setError(null);
            try {
                const { models } = await System.fetchLMStudioModels(basePath);
                setModels(models);
            }
            catch (error) {
...
export function LMStudioEmbeddingOptions({ settings }) {
      ...
      <div className={STYLES.inputGroup}>
        <div className={STYLES.inputContainer}>
          ...
          <div className="flex items-center gap-x-4">
            <input type="url" name="EmbeddingBasePath" className={STYLES.input} placeholder="http://localhost:1234" value={basePathValue} onChange={(e) => basePath.onChange(e.target.value)} onBlur={() => basePath.onBlur()} required autoComplete="off" spellCheck={false}/>
