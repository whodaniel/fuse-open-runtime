...
export function VoyageAiOptions({ settings }) {
    return (<div className={STYLES.container}>
      <div className={STYLES.inputGroup}>
        <div className={STYLES.inputContainer}>
          ...
          <input type="password" name="VoyageAiApiKey" className={STYLES.input} placeholder="Voyage AI API Key" defaultValue={(settings === null || settings === void 0 ? void 0 : settings.VoyageAiApiKey) ? "*".repeat(20) : ""} required autoComplete="off" spellCheck={false}/>
        </div>
        <div className={STYLES.inputContainer}>
          <label className={STYLES.label}>
            Model Preference
          </label>
          <select name="EmbeddingModelPref" required defaultValue={settings === null || settings === void 0 ? void 0 : settings.EmbeddingModelPref} className={STYLES.select}>
            <optgroup label="Available embedding models">
              {VOYAGE_MODELS.map((model) => (<option key={model} value={model}>
                  {model}
                </option>))}
            </optgroup>
          </select>
