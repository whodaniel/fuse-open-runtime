...
var ActivityFeed = function (_a) {
    ...
    return className = { "space-y-4 ": .concat(className) } >
        className;
    "text-lg font-medium text-gray-900" > Activity;
    Feed < /h3>
        < div;
    className = "flow-root" >
        className;
    "-mb-8" >
        { events, : .map(function (event, eventIdx) {
                return key = { event, : .id } >
                    className;
                "relative pb-8" >
                    { eventIdx } !== events.length - 1 ? className = "absolute top-5 left-5 -ml-px h-full w-0.5 bg-gray-200" : ;
                aria - hidden;
                "true" /  > ;
            }), null:  }
        < div;
    ...
    size = "sm" /  >
        className;
    "absolute -right-1 -bottom-1 bg-white rounded-full p-0.5" >
        className;
    "text-sm";
    role = "img";
    aria - label;
    ...
    < /div>;
{
    event.metadata.details && className;
