...
var TableWidget = function (_a) {
    ...
    if (loading) {
        return className = { "bg-white rounded-lg shadow p-6 ": .concat(className) } >
            className;
        "animate-pulse" >
            className;
        "h-4 bg-gray-200 rounded w-1/4 mb-4" > /div>
            < div;
        className = "space-y-3" >
            { __spreadArray([], Array) { } }(pageSize), true;
        map(function (_, i) { return key = { i }; className = "h-10 bg-gray-200 rounded" > /div>; });
    }
}
    < /div>
    < /div>
...
if (error) {
    ...
    table
        < /h3>
        < p;
    className = "mt-1 text-sm text-gray-500" > { error } < /p>
        < /div>
        < /div>;
    ;
}
...
"flex items-center justify-between mb-4" >
    className;
"text-lg font-medium text-gray-900" > { title } < /h3>;
{
...
"bg-gray-50" >
    { columns, : .map(function (column) {
            ...
            className = { "px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider ": .concat(column.width || '') } >
                { column, : .sortable ? className = "group inline-flex items-center space-x-1" : , onClick = { function() { return handleSort(column.key); } } >
                        { column, : .label } < /span>
    ...
    < /div>;
{
    ...
    "flex items-center space-x-2" >
        onClick;
    {
        function () { return setCurrentPage(function (p) { return Math.max(1, p - 1); }); }
    }
...
/span>
    < button;
onClick = { function() { return setCurrentPage(function (p) { return Math.min(totalPages, p + 1); }); } };
