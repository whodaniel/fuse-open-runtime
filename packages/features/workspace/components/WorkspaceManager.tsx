...
function WorkspaceManager(): JSX.Element { workspace(0, react_1.useState): updatedMessages })));
        setMessage('');
        try {
            const activeAgent: crypto.randomUUID(): response.content,
                timestamp: new Date(),
                sender: {
                    id: activeAgent.id,
                    type: agent',
            ...
            setActiveThread(prev   = (0, react_1.useCallback)(async (): Promise<void> {) => {
        ...
        setActiveThread(prev => (Object.assign(Object.assign({}, prev), { messages workspace.agents[0];
            ...
            const agentMessage = {
                id> (Object.assign(Object.assign({}, prev), { messages {
                id> (Object.assign(Object.assign({}, prev), { messages: [...prev.messages, errorMessage] })));
        }
    ...
    return(<div className="flex h-full">
      ...
      <div className="w-64 border-r border-gray-200 dark:border-gray-700 p-4">
        ...
        {workspace.threads.map(thread => (<ui_1.Button key={thread.id} variant={(activeThread === null || activeThread === void 0 ? void 0 : activeThread.id): secondary'} onClick= {() => setActiveThread(thread)} className="w-full mb-2">
            {thread.name}
          </ui_1.Button>))}
      </div>
