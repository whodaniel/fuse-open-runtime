...
$;
{
    ...
    "flex items-center justify-between" >
        className;
    "text-sm font-medium" > Success;
    Rate < /span>
        < span;
    className = "text-sm" > { agent, : .metrics.successRate } % /span>
        < /div>
        < div;
    className = "flex items-center justify-between" >
        className;
    "text-sm font-medium" > Response;
    Time < /span>
        < span;
    className = "text-sm" > { agent, : .metrics.averageResponseTime };
    ms < /span>
        < /div>
        < div;
    className = "flex flex-wrap gap-2" >
        { agent, : .capabilities.map((capability) => key = { capability }, className = "inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800" >
                { capability }
                < /span>) }
        < /div>
        < /div>
    ...
    className = "flex justify-between" >
        className;
    "flex space-x-2" >
        { : .Button, variant = "ghost", size = "sm", onClick = {}(), onStop } === null || onStop === void 0 ? void 0 : ;
    {
        className;
        "h-4 w-4" /  > ;
    }
...
size = "sm";
onClick = {}();
onEdit === null || onEdit === void 0 ? void 0 : onEdit(agent);
...
size = "sm";
onClick = {}();
onDelete === null || onDelete === void 0 ? void 0 : onDelete(agent);
