...
function showWelcomeMessage(context) {
    ...
    if (!hasShownWelcome) {
        ...
        vscode.window.showInformationMessage('The New Fuse is now active! Try out the AI collaboration features.', 'Show AI Agents', 'Start Collaboration', 'Read Quick Start').then(selection => {
            if (selection === 'Show AI Agents') {
                ...
                vscode.workspace.openTextDocument(vscode.Uri.file(context.extensionPath + '/QUICK-START.md')).then(doc => {
    ...
    const walkthrough = vscode.window.createWebviewPanel('fuseWalkthrough', 'The New Fuse - Interactive Walkthrough', vscode.ViewColumn.Active, {
        enableScripts: true,
  ...
  `;
    ...
    walkthrough.webview.onDidReceiveMessage(message => {
