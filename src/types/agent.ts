...
export interface AgentState {
    id: string;
    agentId: string;
    status: 'idle' | 'busy' | 'error'; // Corrected string literal union
    currentTask?: string;
    lastActive: Date;
    // Combine metrics into a single object to avoid duplicate keys
    metrics: {
        totalTasks: number;
    ...
    pendingTasks: any[];
    activeTools: string[];
    // Removed duplicate metrics field
}
...
export interface AgentAction {
    ...
    status: 'pending' | 'running' | 'completed' | 'failed'; // Corrected string literal union
    result?: unknown;
    error?: string;
    startTime: Date;
    endTime?: Date;
...
export interface Message {
  role: 'user' | 'assistant' | 'system'; // Corrected string literal union
  content: string;
