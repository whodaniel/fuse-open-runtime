...
const FilePathSchema = z.string().describe('Relative path within the workspace');
...
export const fileTools = {
    ...
    readFile: async (params, context) => {
        ...
        catch (error) { // Catch as 'any' or 'unknown' and check type
...
export const buildTools = {
    ...
    runBuild: async (params, context) => {
        ...
        try {
            ...
            if (stdout)
                logger.info(`[runBuild] Command output: ${stdout}`);
            if (stderr)
                logger.warn(`[runBuild] Command stderr: ${stderr}`);
            return {
                success: true,
...
export const writeFileSchema = WriteFileSchema;
export const runBuildSchema = RunBuildSchema;
