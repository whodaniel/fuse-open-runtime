...
interface ConfigFile {
  ...
  templates?: Record<string, Partial<MCPServerConfig>>;
  discovery?: {
...
class EnhancedMCPConfigManager {
  ...
  private configCache: Map<string, { config: ConfigFile; lastModified: number }> = new Map();
  private healthChecks: Map<string, NodeJS.Timeout> = new Map();
  private oauthClient: any = null; // Will be injected by OAuth integration
  ...
  private initializeDefaultTemplates() {
    ...
    this.configCache.set('templates', { 
      config: { mcpServers: {}, templates } as ConfigFile, 
      lastModified: Date.now() 
    });
  }

  // Implementation methods
  private async listMCPServers(params: any) {
    try {
      ...
      if (params.filter) {
        ...
        servers = servers.filter(server => {
          if (filter.enabled !== undefined && server.enabled !== filter.enabled) return false;
          if (filter.oauth_required !== undefined && server.oauth?.required !== filter.oauth_required) return false;
          if (filter.tags && filter.tags.length > 0) {
            if (!server.tags || !filter.tags.some(tag => server.tags.includes(tag))) return false;
          }
          return true;
        });
      }
  ...
  private async addMCPServer(params: any) {
    try {
      const configPath = this.getConfigPath(params.config_path);
      ...
      if (params.tags) serverConfig.tags = params.tags;
      if (params.priority) serverConfig.priority = params.priority;
      if (params.oauth_config) serverConfig.oauth = params.oauth_config;
      if (params.health_check) serverConfig.health_check = params.health_check;
  ...
  private async removeMCPServer(params: any) {
    try {
      const configPath = this.getConfigPath(params.config_path);
      const config = await this.readConfigFile(configPath);
  ...
  private async validateConfiguration(params: any) {
    try {
      const configPath = this.getConfigPath(params.config_path);
      const config = await this.readConfigFile(configPath);
  ...
  // Helper methods
  private getConfigPath(providedPath?: string): string {
    if (providedPath) return providedPath;
    return path.join(process.cwd(), 'mcp_config.json');
  }

  private async readConfigFile(configPath: string): Promise<ConfigFile | null> {
    try {
  ...
  private async writeConfigFile(configPath: string, config: ConfigFile): Promise<void> {
    const expandedPath = this.expandPath(configPath);
  ...
  private createEmptyConfig(): ConfigFile {
    return {
      version: '2.0.0',
      mcpServers: {},
      metadata: {
  ...
  private async validateServerConfig(server: MCPServerConfig): Promise<ConfigValidationResult> {
  ...
  private async performSingleHealthCheck(server: MCPServerConfig) {
  ...
  private startHealthMonitoring(serverName: string, config: MCPServerConfig) {
    // Placeholder for health monitoring
    this.logger.log(`Starting health monitoring for ${serverName}`);
  }

  private stopHealthMonitoring(serverName: string) {
  ...
  private async getDiscoveryEndpoints() {
    return JSON.stringify({
      endpoints: [
