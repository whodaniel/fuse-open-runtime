...
exports.EnhancedTheNewFuseMCPServer = void 0;
 ...
 */
class EnhancedTheNewFuseMCPServer extends TheNewFuseMCPServer_1.TheNewFuseMCPServer {
    apiBaseUrl;
    authContext = {};
    enhancedServer;
    constructor(isRemote = false) {
        ...
        this.enhancedServer = new index_1.Server({
    ...
    setupEnhancedHandlers() {
        ...
        this.enhancedServer.setRequestHandler(types_1.ListToolsRequestSchema, async () => {
        ...
        // Handle tool calls
        this.enhancedServer.setRequestHandler(types_1.CallToolRequestSchema, async (request) => {
        ...
        // List resources
        this.enhancedServer.setRequestHandler(types_1.ListResourcesRequestSchema, async () => {
            return {
                resources: [
                    {
        ...
        this.enhancedServer.setRequestHandler(types_1.ReadResourceRequestSchema, async (request) => {
    ...
    async handleEnhancedApiTool(toolName, args) {
        switch (toolName) {
            ...
            default: throw new Error(`Unknown enhanced API tool: ${toolName}`);
        }
    }
    // Enhanced tool handlers
    async handleUnifiedLogin(args) {
    ...
    async handleGatewayHealthCheck(args) {
        ...
        return {
            content: [{
                    type: 'text', text: `🏥 API Gateway Health Status:\n\n${JSON.stringify(result, null, 2)}`,
                }],
        };
    }
    async handleComprehensiveAgentList(args) {
        const result = await this.makeApiRequest({
            method: 'GET',
            endpoint: '/agents',
            query: args,
        ...
        return {
            content: [{
                    type: 'text', text: `🤖 Comprehensive Agent List (${agentCount} agents):\n\n${JSON.stringify(result, null, 2)}`,
                }],
        };
    }
    async handleAdvancedChatSessionCreate(args) {
        const result = await this.makeApiRequest({
            method: 'POST',
    ...
    async handleRealTimeMetrics(args) {
        ...
        return {
            content: [{
                    type: 'text', text: `📊 Real-time Platform Metrics:\n\n${JSON.stringify(result, null, 2)}`,
                }],
        };
    }
    async handleAdvancedWebhookRegister(args) {
        const result = await this.makeApiRequest({
            method: 'POST',
        ...
        return {
            content: [{
                    type: 'text', text: `🔗 Advanced Webhook Registered:\n\nWebhook ID: ${result.id}\nSource: ${args.source}\nMonitoring: ${args.configuration?.monitoring ? 'Enabled' : 'Disabled'}\n\n${JSON.stringify(result, null, 2)}`,
                }],
        };
    }
    async handleAdvancedMcpManagement(args) {
        const endpoint = args.action === 'health_check' ? `/mcp/servers/${args.server_id}/status` : `/mcp/servers${args.server_id ? `/${args.server_id}` : ''}`;
        ...
        return {
            content: [{
                    type: 'text', text: `🔧 MCP Server ${args.action} completed:\n\n${JSON.stringify(result, null, 2)}`,
                }],
        };
    }
    async handleWorkflowExecuteWithMonitoring(args) {
        const result = await this.makeApiRequest({
            method: 'POST', endpoint: `/workflows/${args.workflow_id}/execute`,
            data: {
        ...
        return {
            content: [{
                    type: 'text', text: `⚡ Workflow Execution Started:\n\nWorkflow ID: ${args.workflow_id}\nExecution ID: ${result.execution_id}\nMonitoring: ${args.execution_options?.monitor_real_time ? 'Enabled' : 'Disabled'}\n\n${JSON.stringify(result, null, 2)}`,
                }],
        };
    }
    async handleComprehensiveSystemStatus(args) {
    ...
    async getApiPlatformStatus() {
        try {
            ...
            return {
                contents: [{
                        type: 'text',
                        text: JSON.stringify({ error: `Platform status unavailable: ${error.message}`,
                            enhanced_mcp_integration: true,
    ...
    async handleExistingTool(name, args) {
        // This would delegate to the parent class tool handlers throw new Error(`Existing tool ${name} handler not implemented yet`);
    }
    async run() {
