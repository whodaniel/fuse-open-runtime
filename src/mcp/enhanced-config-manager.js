...
// Removed unused imports: CallToolRequestSchema, ListToolsRequestSchema, ListResourcesRequestSchema, ReadResourceRequestSchema, express
...
class EnhancedMCPConfigManager {
    ...
    constructor() {
        this.logger = new logger_service_1.Logger('EnhancedMCPConfigManager');
    ...
    async listMCPServers(params) {
        try {
            ...
            return {
                content: [{
                        type: 'text',
                        text: JSON.stringify({ error: `Error listing MCP servers: ${error.message}`
                        }, null, 2)
                    }]
            };
        }
        ...
        try {
            ...
            return {
                content: [{
                        type: 'text',
                        text: JSON.stringify({
                            success: true, message: `MCP server "${params.name}" configured successfully`,
            ...
            return {
                content: [{
                        type: 'text',
                        text: JSON.stringify({
                            success: false, error: `Error adding MCP server: ${error.message}`
                        }, null, 2)
                    }]
            };
        }
        ...
        try {
            ...
            if (!config || !config.mcpServers[params.name]) {
                return {
                    content: [{
                            type: 'text',
                            text: JSON.stringify({
                                success: false, error: `MCP server "${params.name}" not found`
                            }, null, 2)
                        }]
                };
            }
            ...
            return {
                content: [{
                        type: 'text',
                        text: JSON.stringify({
                            success: true, message: `MCP server "${params.name}" removed successfully`,
            ...
            return {
                content: [{
                        type: 'text',
                        text: JSON.stringify({
                            success: false, error: `Error removing MCP server: ${error.message}`
                        }, null, 2)
                    }]
            };
        }
    ...
    async validateConfiguration(params) {
        try {
            ...
            if (!config) {
                return {
                    content: [{
                            type: 'text',
                            text: JSON.stringify({
                                valid: false, errors: [`Configuration file not found: ${configPath}`]
                            }, null, 2)
                        }]
                };
            }
            ...
            if (params.check_health) {
                await this.healthCheckServers({ config_path: configPath });
                // Health check results are logged by the method itself
            }
            ...
            return {
                content: [{
                        type: 'text',
                        text: JSON.stringify({
                            valid: false, error: `Validation error: ${error.message}`
                        }, null, 2)
                    }]
            };
        }
    ...
    getConfigPath(providedPath) {
        if (providedPath)
            return providedPath;
        return path_1.default.join(process.cwd(), 'mcp_config.json');
    }
    async readConfigFile(configPath) {
        try {
            ...
            this.configCache.set(cacheKey, {
                config,
                lastModified: fs_1.default.statSync(expandedPath).mtimeMs
            });
            return config;
        }
        catch (error) {
    ...
    async writeConfigFile(configPath, config) {
        const expandedPath = this.expandPath(configPath);
        const dirPath = path_1.default.dirname(expandedPath);
        if (!fs_1.default.existsSync(dirPath)) {
            fs_1.default.mkdirSync(dirPath, { recursive: true });
        }
        fs_1.default.writeFileSync(expandedPath, JSON.stringify(config, null, 2));
        // Update cache
    ...
    async validateServerConfig(server) {
        ...
        if (server.command && !server.command.startsWith('http')) {
            try {
                ...
                if (!fs_1.default.existsSync(commandPath) && !this.isCommandInPath(commandPath)) {
    ...
    isCommandInPath(command) {
        ...
        const paths = PATH.split(path_1.default.delimiter);
        return paths.some(p => {
            const fullPath = path_1.default.join(p, command);
            try {
    ...
    async performSingleHealthCheck(_server) {
    ...
    startHealthMonitoring(serverName, _config) {
        // Placeholder for health monitoring this.logger.log(`Starting health monitoring for ${serverName}`);
    }
    ...
    async validateOAuthClient(_clientId) {
        // Placeholder for OAuth client validation
        return true;
    }
    // Placeholder implementations for remaining methods
    async backupConfiguration(params) {
        // Log the backup request for future implementation this.logger.log(`Backup requested for config: ${params?.config_path || 'default'}`);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
                        error: 'Backup feature not yet implemented',
                        message: 'Configuration backup functionality is planned for a future release',
                        suggestion: 'Use manual file copying for now',
                        requested_config: params?.config_path || 'default'
                    }, null, 2)
                }]
        };
    }
    async restoreConfiguration(params) {
        this.logger.log(`Restore requested for backup: ${params?.backup_path || 'unknown'}`);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
                        error: 'Restore feature not yet implemented',
                        message: 'Configuration restore functionality is planned for a future release',
                        suggestion: 'Use manual file restoration for now',
                        requested_backup: params?.backup_path || 'unknown'
                    }, null, 2)
                }]
        };
    }
    async healthCheckServers(params) {
        this.logger.log(`Health check requested for servers: ${params?.server_names?.join(', ') || 'all'}`);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
                        error: 'Health check feature not yet implemented',
                        message: 'Server health monitoring is planned for a future release',
                        suggestion: 'Use manual server status checking for now',
                        requested_servers: params?.server_names || 'all'
                    }, null, 2)
                }]
        };
    }
    async discoverMCPServers(params) {
        this.logger.log(`Discovery requested for endpoints: ${params?.discovery_endpoints?.join(', ') || 'default'}`);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
                        error: 'Discovery feature not yet implemented',
                        message: 'MCP server discovery is planned for a future release',
                        suggestion: 'Use manual server registration for now',
                        requested_endpoints: params?.discovery_endpoints || 'default'
                    }, null, 2)
                }]
        };
    }
    async applyTemplate(params) {
        this.logger.log(`Template application requested: ${params?.template_name} for ${params?.server_name}`);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
                        error: 'Template feature not yet implemented',
                        message: 'Configuration template application is planned for a future release',
                        suggestion: 'Use manual configuration for now',
                        requested_template: params?.template_name,
                        target_server: params?.server_name
                    }, null, 2)
                }]
        };
    }
    async generateOAuthClient(params) {
        this.logger.log(`OAuth client generation requested for: ${params?.server_name}`);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
                        error: 'OAuth generation feature not yet implemented',
                        message: 'OAuth client generation is planned for a future release',
                        suggestion: 'Use manual OAuth configuration for now',
                        target_server: params?.server_name
                    }, null, 2)
                }]
        };
...
exports.default = EnhancedMCPConfigManager;
//# sourceMappingURL=enhanced-config-manager.js.map