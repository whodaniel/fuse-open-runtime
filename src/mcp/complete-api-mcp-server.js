...
exports.CompleteTheNewFuseApiMcpServer = void 0;
...
const api_wrapper_handlers_1 = require("./api-wrapper-handlers");
class CompleteTheNewFuseApiMcpServer {
        ...
        this.server.setRequestHandler(types_1.ListToolsRequestSchema, async () => {
            return {
                tools: [
                    ...
                    ...api_wrapper_handlers_1.ApiWrapperHandlers.getWorkflowTools(),
                    // User Management Tools (5 tools)
                    ...api_wrapper_handlers_1.ApiWrapperHandlers.getUserManagementTools(),
                    // Marketplace Tools (9 tools)
                    ...api_wrapper_handlers_1.ApiWrapperHandlers.getMarketplaceTools(),
                    // Agency Hub Tools (11 tools)
                    ...api_wrapper_handlers_1.ApiWrapperHandlers.getAgencyHubTools(),
                    // Admin & Monitoring Tools (8 tools)
                    ...api_wrapper_handlers_1.ApiWrapperHandlers.getAdminTools(),
                    // Integration Tools (10 tools)
                    ...api_wrapper_handlers_1.ApiWrapperHandlers.getIntegrationTools(),
                ],
            };
        });
        // Handle tool calls
        this.server.setRequestHandler(types_1.CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                return {
                    content: [
                        {
                            type: 'text', text: `Error executing tool ${name}: ${error.message}`,
                        },
                    ],
                    isError: true,
                };
    ...
    // ========== TOOL HANDLERS ==========
    async handleAuthTool(toolName, args) {
        switch (toolName) {
            case 'auth_login':
            ...
            case 'auth_refresh':
                ...
                return {
                    content: [{
                            type: 'text', text: `✅ Token refreshed successfully!\n\n${JSON.stringify(refreshResult, null, 2)}`,
                        }],
                };
            case 'auth_logout':
            ...
            default: throw new Error(`Unknown auth tool: ${toolName}`);
        }
    }
    async handleAgentTool(toolName, args) {
        switch (toolName) {
            case 'agent_list':
                ...
                return {
                    content: [{
                            type: 'text', text: `🤖 Found ${agentCount} agents\n\n${JSON.stringify(agents, null, 2)}`,
                        }],
                };
            case 'agent_create':
            ...
            case 'agent_get':
                const agent = await this.makeApiRequest({
                    method: 'GET', endpoint: `/agents/${args.id}`,
                });
                return {
                    content: [{
                            type: 'text', text: `🤖 Agent Details:\n\n${JSON.stringify(agent, null, 2)}`,
                        }],
                };
            case 'agent_update':
                const { id, ...updateData } = args;
                const updatedAgent = await this.makeApiRequest({
                    method: 'PUT', endpoint: `/agents/${id}`,
                    data: updateData,
                });
                return {
                    content: [{
                            type: 'text', text: `✅ Agent updated successfully!\n\n${JSON.stringify(updatedAgent, null, 2)}`,
                        }],
                };
            case 'agent_update_status':
                const statusResult = await this.makeApiRequest({
                    method: 'PUT', endpoint: `/agents/${args.id}/status`,
                    data: { status: args.status },
                });
                return {
                    content: [{
                            type: 'text', text: `✅ Agent status updated to: ${args.status}\n\n${JSON.stringify(statusResult, null, 2)}`,
                        }],
                };
            case 'agent_delete':
                await this.makeApiRequest({
                    method: 'DELETE', endpoint: `/agents/${args.id}`,
                });
                return {
                    content: [{
                            type: 'text', text: `✅ Agent ${args.id} deleted successfully.`,
                        }],
                };
    ...
    async handleChatTool(toolName, args) {
        ...
        return {
            content: [{
                    type: 'text', text: `💬 ${toolName} result:\n\n${JSON.stringify(result, null, 2)}`,
                }],
        };
    }
    async handleWebhookTool(toolName, args) {
    ...
    async handleWorkflowTool(toolName, args) { }
    async handleUserTool(toolName, args) { }
    async handleMarketplaceTool(toolName, args) { }
    async handleAgencyTool(toolName, args) { }
    async handleAdminTool(toolName, args) { }
    async handleIntegrationTool(toolName, args) { }
    async handleHealthTool(toolName, args) { }
    async handlePlatformTool(toolName, args) { }
    // Helper methods for endpoint mapping
    mapChatToolToEndpoint(toolName, args) {
        // Map chat tools to appropriate API endpoints
        return { method: 'GET', endpoint: '/chat/sessions' };
    }
    mapWebhookToolToEndpoint(toolName, args) {
        // Map webhook tools to appropriate API endpoints
        return { method: 'GET', endpoint: '/webhooks/status' };
    }
    mapMcpToolToEndpoint(toolName, args) {
        // Map MCP tools to appropriate API endpoints
        return { method: 'GET', endpoint: '/mcp/servers' };
    }
    // ========== RESOURCE HANDLERS ==========
    async getCompleteApiSchema() {
        return {
            contents: [{
                    type: 'text',
                    text: JSON.stringify({
    ...
    async getPlatformStatus() {
        try {
            ...
            return {
                contents: [{
                        type: 'text', text: `Platform status check failed: ${error.message}`,
                    }],
            };
        }
    }
    ...
    async getServicesRegistry() {
        try {
            ...
            return {
                contents: [{
                        type: 'text', text: `Services registry unavailable: ${error.message}`,
                    }],
            };
        }
    }
...
exports.CompleteTheNewFuseApiMcpServer = CompleteTheNewFuseApiMcpServer;
...
//# sourceMappingURL=complete-api-mcp-server.js.map