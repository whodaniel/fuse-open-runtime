...
exports.TheNewFuseApiWrapper = void 0;
    ...
    async handleAuthTool(toolName, args) {
        switch (toolName) {
            case 'auth_login':
                ...
                return {
                    content: [{
                            type: 'text', text: `Login successful. Token stored for subsequent requests. User: ${loginResult.user?.email}`,
                        }],
                };
            case 'auth_register':
                ...
                return {
                    content: [{
                            type: 'text', text: `Registration successful. User created: ${registerResult.user?.email}`,
                        }],
                };
            case 'auth_refresh':
    ...
    async handleAgentTool(toolName, args) {
        switch (toolName) {
            case 'agent_list':
                ...
                return {
                    content: [{
                            type: 'text', text: `Found ${agents.data?.length || 0} agents:\n${JSON.stringify(agents, null, 2)}`,
                        }],
                };
            case 'agent_create':
