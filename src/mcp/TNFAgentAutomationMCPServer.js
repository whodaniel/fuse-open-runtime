...
const RooAgentAutomationService_1 = require("../services/RooAgentAutomationService");
...
class TNFAgentAutomationMCPServer {
    ...
    constructor() {
        ...
        this.agentService = new RooAgentAutomationService_1.RooAgentAutomationService(mcpService);
        ...
        this.server.setRequestHandler(types_1.CallToolRequestSchema, async (request) => {
            try {
                ...
                switch (request.params.name) {
                    ...
                    default:
                        throw new types_1.McpError(types_1.ErrorCode.MethodNotFound, `Unknown tool: ${request.params.name}`);
                }
            }
            catch (error) {
                ...
                throw new types_1.McpError(types_1.ErrorCode.InternalError, `Tool execution failed: ${error.message}`);
            }
        });
    }
            ...
            content: [
                {
                    type: 'text',
                    text: JSON.stringify({
                        success: true, message: `Agent '${agent.name}' created successfully`,
                        agent: {
                            name: agent.name,
                            slug: agent.slug,
            ...
            content: [
                {
                    type: 'text',
                    text: JSON.stringify({
                        success: true, message: `Team '${teamType}' created with ${agents.length} agents`,
                        teamType,
    ...
    async handleGetAgent(args) {
        ...
        if (!agent) {
            throw new types_1.McpError(types_1.ErrorCode.InvalidRequest, `Agent with slug '${slug}' not found`);
        }
        return {
            content: [
                {
            ...
            content: [
                {
                    type: 'text',
                    text: JSON.stringify({
                        success: true, message: `Agent '${slug}' deleted successfully`
                    }, null, 2)
                }
            ]
        };
    ...
    async handleBatchCreateAgents(args) {
        const { requests } = args;
            ...
            content: [
                {
                    type: 'text',
                    text: JSON.stringify({
                        success: true, message: `Batch operation completed: ${result.successful.length} successful, ${result.failed.length} failed`,
...
//# sourceMappingURL=TNFAgentAutomationMCPServer.js.map