...
class TNFMCPConfigManager {
    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(types_1.CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                throw new types_1.McpError(types_1.ErrorCode.InternalError, `Tool execution failed: ${errorMessage}`);
            }
        });
    }
    async listMCPServers(args) {
        ...
        const serverList = await Promise.all(Object.entries(servers).map(async ([name, serverConfig]) => ({
            name,
            ...serverConfig,
    ...
    async addMCPServer(args) {
        const { config_path, client_type, name, command, args: serverArgs = [], env = {}, description = '', tags = [], priority = 5, tnf_integration = false, agent_builder_compatible = false } = args;
        const configPath = this.resolveConfigPath(config_path, client_type);
        const config = await this.loadConfig(configPath);
    ...
    async removeMCPServer(args) {
        ...
        return {
            content: [{
                    type: 'text', text: `MCP server '${name}' removed from ${configPath}${create_backup ? `\nBackup created at ${configPath}.backup.${Date.now()}` : ''}\n\nRestart your MCP client to deactivate the server.`
                }]
        };
    }
    async generateClientConfig(args) {
        const { client_type, output_path, include_servers = [], tnf_passthrough = true, network_config = { host: 'localhost', port: 3001 } } = args;
        // Load TNF registry
    ...
    async syncWithTNFRegistry(args) {
        ...
        return {
            content: [{
                    type: 'text', text: `Sync completed between ${config_path} and TNF registry:\n\n${syncResults.join('\n')}\n\nDirection: ${direction}\nMerge strategy: ${merge_strategy}`
                }]
        };
    }
    async validateConfig(args) {
        ...
        return {
            content: [{
                    type: 'text', text: `Configuration validation for ${configPath}:\n\n${issues.length > 0 ? `Issues:\n${issues.join('\n')}\n\n` : ''}${suggestions.length > 0 ? `Suggestions:\n${suggestions.join('\n')}\n\n` : ''}${issues.length === 0 ? 'Configuration is valid!' : ''}`
                }]
        };
    }
    async getTNFPassthroughConfig(args) {
        const { client_type, tnf_endpoint = 'http://localhost:3001', include_direct_servers = false } = args;
...
//# sourceMappingURL=tnf-mcp-config-manager.js.map