...
exports.OAuthConfigIntegration = void 0;
const logger_service_1 = require("../common/logger.service");
const express_1 = __importDefault(require("express"));
class OAuthConfigIntegration {
    logger;
    oauthServer;
    options;
    constructor(oauthServer, options = {}) {
        this.logger = new logger_service_1.Logger('OAuthConfigIntegration');
     ...
     */
    validateConfigManagementToken(req, res, next) {
        ...
        try {
            ...
            const hasRequiredScopes = requiredScopes.some(scope => tokenScopes.includes(scope) || tokenScopes.includes('mcp:admin'));
     ...
     */
    getRequiredScopesForOperation(method, path) {
        // Admin operations
     ...
     */
    async generateSecureServerConfig(serverName, baseConfig, oauthInfo) {
        const secureConfig = {
            ...baseConfig,
            oauth: {
                required: true,
            ...
            monitoring: {
                oauth_health_check: true, token_validation_endpoint: `${this.options.oauth_server_url}/oauth/introspect`,
                health_check: {
                    enabled: true,
     ...
     */
    async generateOAuthClientForServer(serverName, requestorInfo) {
        try {
            // Use OAuth server's dynamic client registration
            const clientRequest = { client_name: `MCP Server: ${serverName}`,
                redirect_uris: ['urn:ietf:wg:oauth:2.0:oob'],
                grant_types: ['client_credentials'],
                response_types: ['code'], scope: `mcp:read mcp:write tnf:${serverName.toLowerCase()}`,
                token_endpoint_auth_method: 'client_secret_basic',
                mcp_capabilities: ['server-to-server', 'configuration-management']
            };
            // Register client with OAuth server (placeholder implementation) const clientId = `tnf_mcp_${serverName.toLowerCase()}_${Date.now()}`;
     ...
     */
    async getSecureDiscoveryInfo(oauthInfo) {
    ...
    getManagementCapabilities(scope) {
        const scopes = scope.split(' ');
        const capabilities = [];
        if (scopes.includes('mcp:read'))
            capabilities.push('list-servers', 'view-configuration');
        if (scopes.includes('mcp:write'))
            capabilities.push('add-servers', 'update-servers');
        if (scopes.includes('mcp:admin'))
            capabilities.push('remove-servers', 'backup-restore', 'oauth-management');
        if (scopes.includes('tnf:config'))
            capabilities.push('template-management', 'discovery-integration');
        return capabilities;
    }
    ...
    async handleAddServer(req, res) {
        try {
            const serverConfig = await this.generateSecureServerConfig(req.body.name, req.body, req.oauth);
    ...
    async handleRemoveServer(req, res) {
        try {
    ...
    async handleBackupConfig(req, res) {
        try {
            // Implementation for configuration backup res.json({ success: true, backup_id: `backup_${Date.now()}` });
        }
        catch (error) {
            res.status(500).json({ error: error.message });
        }
    ...
    async handleGenerateOAuthClient(req, res) {
        try {
    ...
    async listServersWithOAuthInfo(query, oauth) {
        // Implementation would integrate with enhanced config manager
        return [];
    }
    async addServerWithOAuth(serverConfig, oauth) {
        // Implementation would integrate with enhanced config manager
        return serverConfig;
    }
}
exports.OAuthConfigIntegration = OAuthConfigIntegration;
exports.default = OAuthConfigIntegration;
//# sourceMappingURL=oauth-config-integration.js.map