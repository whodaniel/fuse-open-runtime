...
const oauth_config_integration_1 = __importDefault(require("./oauth-config-integration"));
class UnifiedMCPConfigServer {
    ...
    options;
    constructor(options = {}) {
        this.logger = new logger_service_1.Logger('UnifiedMCPConfigServer');
        this.options = {
            http_port: 3773,
            oauth_port: 3001,
            enable_oauth: true,
    ...
    initializeComponents() {
        ...
        if (this.options.enable_oauth) {
            ...
            this.oauthIntegration = new oauth_config_integration_1.default(this.oauthServer, {
    ...
    setupHttpServer() {
        ...
        // Basic middleware
        this.httpApp.use((0, cors_1.default)({
            origin: ['http://localhost:3000', 'http://localhost:5173', 'http://localhost:3001'],
            credentials: true
        }));
        this.httpApp.use(express_1.default.json({ limit: '10mb' }));
        this.httpApp.use(express_1.default.urlencoded({ extended: true }));
        // Request logging
        ...
        this.httpApp.get('/info', (req, res) => {
            res.json({
                ...
                oauth: {
                    enabled: this.options.enable_oauth, authorization_server: this.options.enable_oauth ? `http://localhost:${this.options.oauth_port}` : null,
                    scopes_supported: [
    ...
    async generateDiscoveryResponse() {
        const baseInfo = {
            ...
            endpoints: {
                ...
                http_api: { base_url: `http://localhost:${this.options.http_port}/api/config`,
                    transport: 'http',
        ...
        if (this.options.enable_oauth) {
            return {
                ...baseInfo,
                oauth_integration: {
                    enabled: true, authorization_server: `http://localhost:${this.options.oauth_port}`,
                    discovery_endpoints: { authorization_server_metadata: `http://localhost:${this.options.oauth_port}/.well-known/oauth-authorization-server`, protected_resource_metadata: `http://localhost:${this.options.oauth_port}/.well-known/oauth-protected-resource`
                    },
                    client_registration: {
                        dynamic_registration_supported: true, registration_endpoint: `http://localhost:${this.options.oauth_port}/oauth/register`
                    }
                }
            };
        }
...
exports.default = UnifiedMCPConfigServer;
//# sourceMappingURL=unified-config-server.js.map