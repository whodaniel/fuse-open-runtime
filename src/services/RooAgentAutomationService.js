...
let RooAgentAutomationService = RooAgentAutomationService_1 = class RooAgentAutomationService extends EventEmitter {
    ...
    workspaceRoot;
    constructor(mcpService, agentOrchestrator, // Inject AgentOrchestrator
    config) {
        super();
    ...
    async createGlobalMode(config) {
        ...
        try {
            ...
            globalModes.customModes = globalModes.customModes.filter((mode) => mode.slug !== config.slug);
            // Add new mode
     ...
     */
    formatGroups(groups, fileRestrictions) {
        if (!fileRestrictions) {
            return groups;
        }
     ...
     * Setup MCP servers
     */
    async setupMCPServers(serverKeys) {
        const { globalConfigPath } = this.getConfigPaths();
     ...
     */
    async createProjectMode(config, projectPath) {
        ...
        try {
            ...
            // Remove existing mode with same slug
            projectModes.customModes = projectModes.customModes.filter((mode) => mode.slug !== config.slug);
            // Add new mode
     ...
     */
    async notifyAgentCreation(config) {
        try {
            await this.communicationService.requestCollaboration('code_generation', {
    ...
    async createDevelopmentTeam(teamType) {
        ...
        for (const memberType of teamConfig.members) {
            try {
                ...
                const errorMsg = `Failed to create agent ${memberType}: ${error.message}`;
     ...
     */
    async deleteAgent(slug, options = {}) {
        try {
            if (options.isGlobal !== false) {
     ...
     */
    async deleteGlobalAgent(slug) {
        const { globalConfigPath } = this.getConfigPaths();
        const globalModesPath = path.join(globalConfigPath, 'global_modes.json');
        try {
            const content = await fs.readFile(globalModesPath, 'utf8');
            const globalModes = JSON.parse(content);
            globalModes.customModes = globalModes.customModes.filter((mode) => mode.slug !== slug);
            await fs.writeFile(globalModesPath, JSON.stringify(globalModes, null, 2));
        }
        catch (error) {
            this.logger.warn(`Could not delete global agent '${slug}': ${error.message}`);
        }
    }
    /**
     * Delete project agent configuration
     */
    async deleteProjectAgent(slug, projectPath) {
        const targetPath = projectPath || this.workspaceRoot;
        if (!targetPath)
            return;
        ...
        try {
            ...
            projectModes.customModes = projectModes.customModes.filter((mode) => mode.slug !== slug);
            await fs.writeFile(projectModesPath, JSON.stringify(projectModes, null, 2));
        }
        catch (error) {
            this.logger.warn(`Could not delete project agent '${slug}': ${error.message}`);
        }
    }
    /**
