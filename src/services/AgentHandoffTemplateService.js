...
export const MASTER_ORCHESTRATOR_HANDOFF_TEMPLATE = {
...
export class AgentHandoffTemplateService {
    async createHandoffPrompt(templateId, variables) {
    ...
    processSimpleArrays(content, variables) {
        // Simple array processing for immediate use
        content = content.replace(/{{#each (\w+)}}([\s\S]*?){{\/each}}/g, (match, arrayName, loopContent) => {
        ...
        content = content.replace(/{{#if (\w+)}}([\s\S]*?){{\/if}}/g, (match, conditionName, conditionalContent) => {
            return variables[conditionName] ? conditionalContent : '';
        });
        return content;
    }
    generateSessionMetrics(sessionData) {
        return {
    ...
    calculateHandoffQuality(sessionData) {
        let score = 10;
        if (!sessionData.nextPriorities?.length)
            score -= 2;
        if (!sessionData.agentNetwork?.length)
            score -= 2;
        if (!sessionData.coordinationProtocols?.length)
            score -= 1;
        if (sessionData.criticalAlerts?.length > 0)
            score -= 1;
        return Math.max(0, score);
    }
}
// Immediate test with current session data
export function generateCurrentSessionHandoff() {
...
//# sourceMappingURL=AgentHandoffTemplateService.js.map