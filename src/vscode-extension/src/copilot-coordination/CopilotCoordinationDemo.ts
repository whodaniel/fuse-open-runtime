...
export class CopilotCoordinationDemo {
    ...
    async demo2_ContextSharing() {
        ...
        const context = {
            ...
            recentActivity: ['context_sharing_demo'],
            currentTask: 'context_sharing'
        };

        // Share context with other instances
        await this.coordinator.shareContext('copilot-sidebar-chat', context);
        console.log('📤 Context shared successfully');
        
        // Listen for context updates from other instances
        this.coordinator.onContextShared((data) => {
            console.log(`📨 Received context from ${data.from}:`, {
                document: data.context.activeDocument?.uri,
                timestamp: data.context.workspaceContext
            });
        });
    }
    ...
    async demo4_TaskHandoff() {
        ...
        await this.coordinator.handoffTask('copilot-sidebar-chat', 'copilot-editor-inline', task);
        
        console.log(`✅ Task handed off to copilot-editor-inline`);
        vscode.window.showInformationMessage(`Task handed off to copilot-editor-inline`);
    ...
    async demo5_RealTimeSync() {
        ...
        this.coordinator.onCompletionSync((data) => {
            console.log(`🔄 Received sync from ${data.from}:`, {
    ...
    async demo6_MultiInstanceWorkflow() {
        ...
        await this.coordinator.shareContext('copilot-sidebar-chat', chatContext);
        ...
        if (chatContext.userQuery.includes('database') || chatContext.userQuery.includes('data')) {
            ...
            await this.coordinator.handoffTask('copilot-sidebar-chat', 'copilot-notebook', dataTask);
        }
        ...
        await this.coordinator.shareContext('copilot-coordinator', finalContext);
