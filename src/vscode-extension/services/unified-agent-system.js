...
exports.createUnifiedAgentSystem = createUnifiedAgentSystem;
...
class UnifiedAgentSystem {
    constructor() {
        this.disposables = [];
        this.agents = new Map();
        this.connectionStatus = shared_1.ConnectionStatus.DISCONNECTED;
    ...
    registerCommands() {
        ...
        this.disposables.push(vscode.commands.registerCommand('thefuse.connectToAgent', async () => {
            ...
            const selectedAgent = await vscode.window.showQuickPick(availableAgents.map(agent => ({
                label: agent.name,
        ...
        this.disposables.push(vscode.commands.registerCommand('thefuse.disconnectFromAgent', async () => {
            ...
            const connectedAgents = Array.from(this.agents.values())
                .filter(agent => agent.status === shared_1.ConnectionStatus.CONNECTED);
            if (connectedAgents.length === 0) {
                vscode.window.showInformationMessage('No connected agents');
                return;
            }
            const selectedAgent = await vscode.window.showQuickPick(connectedAgents.map(agent => ({
                label: agent.name,
                description: agent.description || '',
        ...
        this.disposables.push(vscode.commands.registerCommand('thefuse.endCollaboration', async () => {
            ...
            const selectedSession = await vscode.window.showQuickPick(activeSessions.map(session => ({
                label: session.topic,
    ...
    async connectToAgent(agentId) {
        try {
            ...
            // Update agent status
            agent.status = shared_1.ConnectionStatus.CONNECTED;
    ...
    async disconnectFromAgent(agentId) {
        try {
            ...
            if (agent.status !== shared_1.ConnectionStatus.CONNECTED) {
            ...
            // Update agent status
            agent.status = shared_1.ConnectionStatus.DISCONNECTED;
            this.agents.set(agentId, agent);
            // Update connection status
            this.updateConnectionStatus();
     ...
     */
    async startCollaborationSession(agentId, topic) {
        try {
            const agent = this.agents.get(agentId);
            if (!agent) {
                throw new Error(`Agent with ID ${agentId} not found`);
            ...
            const panel = vscode.window.createWebviewPanel('collaborationSession', `Collaboration: ${topic}`, vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
            ...
            await this.simulateAgentMessage(agentId, `Hello! I'm ${agent.name}. Let's collaborate on: ${topic}. How can I help you?`, sessionId);
     ...
     */
    async sendMessageToAgent(agentId, message, sessionId) {
        try {
            const agent = this.agents.get(agentId);
            if (!agent) {
                throw new Error(`Agent with ID ${agentId} not found`);
     ...
     */
    async simulateAgentMessage(agentId, message, sessionId) {
        const session = this.activeSessions.get(sessionId);
        if (!session) {
            return;
        }
     ...
     */
    async simulateAgentResponse(agentId, userMessage, sessionId) {
     ...
     */
    getCollaborationHtml(webview, session) {
    ...
    updateConnectionStatus() {
        ...
        if (connectedAgents.length === 0) {
            ...
            this.connectionStatus = shared_1.ConnectionStatus.CONNECTED;
 ...
 */
function createUnifiedAgentSystem() {
    return new UnifiedAgentSystem();
}
//# sourceMappingURL=unified-agent-system.js.map