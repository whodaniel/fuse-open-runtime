...
class MCPHandler extends events_1.EventEmitter {
    ...
    async handleRequest(request, sourceAgent) {
        try {
    ...
    async handleContextRequest(request, sourceAgent) {
    ...
    async handleCapabilityRequest(request, sourceAgent) {
        const { action, parameters } = request;
        switch (action) {
            case 'query':
    ...
    async handleCodeExecutionRequest(request, sourceAgent) {
        ...
        try {
            ...
            const result = await this.executeCode(parameters.code, parameters.language, parameters.mode);
            return {
                success: true,
                result
            };
    ...
    hasExecutionPermission(agent, mode) {
    ...
    async executeCode(code, language, mode) {
    ...
    getContext(agentId, contextType) {
