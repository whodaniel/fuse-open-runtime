...
exports.UnifiedLLMOrchestrator = void 0;
exports.createUnifiedLLMOrchestrator = createUnifiedLLMOrchestrator;
...
class UnifiedLLMOrchestrator {
    ...
    registerCommands() {
            ...
            try {
                vscode.window.withProgress({
                    ...
                    if (result) {
                        ...
                        const panel = vscode.window.createWebviewPanel('generatedText', 'Generated Text', vscode.ViewColumn.One, { enableScripts: true });
                        panel.webview.html = this.getGeneratedTextHtml(panel.webview, prompt, result.text);
                    }
                });
            }
            catch (error) {
        ...
        this.disposables.push(vscode.commands.registerCommand('thefuse.optimizeCode', async () => {
            ...
            if (!selection.isEmpty) {
                ...
                selection = new vscode.Selection(0, 0, document.lineCount - 1, document.lineAt(document.lineCount - 1).range.end.character);
            }
            ...
            const selectedOptimizationType = await vscode.window.showQuickPick(optimizationTypes.map(type => ({
                label: type.label,
            ...
            try {
                vscode.window.withProgress({
                    ...
                    const result = await this.optimizeCode(codeToOptimize, document.languageId, { optimizationType: selectedOptimizationType.id });
                    ...
                    const panel = vscode.window.createWebviewPanel('codeOptimization', 'Code Optimization', vscode.ViewColumn.Beside, { enableScripts: true });
                    panel.webview.html = this.getCodeOptimizationHtml(panel.webview, codeToOptimize, result.optimizedCode, result.explanation || 'Code optimized for better performance.', result.improvements || [], document.languageId);
            ...
            try {
                vscode.window.withProgress({
                    ...
                    if (result) {
                        ...
                        const panel = vscode.window.createWebviewPanel('questionAnswer', 'Answer', vscode.ViewColumn.One, { enableScripts: true });
                        panel.webview.html = this.getQuestionAnswerHtml(panel.webview, question, result.answer, result.confidence, result.references);
                    }
                });
            }
            catch (error) {
     ...
     */
    sendWorkerRequest(action, payload) {
        if (!this.worker) {
     ...
     */
    getGeneratedTextHtml(webview, prompt, text) {
     ...
     */
    getCodeOptimizationHtml(webview, originalCode, optimizedCode, explanation, improvements, language) {
        const cspSource = webview.cspSource;
        return `<!DOCTYPE html>
        <html lang="en">
        <head>
     ...
     */
    getQuestionAnswerHtml(webview, question, answer, confidence, references) {
        const cspSource = webview.cspSource;
        return `<!DOCTYPE html>
        <html lang="en">
        <head>
...
exports.UnifiedLLMOrchestrator = UnifiedLLMOrchestrator;
/**
 * Create and initialize the unified LLM orchestrator
 */
function createUnifiedLLMOrchestrator(llmProviderManager) {
    return new UnifiedLLMOrchestrator(llmProviderManager);
}
//# sourceMappingURL=unified-llm-orchestrator.js.map