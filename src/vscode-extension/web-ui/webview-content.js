...
exports.getCommunicationPanelContent = getCommunicationPanelContent;
exports.getMessageHtml = getMessageHtml;
const vscode = __importStar(require("vscode"));
const shared_1 = require("../types/shared");
function getCommunicationPanelContent(webview, extensionUri, agents, conversations, connectionStatus) {
            ...
            <div class="connection-status">
                ${Object.entries(connectionStatus).map(([id, status]) => `
                    <div class="status-indicator ${status.toLowerCase()}">
                        <i class="codicon codicon-${status === shared_1.ConnectionStatus.CONNECTED ? 'check' : 'error'}"></i>
                        ${id}
                    </div>
                `).join('')}
            </div>
