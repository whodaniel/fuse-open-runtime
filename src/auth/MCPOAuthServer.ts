...
export class MCPOAuthServer {
   ...
   */
  private async getAuthServerMetadata(req: Request, res: Response) {
    const metadata = {
      issuer: this.ISSUER,
   ...
   */
  private async getProtectedResourceMetadata(req: Request, res: Response) {
    const metadata = {
  ...
  private async authorize(req: Request, res: Response) {
    ...
    this.authCodes.set(authCode, {
      ...
      scope: scope as string || 'mcp:read',
   ...
   */
  private async mcpDiscovery(req: Request, res: Response) {
    const discovery = {
      mcp_version: '2025-06-18',
   ...
   */
  public validateMCPToken(requiredScopes: string[] = []) {
  ...
  private generateAccessToken(client_id: string, user_id: string | null, scope: string): string {
    const payload = {
      iss: this.ISSUER,
  ...
  private generateRefreshToken(client_id: string, user_id: string | null): string {
    return getRandomBytes(32).toString('hex');
  }

  private getMCPResourcesForScope(scope: string): string[] {
  ...
  private getMCPCapabilitiesForScope(scope: string): string[] {
    const scopes = scope.split(' ');
  ...
  private async mcpCapabilities(req: Request, res: Response) {
    // Implementation for MCP capabilities endpoint
    res.json({ capabilities: ['oauth2', 'dynamic-discovery'] });
  }

  private async mcpTokenExchange(req: Request, res: Response) {
