...
async function initializeEnhancedFeatures(): Promise<void> {
  try {
    ...
    if (
      settingsManager.isFeatureEnabled('websocket-enhancement') &&
      webSocketManager &&
  ...
  try {
    ...
    if (currentPageMapping) {
      ...
      setTimeout(async () => {
        ...
        if (
          currentPageMapping &&
          (currentPageMapping.chatInput ||
            currentPageMapping.sendButton ||
            currentPageMapping.chatOutput)
        ) {
          elementSelector.saveElementMapping(currentPageMapping);
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      if (shouldRedetect) {
        ...
        setTimeout(async () => {
          ...
          if (
            newMapping &&
            (newMapping.chatInput || newMapping.sendButton || newMapping.chatOutput)
          ) {
...
function enterElementSelectionMode(
  targetType: 'input' | 'button' | 'output'
): Promise<ElementInfo> {
...
function getElementTypeDisplayName(type: 'input' | 'button' | 'output'): string {
  switch (type) {
    case 'input':
      return 'chat input field';
    case 'button':
      return 'send button';
    case 'output':
      return 'chat output area';
    default:
      return 'element';
  }
}
...
function handleCapturePageOutput(request: any, sendResponse: (response: any) => void): void {
  ...
  if (!selector) {
    ...
    sendResponse({
      success: false,
      error: 'No chat output selector available. Please configure element mapping first.',
    });
    return;
  }
...
function handleSendToPageInput(request: any, sendResponse: (response: any) => void): void {
  ...
  try {
    const inputElement = document.querySelector(chatInputSelector) as
      | HTMLInputElement
      | HTMLTextAreaElement;
    ...
    if (!inputElement) {
      logger.warn(
        `SEND_TO_PAGE_INPUT: Input element not found with selector: ${chatInputSelector}`
      );
      sendResponse({
        success: false,
        error: `Input element not found with selector: ${chatInputSelector}`,
      });
      return;
    }
    if (!buttonElement) {
      logger.warn(
        `SEND_TO_PAGE_INPUT: Button element not found with selector: ${sendButtonSelector}`
      );
      sendResponse({
        success: false,
        error: `Button element not found with selector: ${sendButtonSelector}`,
      });
      return;
    }
 ...
 */
async function handleEnterSelectionMode(
  request: any,
 ...
 */
async function handleAutoDetectElements(
  request: any,
  sendResponse: (response: any) => void
): Promise<void> {
  try {
 ...
 */
async function handleLoadElementMapping(
  request: any,
  sendResponse: (response: any) => void
): Promise<void> {
  try {
    ...
    if (chatInputSelector) {
      try {
        ...
        if (inputElements.length > 0) {
          ...
          results.push(
            `  - Type: ${firstElement.tagName}, ID: ${firstElement.id || 'none'}, Classes: ${firstElement.className || 'none'}`
          );
        }
      } catch (error) {
