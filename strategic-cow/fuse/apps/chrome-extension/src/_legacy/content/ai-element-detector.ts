...
export class AIElementDetector {
  ...
  private initializeChatPatterns(): void {
    this.chatPatterns = [
      ...
      // Send button patterns
      {
        ...
        textPatterns: [/^send$/i, /submit/i, /post/i, /publish/i, /reply/i],
        confidence: 0.9,
      },
      // Output/message container patterns
      {
        ...
        textPatterns: [/message/i, /conversation/i, /chat/i, /history/i],
        confidence: 0.7,
      },
    ];
  }
  ...
  private rankElementsByAI(elements: ElementInfo[], type: string): ElementInfo[] {
    return elements.sort((a, b) => b.confidence - a.confidence).slice(0, 5); // Return top 5 candidates
  }
