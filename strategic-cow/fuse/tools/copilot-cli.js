...
const CONFIG = {
  ...
  directives:
    'Extract key information regarding user activity, context, and potential needs from this screen.',
  cleanup: true,
  tempDir: os.tmpdir(),
};
...
function log(type, msg) {
  ...
  console.log(
    `${colors.dim}[${timestamp}]${colors.reset} ${color}[${label}]${colors.reset} ${msg}`
  );
}
...
async function main() {
  ...
  console.log(
    `${colors.dim}Security: ${CONFIG.cleanup ? 'Strict (Auto-delete)' : 'Relaxed (Keep files)'}${colors.reset}\n`
  );
