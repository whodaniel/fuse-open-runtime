...
function handleAIQueryMessage(ws, message) {
    console.log('AI query received:', message.data?.query || message.query);

    // Extract the query from either message.data.query or message.query
    const query = message.data?.query || message.query;
...
// Start HTTP server
const PORT = 3712; // Using port 3712 as an alternative since 3710 and 3711 might be in use
server.listen(PORT, () => {
...
server.on('error', (error) => {
    ...
    // If the port is already in use, try to use a different port
    if (error.code === 'EADDRINUSE') {
        console.error(`Port ${PORT} is already in use. Please stop any other processes using this port.`);
        console.error('You can use the following command to find processes using this port:');
        console.error(`  lsof -i :${PORT}`);
        console.error('And to kill a process:');
