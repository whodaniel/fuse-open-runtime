...
function OllamaEmbeddingModelSelection({ settings, basePath = null }) {
    ...
    useEffect(() => {
        async function fetchModels(): Promise<void> {) {
            ...
            setError(null);
            try {
                const { models } = await System.fetchOllamaModels(basePath);
                setModels(models);
            }
            catch (error) {
                console.error("Failed to fetch Ollama models", error);
                setError("Could not fetch models from Ollama server. Ensure your server is running and the URL is correct.");
            }
            setLoading(false);
        }
        fetchModels();
...
export function OllamaEmbeddingOptions({ settings }) {
    ...
    const [maxChunkLength, setMaxChunkLength] = useState((settings === null || settings === void 0 ? void 0 : settings.EmbeddingModelMaxChunkLength) || 8192);
      ...
      <div className={STYLES.inputGroup}>
        <div className={STYLES.inputContainer}>
          ...
          <div className="flex items-center gap-x-4">
            <input type="url" name="EmbeddingBasePath" className={STYLES.input} placeholder="http://localhost:11434" value={basePathValue} onChange={(e) => basePath.onChange(e.target.value)} onBlur={() => basePath.onBlur()} required autoComplete="off" spellCheck={false}/>
...
export function OllamaOptions({ settings }: OllamaOptionsProps) {
  return (
    <div className={COMMON_STYLES.container}>
      <div className={COMMON_STYLES.inputsContainer}>
        <div className={COMMON_STYLES.inputWrapper}>
          <label className={COMMON_STYLES.label} htmlFor="ollamaBaseUrl">
            Ollama Base URL
          </label>
          <input
            id="ollamaBaseUrl"
...
//# sourceMappingURL=OllamaOptions.js.map