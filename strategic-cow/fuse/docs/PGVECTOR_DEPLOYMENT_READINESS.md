...
Railway, utilizing the `pgvector` driver.

## Changes Implemented

### 1. Deployment Configuration (`railway.toml`)
...
### 2. Docker Build Support (`Dockerfile.railway`)
...
- **Hybrid Architecture**: Converted the gRPC microservice into a **Hybrid
  ...
  - **Why**: Railway and many PaaS providers rely on HTTP health checks. Pure
...
- **Database Connectivity**: Updated `main.ts` to natively accept
  `DATABASE_URL`.
  - **Integration**: The service now automatically uses the Railway-provided
...
2. **Health Check**: Visit
   `https://core-vector-db-production.up.railway.app/health` (or your specific
