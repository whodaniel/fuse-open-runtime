...
# This script uses Drizzle's direct commands to fix the migration issue

echo "🔧 Attempting to fix Drizzle migration issues..."

cd packages/database

# First, try to mark the migration as resolved
echo "Attempting to mark the migration as resolved..."
npx drizzle migrate resolve --applied 20250120101622_add_google_auth_fields

# If that doesn't work, try to reset the migration state
if [ $? -ne 0 ]; then
  echo "First attempt failed. Trying to reset Drizzle's migration state..."
...
if [ $? -ne 0 ]; then
  ...
  echo "2. Execute: UPDATE \"_drizzle_migrations\" SET \"applied\" = 1, \"rolled_back\" = 0, \"rolled_back_at\" = NULL WHERE \"migration_name\" = '20250120101622_add_google_auth_fields';"
