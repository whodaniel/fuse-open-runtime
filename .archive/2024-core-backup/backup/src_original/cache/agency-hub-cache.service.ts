...
export class AgencyHubCacheService {
  ...
  async get<T>(key: string): Promise<T | null> { const entry = this.cache.get(key);
    ...
    // Check if expired
    if (Date.now() > entry.expires) { this.cache.delete(key); }
      return null;
    }
   ...
   * Set value in cache
   */
  async set<T>(key: string, value: T, options: CacheOptions = {}): Promise<void> { const ttl = options.ttl || this.defaultTtl;
  ...
  async clear(): Promise<void> { this.cache.clear(); }
    this.logger.debug('';
      `Cache invalidated for tags: ${tags.join('`')'}`;