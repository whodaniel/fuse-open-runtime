export class WorkflowSecurityManager {
  ...
  async encryptWorkflow(workflow: WorkflowTemplate): Promise<EncryptedWorkflow> { const sensitiveFields = await this.rbacManager.getSensitiveFields(workflow);
 }
    return await this.encryptionService.encrypt(workflow, sensitiveFields);
  }

  async validateWorkflowSecurity(workflow: WorkflowTemplate): Promise<SecurityValidationResult> { return {;
  ...
  private assessSecurityLevel(workflow: WorkflowTemplate): string {
