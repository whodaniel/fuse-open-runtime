...
const callGemini = async (
  prompt: string,
  systemInstruction = 'You are a professional poker strategy engine.'
) => {
  try {
    const json = await api('/api/gemini/content', {
      method: 'POST',
      body: { contents: prompt, config: { systemInstruction }, model: 'gemini-2.5-flash' },
    });
    return json?.text || 'Protocol link unstable.';
  } catch (e) {
    console.error(e);
    return 'Protocol link unstable.';
  }
};

// --- THEME & CONSTANTS ---
const THEME = {
  metal: 'bg-gradient-to-b from-[#2a2e38] via-[#1a1c23] to-[#0d0f14]',
  felt: 'bg-[radial-gradient(circle_at_center,_#062b1f_0%,_#020a07_100%)]',
};
...
function AppContent() {
  ...
  useEffect(() => {
    const coordinateSystem = 'origin: top-left, x increases right, y increases down';
    ...
    win.render_game_to_text = () =>
      JSON.stringify({
        ...
        heroCards: gameState?.seats?.find((seat: any) => seat.isHero)?.cards || [],
        turnIndex: gameState?.turnIndex ?? null,
        seats: (gameState?.seats || []).map((seat: any, idx: number) => ({
          index: idx,
          name: seat.name,
          stack: seat.stack,
          bet: seat.bet,
    ...
    if (typeof win.advanceTime !== 'function') {
  ...
  }, [view, user, activeTableId, gameState]);
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
            ...
            <div className="hidden sm:flex h-full py-2">
              <input
                type="number"
                id="raiseAmount"
                defaultValue={gameState.currentBet + gameState.blinds[1]}
  ...
  return (
    <>
      ...
      {view === 'CONTROL CENTER' && renderControlCenter()}
      {view === 'CASH TABLES' && (
        <CashTableBrowser
      ...
      {view === 'TABLE' && (
        <TournamentTableView onLeave={handleLeaveTable}>{renderTableContent()}</TournamentTableView>
      )}
