...
export class PayPalBusinessProcessor {
  ...
  private extractOrgFromPayPal(resource: any): string {
    ...
    return resource.custom_id || resource.invoice_id || resource.reference_id || 'default_org';
  }

  private async processPaymentCapture(
    event: BusinessEvent,
  ...
  private async processPaymentDenied(
    event: BusinessEvent,
    payload: PayPalWebhookPayload
  ): Promise<void> {
    const capture = payload.resource;
  ...
  private async processSaleCompleted(
    event: BusinessEvent,
    payload: PayPalWebhookPayload
  ): Promise<void> {
  ...
  private suggestRetentionStrategies(_subscription: any): string[] {
    return ['discount_offer', 'feature_education', 'customer_success_outreach', 'exit_survey'];
  }
  ...
  private async identifyUpsellOpportunities(_sale: any): Promise<string[]> {
    return ['premium_features', 'additional_licenses', 'professional_services'];
  }
