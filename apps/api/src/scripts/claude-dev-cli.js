...
class ClaudeDevCLI {
    ...
    saveConfig() {
        ...
        try {
            ...
            // eslint-disable-next-line no-console
            console.log(chalk.red(`✗ Failed to save config: ${error instanceof Error ? error.message : String(error)}`));
        }
    }
    async makeRequest(endpoint, method = 'GET', data) {
        ...
        try {
            ...
            if (axiosError.response) {
                ...
                throw new Error(`Request Error: ${axiosError instanceof Error ? axiosError.message : String(axiosError)}`);
            }
        }
    }
...
//# sourceMappingURL=claude-dev-cli.js.map