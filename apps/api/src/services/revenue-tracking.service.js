...
let RevenueTrackingService = RevenueTrackingService_1 = class RevenueTrackingService {
    ...
    logger = new Logger(RevenueTrackingService_1.name);
    ...
    async trackRevenue(data) {
        ...
        try {
            ...
            if (agentNft.isFractionalized) {
                try {
                    await this.smartContractService.addRevenue(Number(utils.formatEther(data.amount)), // Convert from wei to ether, then to number
                    data.source, // Pass the source as the second parameter
                    data.tokenAddress // Pass token address as metadata
                    );
                }
                catch (error) {
    ...
    async checkAndTriggerDistribution(revenueStreamId) {
        try {
            ...
            const totalRevenue = ethers.BigNumber.from(revenueStream.totalRevenue || '0');
            const distributedRevenue = ethers.BigNumber.from(revenueStream.distributedRevenue || '0');
            const pendingRevenue = totalRevenue.sub(distributedRevenue);
            const distributionThreshold = ethers.BigNumber.from(revenueStream.distributionThreshold || '0');
    ...
    async distributeRevenue(revenueStreamId) {
        ...
        try {
            ...
            const distributedRevenue = ethers.BigNumber.from(revenueStream.distributedRevenue || '0');
            const pendingRevenue = totalRevenue.sub(distributedRevenue);
            if (pendingRevenue.lte(ethers.BigNumber.from(0))) {
            ...
            const recipients = revenueStream.agentNft.fractionalShares.map((share) => {
                const shareAmountNum = Number(share.shareAmount);
                const sharePercentage = shareAmountNum / (totalShares || 1);
                const amount = pendingRevenue
                    .mul(ethers.BigNumber.from(shareAmountNum.toString()))
                    .div(ethers.BigNumber.from(totalShares.toString()));
                return {
            ...
            const contractResult = await this.smartContractService.distributeRevenue(recipients.map((r) => ({ address: r.address, amount: parseFloat(r.amount) })));
            // The smart contract service returns { transactionHash, blockNumber }
            // but the code expects { distributedAmount, txHash }
            const txHash = contractResult.transactionHash;
    ...
    async getMockRevenueEvents(_fromBlock, _toBlock) {
    ...
    async getRevenueAnalytics(agentNftId, timeframe = 'month') {
        try {
            ...
            const totalRevenue = revenueStreams.reduce((sum, stream) => sum + parseFloat(utils.formatEther(stream.totalRevenue || '0')), 0);
            const totalDistributed = revenueStreams.reduce((sum, stream) => sum + parseFloat(utils.formatEther(stream.distributedRevenue || '0')), 0);
...
RevenueTrackingService = RevenueTrackingService_1 = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [typeof (_a = typeof PrismaService !== "undefined" && PrismaService) === "function" ? _a : Object, typeof (_b = typeof SmartContractService !== "undefined" && SmartContractService) === "function" ? _b : Object])
], RevenueTrackingService);
export { RevenueTrackingService };
