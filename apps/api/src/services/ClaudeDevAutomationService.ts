...
export class ClaudeDevAutomationService {
  ...
  private async processAutomation(
    ...
    try {
      ...
      automation.metadata.endTime = new Date();
      automation.metadata.duration =
        automation.metadata.endTime.getTime() - automation.metadata.startTime.getTime();
    } catch (error) {
  ...
  async getHealthStatus(): Promise<any> {
    return {
      status: 'healthy',
      activeAutomations: Array.from(this.automations.values()).filter((a) => a.status === 'running')
        .length,
  ...
  async getStatistics(tenantId?: string): Promise<any> {
    ...
    return {
      ...
      totalTemplates: this.templates.size,
      customTemplates: Array.from(this.templates.values()).filter((t) => t.id.startsWith('custom-'))
        .length,
    };
  }
  ...
  async getTasksByAgent(agentId: string, tenantId?: string): Promise<AutomationResult[]> {
    ...
    return results.sort(
      (a, b) => new Date(b.metadata.startTime).getTime() - new Date(a.metadata.startTime).getTime()
    );
  }
