...
let TransactionsService = TransactionsService_1 = class TransactionsService {
    ...
    prisma;
    smartAccountService;
    logger = new Logger(TransactionsService_1.name);
    ...
    async executeTransaction(walletId, transactionData) {
        try {
            ...
            if (shouldUseSmartAccount && smartAccountCapable) {
                ...
                txHash = await this.smartAccountService.executeSmartAccountTransaction(walletId, to, parseEther(value), // Value is already string, parseEther returns bigint
                data);
            }
            else {
                // Execute via EOA
    ...
    async executeBatchTransaction(walletId, batchData) {
        try {
            ...
            const transactionRecords = await this.prisma.$transaction(async (prisma) => {
                return await Promise.all(batchData.transactions.map(tx => prisma.transaction.create({
                    data: {
                        wallet: { connect: { id: walletId } },
