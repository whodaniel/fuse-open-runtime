    ...
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, (0, index_js_1.runLighthouseAudit)(url, [types_js_1.AuditCategory.SEO])];
                case 1:
                    lhr = _a.sent();
                    return [2 /*return*/, extractAIOptimizedData(lhr, url)];
                case 2:
                    error_1 = _a.sent();
                    throw new Error("SEO audit failed: ".concat(error_1 instanceof Error ? error_1.message : String(error_1)));
...
var extractAIOptimizedData = function (lhr, url) {
    var categoryData = lhr.categories[types_js_1.AuditCategory.SEO];
    ...
    auditRefs
        ...
        var details = [];
        if (audit.details) {
            var auditDetails = audit.details;
            if (auditDetails.items && Array.isArray(auditDetails.items)) {
                ...
                auditDetails.items.slice(0, itemLimit).forEach(function (item) {
                    var detail = {};
                    if (item.selector) {
    ...
    Object.entries(categories)
        ...
        switch (category) {
            case "content":
                recommendation = "Improve SEO content (".concat(data.issues_count, " issues): titles, descriptions, and headers");
                break;
            case "mobile":
                recommendation = "Optimize for mobile devices (".concat(data.issues_count, " issues)");
                break;
            case "crawlability":
                recommendation = "Fix crawlability issues (".concat(data.issues_count, " issues): robots.txt, sitemaps, and redirects");
                break;
            default:
                recommendation = "Fix ".concat(data.issues_count, " SEO issues in category: ").concat(category);
        }
        prioritized_recommendations.push(recommendation);
    });
    // Add specific high-impact recommendations
    if (issues.some(function (issue) { return issue.id === "meta-description"; })) {
    ...
    if (issues.some(function (issue) { return issue.id === "hreflang"; })) {
        prioritized_recommendations.push("Fix hreflang implementation for international SEO");
    }
    if (issues.some(function (issue) { return issue.id === "canonical"; })) {
