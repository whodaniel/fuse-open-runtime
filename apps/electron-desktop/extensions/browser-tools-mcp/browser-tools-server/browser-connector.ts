...
function convertPathForCurrentPlatform(inputPath: string): string {
  ...
  logger.debug(`Converting path "${inputPath}" for platform: ${platform}`);
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        testServer.once("error", (err: any) => {
          if (err.code === "EADDRINUSE") {
            logger.debug(`Port ${currentPort} is in use, trying next port...`);
            currentPort++;
            attempts++;
...
function truncateLogsToQueryLimit(logs: any[]): any[] {
  ...
  for (const log of processedLogs) {
    ...
    result.push(log);
    currentSize += logSize;
    logger.debug(`Added log of size ${logSize}, total size now: ${currentSize}`);
  }
...
app.post("/extension-log", (req, res) => {
  ...
  if (!data) {
    logger.warn("Warning: No data received in log request");
    res.status(400).json({ status: "error", message: "No data provided" });
    return;
  }

  logger.debug(`Processing ${data.type} log entry`);
  ...
  switch (data.type) {
    case "page-navigated":
      ...
      logger.info("Received page navigation event with URL:", data.url);
      currentUrl = data.url;

      // Also update the tab ID if provided
      if (data.tabId) {
        logger.debug("Updating tab ID from page navigation event:", data.tabId);
        currentTabId = data.tabId;
      }

      logger.debug("Updated current URL:", currentUrl);
      break;
    case "console-log":
      logger.debug("Adding console log:", {
        level: data.level,
        message:
      ...
      break;
    case "console-error":
      logger.debug("Adding console error:", {
        level: data.level,
        message:
          data.message?.substring(0, 100) +
          (data.message?.length > 100 ? "..." : ""),
    ...
    case "network-request":
      ...
      logger.debug("Adding network request:", logEntry);
    ...
    case "selected-element":
      logger.debug("Updating selected element:", {
      ...
      break;
    default:
      logger.warn("Unknown log type:", data.type);
  }

  logger.debug("Current log counts:", {
    consoleLogs: consoleLogs.length,
    consoleErrors: consoleErrors.length,
    networkErrors: networkErrors.length,
    networkSuccess: networkSuccess.length,
  });
  logger.debug("=== End Extension Log ===\n");
...
app.post("/current-url", (req, res) => {
  ...
  if (req.body && req.body.url) {
    ...
    if (req.body.tabId) {
      ...
      logger.debug(`Updated current tab ID: ${oldTabId} -> ${currentTabId}`);
    }
    ...
    logger.warn("No URL provided in current-url request");
...
app.get("/current-url", (req, res) => {
  logger.debug("Current URL requested, returning:", currentUrl);
  res.json({ url: currentUrl });
});

// Add endpoint to navigate to a URL
app.post("/navigate", (req, res) => {
  logger.debug("Navigation request received:", req.body);
  
  const { url } = req.body;
  
  if (!url) {
    logger.warn("Navigation request missing URL");
  ...
  logger.debug(`Attempting to navigate to: ${url}`);
  
  // Use chrome.tabs.update to navigate the current tab
  if (typeof chrome !== 'undefined' && chrome.tabs) {
    const tabId = typeof currentTabId === 'string' ? parseInt(currentTabId, 10) : currentTabId;
    chrome.tabs.update(tabId, { url: url }, (_tab) => {
      if (chrome.runtime.lastError) {
        logger.error("Navigation failed:", chrome.runtime.lastError);
        res.status(500).json({ 
    ...
    logger.debug("Chrome tabs API not available, using script injection");
    
    // This will be handled by the Chrome extension via WebSocket
    res.json({ 
      success: true, 
      url: url,
      message: `Navigation request sent for ${url}` 
    });
  }
});
...
export class BrowserConnector {
  ...
  constructor(app: express.Application, server: any) {
    ...
    this.app.post(
      ...
      async (req: express.Request, res: express.Response) => {
        ...
        logger.debug("Browser Connector: Request body:", req.body);
        logger.debug(
      ...
      ws.on("message", (message: string | Buffer | ArrayBuffer | Buffer[]) => {
        try {
          ...
          if (data.type === "current-url-response" && data.url) {
            logger.debug("Received current URL from browser:", data.url);
            currentUrl = data.url;
          ...
          if (data.type === "page-navigated" && data.url) {
            logger.debug("Page navigated to:", data.url);
            currentUrl = data.url;
          ...
          if (data.type === "screenshot-data" && data.data) {
            ...
            logger.debug("Screenshot path from extension:", data.path);
            logger.debug("Auto-paste setting from extension:", data.autoPaste);
            ...
            if (callbacks.length > 0) {
              ...
              logger.debug("Found callback, resolving promise");
              ...
              logger.debug("No callbacks found for screenshot");
            }
          }
          // Handle screenshot error
          else if (data.type === "screenshot-error") {
            logger.warn("Received screenshot error:", data.error);
            const callbacks = Array.from(screenshotCallbacks.values());
            if (callbacks.length > 0) {
              const callback = callbacks[0];
              callback.reject(
    ...
    this.app.post(
      ...
      (req: express.Request, res: express.Response): void => {
        ...
        logger.debug("Browser Connector: Request body:", req.body);
        try {
          logger.debug("Received screenshot capture request");
          const { data, path: outputPath } = req.body;

          if (!data) {
            logger.warn("Screenshot request missing data");
          ...
          logger.debug(`Using screenshot path: ${targetPath}`);
          ...
          logger.debug(`Created/verified directory: ${targetPath}`);

          // Generate a unique filename using timestamp
          const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
          const filename = `screenshot-${timestamp}.png`;
          const fullPath = path.join(targetPath, filename);
          logger.debug(`Saving screenshot to: ${fullPath}`);
  ...
  private async getUrlForAudit(): Promise<string | null> {
    try {
      logger.debug("getUrlForAudit called");

      // Use the stored URL if available immediately
      if (currentUrl && currentUrl !== "" && currentUrl !== "about:blank") {
        logger.debug(`Using existing URL immediately: ${currentUrl}`);
        return currentUrl;
      }

      // Wait for a URL to become available (retry loop)
      logger.debug("No valid URL available yet, waiting for navigation...");
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        if (currentUrl && currentUrl !== "" && currentUrl !== "about:blank") {
          logger.debug(`URL became available after waiting: ${currentUrl}`);
          return currentUrl;
        }
      ...
      logger.warn("Timed out waiting for URL, returning null");
      return null;
    } catch (error) {
  ...
  async captureScreenshot(req: express.Request, res: express.Response) {
    logger.debug("Browser Connector: Starting captureScreenshot method");
    logger.debug("Browser Connector: Request headers:", req.headers);
    logger.debug("Browser Connector: Request method:", req.method);
    ...
    try {
      logger.debug("Browser Connector: Starting screenshot capture...");
      const requestId = Date.now().toString();
      logger.debug("Browser Connector: Generated requestId:", requestId);
      ...
      logger.debug("Browser Connector: Waiting for screenshot data...");
      const {
        data: base64Data,
        path: customPath,
        autoPaste,
      } = await screenshotPromise;
      logger.debug("Browser Connector: Received screenshot data, saving...");
      logger.debug("Browser Connector: Custom path from extension:", customPath);
      logger.debug("Browser Connector: Auto-paste setting:", autoPaste);
      ...
      logger.debug(`Browser Connector: Using path: ${targetPath}`);
      ...
      try {
        fs.mkdirSync(targetPath, { recursive: true });
        logger.debug(`Browser Connector: Created directory: ${targetPath}`);
      } catch (err) {
        logger.error(
      ...
      const filename = `screenshot-${timestamp}.png`;
      const fullPath = path.join(targetPath, filename);
      logger.debug(`Browser Connector: Full screenshot path: ${fullPath}`);
      ...
      if (os.platform() === "darwin" && autoPaste === true) {
        ...
        exec(`osascript -e '${appleScript}'`, (error, stdout, stderr) => {
          if (error) {
            ...
            logger.debug(`Browser Connector: AppleScript executed successfully`);
            logger.debug(`Browser Connector: stdout: ${stdout}`);
          }
        });
      } else {
  ...
  public shutdown() {
    return new Promise<void>((resolve) => {
      ...
      if (
        ...
        logger.debug("Notifying client to close connection...");
        try {
          this.activeConnection.send(
  ...
  private setupAuditEndpoint(
    ...
    this.app.post(endpoint, async (req: any, res: any) => {
      try {
        ...
        if (!url) {
          logger.warn(`No URL available for ${auditType} audit`);
        ...
        if (!req.body?.url && url === currentUrl) {
          logger.debug(`Using stored URL for ${auditType} audit:`, url);
        }

        // Check if we're using the default URL
        if (url === "about:blank") {
          logger.warn(`Cannot run ${auditType} audit on about:blank`);
          return res.status(400).json({
  ...
  try {
    ...
    try {
      ...
      if (PORT !== REQUESTED_PORT) {
        logger.warn(`\n====================================`);
        logger.warn(`NOTICE: Requested port ${REQUESTED_PORT} was in use.`);
