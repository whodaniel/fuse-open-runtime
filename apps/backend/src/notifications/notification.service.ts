...
export class NotificationService {
  ...
  async sendMultiChannelNotification(channel: 'SLACK' | 'EMAIL' | 'WEBHOOK', config: any, message: string) {
      try {
          switch (channel) {
              case 'SLACK':
  ...
  async sendEmailNotification(config: { recipientEmail: string, subject?: string }, message: string) {
  ...
  async sendWebhookNotification(config: { url: string, method?: string, headers?: Record<string, string> }, message: string) {
      await axios({
