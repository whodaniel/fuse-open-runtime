...
let SmartContractService = SmartContractService_1 = class SmartContractService {
    ...
    initializeContracts() {
        try {
            ...
            this.agentNftContract = new ethers_1.ethers.Contract(process.env.AGENT_NFT_CONTRACT_ADDRESS || ethers_1.ethers.constants.AddressZero, exports.AGENT_NFT_ABI, this.wallet);
            this.marketplaceContract = new ethers_1.ethers.Contract(process.env.MARKETPLACE_CONTRACT_ADDRESS || ethers_1.ethers.constants.AddressZero, exports.MARKETPLACE_ABI, this.wallet);
            this.revenueDistributorContract = new ethers_1.ethers.Contract(process.env.REVENUE_DISTRIBUTOR_CONTRACT_ADDRESS || ethers_1.ethers.constants.AddressZero, exports.REVENUE_DISTRIBUTOR_ABI, this.wallet);
            this.smartAccountFactoryContract = new ethers_1.ethers.Contract(process.env.SMART_ACCOUNT_FACTORY_ADDRESS || ethers_1.ethers.constants.AddressZero, exports.SMART_ACCOUNT_FACTORY_ABI, this.wallet);
    ...
    async listShares(agentTokenId, shareAmount, pricePerShare, duration) {
        try {
            const tx = await this.marketplaceContract.listShares(agentTokenId, shareAmount, ethers_1.ethers.utils.parseEther(pricePerShare), duration);
            const receipt = await tx.wait();
    ...
    async buyShares(listingId, totalPrice) {
        try {
            const tx = await this.marketplaceContract.buyShares(listingId, {
                value: ethers_1.ethers.utils.parseEther(totalPrice)
            });
            const receipt = await tx.wait();
            return receipt.transactionHash;
        }
    ...
    async makeOffer(listingId, shareAmount, offerPrice) {
        try {
            const tx = await this.marketplaceContract.makeOffer(listingId, shareAmount, {
                value: ethers_1.ethers.utils.parseEther(offerPrice)
            });
            const receipt = await tx.wait();
    ...
    async createRevenueStream(agentTokenId, streamName, tokenAddress, distributionThreshold) {
        try {
            const tx = await this.revenueDistributorContract.createRevenueStream(agentTokenId, streamName, tokenAddress, ethers_1.ethers.utils.parseEther(distributionThreshold));
            const receipt = await tx.wait();
    ...
    async addRevenue(streamId, amount, tokenAddress) {
        try {
            let tx;
            if (tokenAddress === ethers_1.ethers.constants.AddressZero || !tokenAddress) {
                // ETH payment
                tx = await this.revenueDistributorContract.addRevenue(streamId, ethers_1.ethers.utils.parseEther(amount), {
    ...
    async distributeRevenue(streamId) {
        try {
            ...
            return {
                distributedAmount: ethers_1.ethers.utils.formatEther(distributedAmount || '0'),
                txHash: receipt.transactionHash
            };
        }
        catch (error) {
