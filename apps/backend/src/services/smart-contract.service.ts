...
import { Contract, formatEther, JsonRpcProvider, parseEther, Wallet, ZeroAddress } from 'ethers';
...
export class SmartContractService {
  ...
  async transferFractionalShare(
    ...
    try {
      const tx = await this.agentNftContract.transferFractionalShare(
        BigInt(tokenId),
        from,
        to,
        BigInt(shareAmount)
  ...
  async createRevenueStream(
    ...
    try {
      ...
      const streamEvent = receipt.logs?.find(
        (log: any) => log.fragment.name === 'RevenueStreamCreated'
      );
  ...
  async addRevenue(streamId: number, amount: string, tokenAddress?: string): Promise<string> {
    try {
      ...
      if (tokenAddress === ZeroAddress || !tokenAddress) {
        // ETH payment
        tx = await this.revenueDistributorContract.addRevenue(
          BigInt(streamId),
          parseEther(amount),
          {
  ...
  async distributeRevenue(
    streamId: number
  ): Promise<{ distributedAmount: string; txHash: string }> {
    try {
      const tx = await this.revenueDistributorContract.distributeRevenue(BigInt(streamId));
      const receipt = await tx.wait();

      const distributeEvent = receipt.logs?.find(
        (log: any) => log.fragment.name === 'RevenueDistributed'
      );
