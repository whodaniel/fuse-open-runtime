...
let RevenueTrackingService = RevenueTrackingService_1 = class RevenueTrackingService {
        ...
        try {
            ...
            if (!revenueStream) {
                revenueStream = await this.prisma.revenueStream.create({
                    data: {
                        ...
                        tokenAddress: data.tokenAddress,
                        totalRevenue: '0',
                        distributionThreshold: ethers_1.ethers.utils.parseEther('0.1').toString(), // Default threshold
                    }
                });
            }
            ...
            if (agentNft.isFractionalized) {
                try {
                    ...
                    ethers_1.ethers.utils.formatEther(data.amount), data.tokenAddress);
                }
                catch (error) {
    ...
    async checkAndTriggerDistribution(revenueStreamId) {
        try {
            ...
            const totalRevenue = ethers_1.ethers.BigNumber.from(revenueStream.totalRevenue);
            const distributedRevenue = ethers_1.ethers.BigNumber.from(revenueStream.distributedRevenue);
            const pendingRevenue = totalRevenue.sub(distributedRevenue);
            const distributionThreshold = ethers_1.ethers.BigNumber.from(revenueStream.distributionThreshold);
    ...
    async distributeRevenue(revenueStreamId) {
        ...
        try {
            ...
            this.logger.log(`Revenue distributed successfully: ${ethers_1.ethers.utils.formatEther(pendingRevenue)} ETH to ${recipients.length} recipients`);
            return result;
        }
        catch (error) {
    ...
    async getRevenueAnalytics(agentNftId, timeframe = 'month') {
        try {
            ...
            const totalRevenue = revenueStreams.reduce((sum, stream) => sum + parseFloat(ethers_1.ethers.utils.formatEther(stream.totalRevenue || '0')), 0);
            const totalDistributed = revenueStreams.reduce((sum, stream) => sum + parseFloat(ethers_1.ethers.utils.formatEther(stream.distributedRevenue || '0')), 0);
