export class WorkflowValidator {
    ...
    validateNodeParameters(node, nodeTypeData) {
        const errors = [];
        const requiredParams = nodeTypeData.properties.properties.filter((prop) => prop.required);
    ...
    validateNodeCredentials(node, nodeTypeData) {
        ...
        if (nodeTypeData.credentials && nodeTypeData.credentials.length > 0) {
            ...
            if (!node.credentials || !node.credentials[requiredCred.name]) {
                errors.push(`Missing required credentials '${requiredCred.name}' in node ${node.name || node.id}`);
            }
        }
        return errors;
    }
    validateConnections(nodes, edges) {
        ...
        edges.forEach((edge) => {
            ...
            if (!this.isValidConnection(sourceNode, targetNode, sourceHandle, targetHandle)) {
                errors.push(`Invalid connection type from ${sourceNode.name || sourceNode.id} to ${targetNode.name || targetNode.id}`);
            }
        });
        return errors;
    }
    isValidConnection(sourceNode, targetNode, sourceHandle, targetHandle) {
    ...
    checkCircularDependencies(nodes, edges) {
        ...
        const hasCircularDependency = (nodeId) => {
