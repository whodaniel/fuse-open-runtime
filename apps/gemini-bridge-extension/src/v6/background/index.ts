...
class BackgroundService {
    ...
    switch (message.type) {
      ...
      case 'MESSAGE_RECEIVE':
        ...
        if (agentMessage && agentMessage.content) {
          const channelName = (this.channels.get(agentMessage.channel || '') as any)?.name || '';
   ...
   */
  private safeSendMessage(tabId: number, message: any, callback?: (response: any) => void): void {
    try {
  ...
  private frontloadPageAgentContext(agent: Agent): void {
    ...
    this.safeSendMessage(agent.metadata.tabId as number, {
...
[
   ...
   */
  private async registerWebMCPTools(): Promise<void> {
    if (typeof (chrome as any).ai !== 'undefined' && (chrome as any).ai.registerTools) {
      ...
      try {
        await (chrome as any).ai.registerTools(tnfTools, (toolName: string, params: any) => {
   ...
   */
  private async handleToolInvocation(toolName: string, params: any): Promise<any> {
