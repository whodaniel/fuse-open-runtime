      ...
      try {
          ...
          try {
            ...
            if (event.data instanceof ArrayBuffer) {
              ...
              if (this.options.useCompression) {
                try {
                  ...
                  this.log(
                    `Decompressed binary message. Original size: ${event.data.byteLength}, Decompressed size: ${processedData.length}`
                  );
                } catch (compressionError) {
                  this.log(
                    'Failed to decompress ArrayBuffer, attempting to process as uncompressed string. Error:',
                  ...
                  try {
                    ...
                    this.log(
                      'Failed to decode ArrayBuffer as UTF-8 string after decompression failure. Error:',
                      decodeError
                    );
                    this.emit(
                      'error',
                      new Error(
                        'Failed to process binary message: decompression and decode failed.'
                      )
                    );
                    return;
                  }
                ...
                try {
                  ...
                  this.log(
                    'Failed to decode ArrayBuffer as UTF-8 string (compression disabled). Error:',
                    decodeError
                  );
              ...
              if (this.options.useCompression) {
                ...
                this.log(
                  'Received string message while compression is enabled. Treating as uncompressed.'
                );
              }
            ...
            if (this.securityManager) {
              try {
                ...
                this.log(
                  'Failed to decrypt message, using original (potentially decompressed) data. Error:',
                  decryptionError instanceof Error ? decryptionError.message : decryptionError
                );
              }
            }
            ...
            try {
              ...
              this.log(
                'Failed to parse message data as JSON, treating as plain string. Error:',
        ...
        this.ws.onclose = (event) => {
          ...
          if (!event.wasClean && this.reconnectAttempts < this.options.reconnectAttempts!) {
            ...
            this.log(
              `Max reconnect attempts reached (${this.reconnectAttempts}/${this.options.reconnectAttempts}). Not rescheduling.`
            );
            this.state.reconnecting = false;
    ...
    try {
      ...
      if (this.securityManager) {
        if (!(await this.securityManager.isKeyReady())) {
          ...
          this.emit('encryption_unavailable', {
            message: 'Encryption key unavailable; message not sent.',
          }); // Specific event
          return false;
        }
        ...
        if (encrypted === null) {
          ...
          this.emit('encryption_error', {
            message: 'Message encryption failed; message not sent.',
          }); // Specific event
          return false;
        }
      ...
      if (this.options.useCompression && typeof messageToSend === 'string') {
        try {
          ...
          messageToSend = compressedData.buffer.slice(
            compressedData.byteOffset,
            compressedData.byteOffset + compressedData.byteLength
          );
          this.log(
            `Message compressed successfully. Original (encrypted/plain) length: ${(messageToSend as string).length}, Compressed length: ${messageToSend.byteLength}`
          );
        } catch (compressionError) {
          this.log(
            'Failed to compress message, sending uncompressed. Error:',
            compressionError instanceof Error ? compressionError.message : compressionError
          );
        ...
        this.log(
          'Compression enabled, but message is not a string (already binary?). Skipping compression.'
        );
      } else {
