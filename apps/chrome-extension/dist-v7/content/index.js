(() => {
  ...
  const e = new (class {
    ...
    findElements() {
      const e = Array.from(
          document.querySelectorAll(
            'rich-textarea p[contenteditable="true"], rich-textarea [contenteditable="true"], div.ql-editor.textarea, div.ql-editor[contenteditable="true"], div[contenteditable="true"][role="textbox"], div[contenteditable="true"][data-placeholder], #prompt-textarea, textarea, textarea[placeholder*="Ask" i], textarea[placeholder*="follow-up" i], textarea[placeholder*="Message" i], div[contenteditable="true"][aria-label*="Message" i], div[contenteditable="true"][aria-label*="Ask" i], div[contenteditable="true"]:not([role="button"])'
          )
    ...
    async sendMessage(e) {
      ...
      try {
        ...
        let freshBtn = this.findElements().sendButton || n;
        let btnAttempts = 0;
        while (freshBtn && freshBtn.hasAttribute('disabled') && btnAttempts < 10) {
          await this.delay(100);
          freshBtn = this.findElements().sendButton || freshBtn;
          btnAttempts++;
        }
        let sentViaEnter = !1;
        if (
          !freshBtn ||
          'perplexity.ai' === window.location.hostname ||
          window.location.hostname.endsWith('.perplexity.ai') ||
        ...
        !sentViaEnter && freshBtn && freshBtn.click();
        console.log('[SimpleChatBridge] Message sent:', e.substring(0, 50));
        return (this.startWatchingForResponse(s, a), !0);
      } catch (e) {
        return (
    ...
    countModelResponses() {
      const e = document.querySelectorAll('model-response').length;
      if (e > 0) return e;
      const t = document.querySelectorAll(
        'div.prose, div[class*="prose"], div[data-testid*="answer"]'
      ).length;
      if (t > 0) return t;
    ...
    getLatestResponse() {
      const _clean = (el) => {
        if (!el) return null;
        const c = el.cloneNode(!0);
        c.querySelectorAll('button, [role="button"], .chip, [class*="action"]').forEach((e) =>
          e.remove()
        );
      ...
      if (p.length > 0) {
        const txt = _clean(p[p.length - 1]);
        if (txt) return txt;
      }
      const g = document.querySelectorAll('[data-message-author-role="assistant"]');
      if (g.length > 0) return _clean(g[g.length - 1]);
      return null;
    }
    extractResponseArtifacts() {
      ...
      const e = document.querySelectorAll('model-response');
      let n = null;
      if (e.length > 0) n = e[e.length - 1];
      if (!n) {
        const e = document.querySelectorAll(
          'div.prose, div[class*="prose"], div[data-testid*="answer"]'
        );
        e.length > 0 && (n = e[e.length - 1]);
      }
      if (!n) return t;
      const s = Array.from(n.querySelectorAll('a[href]'))
        .map((e) => nUrl((e.getAttribute('href') || '').trim()))
        .filter((e) => /^https?:\/\//i.test(e));
      t.links = Array.from(new Set(s));
      const a = [];
      n.querySelectorAll('img[src],video[src],source[src],iframe[src]').forEach((e) => {
        const t = nUrl((e.getAttribute('src') || '').trim());
        if (!t) return;
        a.push({
      ...
      for (const e of t.links) {
        const t = this.getYouTubeVideoId(e);
        if (t) {
          a.push({
            type: 'thumbnail',
            provider: 'youtube',
      ...
      t.attachments = a.filter((e, t, n) => {
        const s = `${e.type || 'file'}:${e.url || ''}`;
        return n.findIndex((e) => `${e.type || 'file'}:${e.url || ''}` === s) === t;
      });
      return t;
    }
    getYouTubeVideoId(e) {
      try {
        const t = new URL(e);
        if (t.hostname.includes('youtu.be')) return t.pathname.replace('/', '').trim() || null;
        if (t.hostname.includes('youtube.com')) return t.searchParams.get('v');
      } catch (e) {}
      return null;
    }
    enrichContentWithLinks(e, t) {
      const n = (e || '').trim();
      if (!t?.links?.length) return n;
      const s = t.links.filter((e) => !n.includes(e));
    ...
    startWatchingForResponse(countBefore, nBaseline = null) {
      this.stopWatching();
      ...
      ((this.responseCheckInterval = window.setInterval(() => {
        if (watchToken !== this.watchToken) return;
        const eCount = this.countModelResponses(),
          e = this.getLatestResponse(),
          s = this.isStreaming(),
          a = eCount > countBefore || (!!e && e !== nBaseline);
        a && (hasSeenDelta = !0);
        if (hasSeenDelta) {
          console.log('[SimpleChatBridge] Checking response...', {
            ...
            countNow: eCount,
            countBefore,
            hasContentDelta: !!(e && e !== nBaseline),
          });
          if (e)
            if (e === nBaseline && !s) ((t = 0), (n = e));
            else if (e !== n || s) ((t = 0), (n = e));
            else if ((t++, t >= 2)) {
              this.stopWatching();
              if (e !== this.lastResponseText) {
                this.lastResponseText = e;
                console.log('[SimpleChatBridge] Response complete!', e.substring(0, 100));
                const a = this.extractResponseArtifacts();
                this.callbacks.onResponse?.(this.enrichContentWithLinks(e, a), a);
              }
            }
        }
      }, 1e3)),
        (this.responseTimeoutHandle = window.setTimeout(() => {
          if (!this.isWaitingForResponse || watchToken !== this.watchToken) return;
          const e = this.getLatestResponse();
          if (e && e !== nBaseline && e !== this.lastResponseText)
            return (
              console.warn('[SimpleChatBridge] Timeout fallback captured latest response'),
              (this.lastResponseText = e),
              this.stopWatching(),
              void this.callbacks.onResponse?.(e, this.extractResponseArtifacts())
            );
    ...
    stopWatching() {
      ((this.isWaitingForResponse = !1),
        ...
        this.responseTimeoutHandle &&
          (clearTimeout(this.responseTimeoutHandle), (this.responseTimeoutHandle = null)));
    }
    getLastResponse() {
  ...
  try {
    const t = globalThis.customElements;
    if (t && 'function' == typeof t.define) {
      const e = t.define;
      t.define.__isSafeGuarded ||
        ((t.define = function (t, n, s) {
  ...
  class t {
    constructor(e = {}) {
      ((this.container = null),
        ...
        (this.channelStorageListener = null),
        ...
        (this.pageAgentId = e.pageAgentId || null),
        (this.state = {
    ...
    renderChatTab() {
      ...
      return `\n      ${e}\n      <div class="fcp6-chat-scroll" id="fuse-chat-scroll" style="flex: 1; overflow-y: auto; max-height: 300px; padding-right: 4px;">\n        ${
        this.messages.length > 0
          ? this.messages
              .slice(-50)
              .map((e) => this.renderChatMessage(e))
              .join('')
      ...
      this.container &&
        (this.container.addEventListener('click', (e) => {
          ...
          ((this.lastActionEvent = e),
            (this.lastActionChannelId =
              t.closest('[data-channel-id]')?.getAttribute('data-channel-id') || null));
        ...
        this.container.addEventListener('change', (e) => {
          const t = e.target;
          if ('fuse-hidden-file-input' !== t.id) return;
          const n = Array.from(t.files || [])
            .map((e) => e.name)
            .filter(Boolean);
          n.length &&
            (this.messages.push({
              id: `attach-local-${Date.now()}`,
              from: 'Fuse System',
              to: 'You',
              content: `Selected local files (not uploaded by Fuse): ${n.join(', ')}`,
              timestamp: Date.now(),
              type: 'system',
            }),
            this.logActivity('panel_attach_local_selected', { fileNames: n, fileCount: n.length }),
            this.update());
          t.value = '';
        }),
        ...
        (this.channelStorageListener = (e, t) => {
          ...
          if (e.fuse_paused_channels) {
            const n = Array.isArray(e.fuse_paused_channels.newValue)
              ...
              : [];
            ((this.pausedChannels = new Set(n)), this.update());
          }
        }),
        chrome.storage.onChanged.addListener(this.channelStorageListener));
    }
    startDrag(e) {
    ...
    handleAction(e) {
      switch (e) {
        ...
        case 'submit-create-channel':
          ...
          break;
        case 'join-channel':
          this.lastActionChannelId && this.joinChannel(this.lastActionChannelId);
          break;
        case 'pause-channel':
          this.lastActionChannelId && this.toggleChannelPause(this.lastActionChannelId);
          break;
        case 'delete-channel':
          this.lastActionChannelId && this.deleteChannel(this.lastActionChannelId);
          break;
        case 'attach-file':
          this.openPageAttachFlow();
          break;
        case 'start-service':
        case 'stop-service':
    ...
    sendMessage() {
      ...
      const n = `panel-user-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
      if (this.currentChannel && this.pausedChannels.has(this.currentChannel))
        return (
          (e.value = ''),
          void console.log('[FuseConnect] Channel is paused; skipping relay broadcast')
        );
      ((e.value = ''),
        chrome.runtime.sendMessage({
          ...
          senderId: this.pageAgentId,
          metadata: {
            senderId: this.pageAgentId,
            senderPanelId: this.panelId,
            senderHost: window.location.hostname,
            senderUrl: window.location.href,
          },
        }),
        this.logActivity('panel_send_message', {
          messageId: n,
          contentPreview: t.substring(0, 120),
        }),
        this.messages.push({
          id: n,
          from: 'You',
        ...
        chrome.runtime.sendMessage({ type: 'INJECT_MESSAGE', content: t }, (e) => {
          e?.success
            ? (this.messages.push({
              ...
              this.logActivity('panel_inject_message', {
                contentPreview: t.substring(0, 120),
                success: !0,
              }),
              this.update())
    ...
    sendUnifiedMessage() {
      ...
      if (!e || !e.value.trim()) return;
      const t = e.value.trim();
      const s = `panel-unified-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
      ...
      if (
        ...
        'connected' === this.connectionStatus &&
          this.currentChannel &&
          !this.pausedChannels.has(this.currentChannel))
      ) {
        const e = `user:${t}`,
        ...
        if (!n || Date.now() - n > 3e3) {
          ...
          this.logActivity('panel_send_unified', {
            messageId: s,
            contentPreview: t.substring(0, 120),
            channel: this.currentChannel,
          });
    ...
    selectChannel(e) {
      ...
      ((this.currentChannel = e),
        ...
        this.logActivity('channel_select', { from: t, to: e }),
        this.update());
    }
    createChannel() {
    ...
    submitCreateChannel() {
      ...
      ((e.value = ''),
        ...
        this.safeSendMessage({ type: 'CHANNEL_CREATE', name: t }, (e) => {
          if (e?.success || e?.channelId) {
            const t = e?.channelId || null;
    ...
    toggleChannelPause(e) {
      if (!e) return;
      const t = this.pausedChannels.has(e);
      (t ? this.pausedChannels.delete(e) : this.pausedChannels.add(e),
        chrome.storage.local.set({ fuse_paused_channels: Array.from(this.pausedChannels) }),
        chrome.runtime.sendMessage({ type: 'CHANNEL_PAUSE', channelId: e, paused: !t }),
        this.logActivity('channel_pause_toggle', { channelId: e, paused: !t }),
        this.update());
    }
    deleteChannel(e) {
      if (!e) return;
      const t = this.channels.find((t) => t.id === e)?.name || e;
      if (!confirm(`Delete channel \"${t}\"?`)) return;
      (this.pausedChannels.delete(e),
        ...
        this.logActivity('channel_delete', { channelId: e, channelName: t }),
        this.update());
    }
    logActivity(e, t = {}) {
      try {
        chrome.runtime.sendMessage({
          ...
          senderId: this.pageAgentId || null,
          metadata: {
            senderPanelId: this.panelId,
            senderHost: window.location.hostname,
            senderUrl: window.location.href,
      ...
      switch (e.type) {
        ...
        case 'NEW_MESSAGE':
          if (e.message) {
            ...
            const a = t.metadata?.senderId || t.senderId || null,
              i = this.pageAgentId && a && a === this.pageAgentId,
              o = t.messageId || t.id || null,
              n =
                'You' === t.from || 'Browser Agent' === t.from || t.from?.includes('Browser Agent'),
              s = this.messages.some(
                (e) =>
                  ((o && (e.messageId === o || e.id === o)) ||
                    (e.content === t.content && (e.channel || null) === (t.channel || null))) &&
                  Date.now() - e.timestamp < 5e3
              );
            if (i || n || s) {
            ...
            (this.messages.push({
              ...t,
              messageId: o || t.messageId || null,
              artifacts: this.normalizeArtifacts(
                t.artifacts || {
                  links: Array.isArray(t?.metadata?.links) ? t.metadata.links : [],
                  attachments: Array.isArray(t?.metadata?.attachments)
              ...
              this.update());
            const r = t.from && !t.from.includes('You') && !t.from.includes('AI (Page)'),
              l =
            ...
            (console.log('[FuseConnect] NEW_MESSAGE auto-inject check:', {
              ...
              r &&
                ...
                chrome.runtime.sendMessage(
                  {
                    type: 'INJECT_MESSAGE',
                    content: this.appendArtifactsToContent(
                      t.content,
                      t.artifacts || {
                        links: Array.isArray(t?.metadata?.links) ? t.metadata.links : [],
                        attachments: Array.isArray(t?.metadata?.attachments)
        ...
        case 'RESPONSE_COMPLETE':
          if (
            ...
            const a = this.normalizeArtifacts(
              e.artifacts || {
                links: Array.isArray(e?.metadata?.links) ? e.metadata.links : [],
            ...
            ((t = t
              ...
              .trim()),
              (t = this.appendArtifactsToContent(t, a)));
            if (
              !t ||
              t.includes('[User → AI]') ||
              t.includes('[AI → User]') ||
    ...
    renderChatMessage(e) {
      const t = this.getMessageArtifacts(e),
        n = this.renderArtifactsHtml(t);
    ...
    normalizeUrl(e) {
      ...
      try {
        ...
        return /^https?:$/i.test(t.protocol) ? t.toString() : null;
      } catch (e) {
        return null;
      }
    }
    ...
    normalizeArtifacts(e) {
      ...
      const n = new Set();
      t.links = t.links.filter((e) => (n.has(e) ? !1 : (n.add(e), !0)));
      const s = new Set();
      t.attachments = t.attachments.filter((e) => {
        const t = `${e.type || 'file'}:${e.url}`;
        return s.has(t) ? !1 : (s.add(t), !0);
      });
      for (const e of t.links) {
        const n = this.getYouTubeVideoId(e);
        if (!n) continue;
        const s = `https://img.youtube.com/vi/${n}/hqdefault.jpg`;
        t.attachments.find((e) => e.url === s) ||
          t.attachments.push({
            type: 'thumbnail',
            provider: 'youtube',
            url: s,
    ...
    getMessageArtifacts(e) {
      const t = { links: [], attachments: [] },
        n = [e?.artifacts || null, e?.metadata || null];
      for (const e of n) {
        if (!e || 'object' != typeof e) continue;
        Array.isArray(e.links) &&
          e.links.forEach((e) => {
            'string' == typeof e && t.links.push(e);
          });
        Array.isArray(e.attachments) &&
          e.attachments.forEach((e) => {
            e && 'string' == typeof e.url && t.attachments.push(e);
          });
      }
      return this.normalizeArtifacts(t);
    }
    appendArtifactsToContent(e, t) {
      const n = this.normalizeArtifacts(t),
        s = (e || '').trim();
      if (!n.links.length) return s;
      const a = n.links.filter((e) => !s.includes(e));
      return a.length ? `${s}\n\n${a.join('\n')}`.trim() : s;
    }
    renderArtifactsHtml(e) {
      if (!e.links.length && !e.attachments.length) return '';
      const t = (e.attachments || []).map((e) => {
          const t = this.escapeHtml(e.url || ''),
            n = this.escapeHtml(e.type || 'file'),
            s =
              'thumbnail' === e.type ||
              'img' === e.type ||
              /\.(png|jpe?g|gif|webp|svg)(\?|$)/i.test(e.url || '');
          return s
            ? `\n            <a href="${t}" target="_blank" rel="noopener noreferrer" style="display:inline-block;margin:4px 6px 0 0;">\n              <img src="${t}" alt="${n}" loading="lazy" style="width:108px;height:72px;object-fit:cover;border-radius:6px;border:1px solid rgba(255,255,255,0.15);" />\n            </a>\n          `
            : `\n            <a href="${t}" target="_blank" rel="noopener noreferrer" style="display:inline-block;margin:4px 6px 0 0;font-size:11px;color:#7fd7ff;">📎 ${n}</a>\n          `;
        }),
        n = (e.links || []).map(
          (e) =>
            `\n          <a href="${this.escapeHtml(e)}" target="_blank" rel="noopener noreferrer" style="display:block;margin-top:4px;font-size:11px;color:#7fd7ff;word-break:break-all;">${this.escapeHtml(e)}</a>\n        `
        );
      return `\n        <div style="margin-top:8px;padding-top:6px;border-top:1px dashed rgba(255,255,255,0.12);">\n          ${t.join('')}\n          ${n.join('')}\n        </div>\n      `;
    }
    truncate(e, t) {
    ...
    openPageAttachFlow() {
      const e = [
        ...
        t = (e) => e && !e.closest('#fuse-connect-panel-v6');
      const n = [];
      for (const s of e) {
        Array.from(document.querySelectorAll(s)).forEach((e) => n.push([s, e]));
      }
      Array.from(document.querySelectorAll('*')).forEach((s) => {
        if (!s.shadowRoot) return;
        for (const a of e)
          s.shadowRoot.querySelectorAll(a).forEach((e) => {
            n.push([a, e]);
          });
      });
      for (const [e, s] of n) {
        if (!t(s)) continue;
        if ('input' === s.tagName.toLowerCase()) s.removeAttribute('disabled');
        return (
          s.click(),
          this.logActivity('panel_attach_click', {
            selector: e,
            targetTag: s.tagName,
          }),
          void console.log('[FuseConnect] Triggered page attach flow via selector:', e)
        );
      }
      const s = this.container?.querySelector('#fuse-hidden-file-input');
      s &&
        (s.click(),
        this.logActivity('panel_attach_fallback', {
          reason: 'page-attach-control-not-found',
        }),
        this.messages.push({
          id: `attach-note-${Date.now()}`,
          from: 'Fuse System',
          to: 'You',
          content:
            'No page upload control found on this page. Use the page UI directly if available.',
          timestamp: Date.now(),
          type: 'system',
        }),
        this.update());
    ...
    handleMessage(e) {
      switch (e.type) {
        ...
        case 'CHANNEL_PAUSE_UPDATE':
          e.channelId &&
            (e.paused
              ? this.pausedChannels.add(e.channelId)
              : this.pausedChannels.delete(e.channelId),
            this.update());
          break;
        case 'NEW_MESSAGE':
          const t = e.message;
    ...
    destroy() {
      (this.chromeMessageListener &&
        ...
        (this.chromeMessageListener = null)),
        this.channelStorageListener &&
          (chrome.storage.onChanged.removeListener(this.channelStorageListener),
          (this.channelStorageListener = null)),
    ...
    : ((window.__FUSE_CONNECT_INITIALIZED__ = !0),
      new (class {
        ...
        cleanupStalePanels() {
          const e = [
              '#fuse-connect-panel-v6',
              '#fuse-connect-styles-v6',
              '#fuse-connect-panel',
              '#fuse-connect-styles',
              '[id^="fuse-connect-panel"]',
              '[id^="fuse-panel"]',
              '.fcp6-context-warning',
            ],
            t = new Set();
          for (const n of e)
            document.querySelectorAll(n).forEach((e) => {
              t.has(e) || (t.add(e), e.remove());
            });
        }
        async init() {
        ...
        setup() {
          this.isInitialized ||
            (this.cleanupStalePanels(),
            (this.isInitialized = !0),
            console.log('[FuseConnect v6] Content script initialized (panel hidden by default)'),
            e.init({
              onResponse: (e, s = {}) => {
                const t = this.getMessageId(
                  `ai-response:${this.panel?.currentChannel || 'none'}:${e.substring(0, 800)}`
                );
                if (this.isProcessedMessage(t)) return;
                this.markMessageProcessed(t);
                const n = this.panel?.currentChannel || null;
                (console.log('[FuseConnect v6] AI Response received, length:', e.length),
                  this.panel &&
                    this.panel.logActivity('ai_response_captured', {
                      messageId: t,
                      channel: n,
                  ...
                  chrome.runtime.sendMessage({
                    ...
                    metadata: {
                      senderId: this.pageAgentId,
                      senderPanelId: this.panel?.panelId || null,
                      senderHost: window.location.hostname,
                      senderUrl: window.location.href,
                      links: s?.links || [],
            ...
            chrome.runtime.sendMessage(
              ...
              (e) => {
                this.tabId = e?.tabId ?? null;
                this.panelVisibilityKey = `fuse_panel_visible_tab_${this.tabId ?? 'unknown'}`;
                const t = `fuse_page_agent_id_tab_${this.tabId ?? 'unknown'}`;
                chrome.storage.local.get([t], (e) => {
                  this.pageAgentId = e?.[t] || this.createPageAgentId();
                  chrome.storage.local.set({ [t]: this.pageAgentId });
                  console.log('[FuseConnect v6] Page agent ready:', {
                    tabId: this.tabId,
                    pageAgentId: this.pageAgentId,
                  });
                  chrome.runtime.sendMessage({
                    type: 'PAGE_AGENT_REGISTER',
                    pageAgentId: this.pageAgentId,
                    tabId: this.tabId,
                    url: window.location.href,
                    hostname: window.location.hostname,
                    platform: this.getPlatformFromUrl(window.location.hostname),
                  });
                });
              }
            ));
        }
        getPlatformFromUrl(e) {
          if (!e) return 'custom';
          if (e.includes('gemini.google.com')) return 'gemini';
          if (e.includes('perplexity.ai')) return 'perplexity';
          if (e.includes('chatgpt.com')) return 'chatgpt';
          if (e.includes('claude.ai')) return 'claude';
          if (e.includes('poe.com')) return 'poe';
          return 'custom';
        }
        restorePanelVisibility() {
          this.panelVisibilityKey &&
            chrome.storage.local.get([this.panelVisibilityKey], (e) => {
              e?.[this.panelVisibilityKey] && this.showPanel();
            });
        }
        persistPanelVisibility(e) {
          this.panelVisibilityKey && chrome.storage.local.set({ [this.panelVisibilityKey]: !!e });
        }
        createPageAgentId() {
          const e = this.tabId ?? 'unknown',
            t = Math.random().toString(36).substring(2, 8);
          return `page-agent-${e}-${t}`;
        }
        getMessageId(e) {
          let t = 0;
          for (let n = 0; n < e.length; n++) ((t = (t << 5) - t + e.charCodeAt(n)), (t |= 0));
          return `msg-${Math.abs(t)}-${Math.floor(Date.now() / 1e3)}`;
        }
        isProcessedMessage(e) {
          return this.processedMessageIds.has(e);
        }
        markMessageProcessed(e) {
          (this.processedMessageIds.add(e),
            this.processedMessageIds.size > 300 &&
              (this.processedMessageIds = new Set(
                Array.from(this.processedMessageIds).slice(-200)
              )));
        }
        startChatDetection() {
          const t = () => {
        ...
        showPanel() {
          if (!this.panel) {
            this.panel = new t({ pageAgentId: this.pageAgentId });
            ...
            switch (t.type) {
              ...
              case 'NEW_MESSAGE':
                if ((this.panel && this.panel.handleMessage(t), t.message)) {
                  ...
                  if (c && this.panel?.pausedChannels?.has(c))
                    return (
                      console.log(
                        '[FuseConnect v6] Channel paused; skipping NEW_MESSAGE injection',
                        c
                      ),
                      s({ success: !0 }),
                      !0
                    );
                  const n = e.metadata?.senderId || e.senderId || null,
                    a =
                      e.messageId ||
                      e.id ||
                      t.messageId ||
                      this.getMessageId(
                        `${e.channel || e.channelId || 'none'}:${e.from}:${e.content}:${e.timestamp || ''}`
                      ),
                    i = !!(this.pageAgentId && n && n === this.pageAgentId),
                    o =
                      e.content?.startsWith('[AI Response]') ||
                      e.content?.startsWith('[AI → User]') ||
                      'ai-response' === e.messageType;
                  if (this.isProcessedMessage(a))
                    return (
                      console.log('[FuseConnect v6] Skipping already-processed message', {
                        messageId: a,
                      }),
                      s({ success: !0 }),
                      !0
                    );
                  this.markMessageProcessed(a);
              ...
              case 'RESPONSE_COMPLETE': {
                ...
                if (c && this.panel?.pausedChannels?.has(c))
                  return (
                    console.log(
                      '[FuseConnect v6] Channel paused; skipping RESPONSE_COMPLETE injection',
                      c
                    ),
                    s({ success: !0 }),
                    !0
                  );
                const e = t.senderId || null,
                  n = !!(this.pageAgentId && e && e === this.pageAgentId),
                  a =
                    t.messageId ||
                    this.getMessageId(
                      `resp:${t.channel || t.channelId || 'none'}:${e || 'unknown'}:${String(t.content).substring(0, 800)}`
                    );
                if (n || this.isProcessedMessage(a)) return (s({ success: !0 }), !0);
                this.markMessageProcessed(a);
                const i = String(t.content)
                  .replace(/^\[AI\s*→\s*User\]\s*/g, '')
                  .trim();
                return (
                  i &&
        ...
        async injectMessage(t) {
          const n = (t || '').replace(/\s+/g, ' ').trim().substring(0, 1200),
            s = Date.now();
          if (n && this.lastInjectedSignature === n && s - this.lastInjectedAt < 5e3)
            return (console.log('[FuseConnect v6] Skipping duplicate inject within 5s'), !0);
          ((this.lastInjectedSignature = n), (this.lastInjectedAt = s));
          console.log('[FuseConnect v6] Injecting message:', t.substring(0, 50));
          const a = await e.sendMessage(t);
          try {
            chrome.runtime.sendMessage({
              type: 'ACTIVITY_EVENT',
              eventType: 'content_inject_message',
              channel: this.panel?.currentChannel || null,
              senderId: this.pageAgentId || null,
              metadata: {
                senderHost: window.location.hostname,
                senderUrl: window.location.href,
