...
The New Fuse Chrome Extension now supports automatic resolution of decentralized web (Web3) URLs to their HTTP gateway equivalents.
...
- **Resolves to:** `https://ipfs.io/ipfs/QmXoypizjW3WknFiJnKLwHCnL72vedxjQkDDP1mXWo6uco`
...
- **Address Bar:** Type any Web3 URL directly in the browser address bar, and it will automatically resolve to the appropriate gateway
- **Programmatic Navigation:** Web3 URLs passed through the extension's browser control API are automatically resolved
- **Notifications:** When a Web3 URL is resolved, a notification shows the original URL and the resolved gateway URL
...
3. Configure custom gateways:
   - **IPFS Gateway:** For `ipfs://` and `ipns://` URLs (default: `https://ipfs.io`)
   - **Arweave Gateway:** For `ar://` URLs (default: `https://arweave.net`)
   - **Swarm Gateway:** For `bzz://` URLs (default: `https://gateway.ethswarm.org`)
...
1. **Web3 URL Resolver** ([utils/web3-url-resolver.ts](apps/chrome-extension/src/utils/web3-url-resolver.ts))
...
2. **Web3 Interceptor** ([background/web3-interceptor.ts](apps/chrome-extension/src/background/web3-interceptor.ts))
...
3. **Background Script Integration** ([background/index.ts](apps/chrome-extension/src/background/index.ts))
...
4. **Browser Control Handler** ([background/browser-control-handler.ts](apps/chrome-extension/src/background/browser-control-handler.ts))
   - Intercepts Web3 URLs in programmatic navigation
   - Returns Web3 metadata in navigation results

5. **Options UI** ([options/index.tsx](apps/chrome-extension/src/options/index.tsx))
...
```typescript
import { resolveWeb3Url, isWeb3Url, validateWeb3Url } from './utils/web3-url-resolver';
...
Since the TNF Chrome Extension is installed by default in the Electron app's Chromium browser, **Web3 support is automatically available in the Electron app**. No additional configuration is needed.
...
1. Update [web3-url-resolver.ts](apps/chrome-extension/src/utils/web3-url-resolver.ts):
   - Add protocol to `Web3Protocol` type
   - Add resolution logic
   - Add validation logic

2. Update [web3-interceptor.ts](apps/chrome-extension/src/background/web3-interceptor.ts):
