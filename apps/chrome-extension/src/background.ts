...
try {
  ...
  chrome.storage.local.get(['websocketUrl'], (result) => {
    ...
    try {
      ...
      webSocketManager.on(
        'connectionStatus',
        (statusPayload: ConnectionStatusMessage['payload']) => {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  try {
            ...
            chrome.tabs
              ...
              .catch((error) => {
                backgroundLogger.error(
                  'Error forwarding AUTO_DETECT_ELEMENTS to content script:',
                  error
                );
      ...
      case 'SET_FLOATING_PANEL_STATE':
        ...
        if (message.state) {
          chrome.storage.local.set({ [FLOATING_PANEL_STATE_KEY]: message.state }, () => {
            if (chrome.runtime.lastError) {
              backgroundLogger.error(
                'Error setting floating panel state:',
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
          if (tabs[0] && tabs[0].id) {
            chrome.tabs
              ...
              .catch((error) => {
                backgroundLogger.error(
                  'Error forwarding TOGGLE_FLOATING_PANEL to content script:',
                  error
                );
                sendResponse({ success: false, error: error.message });
              });
                ...
                (tab) => {
                  if (chrome.runtime.lastError) {
                    ...
                    sendResponse({
                      success: false,
                      error:
                        'Failed to start WebSocket server. VS Code integration might not be installed.',
    ...
    sendResponse({
      success: false,
      error: (error as Error).message || 'Unknown error in background script',
    });
  }
...
async function autoOptimizePerformance(tabId: number | undefined, parameters: any) {
  ...
  if (health?.results) {
    const memoryIssues = health.results.filter(
      (r: any) => r.component === 'Memory Usage' && r.status !== 'healthy'
    );
    const cpuIssues = health.results.filter(
      (r: any) => r.component === 'CPU Usage' && r.status !== 'healthy'
    );
    const networkIssues = health.results.filter(
      (r: any) => r.component === 'Network Latency' && r.status !== 'healthy'
    );
