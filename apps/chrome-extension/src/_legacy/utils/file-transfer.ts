...
function isTransferRequestMessage(data: FileTransferMessage): data is FileTransferRequestMessage {
  return (
    data.type === 'FILE_TRANSFER_REQUEST' &&
...
export class FileTransferManager {
  ...
  private handleWebSocketMessage(data: any): void {
    if (!isFileTransferMessage(data)) {
      this.logger.debug(
        'FileTransferManager: Received non-standard message or message missing type/fileId, ignoring.',
        data
      );
      return;
    }
  ...
  private handleChunkAck(data: FileTransferChunkAckMessage): void {
    ...
    transfer.progress =
      (transfer.chunks.filter((c) => c.acknowledged).length / transfer.chunks.length) * 100;
  ...
  private handleTransferComplete(data: FileTransferCompleteMessage): void {
    ...
    this.logger.info(
      `File transfer confirmed complete by remote: ${transfer.fileName} (${fileId})`
    );
  ...
  private handleFileChunk(data: FileTransferChunkMessage): void {
    ...
    try {
      ...
      try {
        ...
        this.logger.error(
          `Error decoding Base64 chunk data for ${transfer.fileName} (${fileId}), chunk ${chunkIndex}:`,
          base64Error
        );
      ...
      this.wsManager.send({
        ...
        fileId,
        error: error.message,
      } as FileTransferErrorMessage); // Corrected from 'fileTransferError'
    }
  }
  ...
  private handleTransferCancel(data: FileTransferCancelMessage): void {
    ...
    if (transfer) {
      ...
      if (transfer.onError) {
        // Or a specific onCancel callback
  ...
  private handleTransferAbort(data: FileTransferAbortMessage): void {
    ...
    if (transfer) {
      ...
      if (transfer.onError) {
        // Or a specific onAbort callback
        transfer.onError(new Error(`Transfer aborted by remote: ${abortReason}`));
      }
      this.transfers.delete(fileId);
    } else {
      this.logger.warn(
        `Received abort for unknown transfer: ${fileId}. Reason: ${reason || 'No reason provided'}`
      );
    }
  }
  ...
  private sendSpecificChunk(fileId: string, chunkIndex: number): void {
    ...
    if (chunk.retries < maxRetries) {
      ...
      this.wsManager.send({
        type: 'FILE_TRANSFER_ERROR',
        fileId,
        error: `Max retries for chunk ${chunkIndex}`,
      } as FileTransferErrorMessage);
      if (transfer.onError)
   ...
   * @private
   */
  private validateTransferForChunkSend(transfer: Transfer | undefined, fileId: string): boolean {
    if (!transfer) {
   ...
   * @private
   */
  private handleChunkSendError(transfer: Transfer, chunkIndex: number, error: unknown): void {
  ...
  cancelTransfer(fileId: string, reason: string = 'User cancelled'): void {
    ...
    if (transfer.onError) {
      // Optionally, call onError when a transfer is locally cancelled.
