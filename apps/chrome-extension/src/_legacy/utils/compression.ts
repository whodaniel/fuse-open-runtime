...
export const compressString = (message: string | object): Uint8Array => {
  try {
    ...
    console.warn(
      '[CompressionUtils] Compression failed:',
...
export const decompressArrayBufferToString = (data: ArrayBuffer): string => {
  try {
    ...
    console.warn(
      '[CompressionUtils] Decompression failed:',
      error instanceof Error ? error.message : error
    );
    throw error;
  }
...
export const compressionUtils: CompressionUtils = {
  ...
  decompressMessage(compressed: string): string | object | null {
    try {
      ...
      console.warn(
        '[CompressionUtils] Decompression failed, attempting to parse as JSON or return as string:',
        decompressionError instanceof Error ? decompressionError.message : decompressionError
      );
      try {
        ...
        console.warn(
          '[CompressionUtils] Could not decompress or parse as JSON. Returning raw (if string) or null.:',
          jsonError
        );
        return null;
      }
