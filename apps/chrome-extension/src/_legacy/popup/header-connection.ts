...
export class HeaderConnectionManager {
  ...
  initialize(): void {
    try {
      ...
      this.connectButton.addEventListener('click', () => {
        if (
          this.connectionManager.isConnectedToVSCode() ||
          this.connectionManager.isConnectedToRelay()
        ) {
  ...
  private updateConnectionStatus(): void {
    const isConnected =
      this.connectionManager.isConnectedToVSCode() || this.connectionManager.isConnectedToRelay();
    ...
    if (this.connectionDot) {
      ...
      this.connectionDot.title = isConnected
        ? 'Connected'
        : this.isConnecting
          ? 'Connecting...'
          : 'Disconnected';
    ...
    if (this.connectButton) {
      ...
      this.connectButton.className = isConnected
        ? 'primary-button connected'
        : 'primary-button disconnected';
    }
