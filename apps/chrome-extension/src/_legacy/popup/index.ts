...
function initializeSettings(): void {
  ...
  const keyboardNavigationCheckbox = document.getElementById(
    'keyboard-navigation'
  ) as HTMLInputElement;
  ...
  chrome.runtime.sendMessage({ type: 'GET_SETTINGS' }, (response) => {
    if (response && response.success) {
      ...
      const saveChatHistoryCheckbox = document.getElementById(
        'save-chat-history'
      ) as HTMLInputElement;
      const debugModeCheckbox = document.getElementById('debug-mode') as HTMLInputElement;
      const autoDetectCodeCheckbox = document.getElementById(
        'auto-detect-code'
      ) as HTMLInputElement;
  ...
  chrome.runtime.sendMessage({ type: 'AUTH_STATUS' }, (response) => {
    if (response && response.success) {
      ...
      if (tokenExpiry)
        tokenExpiry.textContent = response.tokenExpiry
          ? new Date(response.tokenExpiry).toLocaleString()
          : 'None';
    }
  });
  ...
  if (saveSettingsButton) {
    saveSettingsButton.addEventListener('click', () => {
      ...
      const wsProtocol =
        (document.getElementById('ws-protocol') as HTMLSelectElement)?.value || 'ws';
      const wsHost = (document.getElementById('ws-host') as HTMLInputElement)?.value || 'localhost';
      const wsPort = parseInt(
        (document.getElementById('ws-port') as HTMLInputElement)?.value || '3712',
        10
      );
      const relayUrl =
      ...
      const autoConnect =
        (document.getElementById('auto-connect') as HTMLInputElement)?.checked !== false;
      const showNotifications =
        (document.getElementById('show-notifications') as HTMLInputElement)?.checked !== false;
      const saveChatHistory =
        (document.getElementById('save-chat-history') as HTMLInputElement)?.checked !== false;
      const debugMode =
        (document.getElementById('debug-mode') as HTMLInputElement)?.checked === true;
      const autoDetectCode =
        (document.getElementById('auto-detect-code') as HTMLInputElement)?.checked !== false;
      const useCompression =
        (document.getElementById('use-compression') as HTMLInputElement)?.checked !== false;
        ...
        if (response && response.success) {
          ...
          chrome.runtime.sendMessage({ type: 'AUTH_STATUS' }, (authResponse) => {
            if (authResponse) {
              ...
              if (tokenExpiry)
                tokenExpiry.textContent = authResponse.tokenExpiry
