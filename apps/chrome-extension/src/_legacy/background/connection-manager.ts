...
export class ConnectionManager {
  ...
  async connect(): Promise<boolean> {
    try {
      ...
      this.wsManager.addListener(
        'encryption_unavailable',
        this.handleEncryptionUnavailable.bind(this)
      ); // New
      ...
      if (connected) {
        ...
        connectionLogger.error(
          'Failed to connect to WebSocket server - timed out or connection failed'
        );
  ...
  private notifyConnectionState(): void {
    ...
    let finalStatus:
      ...
      | 'error'
      | 'authenticating'
      | 'uninitialized' = wsFullStatus.status;
  ...
  private handleEncryptionUnavailable(errorEvent: { message: string }): void {
    const errorMessage =
      errorEvent.message ||
      'Encryption key is not set or available. Cannot send/receive secure messages.';
