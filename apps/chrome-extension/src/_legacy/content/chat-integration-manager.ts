...
export class ChatIntegrationManager {
    ...
    const platforms: ChatPlatform[] = [
      {
        ...
        patterns: {
          ...
          messageSelectors: ['[data-message-id]', '.conversation-turn', '.message'],
        },
        features: {
      ...
      },
      {
        ...
        domain: 'claude.ai',
        patterns: {
          inputSelectors: ['.ProseMirror', '[contenteditable="true"]', 'textarea'],
          buttonSelectors: ['[aria-label*="Send"]', 'button[type="submit"]', '.send-button'],
          outputSelectors: ['[data-is-streaming]', '.font-claude-message', '.message-content'],
          messageSelectors: ['.font-claude-message', '.message', '[data-message]'],
        },
        features: {
          supportsMarkdown: true,
          supportsCodeBlocks: true,
      ...
      },
      {
        ...
        patterns: {
          inputSelectors: ['.ql-editor', '[contenteditable="true"]', 'textarea'],
          buttonSelectors: ['[aria-label*="Send"]', '.send-button', 'button[type="submit"]'],
          outputSelectors: ['.model-response', '.response-container', '.message-content'],
          messageSelectors: ['.conversation-turn', '.message', '.chat-message'],
        },
        features: {
          supportsMarkdown: true,
          supportsCodeBlocks: true,
      ...
      },
      {
        ...
        domain: 'discord.com',
        patterns: {
          inputSelectors: ['[data-slate-editor="true"]', '.slateTextArea', 'div[role="textbox"]'],
          buttonSelectors: ['[aria-label*="Send"]', '.sendButton', 'button[type="submit"]'],
          outputSelectors: ['.messageContent', '.markup', '.message-content'],
          messageSelectors: ['.message', '[id^="chat-messages"]', '.messageListItem'],
        },
        features: {
          supportsMarkdown: true,
          supportsCodeBlocks: true,
      ...
      },
      {
        ...
        domain: 'slack.com',
        patterns: {
          inputSelectors: ['.ql-editor', '[data-qa="message_input"]', '[contenteditable="true"]'],
          buttonSelectors: [
            '[data-qa="texty_send_button"]',
            '.c-button--primary',
            '[aria-label*="Send"]',
          ],
          outputSelectors: ['.c-message__body', '.p-rich_text_section', '.message-content'],
          messageSelectors: ['.c-message', '[data-qa="message"]', '.c-message_kit__message'],
        },
        features: {
  ...
  private async initializePlatformSpecific(platform: ChatPlatform): Promise<boolean> {
    try {
      ...
      this.logger.info(
        `${platform.name} integration initialized with ${this.extractedMessages.length} existing messages`
      );
      return true;
    } catch (error) {
  ...
  private determineSender(
    ...
    const messageElements = document.querySelectorAll(
      platform.patterns.messageSelectors.join(', ')
    );
  ...
  private extractTimestamp(element: HTMLElement): number | null {
    ...
    const timeSelectors = ['time', '.timestamp', '.time', '[datetime]', '.message-time'];
  ...
  private isMessageEdited(element: HTMLElement): boolean {
    return (
      element.querySelector('.edited') !== null ||
  ...
  private isDuplicateMessage(message: ChatMessage): boolean {
    return this.extractedMessages.some(
      (existing) =>
  ...
  private findElementsBySelectors(
    selectors: string[],
