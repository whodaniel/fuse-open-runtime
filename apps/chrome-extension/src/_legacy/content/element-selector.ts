...
export class ElementSelector {
  ...
  public enterSelectionMode(
    elementType: string,
    callback?: (elementInfo: ElementInfo, elementType: string) => void
  ): void {
  ...
  private classifyElement(element: HTMLElement): {
    ...
    // Input detection
    const inputIndicators = ['input', 'textarea', 'message', 'chat', 'text', 'prompt', 'query'];
    ...
    // Button detection
    const buttonIndicators = ['button', 'send', 'submit', 'go', 'enter'];
  ...
  private selectBestCandidate(candidates: HTMLElement[], type: string): HTMLElement | null {
    ...
    return candidates.reduce((best, current) => {
      ...
      const bestScore =
        bestInfo.confidence + (bestInfo.isVisible ? 0.2 : 0) + (bestInfo.isInteractable ? 0.2 : 0);
      const currentScore =
        currentInfo.confidence +
        (currentInfo.isVisible ? 0.2 : 0) +
        (currentInfo.isInteractable ? 0.2 : 0);
