...
export class DebugSettingsManager {
  ...
  private async saveSettings(): Promise<void> {
    try {
      ...
      const debugMode =
        (document.getElementById('debug-mode') as HTMLInputElement)?.checked || false;
      const verboseLogging =
        (document.getElementById('verbose-logging') as HTMLInputElement)?.checked || false;
      const logToConsole =
        (document.getElementById('log-to-console') as HTMLInputElement)?.checked || true;
      const logToStorage =
        (document.getElementById('log-to-storage') as HTMLInputElement)?.checked || true;
      const maxLogSize = parseInt(
        (document.getElementById('max-log-size') as HTMLInputElement)?.value || '1000',
        10
      );
