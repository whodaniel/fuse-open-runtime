// The New Fuse - Backend API Client
// Handles communication with TNF AI Video Intelligence backend
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 && retryCount === 0) {
          console.warn('[TNF API] 401 Unauthorized - JWT token may be expired');

          // Try to get user profile again to refresh JWT
          const stored = await chrome.storage.local.get(['userProfile']);
          if (stored.userProfile) {
            console.log('[TNF API] Attempting to refresh JWT token...');
            try {
              await this.login({
              ...
              console.log('[TNF API] JWT refreshed, retrying original request...');
              return await this.request(endpoint, options, retryCount + 1);
            } catch (refreshError) {
              console.error('[TNF API] Failed to refresh JWT:', refreshError);
              // Clear auth state
