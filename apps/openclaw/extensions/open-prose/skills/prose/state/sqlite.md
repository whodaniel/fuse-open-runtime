...
**SQLite database**. This is an experimental alternative to file-based state
(`filesystem.md`) and in-context state (`in-context.md`).
...
If `sqlite3` is not available, the VM will fall back to filesystem state and
warn the user.
...
- **Structured queries**: Find specific bindings, filter by status, aggregate
  results
...
**Key principle:** The database is a flexible workspace. The VM and subagents
share it as a coordination mechanism, not a rigid contract.
...
`.prose/agents.db`, and for user-scoped agents live in `~/.prose/agents.db`. The
VM initializes these databases on first use and provides the correct path to
subagents.
...
The VM initializes these tables. This is a **minimum viable schema**—extend
freely.
...
- **JSON fields**: Store structured data as JSON text in `metadata`, `*_schema`
  columns
...
- **Anonymous bindings**: Sessions without explicit capture (`session "..."`
  without `let x =`) use auto-generated names: `anon_001`, `anon_002`, etc.
...
For parallel blocks, the VM uses the `metadata` JSON field to track branches.
...
Unlike filesystem state, SQLite state is intentionally **less prescriptive**.
The core schema is a starting point. You are encouraged to:
...
write their own outputs directly to the database. Both maintain the principle
that what happens is recorded, and what is recorded can be queried.
