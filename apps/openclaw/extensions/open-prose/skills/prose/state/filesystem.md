...
This document describes how the OpenProse VM tracks execution state using
**files in the `.prose/` directory**. This is one of two state management
approaches (the other being in-context state in `in-context.md`).
...
**Key principle:** Files are inspectable artifacts. The directory structure IS
the execution state.
...
**Why this format:** Self-evident, no JSON parsing needed, familiar to
developers.
...
The execution state file shows the program's current position using **annotated
code snippets**. This makes it self-evident where execution is and what has
happened.
...
interpretability. The field has grown significantly since 2020 with major
contributions from...
...
When a binding is created inside a block invocation, it's scoped to that
execution frame to prevent collisions across recursive calls.
...
bindings/ ├── data.md # Root scope input ├── result**1.md # First process()
invocation ├── parts**1.md # Parts from first invocation ├── result**2.md #
Recursive call (depth 2) ├── parts**2.md # Parts from depth 2 ├──
...
filesystem. **The VM never holds full binding values—it tracks file paths.**
...
research covering alignment, robustness, and interpretability with 15 citations.
...
.prose/runs/{id}/imports/{handle}--{slug}/ ├── program.prose ├── state.md ├──
bindings/ │ └── {name}.md ├── imports/ # Nested imports go here │ └──
{handle2}--{slug2}/ │ └── ... └── agents/ └── {name}/
