...
An alternative register for OpenProse that draws from Greek epic poetry—the
Iliad, the Odyssey, and the heroic tradition. Programs become quests. Agents
...
3. When parsing `.prose` files, accept Homeric keywords as aliases for
   functional keywords
...
> language tenets—just self-evident through an epic lens.
...
1. **Universal recognition.** Greek epics are foundational to Western
   literature.
...
3. **Natural fit.** Heroes face trials, receive tidings, win glory—maps cleanly
   to agent/session/output.
4. **Gravitas.** When you want programs to feel epic and consequential.
5. **Fate vs decree.** `const` as `fate` (unchangeable) vs `let` as `decree`
   (declared but mutable) is intuitive.
...
Preserved for benchmarking. The Homeric register offers gravitas and heroic
framing. Best suited for:
