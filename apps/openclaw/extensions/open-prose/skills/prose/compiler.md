...
OpenProse is a programming language for AI sessions. An AI session is a
Turing-complete computer; this document provides complete documentation for the
language syntax, semantics, and execution model.
...
The validation criterion: **Would a blank agent with only `prose.md` understand
this program as self-evident?**
...
If a construct is ambiguous or non-obvious, it should be flagged or transformed
into a clearer form.
...
OpenProse provides a declarative syntax for defining multi-agent workflows.
Programs consist of statements that are executed sequentially, with each
`session` statement spawning a subagent to complete a task.

### Design Principles

- **Pattern over framework**: The simplest solution is barely anything at
  all—just structure for English
...
- **Framework-agnostic**: Works with Claude Code, OpenCode, and any future agent
  framework
...
Comments are **stripped during compilation**. The OpenProse VM never sees them.
They have no effect on execution and exist purely for human documentation.
...
Multi-line strings use triple double-quotes (`"""`) and preserve internal
...
- Variables are referenced by wrapping the variable name in curly braces:
  `{varname}`
...
Use statements import other OpenProse programs from the registry at
`p.prose.md`, enabling modular workflows.
...
Use statements are processed before any agent definitions or sessions. The
OpenProse VM:
...
Agents are reusable templates that configure subagent behavior. Once defined,
agents can be referenced in session statements.
...
Skills must be imported before they can be assigned. Referencing an unimported
skill generates a warning.
...
The session statement is the primary executable construct in OpenProse. It
spawns a subagent to complete a task.
...
2. **Spawn a Subagent**: Create a new Claude subagent with the resolved
   configuration
...
Variables allow you to capture the results of sessions and pass them as context
...
This is equivalent to passing an object where each property is a variable
reference.
...
All variable names must be **unique within a program**. No shadowing is allowed
across scopes.
...
**Why this constraint:** Since bindings are stored as `bindings/{name}.md`, two
variables with the same name would collide on the filesystem. Rather than
introduce complex scoping rules, we enforce uniqueness.
...
**Exception:** Imported programs run in isolated namespaces. A variable `result`
in the main program does not collide with `result` in an imported program (they
write to different `imports/{handle}--{slug}/bindings/` directories).

---

## Composition Blocks

Composition blocks allow you to structure programs into reusable, named units
and express sequences of operations inline.

### do: Block (Anonymous Sequential Block)

The `do:` keyword creates an explicit sequential block. All statements in the
...
Named blocks create reusable workflow components. Define once, invoke multiple
times.
...
The `->` operator chains sessions into a sequence on a single line. This is
...
Block definitions are hoisted - you can use a block before it's defined in the
source:
...
Parallel blocks allow multiple sessions to run concurrently. All branches
execute simultaneously, and the block waits for all to complete before
continuing.
...
All three sessions start at the same time and run concurrently. The program
...
The object shorthand `{ a, b, c }` is equivalent to passing an object with
properties `a`, `b`, and `c` where each property's value is the corresponding
variable.
...
The setup runs first, then Task A and Task B run in parallel, and finally
cleanup runs.
...
Each parallel branch contains a sequential workflow. The two workflows run
concurrently.
...
By default, parallel blocks wait for all branches to complete. You can specify
...
Fixed loops provide bounded iteration over a set number of times or over a
collection.
...
Unbounded loops provide iteration with AI-evaluated termination conditions.
Unlike fixed loops, the iteration count is not known ahead of time - the
OpenProse VM evaluates conditions at runtime using its intelligence to determine
when to stop.

### Discretion Markers

Unbounded loops use **discretion markers** (`**...**`) to wrap AI-evaluated
conditions. These markers signal that the enclosed text should be interpreted
intelligently by the OpenProse VM at runtime, not as a literal boolean
expression.
...
**Warning**: Loops without termination conditions or max iterations generate a
warning. Always include a safety limit:
...
The OpenProse VM evaluates the discretion condition after each iteration and
exits when it determines the condition is satisfied.
...
1. **Context Awareness**: The condition is evaluated in the context of what has
   happened so far in the session
2. **Semantic Understanding**: The condition text is interpreted semantically,
   not literally
...
Pipeline operations provide functional-style collection transformations. They
allow you to chain operations like map, filter, and reduce using the pipe
operator (`|`).
...
Inside a map body, the implicit variable `item` refers to the current element
being processed.
...
The session in a filter body should return something the OpenProse VM can
interpret as truthy/falsy (like "yes"/"no").
...
OpenProse provides structured error handling with try/catch/finally blocks,
throw statements, and retry mechanisms for resilient workflows.

### Try/Catch Blocks

The `try:` block wraps operations that might fail. The `catch:` block handles
errors.
...
The error variable (`err`) contains contextual information about what went wrong
and is only accessible within the catch block.
...
Inside a catch block, `throw` without arguments re-raises the caught error to
outer handlers:
...
Try blocks can be nested. Inner catch blocks don't trigger outer handlers unless
they rethrow:
...
This differs from the `on-fail:` policy which controls behavior when unhandled
errors occur.
...
Choice blocks allow the OpenProse VM to select from multiple labeled options
based on criteria. This is useful for branching workflows where the best path
depends on runtime analysis.
...
The criteria is wrapped in discretion markers (`**...**`) and is evaluated by
the OpenProse VM to select which option to execute:
...
If/elif/else statements provide conditional branching based on AI-evaluated
conditions using discretion markers.
...
For the current implementation, state is tracked in-context (conversation
history):
...
All core features through Tier 12 have been implemented. Potential future
enhancements:
...
1. **Read this document** (`compiler.md`) fully to understand all syntax and
   validation rules
...
For direct interpretation without compilation, read `prose.md` and execute
statements as described in the Session Statement section.
