...
This document catalogs patterns that lead to brittle, expensive, slow, or
unmaintainable programs. Each antipattern includes recognition criteria and
...
A single session that tries to do everything. God sessions are hard to debug,
impossible to parallelize, and produce inconsistent results.
...
**Why it's bad**: The session has no clear completion criteria. It mixes
concerns that could be parallelized. Failure anywhere fails everything.
...
Running independent operations sequentially when they could run concurrently.
...
**Why it's bad**: Total time is sum of all research times. Each session waits
for the previous one unnecessarily.
...
Context passed haphazardly without clear data flow. Makes programs hard to
...
**Why it's bad**: Unclear which sessions depend on which outputs. Hard to
...
Spawning parallel agents for related analytical work, then synthesizing, when a
single focused agent could do the entire job more efficiently.
...
**Why it's bad**: For related analysis that feeds into one conclusion, the
coordination overhead and context fragmentation often outweigh parallelism
benefits. Each parallel agent sees only part of the picture.
...
**When parallel IS right**: When analyses are truly independent (security vs
performance), when you want diverse perspectives that shouldn't influence each
other, or when the work is so large it genuinely benefits from division.

#### copy-paste-workflows

Duplicating session sequences instead of using blocks. Leads to inconsistent
changes and maintenance burden.
...
**Why it's bad**: If the workflow needs to change, you must change it
everywhere. Easy to miss one.
...
A loop without max iterations. Can run forever if the condition is never
satisfied.
...
**Why it's bad**: "Perfect" may never be achieved. The program could run
...
Assuming everything will succeed. No error handling for operations that can
fail.
...
**Why it's bad**: If the API fails, subsequent sessions receive no valid input.
Silent corruption.
...
Using `on-fail: "ignore"` when failures actually matter. Masks problems that
should surface.
...
**Why it's bad**: The VM has no clear criteria for evaluation. Results are
unpredictable.
...
Catching errors without meaningful handling. Hides problems without solving
them.
...
**Why it's bad**: Opus costs significantly more than haiku. Simple tasks don't
benefit from advanced reasoning.
...
**Why it's bad**: Large contexts slow processing, increase costs, and can
...
**Why it's bad**: Three sessions when one could handle all items. Session
overhead multiplied.
...
**Why it's bad**: Same research done three times with slightly different
framing.
...
Parallelizing so aggressively that overhead dominates or resources are
exhausted.
...
**Why it's bad**: May overwhelm the system. Coordination overhead can exceed
...
**Why it's bad**: Coordination overhead exceeds task time. Sequential would be
...
**Fix**: Use appropriate patterns for fire-and-forget operations, or batch
logging:
...
**Why it's bad**: Relies on VM implementation details. Fragile across
refactoring.
...
**Why it's bad**: No focus means mediocre results across the board. Can't
...
**Why it's bad**: Zero bugs is unachievable. Loop runs until max (if specified)
or forever.
...
**Why it's bad**: The health check result isn't used. Deploy happens
unconditionally.
...
**Why it's bad**: Missing config is expected, not exceptional. Obscures actual
errors.
...
**Why it's bad**: Each checkpoint is a round-trip waiting for user input. If the
answer is predictable 90% of the time, you're adding latency for no value.
...
**When checkpoints ARE right**: Irreversible actions (production deployments to
critical systems), expensive operations (long-running jobs), or genuine decision
points where the user's preference isn't predictable.
...
**Why it's bad**: Wastes time when the answer is already known. If the observer
detected a fatal error at +5 seconds, why wait another 55 seconds?
