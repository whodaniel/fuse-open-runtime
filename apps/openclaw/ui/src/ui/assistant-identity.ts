...
declare global {
  interface Window {
    __THENEWFUSE_ASSISTANT_NAME__?: string;
    __THENEWFUSE_ASSISTANT_AVATAR__?: string;
  }
}
...
export function resolveInjectedAssistantIdentity(): AssistantIdentity {
  ...
  return normalizeAssistantIdentity({
    name: window.__THENEWFUSE_ASSISTANT_NAME__,
    avatar: window.__THENEWFUSE_ASSISTANT_AVATAR__,
  });
}
