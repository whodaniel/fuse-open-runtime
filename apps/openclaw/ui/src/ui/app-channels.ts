...
export async function handleWhatsAppStart(host: TheNewFuseApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: TheNewFuseApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: TheNewFuseApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: TheNewFuseApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: TheNewFuseApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: TheNewFuseApp): string {
...
export function handleNostrProfileCancel(host: TheNewFuseApp) {
...
export function handleNostrProfileToggleAdvanced(host: TheNewFuseApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: TheNewFuseApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: TheNewFuseApp) {
  const state = host.nostrProfileFormState;
