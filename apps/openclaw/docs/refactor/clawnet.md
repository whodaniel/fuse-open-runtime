---
summary:
  'Clawnet refactor: unify network protocol, roles, auth, approvals, identity'
read_when:
...
- Proposed new state: one protocol, scoped roles, unified auth/pairing, TLS
  pinning.
...
- Full API surface: config, channels, models, sessions, agent runs, logs, nodes,
  etc.
...
- Can receive `invoke` commands (`system.run`, `camera.*`, `canvas.*`,
  `screen.record`, etc).
...
Role is per‑connection, not per device. A device may open both roles,
separately.
...
- **Physical presence window**: after a local approval on gateway host UI, allow
  auto‑pair for a short window (e.g. 10 minutes).
...
Approval happens on node host (mac app node runtime). Prompt appears where node
runs.
...
- Broadcast to all operators; only the active UI shows a modal (others get a
  toast).
...
- Proposal: one WS protocol with explicit roles + scopes, unified pairing + TLS
  pinning, gateway‑hosted approvals, stable device IDs + cute slugs.
- Outcome: simpler UX, stronger security, less duplication, better mobile
  routing.
