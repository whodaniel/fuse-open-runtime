...
- **Discord/Slack**: empty `emoji` removes all of the bot's reactions on the
...
- **WhatsApp**: empty `emoji` removes the bot reaction; `remove: true` maps to
  empty emoji (still requires `emoji`).
