...
This guide summarizes a sane workflow for working on the pi integration in
OpenClaw.
...
For tool call behavior, prompt for a `read` or `exec` action so you can see tool
streaming and payload handling.
...
`agents/<agentId>/sessions.json` for that agent. Keep `credentials/` if you do
not want to reauthenticate.
