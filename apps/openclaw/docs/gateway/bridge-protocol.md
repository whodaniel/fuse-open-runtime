...
**Note:** Current OpenClaw builds no longer ship the TCP bridge listener; this
document is kept for historical reference. Legacy `bridge.*` config keys are no
longer part of the config schema.
...
- **Loopback WS**: the full WS control plane stays local unless tunneled via
  SSH.
...
- Legacy default listener port was `18790` (current builds do not start a TCP
  bridge).
...
- `req` / `res`: scoped gateway RPC (chat, sessions, config, health, voicewake,
  skills.bins)
- `event`: node signals (voice transcript, agent request, chat subscribe, exec
  lifecycle)
...
Nodes can emit `exec.finished` or `exec.denied` events to surface system.run
activity. These are mapped to system events in the gateway. (Legacy nodes may
still emit `exec.started`.)
...
- `exitCode`, `timedOut`, `success`, `output`: completion details (finished
  only).
...
Bridge is currently **implicit v1** (no min/max negotiation). Backward‑compat is
