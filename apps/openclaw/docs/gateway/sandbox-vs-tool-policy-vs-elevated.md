...
1. **Sandbox** (`agents.defaults.sandbox.*` / `agents.list[].sandbox.*`) decides
   **where tools run** (Docker vs host).
2. **Tool policy** (`tools.*`, `tools.sandbox.tools.*`, `agents.list[].tools.*`)
   decides **which tools are available/allowed**.
3. **Elevated** (`tools.elevated.*`, `agents.list[].tools.elevated.*`) is an
   **exec-only escape hatch** to run on the host when you’re sandboxed.
...
- effective sandbox tool allow/deny (and whether it came from
...
See [Sandboxing](/gateway/sandboxing) for the full matrix (scope, workspace
mounts, images).
...
- Binding `/var/run/docker.sock` effectively hands host control to the sandbox;
  only do this intentionally.
- Workspace access (`workspaceAccess: "ro"`/`"rw"`) is independent of bind
  modes.
...
- **Tool profile**: `tools.profile` and `agents.list[].tools.profile` (base
  allowlist)
...
- `/exec` only changes session defaults for authorized senders; it does not
  grant tool access. Provider tool keys accept either `provider` (e.g.
...
- If you’re already running direct, elevated is effectively a no-op (still
  gated).
