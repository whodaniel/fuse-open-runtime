...
如果你能运行 `npm install -g openclaw@latest`，但之后看到
`openclaw: command not found`，这几乎总是 **PATH**
问题：npm 存放全局二进制文件的目录不在你 shell 的 PATH 中。
...
`$(npm prefix -g)`（Windows）**未出现**在 `echo "$PATH"`
的输出中，你的 shell 就无法找到全局 npm 二进制文件（包括 `openclaw`）。
...
如果 `npm install -g ...` 因 `EACCES`
失败，请将 npm 的全局前缀切换到用户可写的目录：
