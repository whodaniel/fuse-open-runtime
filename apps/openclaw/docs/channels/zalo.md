...
- Deterministic routing: replies go back to Zalo; the model never chooses
  channels.
...
Multi-account support: use `channels.zalo.accounts` with per-account tokens and
optional `name`.
...
- Streaming is blocked by default due to the 2000 char limit making streaming
  less useful.
...
- `channels.zalo.allowFrom` accepts numeric user IDs (no username lookup
  available).
...
**Note:** getUpdates (polling) and webhook are mutually exclusive per Zalo API
docs.
...
- `channels.zalo.dmPolicy`: `pairing | allowlist | open | disabled` (default:
  pairing).
- `channels.zalo.allowFrom`: DM allowlist (user IDs). `open` requires `"*"`. The
  wizard will ask for numeric IDs.
