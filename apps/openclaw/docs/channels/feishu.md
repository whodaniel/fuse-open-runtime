...
Status: production-ready, supports bot DMs and group chats. Uses WebSocket long
connection mode to receive events.
...
Lark (global) tenants should use https://open.larksuite.com/app and set
`domain: "lark"` in the Feishu config.
...
If your tenant is on Lark (international), set the domain to `lark` (or a full
domain string). You can set it at `channels.feishu.domain` or per account
...
> Note: Feishu does not support native command menus yet, so commands must be
> sent as text.
...
Feishu does not support message editing, so block streaming is enabled by
default (`blockStreaming: true`). The bot waits for the full reply before
sending.
