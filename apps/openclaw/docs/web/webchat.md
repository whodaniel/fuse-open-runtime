...
- A native chat UI for the gateway (no embedded browser and no local static
  server).
...
- The UI connects to the Gateway WebSocket and uses `chat.history`, `chat.send`,
  and `chat.inject`.
- `chat.inject` appends an assistant note directly to the transcript and
  broadcasts it to the UI (no agent run).
...
- No dedicated `webchat.*` block. WebChat uses the gateway endpoint + auth
  settings below.
