...
2. In the iOS app, open Settings and pick a discovered gateway (or enable Manual
   Host and enter host/port).
...
`openclaw.internal.`) and Tailscale split DNS. See [Bonjour](/gateway/bonjour)
...
- The Gateway canvas host serves `/__openclaw__/canvas/` and
  `/__openclaw__/a2ui/`.
...
- iOS may suspend background audio; treat voice features as best-effort when the
  app is not active.
...
- Pairing prompt never appears: run `openclaw nodes pending` and approve
  manually.
- Reconnect fails after reinstall: the Keychain pairing token was cleared;
  re-pair the node.
