...
- Metrics include token usage, cost, context size, run duration, and
  message-flow counters/histograms (webhooks, queueing, session state, queue
  depth/wait).
...
- If the endpoint already contains `/v1/traces` or `/v1/metrics`, it is used
  as-is.
