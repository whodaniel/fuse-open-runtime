...
Manage OpenClaw’s browser control server and run browser actions (tabs,
snapshots, screenshots, navigation, clicks, typing).
...
- `openclaw`: launches/attaches to a dedicated OpenClaw-managed Chrome instance
...
This mode lets the agent control an existing Chrome tab that you attach manually
(it does not auto-attach).
...
browser actions to that node (no separate browser control server required).
