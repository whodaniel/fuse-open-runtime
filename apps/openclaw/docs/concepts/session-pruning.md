...
- When `mode: "cache-ttl"` is enabled and the last Anthropic call for the
...
- **OAuth or setup-token** profiles: enable `cache-ttl` pruning and set
  heartbeat to `1h`.
- **API key** profiles: enable `cache-ttl` pruning, set heartbeat to `30m`, and
  default `cacheControlTtl` to `1h` on Anthropic models.
- If you set any of these values explicitly, OpenClaw does **not** override
  them.
...
- **What gets cheaper:** pruning reduces the **cacheWrite** size for that first
...
- **What it does not do:** pruning doesn’t add tokens or “double” costs; it only
...
- The last `keepLastAssistants` assistant messages are protected; tool results
  after that cutoff are not pruned.
- If there aren’t enough assistant messages to establish the cutoff, pruning is
  skipped.
- Tool results containing **image blocks** are skipped (never trimmed/cleared).

## Context window estimation

Pruning uses an estimated context window (chars ≈ tokens × 4). The base window
...
If `agents.defaults.contextTokens` is set, it is treated as a cap (min) on the
resolved window.
...
- Pruning only runs if the last Anthropic call is older than `ttl` (default
  `5m`).
...
- Compaction is separate: compaction summarizes and persists, pruning is
