...
import { buildPerplexityProvider } from './perplexity-models.js';
import {
  SYNTHETIC_BASE_URL,
  SYNTHETIC_MODEL_CATALOG,
  buildSyntheticModelDefinition,
} from './synthetic-models.js';
import { VENICE_BASE_URL, discoverVeniceModels } from './venice-models.js';

const ANTIGRAVITY_BASE_URL = 'http://localhost:3001/api/advanced-llm/proxy';
const ANTIGRAVITY_DEFAULT_CONTEXT_WINDOW = 2000000;
...
const TOGETHER_DEFAULT_CONTEXT_WINDOW = 128000;
const TOGETHER_DEFAULT_MAX_TOKENS = 8192;
...
function buildOpenRouterProvider(): ProviderConfig {
  return {
    ...
    models: [
      ...
      {
        id: 'google/gemini-pro-1.5-exp:free',
        name: 'Gemini Pro 1.5 Exp (Free)',
        reasoning: false,
        input: ['text', 'image'],
        cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
    ...
    models: [
      {
        ...
        name: 'Claude 4.5 Thinking (Antigravity)',
        reasoning: true,
        input: ['text', 'image'],
...
export async function resolveImplicitProviders(params: {
  ...
  const antigravityKey =
    resolveEnvApiKeyVarName('antigravity') ??
    resolveEnvApiKeyVarName('tnf') ??
    resolveApiKeyFromProfiles({ provider: 'antigravity', store: authStore });
  if (antigravityKey) {
    providers.antigravity = { ...buildAntigravityProvider(), apiKey: antigravityKey };
  }
