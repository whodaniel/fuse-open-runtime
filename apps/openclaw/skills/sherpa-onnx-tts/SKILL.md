...
The wrapper lives in this skill folder. Run it directly, or add the wrapper to
PATH:
...
- Pick a different model from the sherpa-onnx `tts-models` release if you want
  another voice.
- If the model dir has multiple `.onnx` files, set `SHERPA_ONNX_MODEL_FILE` or
  pass `--model-file`.
