...
description:
  Batch-generate images via OpenAI Images API. Random prompt sampler +
  `index.html` gallery.
...
Generate a handful of “random but structured” prompts and render them via the
OpenAI Images API.
...
Different models support different parameter values. The script automatically
selects appropriate defaults based on the model.
...
- **GPT image models** support additional parameters:
  ...
  - Note: `stream` and `moderation` are available via API but not yet
    implemented in this script
- **dall-e-3** has a `--style` parameter: `vivid` (hyper-real, dramatic) or
