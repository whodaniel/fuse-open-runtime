...
name: ordercli
description:
  Foodora-only CLI for checking past orders and active order status (Deliveroo
  WIP).
...
Use `ordercli` to check past orders and track active order status (Foodora only
right now).
