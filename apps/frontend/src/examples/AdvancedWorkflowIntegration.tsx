...
export function AdvancedWorkflowIntegration() {
  ...
  const { isConnected, activeWorkflows, startWorkflow, stopWorkflow } = useWorkflowIntegration();

  const handleAnalysisStart = useCallback(async () => {
    const tools = [new DataAnalysisTool(), new VisualizationTool(), new StatisticalTool()];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-100 rounded-lg p-4">
        ...
        <pre className="bg-white p-4 rounded">{JSON.stringify(analysisConfig, null, 2)}</pre>
      </div>
    </div>
  );
}
