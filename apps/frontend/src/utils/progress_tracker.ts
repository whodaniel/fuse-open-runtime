...
export class ProgressTracker {
  ...
  async monitorTask(taskId) {
    ...
    while (taskId in this.tasks) {
      try {
        ...
        this.logger.error(
          `Error monitoring task ${taskId}: ${error instanceof Error ? error.message : String(error)}`
        );
        break;
      }
    }
  ...
  async getTaskStatus(taskId) {
    ...
    try {
      ...
      this.logger.error(
        `Error getting status for task ${taskId}: ${error instanceof Error ? error.message : String(error)}`
      );
      throw error;
    }
  }
  shouldAdjustStrategy(taskId) {
    ...
    const progressRate =
      ...
      (metrics.timestamps[metrics.timestamps.length - 1] -
        metrics.timestamps[metrics.timestamps.length - 5]);
  ...
  async rebalanceResources(task) {
    try {
      ...
      this.logger.error(
        `Error rebalancing resources: ${error instanceof Error ? error.message : String(error)}`
      );
      throw error;
    }
  }
