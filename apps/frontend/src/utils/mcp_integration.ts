...
export class MCPWorkflowAdapter {
  ...
  async handleToolUse(toolUse) {
    ...
    try {
      ...
      if (toolName.startsWith('use_')) {
        ...
        const agent = Array.from(this.activeAgents.values()).find((a) =>
  ...
  async cleanup() {
    await Promise.all(Array.from(this.activeAgents.values()).map((agent) => agent.stop()));
  }
}
