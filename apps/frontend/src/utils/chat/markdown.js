...
export default function renderMarkdown(content, options) {
    if (options === void 0) { options = {}; }
    if (!content)
        return '';
    var disableLinks = options.disableLinks, disableHighlight = options.disableHighlight, stripHtml = options.stripHtml;
    ...
    if (disableLinks) {
        renderedContent = renderedContent.replace(/<a [^>]*>/g, '<span class="text-primary-button">').replace(/<\/a>/g, '</span>');
    }
