...
export class HarmlessnessScreen {
  constructor() {
    this.harmful_patterns = {
      ...
      moderate: [/risk/i, /danger/i, /warning/i, /caution/i, /unsafe/i],
      mild: [/issue/i, /problem/i, /concern/i, /attention/i],
    };
  }
  screen(content) {
    const issues = [];
    ...
    if (severeMatches.length > 0) {
      ...
      recommendations.push(
        'Remove or rephrase content with severe security implications',
    ...
    if (moderateMatches.length > 0 && highestCategory === ContentCategory.SAFE) {
      ...
      recommendations.push(
        'Review and possibly rephrase cautionary language',
    ...
    if (mildMatches.length > 0 && highestCategory === ContentCategory.SAFE) {
      ...
      recommendations.push(
        'Consider if the concerning language is necessary',
        'Review for clearer communication'
      );
    }
    return {
