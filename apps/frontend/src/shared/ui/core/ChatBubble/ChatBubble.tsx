...
export function ChatBubble({
  ...
  return (
    <div className={cn(chatBubbleVariants({ align: align || defaultAlign, size }), className)}>
      <div className="space-y-1">
        {showAuthor && (
          <div className="text-xs text-muted-foreground">{authorLabel || t(`common.${type}`)}</div>
        )}
          ...
          >
            {isEditing ? (
              ...
              <div className="flex items-center gap-2">
                <span>{message}</span>
                {status && (
                  <span className="ml-2 inline-flex items-center">{statusIcons[status]}</span>
                )}
              </div>
            )}
          </div>
          {onMessageRemove && !isEditing && (
            <Button
              variant="ghost"
              size="sm"
        ...
        {timestamp && (
          <div className="text-xs text-muted-foreground">
            {typeof timestamp === 'string' ? timestamp : format(timestamp, 'HH:mm')}
          </div>
        )}
      </div>
    </div>
