...
export const UserIcon = forwardRef(
  (
    ...
    const defaultFallback =
      role === 'user' ? '/assets/icons/user-default.svg' : '/assets/icons/workspace-default.svg';
    const handleError = (e) => {
    ...
    return (
      <div
        ref={ref}
        className={cn(userIconVariants({ size, variant }), onClick && 'cursor-pointer', className)}
        onClick={onClick}
      >
        <img
          src={src || fallbackSrc || defaultFallback}
          alt={alt || `${role} profile picture`}
          onError={handleError}
