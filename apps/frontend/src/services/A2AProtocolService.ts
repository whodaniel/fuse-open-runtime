...
export class A2AProtocolService {
  ...
  async sendMessage(message: A2AMessage): Promise<any> {
    try {
      ...
      const outbound = this.normalizeOutboundMessage(message);
      if (!outbound.to) {
        throw new Error('Direct message requires a recipient');
      }

      const response = await fetch(`${this.apiBaseUrl}/a2a/messages/send`, {
        method: 'POST',
        headers: {
  ...
  async broadcastMessage(message: A2AMessage): Promise<any> {
    try {
      ...
      const outbound = this.normalizeOutboundMessage(message);
      const response = await fetch(`${this.apiBaseUrl}/a2a/messages/broadcast`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async sendRequestAndWaitForResponse(message: A2AMessage, timeout: number = 30000): Promise<any> {
    try {
      // Validate message
      this.validateMessage(message);
      const ack = await this.sendMessage(message);
      return {
        ...ack,
        requestTimeoutMs: timeout,
        note: 'Request sent. Synchronous response endpoint is not exposed by backend; consume response via inbox/channel polling.',
      };
    } catch (error) {
  ...
  private normalizeOutboundMessage(message: A2AMessage): A2AOutboundPayload {
    const version = this.getMessageVersion(message);

    if (version === '2.0' && 'header' in message && 'body' in message) {
      return {
        from: message.header.source,
        to: message.header.target,
    ...
    const v1 = message as A2AMessageV1;
    return {
      from: v1.sender,
      to: v1.recipient,
      type: v1.type,
      payload: v1.payload,
      priority: v1.metadata.priority,
    };
  }
}
