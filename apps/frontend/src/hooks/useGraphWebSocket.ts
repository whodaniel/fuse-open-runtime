...
export function useGraphWebSocket({
  ...
  (0, react_1.useEffect)(() => {
    ...
    messageHandlersRef.current.set('select', (payload) => {
      setState((prev) =>
        Object.assign(Object.assign({}, prev), { selectedNodes: payload.nodeIds })
      );
    });
  ...
  const sendMessage = (0, react_1.useCallback)((type, payload) => {
    var _a;
    if (
      ((_a = wsRef.current) === null || _a === void 0 ? void 0 : _a.readyState) === WebSocket.OPEN
    ) {
  ...
  const updateNode = (0, react_1.useCallback)(
    (nodeId, data) => {
      sendMessage('update', {
  ...
  const selectNodes = (0, react_1.useCallback)(
    (nodeIds) => {
      sendMessage('select', { nodeIds });
    },
    [sendMessage]
  );
  const expandNode = (0, react_1.useCallback)(
    (nodeId, expanded) => {
      sendMessage('expand', { nodeId, expanded });
    },
    [sendMessage]
  );
  const filterGraph = (0, react_1.useCallback)(
    (filters) => {
