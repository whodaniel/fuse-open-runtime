...
export function useGraphAnalysis(initialNodes = [], initialEdges = [], options = {}): any {
  ...
  const handleAnalysisResult = (0, react_1.useCallback)((data) => {
    setState((prev) =>
      Object.assign(Object.assign({}, prev), {
        ...
        communities: new Map(Object.entries(data.communities)),
        isAnalyzing: false,
        error: null,
      })
    );
  ...
  const getNodeMetrics = (0, react_1.useCallback)(
    (nodeId) => {
      return state.metrics.get(nodeId);
    },
    [state.metrics]
  );
  const getNodeCommunity = (0, react_1.useCallback)(
    (nodeId) => {
      return state.communities.get(nodeId);
    },
    [state.communities]
  );
  const getCommunityNodes = (0, react_1.useCallback)(
    (communityId) => {
  ...
  const getMetricsSummary = (0, react_1.useCallback)(() => {
    ...
    return {
      ...
      averageClusteringCoeff:
        metrics.reduce((sum, m) => sum + m.clusteringCoefficient, 0) / metrics.length,
