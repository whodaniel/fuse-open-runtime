...
export const useMassOptimization = () => {
  ...
  const optimizeAgent = useCallback(async (
    agentId: string, 
  ...
  const optimizeTopology = useCallback(async (
    agentIds: string[], 
  ...
  const createOptimizedAgent = useCallback(async (
    agentId: string, 
    config: MassOptimizationConfig
  ...
  const getOptimizationJob = useCallback(async (jobId: string): Promise<OptimizationJob> => {
    return apiCall(`/jobs/${jobId}`);
  }, [apiCall]);

  const getUserOptimizationJobs = useCallback(async (
    status?: string, 
    type?: string
  ...
  const getAgentOptimizationHistory = useCallback(async (agentId: string) => {
    return apiCall(`/agents/${agentId}/history`);
  }, [apiCall]);

  const createValidationDataset = useCallback(async (datasetData: {
    name: string;
    description?: string;
  ...
  const getAgentPerformanceAnalytics = useCallback(async (
    agentId: string, 
    timeRange?: string
  ) => {
    const params = new URLSearchParams();
  ...
  const getTopologyPerformanceAnalytics = useCallback(async (topologyId: string) => {
    return apiCall(`/analytics/topology/${topologyId}`);
  }, [apiCall]);

  const exportOptimizedAgent = useCallback(async (agentId: string) => {
    return apiCall(`/export/agent/${agentId}`);
  }, [apiCall]);

  const importOptimizedAgent = useCallback(async (importData: any) => {
  ...
  const exportOptimizedTopology = useCallback(async (topologyId: string) => {
    return apiCall(`/export/topology/${topologyId}`);
  }, [apiCall]);

  const importOptimizedTopology = useCallback(async (importData: any) => {
