...
export function useMessages(selectedAgent, conversationId): any {
  ...
  (0, react_1.useEffect)(() => {
    ...
    socket.on('chatMessage', (message) => {
      setMessages((prevMessages) => [
        ...prevMessages,
        Object.assign(Object.assign({}, message), { timestamp: new Date(message.timestamp) }),
      ]);
      setIsTyping(false);
    });
  ...
  const loadMessages = async (conversationId) => {
    try {
      const messagesResponse =
        await chatService_1.chatService.getConversationMessages(conversationId);
    ...
    try {
      ...
      if (response.status === 'success') {
        socket === null || socket === void 0
          ...
          : socket.emit(
              'sendChatMessage',
              (0, message_utils_1.createSocketPayload)(message, conversationId)
            );
