    ...
    <div className="space-y-8 animate-in slide-in-from-bottom duration-700">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400 mt-2">Real-time monitoring of the Agent-to-Agent message broker and relay system.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard label="Throughput" value={`${stats.throughput} msg/s`} icon={<Zap className="text-yellow-400" />} />
        <StatCard label="Active Exchanges" value={stats.activeExchanges.toString()} icon={<Share2 className="text-blue-400" />} />
        <StatCard label="Total Routed" value={stats.totalMessages.toLocaleString()} icon={<Database className="text-purple-400" />} />
        <StatCard label="Avg Latency" value={stats.avgLatency} icon={<Activity className="text-emerald-400" />} />
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <GlassCard className="overflow-hidden border-cyan-500/10">
            <div className="max-h-[600px] overflow-y-auto divide-y divide-white/5 scrollbar-thin scrollbar-thumb-white/10">
              {messages.map((msg) => (
                <div key={msg.id} className="p-4 hover:bg-white/5 transition-colors flex items-center gap-4 group">
                  <div className="flex flex-col items-center justify-center w-8 shrink-0">
                    <div className="text-[10px] text-gray-500 font-mono">{new Date(msg.timestamp).toLocaleTimeString([], { hour12: false, minute: '2-digit', second: '2-digit' })}</div>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs text-gray-400 truncate max-w-[100px] font-mono">{msg.senderId}</span>
                      <ArrowRight className="w-3 h-3 text-gray-600" />
                      <span className="text-xs text-gray-400 truncate max-w-[100px] font-mono">{msg.receiverId}</span>
                    </div>
                  ...
                  <div className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                    msg.status === 'processed' ? 'bg-emerald-500/10 text-emerald-400' : 
                    msg.status === 'failed' ? 'bg-red-500/10 text-red-400' : 'bg-gray-500/10 text-gray-400'
                  }`}>
                    {msg.status}
                  </div>
                </div>
        ...
        <div className="space-y-6">
          ...
          <GlassCard className="p-6 overflow-hidden relative group">
            ...
            <div className="relative z-10">
                ...
                <p className="text-xs text-gray-400 mb-4">Test inter-agent communication protocols without affecting production swarms.</p>
...
const ProtocolBadge: React.FC<{ label: string; active?: boolean }> = ({ label, active }) => (
  <span className={`px-2 py-1 rounded text-[10px] font-bold border ${
    active ? 'bg-cyan-500/20 border-cyan-500/50 text-cyan-400' : 'bg-white/5 border-white/10 text-gray-500'
  }`}>
    {label}
  </span>
);
...
const MOCK_MESSAGES: A2AMessage[] = [
  { id: '1', senderId: 'researcher-agent', receiverId: 'writer-agent', type: 'DATA_EXCHANGE', payload: { query: 'Latest trends in AI', results: [1,2,3] }, status: 'processed', timestamp: new Date().toISOString() },
  { id: '2', senderId: 'writer-agent', receiverId: 'editor-agent', type: 'TASK_REQUEST', payload: { draft: 'This is a draft...', target: 'blog' }, status: 'sent', timestamp: new Date().toISOString() },
  { id: '3', senderId: 'editor-agent', receiverId: 'writer-agent', type: 'FEEDBACK', payload: { comments: 'Needs more data', score: 0.8 }, status: 'processed', timestamp: new Date().toISOString() },
  { id: '4', senderId: 'worker-node-01', receiverId: 'coordinator', type: 'HEARTBEAT', payload: { load: 0.45, temp: 62 }, status: 'processed', timestamp: new Date().toISOString() },
  { id: '5', senderId: 'coordinator', receiverId: 'worker-node-02', type: 'NODE_CMD', payload: { cmd: 'RESTART', delay: 0 }, status: 'failed', timestamp: new Date().toISOString() },
];
