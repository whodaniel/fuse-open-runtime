import { PLATFORM_PARITY_FEATURES, type ParityStatus } from '@/config/platformParityFeatures';
...
export default function PlatformParityDashboard() {
  const implemented = PLATFORM_PARITY_FEATURES.filter(
    (feature) => feature.status === 'implemented'
  ).length;
  const partial = PLATFORM_PARITY_FEATURES.filter((feature) => feature.status === 'partial').length;
  const planned = PLATFORM_PARITY_FEATURES.filter((feature) => feature.status === 'planned').length;

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-white">Platform Feature Parity</h1>
        <p className="text-slate-300 mt-2">
          Competitive parity tracker generated from verified platform crawl artifacts.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-white/10 bg-slate-950/40 overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-slate-900/60 border-b border-white/10">
            <tr>
              <th className="px-4 py-3 text-xs uppercase tracking-wide text-slate-300">
                Target Feature
              </th>
              <th className="px-4 py-3 text-xs uppercase tracking-wide text-slate-300">
                TNF Route
          ...
          <tbody>
            {PLATFORM_PARITY_FEATURES.map((feature) => (
              <tr key={feature.id} className="border-b border-white/5 last:border-b-0">
                ...
                <td className="px-4 py-3">
                  <span
                    className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${statusStyles[feature.status]}`}
                  >
