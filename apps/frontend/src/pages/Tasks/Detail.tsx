...
import { getTask } from '@/services/unifiedLedgerApi';
import {
  AlertTriangle,
  Bot,
  Calendar,
...
const mockTaskDetails = {
  ...
  comments: [
    ...
    },
    {
      ...
      authorIsAgent: true,
      content:
        "I've started implementing the endpoint. I'll make sure to follow the guidelines for error responses.",
...
const TaskDetail: React.FC = () => {
  ...
  const [task, setTask] = useState<any>(mockTaskDetails);
    ...
    getTask(id)
      .then((row) => {
        ...
        setTask({
            ...
            row.status === 'in_progress'
              ...
              : row.status === 'completed'
                ...
                : row.status === 'under_review'
                  ...
                  : 'Not Started',
          priority:
            row.priority === 'urgent' || row.priority === 'critical'
              ? 'Critical'
              : row.priority === 'high'
                ? 'High'
                : row.priority === 'low'
          ...
          progress: row.fractal?.progressPercent || 0,
          votes: { upvotes: row.votes?.up || 0, downvotes: row.votes?.down || 0, userVote: null },
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium flex items-center">
                        <span
                          className={`w-5 h-5 rounded-full bg-${task.assignedColor}-100 text-${task.assignedColor}-600 flex items-center justify-center text-xs mr-1`}
                        >
                          {task.assignedAvatar}
                        </span>
                        {task.assignedTo}
                      ...
                      <div className="space-y-4 mb-6">
                        {task.comments.map((comment) => (
                          <div key={comment.id} className="flex space-x-3">
                            <div
                              className={`w-8 h-8 rounded-full ${comment.authorIsAgent ? `bg-blue-100 text-blue-600` : 'bg-gray-100 text-gray-600'} flex items-center justify-center shrink-0`}
                            >
