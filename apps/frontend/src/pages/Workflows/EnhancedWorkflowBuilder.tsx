...
const EnhancedWorkflowBuilder: React.FC = () => {
  ...
  const { isOpen: isNodeLibraryOpen, onOpen: onNodeLibraryOpen, onClose: onNodeLibraryClose } = useDisclosure();
  const { isOpen: isNodeSettingsOpen, onOpen: onNodeSettingsOpen, onClose: onNodeSettingsClose } = useDisclosure();
  const { isOpen: isSaveModalOpen, onOpen: onSaveModalOpen, onClose: onSaveModalClose } = useDisclosure();
  const { isOpen: isExecutionLogOpen, onOpen: onExecutionLogOpen, onClose: onExecutionLogClose } = useDisclosure();
    ...
    try {
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        setNodes((nds) =>
          nds.map((n) =>
            n.id === node.id
              ? { ...n, data: { ...n.data, status: 'running', progress: 50 } }
              : n
          )
        );
          ...
          <ModalBody pb={6}>
            <VStack spacing={4} align="stretch">
              ...
              <HStack justify="flex-end" spacing={3}>
                <Button onClick={onSaveModalClose}>
                  Cancel
                </Button>
          ...
          <DrawerBody>
            <VStack spacing={2} align="stretch">
              {executionState.logs.map((log, index) => (
                <Alert
                  key={index}
                  status={log.level === 'error' ? 'error' : log.level === 'success' ? 'success' : 'info'}
                  variant="left-accent"
                  borderRadius="md"
                >
                  <AlertIcon />
