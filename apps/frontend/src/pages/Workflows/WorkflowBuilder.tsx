    ...
    <Card bg="purple.50" borderColor="purple.200" borderWidth={2}>
      <CardBody p={3}>
        <VStack spacing={2}>
          ...
          <Text fontSize="sm" fontWeight="bold">{data.label}</Text>
          {data.model && (
            <Badge size="sm" colorScheme="purple">{data.model}</Badge>
          )}
        </VStack>
      </CardBody>
    </Card>
...
const WorkflowBuilder: React.FC = () => {
  ...
  const { isOpen: isNodePanelOpen, onOpen: onNodePanelOpen, onClose: onNodePanelClose } = useDisclosure();
              ...
              {['Triggers', 'AI', 'Data', 'Communication', 'Logic', 'Human'].map(category => {
                ...
                return (
                  <VStack key={category} align="stretch" spacing={2}>
                    <Text fontWeight="bold" color="gray.700" fontSize="sm" textTransform="uppercase">
                      {category}
                    </Text>
