...
export const AgencyDashboard: React.FC = () => {
  ...
  const [recentAgencies, setRecentAgencies] = useState<any[]>(MOCK_TENANTS);
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      setStats({
        totalAgents: Array.isArray(agentsRes) ? agentsRes.length : 12,
        activeWorkflows: Array.isArray(workflowsRes) ? workflowsRes.length : 5,
        totalTasks: 1240, // Mock for now until task service is audited
        revenue: '$4,250.80' // Mock for now until revenue router is audited
      });
    } catch (e) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-5xl font-extrabold text-white tracking-tight">
            Agency <span className="bg-gradient-to-r from-blue-400 to-indigo-600 bg-clip-text text-transparent">Command</span>
          </h1>
          <p className="text-gray-400 mt-2 text-lg">Your sovereign multi-tenant orchestration layer is fully operational.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <MetricBox icon={<Bot className="text-blue-400" />} label="Provisioned Agents" value={stats.totalAgents.toString()} sub="+2 this week" />
        <MetricBox icon={<Zap className="text-amber-400" />} label="Active Workflows" value={stats.activeWorkflows.toString()} sub="Running stable" />
        <MetricBox icon={<Activity className="text-emerald-400" />} label="Total Tasks" value={stats.totalTasks.toLocaleString()} sub="99.9% Success" />
        <MetricBox icon={<Rocket className="text-rose-400" />} label="Ecosystem Revenue" value={stats.revenue} sub="Pending router sync" />
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            {recentAgencies.map((agency) => (
              <GlassCard key={agency.id} className="p-5 border-white/5 hover:border-blue-500/30 transition-all cursor-pointer group">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-8">
                    ...
                    <div className="hidden md:block text-right">
                        <div className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Revenue Split</div>
        ...
        <div className="space-y-6">
          ...
          <GlassCard className="p-6 space-y-4 bg-gradient-to-br from-emerald-600/5 to-transparent">
            <div className="flex items-center justify-between">
                ...
                <span className="px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 text-[10px] font-extrabold uppercase ring-1 ring-emerald-500/50">ACTIVE</span>
            </div>
            <div className="flex items-center justify-between">
                <span className="text-sm text-gray-300">Cross-Tenant Isolation</span>
                <span className="px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 text-[10px] font-extrabold uppercase ring-1 ring-emerald-500/50">LOCKED</span>
            </div>
            <div className="flex items-center justify-between">
                <span className="text-sm text-gray-300">Revenue Router Sync</span>
                <span className="px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-400 text-[10px] font-extrabold uppercase ring-1 ring-amber-500/50">PENDING</span>
            </div>
          ...
          <GlassCard className="p-6 border-indigo-500/20 relative overflow-hidden group">
            ...
            <p className="text-sm text-gray-400 mb-6">Provision new agents, workflows, and marketplace nodes for your sub-tenants.</p>
