...
export default function PlansPage() {
  ...
  const onCreate = async () => {
    if (!name.trim()) return;
    try {
      await createPlan({ name, objective, owner: 'ui-user' });
      setName('');
      setObjective('');
      await load();
  ...
  const onLinkPlanToGoal = async () => {
    ...
    try {
      await linkPlan(linkPlanId, { goalId: linkGoalId, actor: 'ui-user' });
      await load();
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Primary project plans linked to goals and records.</p>
        </div>
      ...
      <Card className="p-4 space-y-3">
        ...
        <Textarea value={objective} onChange={(e) => setObjective(e.target.value)} rows={3} placeholder="Plan objective" />
      ...
      <div className="space-y-3">
        {plans.map((p) => (
          <Card key={p.id} className="p-4">
            <Link to={`/plans/${p.id}`} className="font-semibold underline-offset-2 hover:underline">
              {p.name}
            </Link>
            <div className="text-sm text-muted-foreground">{p.objective}</div>
            <div className="text-xs text-muted-foreground mt-2">
              status: {p.status} | linked goals: {p.linkedGoalIds.length} | linked records: {p.linkedRecordIds.length}
            </div>
