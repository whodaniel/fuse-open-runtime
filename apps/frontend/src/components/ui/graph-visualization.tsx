    ...
    >
      ...
      <div style={{ padding: '20px', height: 'calc(100% - 100px)', overflow: 'hidden' }}>
        <svg
          width="100%"
          height="100%"
          style={{ border: '1px solid #e5e7eb', borderRadius: '4px', background: '#ffffff' }}
        >
          {/* Render edges */}
          ...
          {nodes.map((node) => (
            <g key={node.id}>
              ...
              <text x={node.x} y={node.y + 35} textAnchor="middle" fontSize="10" fill="#374151">
                {node.label}
              </text>
            </g>
          ))}
        ...
        {showMiniMap && (
          ...
          >
            <div style={{ padding: '8px', fontSize: '10px', fontWeight: '600', color: '#6b7280' }}>
              Mini Map
            </div>
          </div>
        )}
