...
export const AppStack_Card = ({ children, variant = 'default', className = '' }) => {
  ...
  const variantStyles = {
    ...
    gradient:
      'bg-gradient-to-br from-white to-gray-50 border border-gray-100 shadow-xl hover:shadow-2xl',
    hover: 'bg-white border hover:border-blue-500 shadow-lg hover:shadow-xl hover:scale-[1.02]',
  };
  return (
    <div className={`${baseStyles} ${variantStyles[variant]} ${className} p-6`}>{children}</div>
  );
};
