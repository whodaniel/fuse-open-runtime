          ...
          <GlassCard title="AI Assistant" className="w-80 h-96 flex flex-col">
            ...
            <div className="flex-grow overflow-auto">
              <div className="space-y-4">
                {messages.map((msg, index) => (
                  <div key={index} className={`p-2 rounded-lg ${msg.type === 'user' ? 'bg-blue-500 text-white self-end' : 'bg-gray-200 text-black self-start'}`}>
                    {msg.content}
                  </div>
                ))}
              </div>
            ...
            <div className="p-4 border-t flex">
              <PremiumInput
                ...
                className="flex-grow"
              />
              <PremiumButton onClick={handleSendMessage} className="ml-2">Send</PremiumButton>
            </div>
          </GlassCard>
        )}
      </div>
