...
export function AIAssistant() {
  ...
  return (
    <div className="fixed bottom-4 right-4">
      {!isOpen && (
        <button_1.Button onClick={() => setIsOpen(true)} className="rounded-full h-16 w-16">
          <lucide_react_1.Bot className="h-8 w-8" />
        </button_1.Button>
      )}
      {isOpen && (
        <card_1.Card className="w-80 h-96 flex flex-col">
          ...
          <div className="p-4 border-t flex">
            <input_1.Input
              ...
              className="flex-grow"
            />
            <button_1.Button onClick={handleSendMessage} className="ml-2">
              Send
            </button_1.Button>
          </div>
        </card_1.Card>
