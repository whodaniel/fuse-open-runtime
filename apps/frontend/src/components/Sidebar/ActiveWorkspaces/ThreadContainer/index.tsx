...
export default function ThreadContainer({ workspace }) {
  ...
  const activeThreadIdx = !!threads.find(
    (thread) => (thread === null || thread === void 0 ? void 0 : thread.slug) === threadSlug
  )
    ? threads.findIndex(
        (thread) => (thread === null || thread === void 0 ? void 0 : thread.slug) === threadSlug
      ) + 1
    : 0;
  return (
    <div className={STYLES.container} role="list" aria-label="Threads">
      <ThreadItem
        idx={0}
    ...
    <button onClick={onClick} className={STYLES.newThreadButton}>
      <div className={STYLES.buttonContent}>
        <div className={STYLES.iconContainer}>
          {loading ? (
            <CircleNotch weight="bold" size={14} className={`${STYLES.icon} animate-spin`} />
          ) : (
            <Plus weight="bold" size={14} className={STYLES.icon} />
          )}
        </div>

        <p className={STYLES.buttonText}>{loading ? 'Starting Thread...' : 'New Thread'}</p>
      </div>
    </button>
  );
}
