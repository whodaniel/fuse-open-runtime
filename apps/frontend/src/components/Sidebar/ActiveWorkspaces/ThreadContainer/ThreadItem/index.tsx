...
const STYLES = {
  ...
  downstroke: (idx, activeIdx, isActive) =>
    `${
  ...
  threadContent: (isActive) =>
    `flex w-full items-center justify-between pr-2 group relative ${
      isActive
  ...
  deleteButton:
    'hover:bg-red-500/20 text-slate-300 light:text-theme-text-primary hover:text-red-100',
};
    ...
    return (
      <div className={STYLES.container} role="listitem">
        <div
          ...
          className={STYLES.curvedLine(isActive)}
        />

        {hasNext && (
          <div
            style={{ width: THREAD_CALLOUT_DETAIL_WIDTH / 2 }}
            className={STYLES.downstroke(idx, activeIdx, isActive)}
          />
        )}
        ...
        <div className={STYLES.threadContent(isActive)}>
          {thread.deleted ? (
            ...
            >
              <p className={STYLES.threadName(isActive)}>{truncate(thread.name, 25)}</p>
            </a>
          )}
            ...
            <div ref={optionsContainer} className="flex items-center">
              {ctrlPressed ? (
                <button
                  ...
                  title="Mark thread for deletion"
                >
...
function OptionsMenu({ containerRef, workspace, thread, onRemove, close }) {
  ...
  const outsideClick = (e) => {
    ...
    if (
      !((_a = menuRef.current) === null || _a === void 0 ? void 0 : _a.contains(target)) &&
  ...
  useEffect(() => {
    function setListeners() {
      if (
        !(menuRef === null || menuRef === void 0 ? void 0 : menuRef.current) ||
  ...
  const renameThread = async () => {
    var _a;
    const name =
      (_a = window.prompt('What would you like to rename this thread to?')) === null ||
      _a === void 0
        ? void 0
        : _a.trim();
