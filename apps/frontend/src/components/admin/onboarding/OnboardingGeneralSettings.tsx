...
type GeneralSettings = {
  ...
  trackOnboardingAnalytics: boolean;
  collectFeedback: boolean;
};

const CustomSwitch: React.FC<CustomSwitchProps> = ({ id, isChecked, onChange, label }) => (
...
export const OnboardingGeneralSettings: React.FC<OnboardingGeneralSettingsProps> = ({
  ...
  const handleChange = (
    ...
    if (type === 'checkbox') {
      ...
      setSettings((prev) => ({ ...prev, [name]: checked }) as GeneralSettings);
    } else {
      setSettings((prev) => ({ ...prev, [name]: value }) as GeneralSettings);
    }
  ...
  const handleSwitchChange = (name: string) => (e: React.ChangeEvent<HTMLInputElement>) => {
    setSettings((prev) => ({ ...prev, [name]: e.target.checked }) as GeneralSettings);
    onChange();
  };
  ...
  const handleNumberChange = (name: string, value: string) => {
    ...
    setSettings((prev) => ({ ...prev, [name]: isNaN(num) ? 0 : num }) as GeneralSettings);
    onChange();
  };
