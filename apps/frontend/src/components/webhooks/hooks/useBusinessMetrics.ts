...
export function useBusinessMetrics(initialFilter: MetricsFilter = { timeRange: '24h' }) {
  ...
  const loadMetrics = useCallback(
    async (currentFilter?: MetricsFilter): Promise<void> => {
  ...
  const updateFilter = useCallback(
    (newFilter: Partial<MetricsFilter>) => {
  ...
  const getTopEventTypes = useCallback(
    (limit: number = 5): Array<{ type: BusinessEventType; count: number; percentage: number }> => {
  ...
  const getTopSources = useCallback(
    (
      limit: number = 5
    ): Array<{ source: IntegrationSource; count: number; percentage: number }> => {
      if (!state.metrics) return [];
