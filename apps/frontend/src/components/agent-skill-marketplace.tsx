  ...
  const [skills] = useState([
    {
      ...
      description: 'Advanced text processing and understanding',
      level: 1,
    },
    ...
    <div className="p-4">
      ...
      <div className="flex flex-col gap-2">
        {filteredSkills.map((skill) => (
          <div
            key={skill.id}
            className="p-4 border border-gray-200 rounded-md flex justify-between items-center bg-transparent shadow-none"
          >
            <div>
              <p className="font-bold text-gray-900">{skill.name}</p>
              <p className="text-sm text-muted-foreground">{skill.description}</p>
            </div>
            <Button variant="primary" size="sm" onClick={() => handleInstallSkill(skill.id)}>
              Install
            </Button>
          </div>
        ))}
