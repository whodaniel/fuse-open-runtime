  ...
  const handleRetryConnection = async () => {
    ...
    try {
      ...
      setErrorMessage(
        error instanceof Error 
          ? error.message 
          : 'Failed to reconnect to LLM service'
      );
      setLlmClient(null);
    }
  };
  ...
  if (connectionStatus === 'connecting') {
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement('p', { className: 'text-muted-foreground' }, 'Establishing connection with available LLM providers...'),
        React.createElement('div', { className: 'animate-spin rounded-full h-8 w-8 border-2 border-blue-500 border-t-transparent mt-4' })
      )
    );
  }
  ...
  if (connectionStatus === 'offline' || connectionStatus === 'error') {
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement('h3', { className: 'text-red-800 font-bold flex items-start' },
          React.createElement('svg', { className: 'flex-shrink-0 w-5 h-5 mt-0.5', xmlns: 'http://www.w3.org/2000/svg', fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor' }, 
      ...
      React.createElement(
        ...
        React.createElement('ul', { className: 'mt-2 list-disc list-inside text-sm' },
      ...
      React.createElement('div', { className: 'space-y-4' }, 
        messages.map((msg, index) => 
          React.createElement('div', 
            { key: index, className: `flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} max-w-[80%]` },
            React.createElement('div', 
              { 
                className: `max-w-xs px-4 py-2 rounded-lg ${msg.role === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-900'} 
        ...
        isLoading && React.createElement(
          ...
          React.createElement('div', { className: 'animate-spin rounded-full h-6 w-6 border-2 border-blue-500 border-t-transparent' })
        )
      ),
    ...
    React.createElement(
      ...
      React.createElement(
        'textarea',
        {
          ...
          className: 'flex-1 min-h-[60px] resize-none border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500',
          rows: 2,
