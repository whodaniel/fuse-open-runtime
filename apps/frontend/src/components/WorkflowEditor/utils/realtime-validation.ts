  ...
  validate(nodes, edges) {
    ...
    nodes.forEach((node) => {
      ...
      (nodeType.properties || []).forEach((prop) => {
        if (
          prop.required &&
          (node.data.parameters === undefined || node.data.parameters[prop.name] === undefined)
        ) {
          errors.push(
            `Node '${node.data.name || node.type}' (ID: ${node.id}) is missing required parameter: ${prop.displayName}`
          );
        }
      });
      if (
        ...
        errors.push(
          `Node '${node.data.name || node.type}' (ID: ${node.id}) is missing credentials.`
        );
      }
    });
    return errors;
