...
const PromptNode: React.FC<NodeProps> = memo(({ id, data }) => {
  ...
  const renderContent = () => (
    <div className="space-y-3">
      <div>
        <Label htmlFor={`template-${id}`} className="text-xs">Template</Label>
        <Select
          value={data.config?.templateId || ''}
          onValueChange={handleTemplateChange}
        >
          <SelectTrigger id={`template-${id}`} className="text-xs h-8 mt-1">
      ...
      {selectedTemplate?.id === 'custom' && (
        <div>
          <Label htmlFor={`custom-prompt-${id}`} className="text-xs">Custom Prompt</Label>
          <Textarea
            id={`custom-prompt-${id}`}
            value={customPrompt}
