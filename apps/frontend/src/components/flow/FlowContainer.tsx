...
export const FlowContainer = ({ initialNodes = [], initialEdges = [], onSave }) => {
  ...
  const { saveNodeMemory, loadNodeMemory, updateFlowMemoryGraph } = (0,
  useFlowMemory_1.useFlowMemory)(memoryManager);
  ...
  const handleNodeUpdate = (0, react_1.useCallback)(
    async (nodeId, updates) => {
      const updatedNodes = nodes.map((node) =>
        node.id === nodeId ? Object.assign(Object.assign({}, node), updates) : node
      );
  ...
  const handleNodeSelect = (0, react_1.useCallback)(
    async (nodeId) => {
    ...
    <div className="h-screen flex">
      ...
      <div className="flex-1">
        <react_router_dom_1.Routes>
          ...
          <react_router_dom_1.Route
            path="/node/:nodeId"
            element={<NodeDetails_1.NodeDetails nodes={nodes} onNodeUpdate={handleNodeUpdate} />}
          />
        </react_router_dom_1.Routes>
      </div>
    </div>
