...
analyze_phase() {
    ...
    if bun list | grep -q madge; then
        echo "Generating dependency graph..."
        bun madge --typescript --exclude '^node_modules' --image dependency-graph.svg ./packages || echo "Warning: Could not generate dependency graph"
    else
