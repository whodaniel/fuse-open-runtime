...
class TNFAgentRelay {
  ...
  async discoverAgents() {
    ...
    try {
      const stdout = execSync('ps aux | grep -E "(applescript-mcp|browsermcp)" | grep -v grep', {
        encoding: 'utf8',
      });
    ...
    const configs = [
      ...
      {
        path: `${homeDir}/Library/Application Support/Code/User/mcp_config.json`,
        type: 'VSCODE_MCP',
      },
      {
        path: `${homeDir}/Library/Application Support/GitHub Copilot/mcp_config.json`,
        type: 'COPILOT_MCP',
      },
    ];
    ...
    try {
      switch (command) {
        ...
        default:
          ...
          console.log('');
          console.log('Usage:');
          console.log('  node cli-fixed.js discover  # Find available agents');
          console.log('  node cli-fixed.js test      # Send test message');
          console.log('  node cli-fixed.js status    # Show system status');
          console.log('');
      }
    } catch (error) {
