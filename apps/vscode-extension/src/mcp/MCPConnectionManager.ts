...
import { CircuitBreaker, MCPServerConfig, MCPServerStatus, MCPServerStatusSummary } from '../types';
...
export class MCPConnectionManager {
  ...
  private connectionPool = new Map<
    string,
    { client: Client; config: MCPServerConfig; connectedAt: number }
  >();
  private toolRegistry = new Map<string, string>();
  private resourceRegistry = new Map<string, string>();
  private promptRegistry = new Map<string, string>();

  // Health monitoring
  private healthChecks = new Map<
    string,
    { status: 'healthy' | 'unhealthy' | 'unknown'; lastCheck: number; responseTime?: number }
  >();
   ...
   */
  async retryConnection(serverConfig: MCPServerConfig, attempt = 1): Promise<string> {
