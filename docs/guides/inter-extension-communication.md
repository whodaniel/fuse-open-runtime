...
This document describes the technical implementation of inter-extension
communication protocols used by The New Fuse to enable different AI extensions
to communicate with each other.

## Overview

The New Fuse implements multiple communication protocols to enable AI extensions
to exchange messages, collaborate on tasks, and coordinate their actions. Each
protocol has different advantages and use cases.
...
- **Capabilities**: Describes what the agent can do (e.g., `code-generation`,
  `text-explanation`)
...
The simplest protocol, using VS Code's workspace state storage for message
passing.
...
```typescript
// Send a message
const messages = context.workspaceState.get<AgentMessage[]>(
  'thefuse.messages',
  []
);
...
const myMessages = messages.filter(
  (m) => m.recipient === myAgentId || m.recipient === '*'
);
// Process messages...
```
...
- Messages are written to JSON files in a designated folder (e.g.,
  `ai-communication/`)
...
const filePath = path.join(
  communicationDir,
  `${Date.now()}-${messageId.substring(0, 8)}.json`
);
...
- Adds a layer on top of other protocols (typically File Protocol or Workspace
  State)
...
New protocols can be added by implementing the core `MessageTransport`
interface:
...
await agentClient.sendMessage('other-extension.id', 'doSomething', {
  data: 'value',
});
...
const communicator = createFileProtocolCommunicator(
  context,
  agentClient,
  outputChannel
);
...
// Send a message
await communicator.sendMessage('other-extension.id', 'processData', {
  data: 'large-payload',
});
```
...
const mcpManager = createMCPProtocolManager(
  context,
  agentClient,
  outputChannel
);
