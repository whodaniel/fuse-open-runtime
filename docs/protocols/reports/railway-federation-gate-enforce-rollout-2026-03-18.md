# Railway Federation Gate Enforce Rollout Report
...
Promote production Railway gate policy from `warn` to `enforce` using phased
canary:
...
1. Relay canary:
   - command:
     `APPLY_API=0 APPLY_RELAY=1 bash scripts/railway/set-federation-gate-mode.sh enforce`
   - deployment: `15627f69-c8ef-422c-a826-1db456bd5677`
   - result: `SUCCESS`
2. API promotion:
   - command:
     `APPLY_API=1 APPLY_RELAY=0 bash scripts/railway/set-federation-gate-mode.sh enforce`
