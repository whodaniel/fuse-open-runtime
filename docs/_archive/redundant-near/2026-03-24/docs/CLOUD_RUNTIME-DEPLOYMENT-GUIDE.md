# CloudRuntime Deployment Guide - The New Fuse Monorepo
...
- **CRITICAL**: Set the "Root Directory" in the CloudRuntime UI to the repository root (`.`).
- **CRITICAL**: Set the "Dockerfile Path" to `./Dockerfile.cloud_runtime`.
- This ensures CloudRuntime can find the Dockerfile and has access to the entire monorepo to resolve workspace dependencies.

---

## 📁 Standardized Dockerfile Configuration

A single, multi-stage `Dockerfile.cloud_runtime` in the root of the repository is used to build and deploy all services.

### `Dockerfile.cloud_runtime`
```dockerfile
...
- **Healthcheck**: Includes a standardized healthcheck for CloudRuntime to monitor the service.
...
#### 1. **Prepare CloudRuntime Service in UI**
1. Go to the CloudRuntime Dashboard.
...
3. Set the **Dockerfile Path** to `./Dockerfile.cloud_runtime`.
...
3. Set the value to the path of the service you want to deploy (e.g., `api`, `backend`, `frontend`). This variable is used by the `Dockerfile.cloud_runtime` to build the correct service.

#### 4. **Set Environment Variables in CloudRuntime**
...
2. Monitor the build logs in the CloudRuntime dashboard.
...
**Solution:**
- Ensure that all necessary `package.json` files are copied in the `base` stage of the `Dockerfile.cloud_runtime`.

### Issue 2: Workspace Dependencies Not Found at Runtime
**Solution:**
- Verify that the `COPY --from=build /app/packages/*/dist ./packages/` command in the `Dockerfile.cloud_runtime` is correctly copying the built packages.
...
- Make sure the `PORT` environment variable is being used by the application, as CloudRuntime sets this automatically.
...
- [CloudRuntime Dockerfile deployments](https://docs.thenewfuse.com/deploy/dockerfiles)
