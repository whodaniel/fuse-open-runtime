# CloudRuntime Clean Deployment Plan
...
8. **frontend**
   - Path: `apps/frontend`
   - Port: $PORT (CloudRuntime provides this)
...
The following services are NOT deployed to CloudRuntime:
...
## 📦 Required CloudRuntime Resources
...
1. **PostgreSQL**
   ...
   - Create with: CloudRuntime Dashboard → New → Database → PostgreSQL

2. **Redis**
   - For: relay-core, backend services
   - Create with: CloudRuntime Dashboard → New → Database → Redis
...
# Service URLs (internal CloudRuntime URLs)
VECTOR_DB_URL=https://core-vector-db.cloud_runtime.internal:50051
RELAY_CORE_URL=https://relay-core.cloud_runtime.internal
...
### Step 1: Ensure All Services Exist in CloudRuntime

Before running the deployment script, verify all 8 services exist in the CloudRuntime dashboard:

1. Go to: https://cloud_runtime.app/project/041cee9d-8648-4074-b5a6-0eae436de1d1
...
```bash
cd .
chmod +x cloud_runtime-clean-deploy.sh
./cloud_runtime-clean-deploy.sh
```

### Step 4: Configure Environment Variables

For each service, add the required environment variables via:
- CloudRuntime Dashboard → Service → Variables

Or use the CLI:
```bash
cloud_runtime service core-vector-db
...
- Configuration: `cloud_runtime.toml` specifies `builder = "DOCKERFILE"`
- File: `Dockerfile` in service directory
- Process: CloudRuntime builds the Docker image and runs it

### Nixpacks-based (Some services)
- Configuration: `nixpacks.toml` in service directory
- Process: CloudRuntime auto-detects and builds with Nixpacks
...
- Check build logs in CloudRuntime dashboard
...
- Check that PORT is used correctly (CloudRuntime provides $PORT)
...
- Verify network connectivity (CloudRuntime provides internal URLs)
...
- **CloudRuntime CLI Docs:** https://docs.cloud_runtime.com/develop/cli

---

## 📞 Support

For CloudRuntime-specific issues:
- Discord: https://discord.gg/cloud_runtime
- Docs: https://docs.cloud_runtime.com

For TNF project issues:
- Check service logs: `cloud_runtime logs --service <name>`
