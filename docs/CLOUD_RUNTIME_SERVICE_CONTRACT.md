...
flow, and required environment-variable wiring using CloudRuntime service references.
...
- `api-gateway`:
  - `API_URL=${{api.CLOUD_RUNTIME_STATIC_URL}}`
  - `BACKEND_URL=${{backend.CLOUD_RUNTIME_STATIC_URL}}`
  - `API_GATEWAY_URL=${{api-gateway.CLOUD_RUNTIME_STATIC_URL}}`

Frontend/API routing:

- `TheNewFuse`:
  - `VITE_API_URL=${{api-gateway.CLOUD_RUNTIME_STATIC_URL}}`
  - `VITE_API_GATEWAY_URL=${{api-gateway.CLOUD_RUNTIME_STATIC_URL}}`
  - `VITE_WS_URL=${{relay-server.CLOUD_RUNTIME_STATIC_URL}}`
- `Frontend Application`:
  - `VITE_API_URL=${{api-gateway.CLOUD_RUNTIME_STATIC_URL}}`
  - `VITE_API_GATEWAY_URL=${{api-gateway.CLOUD_RUNTIME_STATIC_URL}}`
  - `VITE_WS_URL=${{relay-server.CLOUD_RUNTIME_STATIC_URL}}`
- `ai-arcade`:
  - `VITE_API_URL=${{api-gateway.CLOUD_RUNTIME_STATIC_URL}}`
  - `VITE_RELAY_URL=${{relay-server.CLOUD_RUNTIME_STATIC_URL}}`
  - `VITE_WS_URL=${{relay-server.CLOUD_RUNTIME_STATIC_URL}}`

## Domain Hardening Checklist (UI)

CloudRuntime CLI supports domain creation, but domain removal/toggling is managed in
CloudRuntime UI.

For each internal-only service:

1. Open service `Settings -> Networking`.
2. Remove CloudRuntime public domain if present.
