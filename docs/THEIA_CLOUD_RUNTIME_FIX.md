...
# SkIDEancer IDE CloudRuntime Deployment - Diagnosis & Fix
...
- ⚠️ Port mismatch: CloudRuntime expects 3000, SkIDEancer listens on 3007

---

## 🛠️ SOLUTION

### Option 1: Update CloudRuntime Port (Recommended)

Change CloudRuntime's public networking port from 3000 to 3007:

1. Go to CloudRuntime project:
...
**In CloudRuntime Service Settings**:
...
### Option 3: Use CloudRuntime's $PORT Variable (Best Practice)

Let CloudRuntime dynamically assign the port:
...
Ensure these are set in CloudRuntime:

```bash
NODE_ENV=production
THEIA_WEBVIEW_EXTERNAL_ENDPOINT={{CLOUD_RUNTIME_STATIC_URL}}
...
2. Redeploy service
3. Let CloudRuntime auto-configure the port

---

## 📊 PORT CONFIGURATION REFERENCE

| Service                 | Default Port | CloudRuntime Port | Status       |
| ----------------------- | ------------ | ------------ | ------------ |
...
The CloudRuntime dashboard shows "Online" because:
...
2. ❌ CloudRuntime routes traffic to port 3000
...
1. **Update CloudRuntime port configuration** (Option 1 above)
2. **Test the subdomain**: https://ide.thenewfuse.com
3. **If still broken**: Check deployment logs in CloudRuntime dashboard
