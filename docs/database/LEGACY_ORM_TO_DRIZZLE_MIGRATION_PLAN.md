...
Drizzle ORM. The migration is currently **40% complete** with all infrastructure
in place but runtime code still using Drizzle.
...
- apps/api uses DatabaseService via DatabaseModule
- packages/core uses DatabaseService for metrics/monitoring
...
1. **src/drizzle/drizzle.service.ts** → Create DrizzleService wrapper
   - Current: `DatabaseService` extends `DrizzleClient`
...
1. **src/modules/drizzle/drizzle.service.ts** → Deprecated, create
   drizzle.service.ts
...
**Document Status**: Active **Last Updated**: December 29, 2024 **Next Review**:
