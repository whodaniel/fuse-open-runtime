...
- Script exists but testing with Docker would require Docker daemon to be
  running
...
The port management system has been successfully implemented and is working as
expected. The ES Module issue has been fixed by updating the script to use
proper ES Module syntax (`import` instead of `require`).
