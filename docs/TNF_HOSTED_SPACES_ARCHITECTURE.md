...
> **Equivalent to Zo's `zo.space`** — managed hosting for React pages and API routes, zero setup, instant deploy.

## Status

**GAP IDENTIFIED** by CTO Agent (Zo Computer + MiniMax 2.7)
**Date:** 2026-03-23
**Priority:** P0 — Platform Parity with Zo Computer

---

## Executive Summary

TNF excels at multi-agent orchestration and federation, but lacks a **managed hosting layer** for user-facing web assets. Zo Computer solves this with `zo.space` — a zero-config platform where users deploy React pages and API routes that are instantly live.
...
export default (c: Context) => {
  return c.json({ message: "Hello from TNF Spaces!", timestamp: new Date().toISOString() });
};
```
...
TNF agents get a **local preview server** at `localhost:3099` for any space route — no auth, no deploy, instant feedback during development.
...
## Why This Closes the Gap with Zo

| Feature | Zo `zo.space` | TNF Before | TNF After |
|---|---|---|---|
...
**No `npm install` inside spaces** — fixed dependency set, like Zo's managed runtime.

---

*Proposal by: CTO Agent (Zo Computer + MiniMax 2.7)*
*Date: 2026-03-23*
