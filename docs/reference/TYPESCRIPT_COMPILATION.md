...
- **build**: `pnpm run clean:js && turbo run build` - Clean up and build all packages
- **build:ts**: `pnpm run clean:js && tsc -b` - Clean up and build TypeScript files
...
5. **Fix template literals in JSX/TSX files**: Use the `pnpm run fix:template-literals` script to fix template literals in JSX/TSX files
6. **Use incremental builds**: Use the `pnpm run build:ts:incremental` script to build TypeScript files incrementally
7. **Type check without compilation**: Use the `pnpm run type-check` script to type check the project without compilation
