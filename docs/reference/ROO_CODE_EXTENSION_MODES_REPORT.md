...
This document details how each Roo Code user‑selectable mode (“Orchestrator”,
“Debug”, “Ask”, “Architect”, “Code”) is implemented, and provides instructions
for achieving full parity in The New Fuse VSCode extension.
...
Define context key `thefuse.currentMode` via
`commands.executeCommand('setContext', 'thefuse.currentMode', mode)`.  
...
- Instantiate `LLMProviderManager`, `MonitoredLLMProviderManager`,
...
context.subscriptions.push(
  commands.registerCommand('thefuse.mode.orchestrator', () =>
...
Function `setMode(mode: string)` updates `globalState` and executes
`setContext`.
...
- Delegate to handlers:
  ...
  switch (currentMode) {
    case 'orchestrator':
      OrchestratorHandler.handle(cmd);
      break;
    case 'debug':
      DebugHandler.handle(cmd);
      break;
    case 'ask':
      AskHandler.handle(cmd);
      break;
    case 'architect':
      ArchitectHandler.handle(cmd);
      break;
    case 'code':
      CodeHandler.handle(cmd);
      break;
  }
  ```
...
Register each view via `window.registerWebviewViewProvider(viewType, provider)`
in `activate()`.

## 8. Implementing Parity in “The New Fuse”

1. Copy the above `commands`, `menus`, and `views` sections into
   `packages/vscode-extension/package.json`.
2. Create `src/command-monitor.ts` with `setMode` logic and
   `onDidExecuteCommand` routing.
3. Add mode handlers under `src/modes/*Handler.ts`.
4. Register the mode commands and status bar in `src/extension.ts`.
5. Define view providers under `src/views/*ViewProvider.ts` and register in
   `activate`.
6. Use `commands.executeCommand('setContext','thefuse.currentMode',mode)` for
   context switching.
7. Add `when` clauses on UI contributions to bind visibility to
   `thefuse.currentMode`.

> This document ensures complete parity for Roo Code modes in The New Fuse
> VSCode extension.
