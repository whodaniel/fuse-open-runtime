...
The New Fuse VS Code extension is an advanced AI assistant with a Copilot-like
chat interface for coding, workflow automation, and agent coordination. It
provides a seamless experience for developers to work with multiple AI agents,
coordinate their activities, and create complex workflows.

## Features

### Multiple Operational Modes

The New Fuse offers five specialized operational modes, each tailored to
different development needs:

- **Orchestrator Mode**: Coordinates multiple AI agents working together on
  complex tasks
- **Debug Mode**: Provides detailed inspection of agent-to-agent message traffic
  and tool execution
- **Ask Mode**: Offers a simplified interface for direct queries to specific
  agents
...
The VS Code extension is built on a modular architecture that enables seamless
communication between different AI agents and tools:
...
All commands can be accessed via the Command Palette (Cmd+Shift+P or
Ctrl+Shift+P):
...
The Workspace State Protocol enables communication between extensions in the
same VS Code window:
...
The VS Code extension integrates with MCP to provide a seamless experience for
developers:
...
- **Web Interaction Tools**: Search the web, fetch content, and interact with
  web services
- **Code Analysis Tools**: Analyze code, retrieve information from the codebase,
  and get diagnostics
- **Integration Tools**: Connect with GitHub, Linear, Jira, Notion, and other
  services
...
function registerMCPIntegration(
  ...
  const agentIntegration = new MCPAgentIntegration(
    context,
    mcpManager,
    lmBridge
  );
  ...
  const componentAnalysisTool = registerComponentAnalysisTool(
    context,
    mcpManager
  );
...
The `utils` directory contains utility functions that are used throughout the
extension:
...
**Note:** Some utilities previously implemented as separate files (such as
fs-utils, webview-utils, uri-utils, and vscode-utils) have been refactored and
their functionality has been integrated directly into the relevant service and
component classes.

## Migration Guide for Refactored Project Structure

As of May 15, 2025, the VS Code extension project structure has been refactored
to improve organization and maintainability. This section provides guidance for
developers adapting to these changes.

### Key Changes

1. **Utility Functions Consolidation**:
   - File system operations (previously in `fs-utils.ts/js`) now use Node's
     `fs/promises` directly
   - WebView utilities (previously in `webview-utils.ts/js`) integrated into
     individual view providers
   - URI handling (previously in `uri-utils.ts/js`) now uses VS Code's
     `vscode.Uri` methods directly
   - VS Code-specific utilities (previously in `vscode-utils.ts/js`) integrated
...
1. **Command Consolidation**: Always use the CommandRegistry to register
   commands
2. **Error Handling**: Use the wrapAsyncCommand utility to handle errors
   properly
3. **Status Bar Integration**: Update status bar items to reflect the current
   state
...
1. **Command Not Found**: Ensure the command is registered in the
   CommandRegistry
...
2. **Check Output Panel**: Open the Output panel and select "The New Fuse" from
   the dropdown
3. **Inspect WebSocket Traffic**: Use the Network tab in Developer Tools
4. **Check Error Logs**: Look for error messages in the VS Code Developer
   Console
...
3. **Improved Collaborative Completion**: Enhance the collaborative completion
   feature
...
7. **UI Improvements**: Enhance the user interface with more interactive
   elements
...
10. **Anthropic XML Integration**: Enhance support for Anthropic's XML function
    call format
