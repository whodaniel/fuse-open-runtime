...
###############################################################################
# The New Fuse - CloudRuntime Deployment Utility
# Advanced CloudRuntime deployment with environment management and monitoring
###############################################################################
...
print_banner() {
  echo ""
  echo -e "${BOLD}${CYAN}╔════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${BOLD}${CYAN}║${NC}  ${BOLD}CloudRuntime Deployment Utility${NC}                                 ${BOLD}${CYAN}║${NC}"
  echo -e "${BOLD}${CYAN}╚════════════════════════════════════════════════════════════════╝${NC}"
  echo ""
}
...
check_cloud_runtime_cli() {
  print_section "Checking CloudRuntime CLI"

  if ! command -v cloud_runtime &>/dev/null; then
    log ERROR "CloudRuntime CLI is not installed"
    log INFO "Install with: npm install -g @cloud_runtime/cli"
    exit 1
  fi

  log SUCCESS "CloudRuntime CLI found"

  local version=$(cloud_runtime --version 2>&1 || echo "unknown")
...
check_authentication() {
  ...
  if ! cloud_runtime whoami &>/dev/null; then
    log ERROR "Not authenticated with CloudRuntime"
    log INFO "Run: cloud_runtime login"
    exit 1
  fi
...
check_project_link() {
  ...
  if ! cloud_runtime status &>/dev/null; then
    log ERROR "Not linked to a CloudRuntime project"
    log INFO "Run: cloud_runtime link"
    exit 1
  fi
  ...
  cloud_runtime status 2>&1 | head -10
}
...
sync_environment_variables() {
  ...
  # Read .env file and set variables in CloudRuntime
  while IFS='=' read -r key value; do
    ...
    # Skip CloudRuntime internal references (they're already set)
    if [[ "$value" =~ ^\$\{\{.*\}\}$ ]]; then
      log INFO "Skipping CloudRuntime internal reference: $key"
      continue
    fi

    log INFO "Setting $key..."
    cloud_runtime variables --set "$key=$value" &>/dev/null || {
...
list_services() {
  ...
  for service_dir in "$services_dir"/*; do
    if [[ -d "$service_dir" ]]; then
      ...
      # Check if service has Dockerfile or cloud_runtime.toml
      if [[ -f "$service_dir/Dockerfile" ]] || [[ -f "$service_dir/cloud_runtime.toml" ]]; then
        echo "  - $service_name"
...
deploy_service() {
  ...
  if [[ ! -f "Dockerfile" ]] && [[ ! -f "Dockerfile.cloud_runtime" ]]; then
    log WARNING "No Dockerfile found for $service"
  fi

  # Deploy using CloudRuntime
  log STEP "Triggering deployment..."

  if [[ "$WAIT_FOR_DEPLOY" == "true" ]]; then
    cloud_runtime up --service "$service" || {
      log ERROR "Deployment failed for $service"
      cd "$PROJECT_ROOT"
      return 1
    }
  else
    cloud_runtime up --detach --service "$service" || {
      log ERROR "Deployment failed for $service"
      cd "$PROJECT_ROOT"
      return 1
    }
...
get_service_url() {
  ...
  local domain=$(cloud_runtime domain --service "$service" 2>&1 || echo "")
...
check_service_health() {
  ...
  if cloud_runtime status --service "$service" &>/dev/null; then
    ...
    # Get service info
    cloud_runtime status --service "$service" 2>&1 | head -10
...
promote_to_production() {
  ...
  # CloudRuntime environment promotion
  cloud_runtime environment --promote || {
...
${BOLD}Commands:${NC}
  deploy [service]    Deploy service(s) to CloudRuntime
