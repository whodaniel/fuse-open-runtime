...
RELAY_SERVICE="${RAILWAY_RELAY_SERVICE:-relay-server}"
...
usage() {
  cat <<'USAGE'
Usage:
  bash scripts/railway/set-federation-gate-mode.sh [off|warn|enforce]

Environment overrides:
  RAILWAY_ENVIRONMENT      Railway environment (default: production)
  RAILWAY_API_SERVICE      API service name (default: api)
  RAILWAY_RELAY_SERVICE    Relay service name (default: relay-server)
...
if [[ "${APPLY_API}" == "1" ]]; then
  railway variable set -s "${API_SERVICE}" -e "${ENVIRONMENT}" "${api_vars[@]}"
fi
if [[ "${APPLY_RELAY}" == "1" ]]; then
  railway variable set -s "${RELAY_SERVICE}" -e "${ENVIRONMENT}" "${relay_vars[@]}"
fi

echo
echo "Verifying variables (non-secret fields):"
if [[ "${APPLY_API}" == "1" ]]; then
  railway variable list -s "${API_SERVICE}" -e "${ENVIRONMENT}" --kv \
    | awk -F= '/^(TNF_GATE_POLICY_MODE|TNF_GATE_POLICY_ENDPOINT)=/{print "api " $1 "=" $2}'
fi
if [[ "${APPLY_RELAY}" == "1" ]]; then
  railway variable list -s "${RELAY_SERVICE}" -e "${ENVIRONMENT}" --kv \
...
while true; do
  status_json="$(railway service status -a --json)"
