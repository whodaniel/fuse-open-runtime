...
RUNNER_SERVICE="${CLOUD_RUNTIME_RUNNER_SERVICE:-}"
RUNNER_SERVICES_CSV="${CLOUD_RUNTIME_RUNNER_SERVICES:-api3,api,backend}"

echo "🚄 [TNF CloudRuntime Run] Executing supercycle remotely..."
...
for svc in "${CANDIDATES[@]}"; do
  ...
  RUNNER_PUBLIC_DOMAIN="$(
    cloud_runtime variable list -s "$svc" -e "$ENVIRONMENT" -k 2>/dev/null \
      | sed -n 's/^CLOUD_RUNTIME_PUBLIC_DOMAIN=//p' \
      | head -n 1
  )"
  ...
  for attempt in 1; do
    if cloud_runtime ssh -s "$svc" -e "$ENVIRONMENT" "$REMOTE_CMD"; then
...
echo "   Falling back to local execution with CloudRuntime public endpoints..."

REDIS_PUBLIC_URL=""
if [ -x "$REDIS_RESOLVER" ]; then
  REDIS_PUBLIC_URL="$(CLOUD_RUNTIME_ENVIRONMENT_NAME="$ENVIRONMENT" "$REDIS_RESOLVER" 2>/dev/null || true)"
fi
if [ -z "$REDIS_PUBLIC_URL" ]; then
  REDIS_PUBLIC_URL="$(
    cloud_runtime variable list -s Redis -e "$ENVIRONMENT" -k 2>/dev/null \
...
SEARXNG_BASE_URL="$(
  cloud_runtime variable list -s "${CANDIDATES[0]}" -e "$ENVIRONMENT" -k 2>/dev/null \
...
if [ -z "$SEARXNG_BASE_URL" ]; then
  SEARXNG_DOMAIN="$(
    cloud_runtime variable list -s Redis -e "$ENVIRONMENT" -k 2>/dev/null \
      | sed -n 's/^CLOUD_RUNTIME_SERVICE_SEARXNG_URL=//p' \
      | head -n 1
  )"
...
if [ -z "$REDIS_PUBLIC_URL" ]; then
  echo "❌ No production Redis URL resolved for CloudRuntime fallback."
  exit 1
fi

if [ -z "$SEARXNG_BASE_URL" ]; then
  echo "❌ SEARXNG_BASE_URL could not be resolved from CloudRuntime variables."
  exit 1
fi

echo "🌐 Local fallback with CloudRuntime endpoints:"
