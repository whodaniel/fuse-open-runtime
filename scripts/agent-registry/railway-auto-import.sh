...
if [ -z "$API_BASE" ]; then
  VARS="$(railway variable list -s "$SERVICE" -e "$ENVIRONMENT" -k 2>/dev/null || true)"
...
if [ -z "$API_BASE" ]; then
  API_BASE="$(railway domain -s "$SERVICE" -e "$ENVIRONMENT" 2>/dev/null | rg -o 'https?://[^[:space:]]+' -m 1 || true)"
fi

if [ -z "${AGENT_REGISTRY_IMPORT_TOKEN:-}" ]; then
  VARS_FOR_TOKEN="$(railway variable list -s "$SERVICE" -e "$ENVIRONMENT" -k 2>/dev/null || true)"
