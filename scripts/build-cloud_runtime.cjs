 ...
 *
 * Builds only what's needed for CloudRuntime deployment:
...
class CloudRuntimeBuilder {
  ...
  async build() {
    try {
      console.log(`\n${colors.bright}${colors.cyan}CloudRuntime Production Build${colors.reset}\n`);

      this.log('Starting CloudRuntime-optimized build...', 'info');
...
// Run the build
const builder = new CloudRuntimeBuilder();
builder.build();
