...
function usage() {
  console.log(`Usage:
  node scripts/cloud_runtime/spawn-openclaw-logical-instance.mjs [options]

Options:
  --service NAME        CloudRuntime service (default: openclaw-sandbox-cloud)
...
function getCloudRuntimeVars(service) {
  const raw = run(`cloud_runtime variable list --service ${service} --json`);
  ...
  if (!domain) throw new Error(`CLOUD_RUNTIME_PUBLIC_DOMAIN missing on service: ${service}`);
  return { token, domain };
}
...
async function main() {
  ...
  const { token, domain } = getCloudRuntimeVars(cfg.service);
