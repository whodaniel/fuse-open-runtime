...
function usage() {
  console.log(`Usage:
  node scripts/cloud_runtime/launch-openclaw-qa-swarm.mjs [options]

Options:
  --services CSV            CloudRuntime services (default: openclaw-cloud,openclaw-primary,openclaw-sandbox-cloud)
...
function getServiceVars(service) {
  const raw = sh(`cloud_runtime variable list --service ${service} --json`);
  const vars = JSON.parse(raw);
  const token = String(vars.OPENCLAW_GATEWAY_TOKEN || '').trim();
  const domain = String(vars.CLOUD_RUNTIME_PUBLIC_DOMAIN || '').trim();
  if (!token) throw new Error(`[${service}] OPENCLAW_GATEWAY_TOKEN missing`);
  if (!domain) throw new Error(`[${service}] CLOUD_RUNTIME_PUBLIC_DOMAIN missing`);
  const urls = [];
  for (const [k, v] of Object.entries(vars)) {
    if (typeof v !== 'string') continue;
    if ((k.startsWith('CLOUD_RUNTIME_SERVICE_') && k.endsWith('_URL')) || k === 'CLOUD_RUNTIME_STATIC_URL') {
...
function buildMission({ service, sessionKey, urls }) {
  ...
  const urlBlock = scoped.length
    ...
    : '1. https://openclaw-cloud-production-934c.thenewfuse.com';
