...
# Deploy remaining package services to CloudRuntime
# This script creates the services first, then deploys them

set -e

echo "🚀 Deploying remaining package services to CloudRuntime..."
echo ""
...
for service_entry in "${services[@]}"; do
    ...
    if cloud_runtime service create "$service_name" 2>&1 | grep -q "Service created"; then
