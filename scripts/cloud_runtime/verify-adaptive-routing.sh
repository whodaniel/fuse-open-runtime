...
for target in "${targets[@]}"; do
  ...
  for url in "${urls[@]}"; do
    for attempt in $(seq 1 "$VERIFY_MAX_ATTEMPTS"); do
      ...
      cloud_runtime_req_id="$(awk 'BEGIN{IGNORECASE=1} /^x-cloud_runtime-request-id:/ {sub(/^x-cloud_runtime-request-id:[[:space:]]*/, ""); gsub(/\r/, ""); print; exit}' "${header_file}")"
      ...
      fail_detail="url=${url} attempt=${attempt}/${VERIFY_MAX_ATTEMPTS} curl_exit=${curl_exit} http=${http_code:-n/a} x-request-id=${req_id:-n/a} x-cloud_runtime-request-id=${cloud_runtime_req_id:-n/a} curl_error=${err_snippet:-n/a} body=${body_snippet:-n/a}"
      rm -f "${header_file}" "${body_file}"
