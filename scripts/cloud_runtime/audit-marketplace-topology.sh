...
# Audits CloudRuntime marketplace-related topology across TNF and MCP-DRS projects.
# Requires: cloud_runtime CLI, jq
...
CLOUD_RUNTIME_ENV="${CLOUD_RUNTIME_ENV:-production}"
...
capture_project_status() {
  ...
  cloud_runtime link -p "$project_id" >/dev/null
  cloud_runtime status --json >"$out_json"
}

list_services() {
  local json_file="$1"
  jq -r --arg env "$CLOUD_RUNTIME_ENV" '
    .environments.edges[]
...
service_meta() {
  ...
  jq -r --arg env "$CLOUD_RUNTIME_ENV" --arg svc "$service_name" '
    .environments.edges[]
    | select(.node.name == $env)
...
echo
echo "=== TNF services ($CLOUD_RUNTIME_ENV) ==="
list_services "$TNF_JSON"

echo
echo "=== MCP-DRS services ($CLOUD_RUNTIME_ENV) ==="
