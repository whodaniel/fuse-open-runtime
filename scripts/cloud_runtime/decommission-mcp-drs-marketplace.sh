...
cloud_runtime link -p "$MCP_DRS_PROJECT_ID" -e "$MCP_DRS_ENV" >/dev/null

for svc in "${SERVICES[@]}"; do
  echo "Removing latest deployment for: $svc"
  cloud_runtime down --service "$svc" --environment "$MCP_DRS_ENV" -y || true
done

echo
echo "Current status for MCP-DRS services:"
cloud_runtime service status --all
