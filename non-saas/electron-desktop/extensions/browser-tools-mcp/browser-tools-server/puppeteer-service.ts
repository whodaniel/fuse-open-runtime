...
async function getHeadlessBrowserInstance(): Promise<Browser> {
  logger.debug("Browser instance request started");
...
function createTempUserDataDir(): string {
  ...
  logger.debug(`Using temporary user data directory: ${userDataDir}`);
  return userDataDir;
}
...
async function setCustomBrowserExecutable(launchOptions: any): Promise<void> {
  ...
  try {
    if (detectedBrowserPath && fs.existsSync(detectedBrowserPath)) {
      logger.debug(`Using cached browser path: ${detectedBrowserPath}`);
      ...
      if (detectedBrowserPath.includes("firefox")) {
        launchOptions.product = "firefox";
        logger.debug("Setting product to firefox for Firefox browser");
      }
    } else {
...
async function findBrowserExecutablePath(): Promise<string> {
  ...
  try {
    logger.debug("Attempting to find Chrome using chrome-launcher...");
    ...
    if (chrome.process && chrome.process.spawnfile) {
      ...
      logger.debug("Found Chrome path from process.spawnfile");
    } else {
      // Try to get the Chrome path from chrome-launcher
      // In newer versions, it's directly accessible
      logger.debug("Trying to determine Chrome path using other methods");
      ...
      for (const p of possiblePaths) {
        if (p && fs.existsSync(p)) {
          chromePath = p;
          logger.debug("Found Chrome path from common locations");
          break;
        }
      }
    }
    ...
    if (chromePath) {
      ...
      logger.debug("Chrome launched but couldn't determine executable path");
    }
  } catch (error) {
    ...
    if (
      ...
      logger.debug("Chrome not installed. Falling back to manual detection");
    } else {
      logger.warn("Failed to find Chrome using chrome-launcher:", error);
      logger.debug("Falling back to manual detection");
    }
  }
  ...
  logger.debug(`Attempting to detect browser executable path on ${platform}...`);
  ...
  if (platform === "win32") {
    ...
    try {
      logger.debug("Checking Windows registry for Chrome...");
      // Try HKLM first
      ...
      try {
        logger.debug("Checking user registry for Chrome...");
        const regOutput = execSync(
    ...
    try {
      logger.debug("Checking Chrome BLBeacon registry...");
      const regOutput = execSync(
      ...
      logger.debug("Failed to find Chrome via BLBeacon registry");
    }
...
function setupBrowserCleanupHandlers(
  ...
  browser.on("disconnected", () => {
    logger.debug(`Browser disconnected. Scheduling cleanup for: ${userDataDir}`);
    ...
    setTimeout(() => {
      ...
      if (!headlessBrowserInstance) {
        logger.debug(`Cleaning up temporary directory: ${userDataDir}`);
        try {
          fs.rmSync(userDataDir, { recursive: true, force: true });
          logger.debug(`Successfully removed directory: ${userDataDir}`);
        } catch (error) {
...
function cancelScheduledCleanup(): void {
  if (browserCleanupTimeout) {
    logger.debug("Cancelling scheduled browser cleanup");
...
export function scheduleBrowserCleanup(): void {
  ...
  if (headlessBrowserInstance) {
    ...
    browserCleanupTimeout = setTimeout(() => {
      logger.debug("Executing scheduled browser cleanup");
      if (headlessBrowserInstance) {
        logger.debug("Closing headless browser instance");
...
export async function connectToHeadlessBrowser(
  ...
  try {
    ...
    logger.debug("Creating a new page for this audit");
    ...
    if (options.headers && Object.keys(options.headers).length > 0) {
      ...
      logger.debug("Set custom HTTP headers");
    }
    ...
    if (options.cookies && options.cookies.length > 0) {
      ...
      logger.debug(`Set ${options.cookies.length} cookies`);
    }
    ...
    if (options.viewport) {
      ...
      logger.debug(`Emulating ${options.emulateDevice} device`);
    }
    ...
    if (options.locale) {
      ...
      logger.debug(`Set locale to ${options.locale}`);
    }

    if (options.timezoneId) {
      await page.emulateTimezone(options.timezoneId);
      logger.debug(`Set timezone to ${options.timezoneId}`);
    }
