...
// DOM elements
let serverTab = null;
let serverStatusElement = null;
let serverControlsElement = null;
let startServerButton = null;
let stopServerButton = null;
let serverStatusIndicator = null;
let redisConfigForm = null;
let redisTestButton = null;
let redisEnvSelector = null;
let redisCustomConfig = null;
/**
...
function setupUI() {
    ...
    startServerButton = document.getElementById('start-server-button');
    stopServerButton = document.getElementById('stop-server-button');
    serverStatusIndicator = document.getElementById('server-status-icon');
    redisConfigForm = document.getElementById('redis-config-form');
    redisTestButton = document.getElementById('test-redis');
    redisEnvSelector = document.getElementById('redis-env');
    redisCustomConfig = document.getElementById('redis-custom-config');
...
function toggleCustomRedisConfig() {
    if (!redisEnvSelector || !redisCustomConfig)
        return;
...
function updateRedisConfigFromForm() {
    ...
    // Update Redis config
    redisConfig.env = formData.get('env');
    redisConfig.host = formData.get('host') || 'localhost';
    redisConfig.port = parseInt(formData.get('port') || '6379');
    redisConfig.username = formData.get('username') || '';
    redisConfig.password = formData.get('password') || '';
...
function populateRedisForm() {
    if (!redisConfigForm)
        return;
    // Set form values
    const envField = redisConfigForm.querySelector('#redis-env');
    const hostField = redisConfigForm.querySelector('#redis-host');
    const portField = redisConfigForm.querySelector('#redis-port');
    const usernameField = redisConfigForm.querySelector('#redis-username');
    const passwordField = redisConfigForm.querySelector('#redis-password');
    const tlsField = redisConfigForm.querySelector('#redis-tls');
    if (envField)
        envField.value = redisConfig.env;
    if (hostField && redisConfig.host !== undefined)
        hostField.value = redisConfig.host;
    if (portField && redisConfig.port !== undefined)
        portField.value = redisConfig.port.toString();
    if (usernameField && redisConfig.username !== undefined)
        usernameField.value = redisConfig.username;
    if (passwordField && redisConfig.password !== undefined)
        passwordField.value = redisConfig.password;
    if (tlsField && redisConfig.tls !== undefined)
        tlsField.checked = redisConfig.tls;
}
/**
