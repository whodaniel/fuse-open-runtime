...
function initializeSettings() {
    ...
    const keyboardNavigationCheckbox = document.getElementById('keyboard-navigation');
    ...
    chrome.runtime.sendMessage({ type: 'GET_SETTINGS' }, (response) => {
        if (response && response.success) {
            ...
            const saveChatHistoryCheckbox = document.getElementById('save-chat-history');
            const debugModeCheckbox = document.getElementById('debug-mode');
            const autoDetectCodeCheckbox = document.getElementById('auto-detect-code');
            const useCompressionCheckbox = document.getElementById('use-compression');
            if (wsProtocolSelect)
                wsProtocolSelect.value = settings.wsProtocol || 'ws';
            if (wsHostInput)
                wsHostInput.value = settings.wsHost || 'localhost';
            if (wsPortInput)
                wsPortInput.value = settings.wsPort?.toString() || '3712';
            if (relayUrlInput)
                relayUrlInput.value = settings.relayUrl || 'https://localhost:3000';
            ...
            if (saveChatHistoryCheckbox)
                saveChatHistoryCheckbox.checked = settings.saveChatHistory !== false;
            if (debugModeCheckbox)
                debugModeCheckbox.checked = settings.debugMode === true;
            if (autoDetectCodeCheckbox)
                autoDetectCodeCheckbox.checked = settings.autoDetectCode !== false;
            if (useCompressionCheckbox)
                useCompressionCheckbox.checked = settings.useCompression !== false;
    ...
    chrome.runtime.sendMessage({ type: 'AUTH_STATUS' }, (response) => {
        if (response && response.success) {
            ...
            if (authStatusText)
                authStatusText.textContent = response.state || 'Unknown';
            if (tokenValidity)
                tokenValidity.textContent = response.tokenValid ? 'Valid' : 'Invalid';
            if (tokenExpiry)
                tokenExpiry.textContent = response.tokenExpiry ? new Date(response.tokenExpiry).toLocaleString() : 'None';
        }
    });
    ...
    if (highContrastCheckbox || largeTextCheckbox || reducedMotionCheckbox ||
        ...
        if (highContrastCheckbox)
            highContrastCheckbox.checked = accessSettings.highContrast;
        if (largeTextCheckbox)
            largeTextCheckbox.checked = accessSettings.largeText;
        if (reducedMotionCheckbox)
            reducedMotionCheckbox.checked = accessSettings.reducedMotion;
        if (keyboardNavigationCheckbox)
            keyboardNavigationCheckbox.checked = accessSettings.keyboardNavigation;
        if (screenReaderCheckbox)
            screenReaderCheckbox.checked = accessSettings.screenReader;
        // Add event listeners
        if (highContrastCheckbox) {
    ...
    if (saveSettingsButton) {
        saveSettingsButton.addEventListener('click', () => {
            ...
            const wsProtocol = document.getElementById('ws-protocol')?.value || 'ws';
            const wsHost = document.getElementById('ws-host')?.value || 'localhost';
            const wsPort = parseInt(document.getElementById('ws-port')?.value || '3712', 10);
            const relayUrl = document.getElementById('relay-url')?.value || 'https://localhost:3000';
            const autoConnect = document.getElementById('auto-connect')?.checked !== false;
            const showNotifications = document.getElementById('show-notifications')?.checked !== false;
            const saveChatHistory = document.getElementById('save-chat-history')?.checked !== false;
            const debugMode = document.getElementById('debug-mode')?.checked === true;
            const autoDetectCode = document.getElementById('auto-detect-code')?.checked !== false;
            const useCompression = document.getElementById('use-compression')?.checked !== false;
            // Save settings
    ...
    if (testWsButton) {
        testWsButton.addEventListener('click', () => {
            const protocol = document.getElementById('ws-protocol')?.value || 'ws';
            const host = document.getElementById('ws-host')?.value || 'localhost';
            const port = document.getElementById('ws-port')?.value || '3712';
                ...
                if (response && response.success) {
                    ...
                    chrome.runtime.sendMessage({ type: 'AUTH_STATUS' }, (authResponse) => {
                        if (authResponse) {
                            ...
                            const tokenExpiry = document.getElementById('token-expiry');
                            if (authStatusText)
                                authStatusText.textContent = authResponse.state || 'Unknown';
                            if (tokenValidity)
                                tokenValidity.textContent = authResponse.tokenValid ? 'Valid' : 'Invalid';
                            if (tokenExpiry)
                                tokenExpiry.textContent = authResponse.tokenExpiry ? new Date(authResponse.tokenExpiry).toLocaleString() : 'None';
                        }
                    });
                }
                else {
            ...
            if (confirm('Are you sure you want to clear authentication data?')) {
                chrome.runtime.sendMessage({ type: 'CLEAR_AUTH' }, (response) => {
                    if (response && response.success) {
                        ...
                        if (tokenValidity)
                            tokenValidity.textContent = 'Invalid';
                        if (tokenExpiry)
                            tokenExpiry.textContent = 'None';
                    }
                    else {
