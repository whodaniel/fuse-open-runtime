/**
 * TNF AI Bridge - Background Script
 * Fully integrated with The New Fuse infrastructure
 */
...
class TNFAIBridge {
  constructor() {
    ...
    this.tnfConfig = null;
    this.relayUrls = {
      main: 'ws://localhost:8765',           // TNF main relay
      comprehensive: 'ws://localhost:3001', // TNF comprehensive relay  
      websocket: 'ws://localhost:3001',     // TNF WebSocket
      backup: 'ws://localhost:3000'         // TNF backup relay
    };
    this.currentRelayUrl = this.relayUrls.comprehensive;
    this.isConnected = false;
    this.reconnectAttempts = 0;
    this.maxReconnectAttempts = 5;
    this.pendingMessages = new Map();
    this.agentId = `tnf-chrome-extension-${Date.now()}`;
    ...
    this.serverStatus = {
      ...
      port: 0,
      connectedClients: 0,
      message: 'Connecting to TNF infrastructure...'
    };
    
    // Unified Response Monitor
    this.responseMonitors = new Map(); // tabId -> monitor instance
    this.processedResponses = new Set();
    this.activeConversations = new Map(); // conversationId -> tab data
    
    this.init();
  }

  async init() {
    console.log('🚀 TNF AI Bridge initializing with full TNF integration...');
    
    // Load TNF configuration first
    await this.loadTNFConfiguration();
    
    // Set up alarms for keep-alive and port monitoring
    chrome.alarms.create('keep-alive', { periodInMinutes: 1 });
    chrome.alarms.create('tnf-health-check', { periodInMinutes: 5 });
    chrome.alarms.onAlarm.addListener(this.handleAlarm.bind(this));
    
    // Load saved relay URL or use TNF default
    chrome.storage.local.get(['relayUrl', 'tnfConfig'], ({ relayUrl, tnfConfig }) => {
      if (tnfConfig) {
  ...
  async loadTNFConfiguration() {
    try {
      console.log('📋 Loading TNF configuration...');
      
      // Try to fetch TNF config from local file or API
      const response = await fetch('http://localhost:3000/tnf/config').catch(() => null);
      if (response && response.ok) {
        this.tnfConfig = await response.json();
        this.tnfIntegrationStatus.mcpConfigLoaded = true;
        console.log('✅ TNF configuration loaded from API');
      } else {
        // Use default TNF configuration
        this.tnfConfig = {
          version: "2.1.0",
          servers: {
            "the-new-fuse-main": {
        ...
        console.log('⚠️ Using default TNF configuration');
      }
      
      // Save to storage
      chrome.storage.local.set({ tnfConfig: this.tnfConfig });
    } catch (error) {
      console.error('❌ Failed to load TNF configuration:', error);
    }
  }

  updateRelayUrls() {
    if (this.tnfConfig && this.tnfConfig.ports) {
      this.relayUrls = {
        main: `ws://localhost:${this.tnfConfig.ports.main || 8765}`,
        comprehensive: `ws://localhost:${this.tnfConfig.ports.comprehensive || 3001}`,
        websocket: `ws://localhost:${this.tnfConfig.ports.websocket || 3001}`,
        backup: `ws://localhost:${this.tnfConfig.ports.backup || 3000}`
      };
      this.currentRelayUrl = this.relayUrls.comprehensive;
    }
  }

  async checkTNFInfrastructure() {
    const tnfPorts = [8765, 3000, 3001, 3002];
    const statuses = {};
    
    await Promise.all(tnfPorts.map(async port => {
      try {
        ...
        if (response.ok) {
          ...
          statuses[port] = true;
          if (port === 3000 && data.redis === 'connected') {
            this.tnfIntegrationStatus.databaseConnected = true;
          }
        } else {
          statuses[port] = false;
        }
        console.log(`✅ TNF service running on port ${port}`);
      } catch {
        statuses[port] = false;
      }
    }));
    
    this.portStatuses = { ...this.portStatuses, ...statuses };
    this.broadcastTNFStatus();
  }

  connectToTNFRelay() {
    // Temporarily disable WebSocket connection to prevent console errors
    console.log('⚠️ TNF WebSocket connection disabled to prevent errors');
    console.log('💡 Extension will work in standalone mode without TNF relay');
    
    // Set status to indicate we're working without TNF relay
    this.isConnected = false;
    this.tnfIntegrationStatus.relayConnected = false;
    this.serverStatus.message = 'Working in standalone mode (TNF relay disabled)';
    
    // Still broadcast status for UI
    this.broadcastTNFStatus();
    this.broadcastServerStatus();
    ...
    try {
      ...
      console.log(`🔌 Connecting to TNF relay: ${this.currentRelayUrl}`);
      this.ws = new WebSocket(this.currentRelayUrl);
      
      this.ws.onopen = () => {
        console.log('✅ Connected to TNF relay');
        this.isConnected = true;
        this.tnfIntegrationStatus.relayConnected = true;
        this.reconnectAttempts = 0;
