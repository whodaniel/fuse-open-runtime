...
export class PerformanceOptimizer {
    ...
    async initialize() {
        ...
        settingsManager.addEventListener('feature-toggled', (data) => {
    ...
    async applyOptimization(optimization) {
        try {
            ...
            if (optimization.type === 'setting') {
                ...
                const success = await settingsManager.toggleFeature(optimization.target, optimization.action === 'enable');
                if (success) {
    ...
    calculateImprovement(before, after) {
        const weightedScore = (metrics) => {
            return ((metrics.memoryUsage / 1024 / 1024) * 0.3 + // Weight memory usage
