...
import { MessageType, MessageSource } from '../shared-protocol'; // Added BaseMessage, ConnectionStatusMessage
...
export class WebSocketManager extends EventEmitter {
        ...
        return new Promise((resolve) => {
            try {
                ...
                this.ws.onmessage = async (event) => {
                    try {
                        let processedData; // This will hold the string data after potential decompression
                        ...
                        if (parsedMessage && parsedMessage.type === MessageType.CONNECTION_STATUS) {
                            ...
                            this.handleConnectionStatus(parsedMessage);
                        }
                ...
                this.ws.onerror = (error) => {
                    // Extract meaningful error information instead of logging "[object Event]"
                    ...
                    if (error.target && error.target.readyState !== undefined) {
                        ...
                        const readyStateText = readyStates[error.target.readyState] || error.target.readyState;
                        errorMessage += ` (ReadyState: ${readyStateText})`;
                        errorDetails.readyState = readyStateText;
                    }
                    
                    if (error.target && error.target.url) {
                        errorMessage += ` for URL: ${error.target.url}`;
                        errorDetails.url = error.target.url;
                    }
                    
                    // Log meaningful error information
                    this.log('WebSocket error details:', {
                    ...
                    this.emit('error', { message: errorMessage, details: errorDetails, originalError: error });
                    resolve(false);
                };
            }
            catch (error) {
    ...
    async send(data) {
        ...
        try {
            ...
            let messageToSend = jsonMessage; // Default to jsonMessage if no encryption/compression
            if (this.securityManager) {
            ...
            if (this.options.useCompression && typeof messageToSend === 'string') {
                try {
                    const compressedData = compressString(messageToSend); // Cast is safe here
                    messageToSend = compressedData.buffer.slice(compressedData.byteOffset, compressedData.byteOffset + compressedData.byteLength);
                    this.log(`Message compressed successfully. Original (encrypted/plain) length: ${messageToSend.length}, Compressed length: ${messageToSend.byteLength}`);
                }
                catch (compressionError) {
