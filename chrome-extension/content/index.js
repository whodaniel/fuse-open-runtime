...
function setupGlobalErrorHandling() {
    ...
    window.addEventListener('unhandledrejection', (event) => {
        logger.reportError(new Error(`Unhandled promise rejection: ${event.reason}`), 'UnhandledRejection', { reason: event.reason });
    });
    // Catch uncaught errors
    window.addEventListener('error', (event) => {
        logger.reportError(event.error || new Error(event.message), 'UncaughtError', {
 ...
 */
async function initializeElementDetection() {
    if (isInitialized) {
...
function injectSelectionModeStyles() {
    if (selectionModeStylesInjected)
        return;
...
function enterElementSelectionMode(targetType) {
...
function addElementEventListeners() {
    ...
    allElements.forEach((element) => {
        if (isSelectableElement(element, selectionMode.targetElement)) {
...
function isSelectableElement(element, targetType) {
    const tagName = element.tagName.toLowerCase();
    const type = element.getAttribute('type')?.toLowerCase();
    const role = element.getAttribute('role')?.toLowerCase();
    switch (targetType) {
        case 'input':
            return (tagName === 'input' && (!type || ['text', 'search', 'email'].includes(type)) ||
                tagName === 'textarea' ||
        ...
        case 'output':
            return (tagName === 'div' || tagName === 'section' || tagName === 'main' ||
 ...
 */
function handleCapturePageOutput(request, sendResponse) {
 ...
 */
function handleSendToPageInput(request, sendResponse) {
    ...
    try {
        const inputElement = document.querySelector(chatInputSelector);
        const buttonElement = document.querySelector(sendButtonSelector);
        if (!inputElement) {
 ...
 */
async function handleEnterSelectionMode(request, sendResponse) {
    try {
 ...
 */
async function handleAutoDetectElements(request, sendResponse) {
    try {
 ...
 */
function handleSaveElementMapping(request, sendResponse) {
 ...
 */
async function handleLoadElementMapping(request, sendResponse) {
    try {
 ...
 */
function handleValidateElementMapping(request, sendResponse) {
    if (!currentPageMapping) {
 ...
 */
async function handleExecuteAIAction(request, sendResponse) {
 ...
 */
async function executeAISendMessage(parameters) {
    ...
    await simulateTyping(inputElement, message);
    // Click send button
    buttonElement.click();
    if (waitForResponse) {
 ...
 */
async function executeAIExtractResponse(parameters) {
 ...
 */
async function waitForNewResponse(timeout = 30000) {
    if (!currentPageMapping?.chatOutput) {
        throw new Error('Chat output not configured');
    }
    const outputElement = elementSelector.findElement(currentPageMapping.chatOutput);
 ...
 */
async function executeAIAnalyzeInterface(parameters) {
    const analysis = {
 ...
 */
async function handleSimulateUserInteraction(request, sendResponse) {
    ...
    try {
        ...
        switch (interactionType) {
            case 'click':
                ...
                break;
            case 'type':
                await simulateTyping(element, parameters.text);
                break;
            case 'hover':
 ...
 * Simulate scroll
 */
async function simulateScroll(element, parameters) {
 ...
 */
function handleControlIframeVisibility(request, sendResponse) {
 ...
 */
function handleTestSelectors(request, sendResponse) {
    try {
        ...
        if (chatInputSelector) {
            try {
                ...
                results.push(`Input: Invalid selector - ${error.message}`);
            }
        }
        ...
        if (chatOutputSelector) {
            try {
                ...
                if (outputElements.length > 0) {
                    const firstElement = outputElements[0];
                    results.push(`  - Type: ${firstElement.tagName}, ID: ${firstElement.id || 'none'}, Classes: ${firstElement.className || 'none'}`);
                }
            }
            catch (error) {
                results.push(`Output: Invalid selector - ${error.message}`);
            }
        }
        ...
        if (sendButtonSelector) {
            try {
                ...
                results.push(`Button: Invalid selector - ${error.message}`);
            }
        }
        sendResponse({
            success: true,
 ...
 */
function handleToggleFloatingPanel(request, sendResponse) {
    try {
        logger.info('Toggling floating panel');
        // Check if floating panel iframe already exists
        let floatingPanel = document.getElementById('tnf-floating-panel');
        let isVisible = false;
        if (!floatingPanel) {
 ...
 */
function handleAutoDetectSelectors(request, sendResponse) {
    try {
        const selectors = {
            input: null,
            output: null,
        ...
        for (const selector of inputCandidates) {
            try {
                ...
                if (elements.length > 0) {
                    selectors.input = generateSimpleSelector(elements[0]);
                    break;
                }
            }
            catch (error) {
        ...
        for (const selector of outputCandidates) {
            try {
                const elements = document.querySelectorAll(selector);
                if (elements.length > 0) {
                    selectors.output = generateSimpleSelector(elements[0]);
                    break;
                }
            }
            catch (error) {
            ...
            try {
                ...
                if (elements.length > 0) {
                    ...
                    if (visibleElements.length > 0) {
                        selectors.button = generateSimpleSelector(visibleElements[0]);
                        break;
                    }
                }
            }
        ...
        sendResponse({
            success: false,
            error: `Failed to auto-detect selectors: ${error.message}`
        });
    }
}
