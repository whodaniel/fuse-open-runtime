...
export class AIElementDetector {
    ...
    async detectChatElements() {
        ...
        Object.keys(results).forEach(type => {
            results[type] = this.rankElementsByAI(results[type], type);
        });
        this.logger.info('Chat elements detected:', results);
        return results;
    }
    findElementsByPattern(pattern) {
        const elements = [];
    ...
    searchByTextPatterns(pattern, foundElements, elements) {
    ...
    isRelevantTagForPattern(element, pattern) {
    ...
    createElementInfo(element, pattern) {
        try {
    ...
    calculateElementConfidence(element, pattern) {
        ...
        const keywordMatches = chatKeywords.filter(keyword => textContent.includes(keyword)).length;
        ...
        if (pattern.type === 'input' && element.closest('form'))
            confidence += 0.1;
        if (pattern.type === 'button' && element.type === 'submit')
            confidence += 0.2;
    ...
    rankElementsByAI(elements, type) {
        return elements
     ...
     */
    async getBestChatOutput() {
        const detected = await this.detectChatElements();
        return detected.output[0] || null;
    }
}
//# sourceMappingURL=ai-element-detector.js.map