...
setup_cli() {
    ...
    if [ -f "$cli_file" ]; then
        ...
        print_status "You can now use: cd apps/api && bun claude-dev <command>"
    fi
}
...
validate_typescript() {
    ...
    if command_exists bun && bun tsc --noEmit --skipLibCheck 2>/dev/null; then
...
# List templates
bun claude-dev templates

# Execute automation
bun claude-dev run code-review params.json

# Check automation status
bun claude-dev get <automation-id>

# View statistics
bun claude-dev stats
```
