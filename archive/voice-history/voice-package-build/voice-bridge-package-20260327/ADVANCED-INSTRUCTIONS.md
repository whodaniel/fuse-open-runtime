...
This guide covers power-user setup, tandem mode with the Audio Trigger KWS app, and troubleshooting.

## 1) Tandem Mode: Local Injection + KWS/LLM Pipeline

Voice Bridge can now do both at once:
- Keep existing local destination injection (`<THE_NEW_FUSE_ROOT>/.voicebridge/voice_stream.txt` + `stream_watch.py`)
- Forward each transcript chunk to KWS API (`/v1/ingest/text`) and periodic flush (`/v1/flush`)
...
- Forwarding is async and non-blocking. UI/input responsiveness remains local-first.
...
## 6) Drift Prevention Runbook

Use this exact sequence to avoid configuration/runtime drift across sessions.
...
Expected:
- target is set (terminal/app/point)
- `enabled: true` for cloud forwarding

Process checks:

```bash
ps -axo pid,ppid,stat,etime,command | rg -n 'voice_server.py|stream_watch.py|voice-target-click-daemon' -S
```

Expected:
- all three processes present

Self-heal (safe to run anytime):

```bash
curl -X POST http://127.0.0.1:50005/activate
```

If injection is missing but browser transcription works:
- re-anchor target (Cmd+Option+Click or `voice-target-here`)
- run `/activate` again
- confirm `stream_watch.py` process exists

If cloud forwarding is missing but local injection works:
- check `voice_bridge_cloud.env`
- re-run `voice` or restart launch agent
- re-check `/kws_state`

Do not edit random copies of scripts:
- only edit the package source folder
- reinstall from package after changes
- rebuild DMG from package source

## 7) Optional AI Response Audio (Terminal Read-Aloud)
