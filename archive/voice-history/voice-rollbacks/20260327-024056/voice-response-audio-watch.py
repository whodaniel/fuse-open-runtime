...
AI_OUTPUT_SUPPRESSION_SECONDS = float(
    os.environ.get("VOICE_RESPONSE_AUDIO_SUPPRESSION_SECONDS", "0.90")
)
REQUESTED_VOICE_NAME = os.environ.get("VOICE_RESPONSE_AUDIO_VOICE", "Daniel").strip()
...
def choose_voice_name() -> str:
    available = list_available_voices()
    if not available:
        return REQUESTED_VOICE_NAME or "Alex"

    lowered = {name.lower(): name for name in available}
    if REQUESTED_VOICE_NAME and REQUESTED_VOICE_NAME.lower() in lowered:
        return lowered[REQUESTED_VOICE_NAME.lower()]

    fallbacks = [
        name.strip()
        for name in os.environ.get(
            "VOICE_RESPONSE_AUDIO_FALLBACK_VOICES",
            "Daniel,Alex,Samantha,Victoria",
        ).split(",")
        if name.strip()
    ]
    for fallback in fallbacks:
        resolved = lowered.get(fallback.lower())
        if resolved:
            return resolved
...
def speak(text: str) -> None:
    ...
    try:
        ...
        cooldown = max(POST_SPEECH_DELAY_SECONDS, AI_OUTPUT_SUPPRESSION_SECONDS)
        if cooldown > 0:
            time.sleep(cooldown)
    finally:
        try:
...
def main() -> None:
    ...
    print(f"🔊 voice-response-audio using voice: {VOICE_NAME}")
