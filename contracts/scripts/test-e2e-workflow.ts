...
async function main() {
  ...
  const createAccountTx = await smartAccountFactory
    .connect(agentOwner)
    .createAccount(agentOwner.address, salt);
  const createAccountReceipt = await createAccountTx.wait();

  const accountCreatedEvent = createAccountReceipt.events?.find(
    (e) => e.event === 'AccountCreated'
  );
  ...
  const createStreamTx = await distributor
    .connect(agentOwner)
    .createRevenueStream(agentTokenId, streamName, tokenAddress, distributionThreshold);
  ...
  const listSharesTx = await marketplace
    .connect(agentOwner)
    .listShares(agentTokenId, sharesToSell, pricePerShare, listingDuration);
  ...
  console.log(
    `   Shares for Sale: ${sharesToSell.toLocaleString()} (${(sharesToSell / 100).toFixed(1)}%)`
  );
  ...
  await buyTx1.wait();
  console.log(
    `✅ Buyer 1 purchased ${sharesForBuyer1.toLocaleString()} shares (${(sharesForBuyer1 / 100).toFixed(1)}%)`
  );
  ...
  await buyTx2.wait();
  console.log(
    `✅ Buyer 2 purchased ${sharesForBuyer2.toLocaleString()} shares (${(sharesForBuyer2 / 100).toFixed(1)}%)`
  );
  ...
  console.log(
    `   Agent Owner: ${ownerShares.toString().padStart(4)} shares (${(ownerShares.toNumber() / 100).toFixed(1).padStart(5)}%)`
  );
  console.log(
    `   Buyer 1:     ${buyer1Shares.toString().padStart(4)} shares (${(buyer1Shares.toNumber() / 100).toFixed(1).padStart(5)}%)`
  );
  console.log(
    `   Buyer 2:     ${buyer2Shares.toString().padStart(4)} shares (${(buyer2Shares.toNumber() / 100).toFixed(1).padStart(5)}%)`
  );
  console.log(
    `   Buyer 3:     ${buyer3Shares.toString().padStart(4)} shares (${(buyer3Shares.toNumber() / 100).toFixed(1).padStart(5)}%)`
  );
  ...
  for (const claimer of claimers) {
    ...
    try {
      const claimTx = await distributor
        .connect(claimer.signer)
        .claimRevenue(streamId, tokenAddress);
      ...
      console.log(
        `✅ ${claimer.name} claimed revenue: ${ethers.utils.formatEther(netReceived)} ETH`
      );
    } catch (error) {
  ...
  console.log(
    `   Total Distributed: ${ethers.utils.formatEther(finalRevenueStream.distributedRevenue)} ETH`
  );
  console.log(
    `   Remaining: ${ethers.utils.formatEther(finalRevenueStream.totalRevenue.sub(finalRevenueStream.distributedRevenue))} ETH`
  );
  ...
  console.log(
    `   Total Volume: ${ethers.utils.formatEther(priceForBuyer1.add(priceForBuyer2).add(offerPrice))} ETH`
  );
