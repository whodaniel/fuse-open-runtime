...
async function main() {
  ...
  const revenueDistributor = await ethers.getContractAt(
    'AgentRevenueDistributor',
    revenueDistributorAddress
  );
  const smartAccountFactory = await ethers.getContractAt(
  ...
  const smartAccountAddress = accountReceipt.events?.find((e) => e.event === 'AccountCreated')?.args
    ?.account;
  ...
  const streamId = streamReceipt.events?.find((e) => e.event === 'RevenueStreamCreated')?.args
    ?.streamId;
  ...
  const distributedAmount = distributeReceipt.events?.find((e) => e.event === 'RevenueDistributed')
    ?.args?.totalAmount;
