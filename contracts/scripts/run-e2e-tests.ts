...
class E2ETestRunner {
    ...
    private async testGasOptimization(): Promise<void> {
        ...
        const tokenId = 1;
        const fractionalizeTx = await agentNFT.connect(user).fractionalizeAgent(
            tokenId, "Test Agent", "Description", "TYPE", ["cap1"], 10000, true
        );
    ...
    private async testSecurity(): Promise<void> {
        ...
        try {
            await agentNFT.connect(attacker).fractionalizeAgent(
                tokenId, "Hacked", "Desc", "TYPE", [], 10000, true
            );
    ...
    private async testLoadStress(): Promise<void> {
        ...
        const fractionalizePromises = users.slice(0, 5).map(async (user, index) => {
            ...
            const tx = await agentNFT.connect(user).fractionalizeAgent(
                tokenId, `Agent ${index}`, "Description", "TYPE", ["capability"], 10000, true
            );
            return tx.wait();
        });
    ...
    private printFinalReport(): void {
        ...
        this.results.forEach((result, index) => {
            ...
            console.log(`   ${index + 1}. ${status} | ${result.testName.padEnd(25)} | ${duration.padStart(8)}`);
