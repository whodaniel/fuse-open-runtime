...
export class MCPActionTrigger {
  ...
  async triggerActions(businessEvent: BusinessEvent): Promise<MCPActionResult[]> {
    ...
    try {
      ...
      this.logger.info(`MCP action execution completed: ${successCount}/${actionResults.length} successful`, {
        eventId: businessEvent.id,
  ...
  private findMatchingActions(businessEvent: BusinessEvent): MCPAction[] {
    return this.actions.filter(action => {
      ...
      return action.triggers.some(trigger => {
        // Check event type
        const eventTypeMatch = trigger.eventTypes.includes('*') || 
                             trigger.eventTypes.includes(businessEvent.type);
        
        if (!eventTypeMatch) return false;

        // Check source
        const sourceMatch = trigger.sources.includes('*') || 
                          trigger.sources.includes(businessEvent.source);
  ...
  private async storeActionResults(businessEvent: BusinessEvent, results: MCPActionResult[]): Promise<void> {
    try {
      const analyticsData = {
