...
;
  ...
  useEffect(() => {
    if (isVoiceEnabled:unknown){
      (voiceEngine as any).start(dashboardState).catch((error)=>{
        (console asany).error(Error startingvoicecontrol:,error);
      ...
      (voiceEngine asany).registerCommandHandler("{
      type: "action,;
  ...
  useEffect(() = "> {
    if (isGestureEnabled: unknown){
     ...
     (gestureEngineasany).registerHandler("pinch", async (): Promise<void> {event) => { 
        const newState = { ...dashboardState    };
