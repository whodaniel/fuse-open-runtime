...
interface TechStackSnippet {
  heading: string;
  content: string;
}

interface TechStackReport {
  [url: string]: TechStackSnippet[];
}
...
async function main(): Promise<void> {
  try {
    ...
    const urls: string[] = JSON.parse(fsSync.readFileSync(urlsPath, 'utf-8'));
    ...
    const report: TechStackReport = {};

    for (const url of filtered) {
      try {
        const response = await axios.get(url);
        const $ = cheerio.load(response.data);
        const snippets: TechStackSnippet[] = [];
