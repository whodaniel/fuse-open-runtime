...
class SubscriptionService {
  constructor() {
    this.apiUrl = process.env.API_URL || 'https://thenewfuse.com/api';
    this.tnfBilling = {
      pricingUrl: 'https://thenewfuse.com/pricing',
  ...
  async upgrade(targetTier) {
    try {
      // Legacy local checkout is deprecated in TNF; route to TNF billing.
      const checkoutUrl = `${this.tnfBilling.pricingUrl}?plan=${encodeURIComponent(targetTier)}`;
  ...
  async verifySubscription(sessionId) {
    throw new Error(
      `Legacy verification disabled. Use TNF billing controller: ${this.tnfBilling.controllerPath}`
    );
  }
  ...
  async cancelSubscription() {
    throw new Error(
      `Legacy cancellation disabled. Use TNF billing controller: ${this.tnfBilling.controllerPath}`
    );
  }
