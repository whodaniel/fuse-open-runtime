...
export class Logger {
  ...
  constructor(options: LoggerOptions = {}) {
    ...
    this.storageKey = `logger_${this.name.toLowerCase()}`;
    ...
    if (this.saveToStorage && typeof chrome?.storage?.local === 'undefined') {
      console.warn(
        `Logger "${this.name}": chrome.storage.local is not available. Disabling saveToStorage.`
      );
  ...
  async getStoredLogs(): Promise<LogEntry[]> {
    if (!this.saveToStorage || !chrome?.storage?.local) {
      console.warn(
        `Logger "${this.name}": Cannot get stored logs. saveToStorage is disabled or chrome.storage is unavailable.`
      );
      return [];
    }
    const storageKey = `logs_${this.name}`;
  ...
  async clearStoredLogs(): Promise<void> {
    if (!this.saveToStorage || !chrome?.storage?.local) {
      console.warn(
        `Logger "${this.name}": Cannot clear stored logs. saveToStorage is disabled or chrome.storage is unavailable.`
      );
      return;
    }
    const storageKey = `logs_${this.name}`;
  ...
  /**
   * Get stored logs from chrome storage
   */
