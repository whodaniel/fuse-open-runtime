    ...
    try {
      ...
      switch (mode) {
        case "comprehensive":
          ...
          break;
        case "basic":
          relayScript = path.join(this.workspaceDir, 'tnf-relay.js');
          break;
        case "websocket":
          relayScript = path.join(this.workspaceDir, 'relay-server/index.js');
          break;
        default:
        ...
        contents: [
          {
            uri: "tnf://config/mcp",
            mimeType: "application/json",
            text: `{"error": "Could not read MCP config: ${error.message}"}`
          }
        ]
      };
    }
