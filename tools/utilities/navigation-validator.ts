...
async function validateNavigation(): Promise<void> {
  ...
  try {
    ...
    // Save report
    fs.writeFileSync(path.join(process.cwd(), CONFIG.reportFile), report.join('\n'));
...
async function findRouteDefinitions(): Promise<Route[]> {
  try {
    const routes: Route[] = [];
    const ignore = CONFIG.excludeDirs.map((dir) => `**/${dir}/**`);

    // Find all route files
    const routeFiles = globSync(CONFIG.routePatterns, { cwd: CONFIG.rootDir, ignore });
    ...
    for (const file of routeFiles) {
      ...
      try {
        ...
        // Find React Router route definitions (single-line and multi-line)
        // v5-style component prop
        const routeMatches = content.matchAll(
          /<Route[\s\S]*?path=["']([^"']+)["'][\s\S]*?component=\{([^}]+)\}[\s\S]*?\/?>/g
        );
        ...
        // v6-style element prop
        const routeV6Matches = content.matchAll(
          /<Route[\s\S]*?path=["']([^"']+)["'][\s\S]*?element=\{([^}]+)\}[\s\S]*?\/?>/g
        );
        ...
        console.warn(
          `   Warning: Could not process route file ${file}: ${(error as Error).message}`
        );
      }
    }
    ...
    console.error(`Error finding route definitions: ${(error as Error).message}`);
    return [];
  }
}

// Find all navigation links in the codebase
async function findNavigationLinks(): Promise<Link[]> {
  try {
    const links: Link[] = [];
    const ignore = CONFIG.excludeDirs.map((dir) => `**/${dir}/**`);

    // Find all files that might contain links
    const linkFiles = globSync(CONFIG.linkPatterns, { cwd: CONFIG.rootDir, ignore });
    ...
    for (const file of linkFiles) {
      ...
      try {
        ...
        console.warn(
          `   Warning: Could not process link file ${file}: ${(error as Error).message}`
        );
      }
    }
    ...
    console.error(`Error finding navigation links: ${(error as Error).message}`);
    return [];
  }
}
...
function validateLinks(routes: Route[], links: Link[]): ValidationResults {
...
function generateNavigationMap(routes: Route[], links: Link[]): NavigationMap {
...
function generateReport(
  ...
  function printNodeAndChildren(nodePath: string, depth: number = 0): void {
  ...
  const rootNodes = Object.keys(navigationMap).filter(
    (path) =>
