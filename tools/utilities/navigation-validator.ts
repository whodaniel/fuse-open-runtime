  ...
  try {
    ...
    for (const file of routeFiles) {
      ...
      try {
        ...
        console.warn(`   Warning: Could not process route file ${file}: ${(error as Error).message}`);
      }
    }
  ...
  try {
    ...
    for (const file of linkFiles) {
      ...
      try {
        ...
        console.warn(`   Warning: Could not process link file ${file}: ${(error as Error).message}`);
      }
    }
...
function generateReport(routes: Route[], links: Link[], validationResults: ValidationResults, navigationMap: NavigationMap): string[] {
  const report: string[] = [];
