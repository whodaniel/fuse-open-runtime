...
import { FileSystemScanner, FileType, PackageType } from '../scanner/FileSystemScanner';
import * as path from 'path';

describe('FileSystemScanner', () => {
  const testRootPath = path.join(__dirname, '../../../../'); // Go to project root
  
  it('should scan file system and identify packages', async () => {
  ...
  it('should identify packages with package.json', async () => {
    const scanner = new FileSystemScanner(testRootPath);
    const result = await scanner.scanFileSystem();
