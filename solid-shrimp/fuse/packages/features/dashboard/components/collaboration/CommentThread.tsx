...
var CommentThread = function (): JSX.Element _a) {
    var _b;
    ...
    var handleSubmitReply = function (): JSX.Element ) {
    ...
    var renderComment = function (): JSX.Element comment, isReply) {
      ...
      <div className="flex-1">
        <div className="bg-white rounded-lg border p-3">
          <div className="flex items-center justify-between">
            ...
            {comment.author.id === currentUser.id && !comment.resolved && (<button onClick={function (): JSX.Element ) { return onResolve(comment); }} className="text-sm text-gray-500 hover:text-gray-700">
                Resolve
              </button>)}
          </div>
          ...
          {comment.attachments && comment.attachments.length > 0 && (<div className="mt-2 space-y-2">
              {comment.attachments.map(function (): JSX.Element attachment) { return (<div key={attachment.id} className="flex items-center space-x-2 text-sm">
          ...
          {comment.reactions && comment.reactions.length > 0 && (<div className="mt-2 flex flex-wrap gap-2">
              {comment.reactions.map(function (): JSX.Element reaction) { return (<button key={reaction.type} onClick={function (): JSX.Element ) { return onReact(comment.id, reaction.type); }} className={"inline-flex items-center space-x-1 rounded-full px-2 py-0.5 text-xs ".concat(reaction.users.includes(currentUser.id)
          ...
          <div className="mt-2 flex items-center space-x-4 text-sm">
            <button onClick={function (): JSX.Element ) { return setIsReplying(!isReplying); }} className="text-gray-500 hover:text-gray-700">
              Reply
            </button>
            <button onClick={function (): JSX.Element ) { return onReact(comment.id, '👍'); }} className="text-gray-500 hover:text-gray-700">
              React
            </button>
          </div>
        </div>
        ...
        {comment.replies && comment.replies.length > 0 && (<div className="mt-3">
            {comment.replies.map(function (): JSX.Element reply) { return (<div key={reply.id}>{renderComment(reply, true)}</div>); })}
          </div>)}

        {/* Reply Input */}
        {isReplying && (<div className="mt-3 ml-8">
            <textarea value={replyContent} onChange={function (): JSX.Element e) { return setReplyContent(e.target.value); }} placeholder="Write a reply..." className="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" rows={3}/>
            <div className="mt-2 flex justify-end space-x-2">
              <Button_1.Button variant="outline" size="sm" onClick={function (): JSX.Element ) { return setIsReplying(false); }}>
                Cancel
              </Button_1.Button>
