...
import type { PackageRedundancyAnalysis, ApplicationLayerAnalysis } from './analyzer/ArchitectureAnalyzer';
...
async function main() {
  ...
  try {
    ...
    if (result.architectureAnalysis) {
      ...
      const highRedundancy = result.architectureAnalysis.packageRedundancy.filter((p: PackageRedundancyAnalysis) => p.redundancyScore > 70);
      const mediumRedundancy = result.architectureAnalysis.packageRedundancy.filter((p: PackageRedundancyAnalysis) => p.redundancyScore > 40 && p.redundancyScore <= 70);
      ...
      const appsToMerge = result.architectureAnalysis.applicationLayer.filter((app: ApplicationLayerAnalysis) => 
        app.consolidationRecommendation.action === 'merge_into'
      );
