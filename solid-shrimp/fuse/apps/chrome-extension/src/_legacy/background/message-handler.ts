...
export class MessageHandler {
   ...
   */
  constructor(connectionManager: ConnectionManager, backgroundLoggerInstance: Logger) {
    // Modified
  ...
  private async handleBgInjectScriptRequest(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        messageLogger.info(
          `Background: Successfully sent INJECT_SCRIPT_REQUEST to tab ${tabId} on attempt ${attempt}`
        );
        sendResponse(responseFromTab); // Forward the content script's response
        return;
      } catch (error: any) {
        messageLogger.warn(
          `Background: Attempt ${attempt} failed for INJECT_SCRIPT_REQUEST to tab ${tabId}: ${error.message}`
        );
        if (error.message && error.message.includes('Receiving end does not exist')) {
          if (attempt < maxRetries) {
            ...
            // Max retries reached
            messageLogger.error(
              `Background: Max retries reached for INJECT_SCRIPT_REQUEST to tab ${tabId}.`
            );
            sendResponse({
              success: false,
              message: `Failed to connect to content script in tab ${tabId} after ${maxRetries} attempts. The page might be restricted or not fully loaded.`,
            });
            return;
          }
        } else {
          ...
          messageLogger.error(
            `Background: Non-retryable error for INJECT_SCRIPT_REQUEST to tab ${tabId}: ${error.message}`
          );
          sendResponse({
            success: false,
            message: `Error sending message to tab ${tabId}: ${error.message}`,
          });
          return;
        }
      }
    ...
    try {
      ...
      const response: SharedSecretAckMessage = {
        ...
        payload: {
          success: true,
          message: connected
            ? 'Shared secret set and reconnected'
            : 'Shared secret set but reconnection failed',
        },
      };
      ...
      this.showNotification(
        ...
        connected
          ? 'Shared secret updated and reconnected to VS Code'
          : 'Shared secret updated but reconnection failed',
