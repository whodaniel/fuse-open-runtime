...
export const DynamicNode = ({ data, id, isConnectable }) => {
  ...
  if (loading) {
    return (
      <div className="dynamic-node loading">
  ...
  if (error || !nodeSchema) {
    return (
      <div className="dynamic-node error">
    ...
    <div className="dynamic-node">
      {nodeSchema.inputs.map((input, index) => (
        <Handle
          ...
          id={`input-${input.name || 'main'}`}
          style={{ left: `${(index + 1) * (100 / (nodeSchema.inputs.length + 1))}%` }}
      ...
      <div className="node-content">
        {Object.entries(nodeSchema.parameters).map(([paramName, paramConfig]) => (
          <div key={paramName} className="param-group">
            ...
            {renderParamInput(paramConfig, data.parameters[paramName], (value) =>
              updateNodeData(paramName, value)
            )}
            {paramConfig.description && (
              <div className="param-description">{paramConfig.description}</div>
            )}
          </div>
        ))}

        {nodeSchema.credentials.length > 0 && (
          <CredentialSelector
            credentialType={nodeSchema.credentials[0].name}
            value={(_a = data.credentials) === null || _a === void 0 ? void 0 : _a.id}
            onChange={(credId) => {}}
          />
        )}
      </div>
      ...
      {nodeSchema.outputs.map((output, index) => (
        <Handle
          ...
          id={`output-${output.name || 'main'}`}
          style={{ left: `${(index + 1) * (100 / (nodeSchema.outputs.length + 1))}%` }}
          isConnectable={isConnectable}
        />
      ))}
    </div>
