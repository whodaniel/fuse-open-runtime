...
Currently configured project: `The-New-Fuse` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/The-New-Fuse/SETUP.md` to fill in
   `data/The-New-Fuse/INFO.md`.
...
`--project-id The-New-Fuse` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/The-New-Fuse/`.
